...
#property description "Date-independent XAUUSD M15 overnight-drift continuation research EA"
...
input ulong  InpMagicNumber = 26071991;
...
input group "Overnight Drift Signal"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_M15;
input ENUM_TIMEFRAMES InpATRTimeframe = PERIOD_D1;
input int    InpATRPeriod = 14;
input int    InpEntryHour = 7;
input int    InpEntryWindowBars = 4;
input int    InpExitHour = 17;
input double InpMinimumPriorDayMoveATR = 0.30;
input double InpMaximumPriorDayMoveATR = 1.50;
input double InpMinimumPriorDayBodyPercent = 45.0;
input double InpMinimumAsianRangeATR = 0.08;
input double InpMaximumAsianRangeATR = 0.55;
input double InpMinimumAsianDriftATR = -0.15;
input double InpMaximumAsianDriftATR = 0.35;
input double InpMinimumSignalBodyPercent = 35.0;
input bool   InpRequireSignalDirection = true;
input bool   InpRequireCloseBeyondAsianMidpoint = true;
...
input double InpMaximumStopATR = 0.60;
...
input int    InpLossCooldownHours = 168;
...
input string InpLogFileName = "Independent_XAUUSD_M15_Overnight_Drift_Continuation_Trades.csv";
...
datetime g_lastEvaluatedDay = 0;
double g_peakEquity = 0.0;
int g_dailyContexts = 0;
int g_driftCandidates = 0;
int g_signalRejects = 0;
...
{
   return "IM15ODC_RISK_" + IntegerToString((long)ticket);
}
...
bool EntrySafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
        InpRealAccountApprovalCode != "M15ODC-LIVE-ACK"))
   {
...
bool PriorDayBar(MqlRates &bar)
{
   ...
   if(CopyRates(_Symbol, PERIOD_D1, 1, 1, rates) != 1)
      return false;
   bar = rates[0];
   return bar.high > bar.low && bar.open > 0.0 && bar.close > 0.0;
}

bool SessionRange(const datetime fromTime,
                  const datetime untilTime,
                  double &sessionOpen,
                  double &sessionHigh,
                  double &sessionLow,
                  double &sessionClose,
                  int &barCount)
{
   ...
   if(fromTime <= 0 || untilTime <= fromTime)
      return false;
   MqlRates rates[];
   int copied = CopyRates(_Symbol, InpSignalTimeframe, fromTime, untilTime - 1, rates);
   if(copied <= 0)
      return false;
   ...
   {
      if(rates[i].time < fromTime || rates[i].time >= untilTime || rates[i].high <= rates[i].low)
         continue;
      if(firstTime == 0 || rates[i].time < firstTime)
      {
         firstTime = rates[i].time;
         sessionOpen = rates[i].open;
      }
      if(lastTime == 0 || rates[i].time > lastTime)
      {
         lastTime = rates[i].time;
         sessionClose = rates[i].close;
      }
      sessionHigh = MathMax(sessionHigh, rates[i].high);
      sessionLow = MathMin(sessionLow, rates[i].low);
      barCount++;
   }
   return barCount >= 4 && sessionOpen > 0.0 && sessionClose > 0.0 &&
          sessionHigh > sessionLow && sessionLow < DBL_MAX;
}

bool OpenDriftPosition(const bool buy,
                       const double atr,
                       const double rawStop)
{
   string safetyReason = "";
   ...
   string comment = buy ? "M15ODC_BUY" : "M15ODC_SELL";
   ...
   g_ordersOpened++;
   g_lastEvaluatedDay = g_dayStart;
   RegisterRiskForNewestPosition(stopDistance);
   LogEvent("entry", trade.ResultOrder(), buy ? "buy" : "sell", lots,
             entryPrice, stopPrice, targetPrice, 0.0, "prior-day and Asian drift continuation");
   return true;
}

bool TryOvernightDriftEntry(const double atr)
{
   datetime currentBarTime = iTime(_Symbol, InpSignalTimeframe, 0);
   if(currentBarTime <= 0 || g_dayStart <= 0 || g_lastEvaluatedDay == g_dayStart)
      return false;
   MqlDateTime nowParts;
   if(!TimeToStruct(currentBarTime, nowParts))
      return false;
   if(nowParts.day_of_week == 0 || nowParts.day_of_week == 6 ||
      (nowParts.day_of_week == 5 && !InpTradeFriday))
      return false;
   int signalSeconds = PeriodSeconds(InpSignalTimeframe);
   if(signalSeconds <= 0)
      return false;
   int minuteOfDay = nowParts.hour * 60 + nowParts.min;
   int entryMinute = InpEntryHour * 60;
   int endMinute = entryMinute + InpEntryWindowBars * signalSeconds / 60;
   if(minuteOfDay < entryMinute || minuteOfDay >= endMinute)
      return false;

   MqlRates priorDay;
   if(!PriorDayBar(priorDay) || atr <= 0.0)
      return false;
   g_dailyContexts++;
   double priorMove = priorDay.close - priorDay.open;
   double priorMoveATR = MathAbs(priorMove) / atr;
   if(priorMove == 0.0 || priorMoveATR < InpMinimumPriorDayMoveATR ||
      priorMoveATR > InpMaximumPriorDayMoveATR ||
      CandleBodyPercent(priorDay) < InpMinimumPriorDayBodyPercent)
      return false;
   bool buy = priorMove > 0.0;
   if((buy && !InpAllowBuy) || (!buy && !InpAllowSell))
      return false;

   double asianOpen = 0.0;
   double asianHigh = 0.0;
   double asianLow = 0.0;
   double asianClose = 0.0;
   int asianBars = 0;
   if(!SessionRange(g_dayStart, currentBarTime, asianOpen, asianHigh,
                    asianLow, asianClose, asianBars))
      return false;
   double asianRangeATR = (asianHigh - asianLow) / atr;
   double signedAsianDriftATR = (buy ? asianClose - asianOpen
                                     : asianOpen - asianClose) / atr;
   if(asianRangeATR < InpMinimumAsianRangeATR ||
      asianRangeATR > InpMaximumAsianRangeATR ||
      signedAsianDriftATR < InpMinimumAsianDriftATR ||
      signedAsianDriftATR > InpMaximumAsianDriftATR)
      return false;
   g_driftCandidates++;

   MqlRates signalBars[];
   ArraySetAsSeries(signalBars, true);
   if(CopyRates(_Symbol, InpSignalTimeframe, 1, 1, signalBars) != 1)
      return false;
   MqlRates signal = signalBars[0];
   double signalMove = signal.close - signal.open;
   double asianMidpoint = 0.5 * (asianHigh + asianLow);
   bool directionAllows = !InpRequireSignalDirection || (buy ? signalMove > 0.0 : signalMove < 0.0);
   bool locationAllows = !InpRequireCloseBeyondAsianMidpoint ||
                         (buy ? signal.close > asianMidpoint : signal.close < asianMidpoint);
   if(!directionAllows || !locationAllows ||
      CandleBodyPercent(signal) < InpMinimumSignalBodyPercent)
   {
      g_signalRejects++;
      return false;
   }
   double rawStop = buy ? asianLow - InpStopBufferATR * atr
                        : asianHigh + InpStopBufferATR * atr;
   return OpenDriftPosition(buy, atr, rawStop);
}
...
int OnInit()
{
   if(InpATRPeriod < 2 || InpEntryHour < 0 || InpEntryHour > 23 ||
      InpEntryWindowBars < 1 || InpExitHour <= InpEntryHour || InpExitHour > 23 ||
      InpMinimumPriorDayMoveATR < 0.0 ||
      InpMaximumPriorDayMoveATR <= InpMinimumPriorDayMoveATR ||
      InpMinimumPriorDayBodyPercent < 0.0 || InpMinimumPriorDayBodyPercent > 100.0 ||
      InpMinimumAsianRangeATR < 0.0 ||
      InpMaximumAsianRangeATR <= InpMinimumAsianRangeATR ||
      InpMaximumAsianDriftATR <= InpMinimumAsianDriftATR ||
   ...
   {
      Print("M15ODC account contract failure: ", accountReason);
      return INIT_FAILED;
   }
...
{
   PrintFormat("M15ODC_DIAGNOSTIC daily_contexts=%d drift_candidates=%d signal_rejects=%d geometry_rejects=%d safety_rejects=%d minimum_lot_rejects=%d opened=%d order_failures=%d",
                g_dailyContexts, g_driftCandidates, g_signalRejects, g_geometryRejects,
