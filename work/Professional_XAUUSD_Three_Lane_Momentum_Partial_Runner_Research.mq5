...
#property description "Research fork with restart-safe momentum partial-profit runner"
...
input bool   InpMOUsePartialRunner = false;
input double InpMOPartialClosePercent = 60.0;
input double InpMOPartialTriggerR = 2.00;
input double InpMOPartialTargetR = 4.00;
input double InpMOPartialStopLockR = 1.25;
...
double ClosedPositionExitProfit(const ulong positionIdentifier)
{
   ...
   for(int i = 0; i < HistoryDealsTotal(); ++i)
   {
      ...
      if(deal == 0 || (ulong)HistoryDealGetInteger(deal, DEAL_POSITION_ID) != positionIdentifier)
         continue;
...
void PortfolioLossStreak(int &streak, datetime &lastLossTime)
{
   ...
   for(int i = HistoryDealsTotal() - 1; i >= 0; --i)
   {
      ...
      if(positionIdentifier == 0 || PositionIdentifierOpen(positionIdentifier))
         continue;
      bool processed = false;
      for(int j = 0; j < ArraySize(processedPositions); ++j)
      {
         if(processedPositions[j] == positionIdentifier)
         {
            processed = true;
            break;
         }
      ...
      if(processed)
         continue;
      int count = ArraySize(processedPositions);
      if(ArrayResize(processedPositions, count + 1) != count + 1)
      {
         g_persistenceHealthy = false;
         return;
      }
      processedPositions[count] = positionIdentifier;
      double profit = ClosedPositionExitProfit(positionIdentifier);
      if(profit >= 0.0)
         return;
      if(lastLossTime == 0)
...
private:
   ...
   {
      return "TLP_MO_PR_" + IntegerToString((long)AccountInfoInteger(ACCOUNT_LOGIN)) +
             "_" + IntegerToString((long)InpMOMagicNumber) + "_";
   }
                       ...
                       double &remainingVolume)
   {
      ...
      if(totalVolume <= 0.0 || step <= 0.0 || minimum <= 0.0)
         return false;
      closeVolume = MathFloor(totalVolume * InpMOPartialClosePercent /
   ...
   void CleanupOrphanedRunnerState()
   {
      ...
      for(int i = GlobalVariablesTotal() - 1; i >= 0; --i)
      {
         ...
         if(ticket == 0 || !SelectOwnedPosition(ticket, InpMOMagicNumber))
            DeleteRunnerState(ticket);
      }
   }
   ...
   bool ProtectRunnerPosition(const ulong ticket, const bool buy)
   {
      ...
      double protectedStop = NormalizeDouble(buy
                                             ? openPrice + InpMOPartialStopLockR * riskDistance
                                             : openPrice - InpMOPartialStopLockR * riskDistance,
                                             _Digits);
      double tolerance = MathMax(_Point, SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE)) * 0.5;
      bool alreadyProtected = buy ? oldSl >= protectedStop - tolerance
                                  : oldSl <= protectedStop + tolerance;
      if(alreadyProtected)
         return true;
      string modifyReason = "";
      if(ModifyOwnedPosition(m_trade, ticket, InpMOMagicNumber,
   ...
   void ManagePartialRunner()
   {
      ...
      for(int i = PositionsTotal() - 1; i >= 0; --i)
      {
         ...
         double openPrice = PositionGetDouble(POSITION_PRICE_OPEN);
         double riskDistance = GlobalVariableGet(RiskKey(ticket));
         double triggerPrice = buy ? openPrice + InpMOPartialTriggerR * riskDistance
                                   : openPrice - InpMOPartialTriggerR * riskDistance;
         ...
         bool partialConfirmed = CloseOwnedPositionPartial(m_trade, ticket, InpMOMagicNumber,
         ...
         WriteEvidenceEvent(m_logHandle, "tlp_mo_pr", "partial_runner", ticket,
                            buy ? "buy" : "sell", closeVolume, currentPrice,
                            PositionGetDouble(POSITION_SL), 0.0,
                            "2R trigger reached; protected remainder retained");
      }
   }
   ...
   bool OpenPosition(const bool buy, const double atr)
   {
      ...
      double closeVolume = 0.0, remainingVolume = 0.0;
      bool runnerRequested = InpMOUsePartialRunner &&
                             PartialVolumes(lots, closeVolume, remainingVolume);
      double targetR = runnerRequested ? InpMOPartialTargetR : InpMOTakeProfitR;
      ...
      {
         if(!SelectOwnedPosition(ticket, InpMOMagicNumber) ||
            !PartialVolumes(PositionGetDouble(POSITION_VOLUME), closeVolume, remainingVolume) ||
            !RegisterRunnerState(ticket, PositionGetDouble(POSITION_VOLUME)))
         {
            string rejectReason = "";
            return RejectPostFill(m_trade, InpMOMagicNumber,
                                  "momentum partial-runner registration", rejectReason);
         }
      }
      WriteEvidenceEvent(m_logHandle, "tlp_mom_e20", "entry", ticket,
                         buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
                         runnerRequested
                         ? "daily momentum plus fresh H1 breakout; partial runner eligible"
                         : "daily momentum plus fresh H1 breakout; standard 2R exit");
      return true;
   }
...
bool InputsValid()
{
   ...
   if(InpMOMomentumLookbackBars < 20 || InpMOEntryLookbackBars < 3 ||
      ...
      (InpMOUsePartialRunner &&
       (InpMOPartialClosePercent <= 0.0 || InpMOPartialClosePercent >= 100.0 ||
        InpMOPartialTriggerR < InpMOTakeProfitR ||
        InpMOPartialTargetR <= InpMOPartialTriggerR || InpMOPartialTargetR > 10.0 ||
        InpMOPartialStopLockR < 0.0 || InpMOPartialStopLockR >= InpMOPartialTriggerR)) ||
