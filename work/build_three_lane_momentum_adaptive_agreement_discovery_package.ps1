[CmdletBinding()]
param(
   [string]$SourcePath = 'work\Professional_XAUUSD_Three_Lane_Momentum_Adaptive_Agreement_Research.mq5',
   ...
   [string]$PackageDir = 'outputs\three_lane_momentum_adaptive_agreement_discovery_model1_package',
   [string]$ManifestPath = 'outputs\THREE_LANE_MOMENTUM_ADAPTIVE_AGREEMENT_DISCOVERY_MODEL1_MANIFEST.csv'
)
...
$expectedSourceHash = '6402A284BE2C4BDBEC2F44B8851650C64A6AEBAD87A35CF8CA2BD8A0275206D2'
...
& (Join-Path $PSScriptRoot 'test_three_lane_momentum_adaptive_agreement_source.ps1') | Out-Null
...
if($sourceHash -ne $expectedSourceHash) { throw "Agreement research source identity changed: $sourceHash" }
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_momentum_adaptive_agreement_discovery_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='maa_control';Role='control';Enabled='false';Risk='0.250'},
   [pscustomobject]@{Name='maa_r020';Role='conservative_neighbor';Enabled='true';Risk='0.200'},
   [pscustomobject]@{Name='maa_r0225';Role='lower_neighbor';Enabled='true';Risk='0.225'},
   [pscustomobject]@{Name='maa_center_r025';Role='center';Enabled='true';Risk='0.250'},
   [pscustomobject]@{Name='maa_r0275';Role='upper_neighbor';Enabled='true';Risk='0.275'}
)
$windows = @(
...
$stopRule = 'Data-informed architecture, sealed pre-2021 parameter discovery. Center positive/no worse than control in both eras; continuous net >= control +3%; CAGR >= control +0.05 point; PF/recovery/return-DD >= control; DD <=1.20% and <=control +0.10 point; trades >=control -2. At least 2 of 3 enabled neighbors pass both eras, >=2% net improvement, >=97% efficiency, DD <=1.25%, and trades >=control -2. No post-result rescue. Recent data is architecture-seen, not pristine holdout.'
...
foreach($variant in $variants) {
   ...
   Set-FixedInput $inputs 'InpMOUseAdaptiveAgreementRisk' $variant.Enabled
   Set-FixedInput $inputs 'InpMOAdaptiveAgreementRiskPercent' $variant.Risk
   ...
   foreach($window in $windows) {
      ...
      $rows.Add([pscustomobject][ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='momentum_adaptive_agreement_discovery_model1'
         Window=$window.Name;From=$window.From;To=$window.To;Model=1;Deposit=10000
         AdaptiveAgreementRiskEnabled=$variant.Enabled;AdaptiveAgreementRiskPercent=$variant.Risk
         BaseMomentumRiskPercent='0.15';ReversionRiskPercent='0.45';AdaptiveRiskPercent='0.15'
...
@(
   '# Momentum / Adaptive Agreement Allocation Discovery Contract','',
   ...
   '- Frozen center: an otherwise-valid momentum entry may request 0.25% risk only while an exact same-direction adaptive-trend position is already open; broker-valued sizing and the 0.75% account-wide cap remain authoritative.',
   '- Frozen neighborhood: disabled control plus agreement risk 0.20%, 0.225%, 0.25%, and 0.275%.',
