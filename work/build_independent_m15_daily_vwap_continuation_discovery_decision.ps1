param(
   [string]$QueuePath = "outputs\INDEPENDENT_M15_DAILY_VWAP_CONTINUATION_DISCOVERY_MODEL1_QUEUE.csv",
   [string]$ReportDir = "outputs\independent_m15_daily_vwap_continuation_discovery_model1_package\reports_here",
   [string]$ResultsPath = "outputs\INDEPENDENT_M15_DAILY_VWAP_CONTINUATION_DISCOVERY_MODEL1_RESULTS.csv",
   [string]$SummaryPath = "outputs\INDEPENDENT_M15_DAILY_VWAP_CONTINUATION_DISCOVERY_MODEL1_SUMMARY.csv",
   [string]$MetricsPath = "outputs\INDEPENDENT_M15_DAILY_VWAP_CONTINUATION_DISCOVERY_MODEL1_METRICS.md",
   [string]$DecisionCsvPath = "outputs\INDEPENDENT_M15_DAILY_VWAP_CONTINUATION_DISCOVERY_DECISION.csv",
   [string]$DecisionMarkdownPath = "outputs\INDEPENDENT_M15_DAILY_VWAP_CONTINUATION_DISCOVERY_DECISION.md"
)
...
$rawResults = Join-Path $repo "work\M15DVC_RAW_RESULTS.csv"
$rawSummary = Join-Path $repo "work\M15DVC_RAW_SUMMARY.csv"
$rawMarkdown = Join-Path $repo "work\M15DVC_RAW_METRICS.md"
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "collect_validation_results.ps1") `
   ...
   -OutResults "work\M15DVC_RAW_RESULTS.csv" -OutSummary "work\M15DVC_RAW_SUMMARY.csv" -OutMarkdown "work\M15DVC_RAW_METRICS.md" | Out-Null
...
$runnerByRank = @{}
foreach($workerPath in (Get-ChildItem -LiteralPath (Join-Path $repo "outputs") -Filter "M15_DAILY_VWAP_CONTINUATION_PORTABLE_*.csv" -ErrorAction SilentlyContinue)) {
...
foreach($item in ($queue | Sort-Object { [int]$_.QueueRank })) {
   ...
   $results.Add([pscustomobject]@{
      ...
      PullbackLookbackBars = $item.PullbackLookbackBars; MinimumADX = $item.MinimumADX
      TakeProfitR = $item.TakeProfitR; MinimumSignalBodyPercent = $item.MinimumSignalBodyPercent
      RequirePriorCloseBeyondVWAP = $item.RequirePriorCloseBeyondVWAP
      UseSignalVolumeFilter = $item.UseSignalVolumeFilter
...
if($results.Count -ne 36 -or @($results | Where-Object Status -ne "PARSED").Count -ne 0) {
   throw "Expected 36 parsed discovery reports."
}
$results | Export-Csv -LiteralPath (Resolve-RepoPath $ResultsPath) -NoTypeInformation -Encoding ASCII

$adjacency = @{
   m15dvc_center = @('m15dvc_pb2','m15dvc_pb4','m15dvc_adx16','m15dvc_adx20','m15dvc_adx22','m15dvc_tp150','m15dvc_tp250','m15dvc_body45','m15dvc_touch_only','m15dvc_volume105','m15dvc_session7_17')
   m15dvc_pb2 = @('m15dvc_center','m15dvc_pb4'); m15dvc_pb4 = @('m15dvc_center','m15dvc_pb2')
   m15dvc_adx16 = @('m15dvc_center','m15dvc_adx20'); m15dvc_adx20 = @('m15dvc_center','m15dvc_adx16','m15dvc_adx22'); m15dvc_adx22 = @('m15dvc_center','m15dvc_adx20')
   m15dvc_tp150 = @('m15dvc_center','m15dvc_tp250'); m15dvc_tp250 = @('m15dvc_center','m15dvc_tp150')
   m15dvc_body45 = @('m15dvc_center'); m15dvc_touch_only = @('m15dvc_center')
   m15dvc_volume105 = @('m15dvc_center'); m15dvc_session7_17 = @('m15dvc_center')
}
$numericPass = @{}
$candidateRows = @{}
foreach($group in ($results | Group-Object Candidate)) {
   ...
   $pass = [double]$older.NetProfit -gt 0.0 -and [double]$later.NetProfit -gt 0.0 -and `
           [double]$continuous.ProfitFactor -ge 1.30 -and [int]$continuous.TotalTrades -ge 120 -and `
           [double]$continuous.MaxDrawdownPercent -le 3.0
...
$md.Add('# Independent M15 Daily-VWAP Continuation Discovery Decision')
$md.Add('')
...
$md.Add('- Exported Model 1 reports: `36 / 36`')
$md.Add('- Discovery windows: `2015-2018`, `2019-2020`, and continuous `2015-2020`')
$md.Add("- Numeric gate passes: ``$numericPassCount / 12``")
$md.Add("- Neighbor-supported eligible profiles: ``$($eligibleRows.Count) / 12``")
$md.Add('')
...
if($gatePassed) {
   ...
   $md.Add('- Reject this daily-VWAP continuation family without inspecting 2021-2026.')
...
$metrics.Add('# Independent M15 Daily-VWAP Continuation Discovery Metrics')
$metrics.Add('')
...
$metrics.Add('')
$metrics.Add('See `outputs/INDEPENDENT_M15_DAILY_VWAP_CONTINUATION_DISCOVERY_DECISION.md` for the gated interpretation.')
