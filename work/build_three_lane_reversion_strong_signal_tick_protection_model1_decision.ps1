param(
   [string]$ManifestPath = "outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_MODEL1_MANIFEST.csv",
   [string]$ReportDir = "outputs\three_lane_reversion_strong_signal_tick_protection_model1_package\reports_here",
   [string]$ResultsPath = "outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_MODEL1_RESULTS.csv",
   [string]$SummaryPath = "outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_MODEL1_SUMMARY.csv",
   [string]$DecisionCsvPath = "outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_MODEL1_DECISION.csv",
   [string]$DecisionMarkdownPath = "outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_MODEL1_DECISION.md",
   [string]$RunAttestationPath = "outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_MODEL1_RUN_ATTESTATION.csv"
)
...
$controlName = "rvsstp_control"
$strongControlName = "rvsstp_strong_only"
$lowerName = "rvsstp_lock005"
$candidateName = "rvsstp_center"
$upperName = "rvsstp_trigger125"
...
$rawResults = "work\RVSSTPM1_RAW_RESULTS.csv"
$rawSummary = "work\RVSSTPM1_RAW_SUMMARY.csv"
$rawMetrics = "work\RVSSTPM1_RAW_METRICS.md"
...
$workerFiles = @(
   Get-ChildItem (Join-Path $repo "outputs") -Filter "THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_MODEL1_EXACT_?.csv" -File
   Get-ChildItem (Join-Path $repo "outputs") -Filter "THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_MODEL1_RETRY_?.csv" -File
) | Sort-Object Name
...
$lines.Add("# Three-Lane Reversion Strong-Signal Tick Protection Model 1 Decision")
$lines.Add("")
$lines.Add($(if($passed) {
...
$lines.Add("- Fixed every-tick protection neighborhood: ``1.00R / 0.05R``, ``1.00R / 0.10R``, ``1.25R / 0.10R``")
...
$lines.Add("")
$lines.Add("The branch keeps the completed-H1 body 0.25 / risk 0.70% allocation and evaluates one tightening-only stop path on every executable tick. The center changed behavior and improved strong-only drawdown from ``$($strongControl.MaxDrawdownPercent)%`` to ``$($candidate.MaxDrawdownPercent)%`` and recovery from ``$($strongControl.RecoveryFactor)`` to ``$($candidate.RecoveryFactor)``, but net fell to ``$(Money ([double]$candidate.NetProfit))`` and CAGR to ``$($candidate.CagrPercent)%``.")
$lines.Add("")
$lines.Add($(if($passed) {
   "The center and both tick-protection neighbors changed behavior and passed the preregistered Model 1 growth and efficiency gates. A fixed Model 4 real-tick comparison may open, but this is not a promotion and the forward candidate remains unchanged."
} else {
   "The center improved risk efficiency but failed the frozen growth and strong-only retention gates; the 1.25R neighbor also failed its independent support gate. No threshold is substituted inside this observed contract. Model 4 stays closed and ATB150 remains the historical champion. Center recovery difference versus strong-only: ``$('{0:F4}' -f ([double]$candidate.RecoveryFactor - [double]$strongControl.RecoveryFactor))``."
}))
$lines.Add("")
