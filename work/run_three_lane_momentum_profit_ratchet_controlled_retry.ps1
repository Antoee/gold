...
if(!$UserAuthorizedFocusRisk) { throw 'Controlled momentum profit-ratchet recovery requires explicit focus/window-risk authorization.' }
...
$manifest = Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_PROFIT_RATCHET_DISCOVERY_MODEL1_RETRY_MANIFEST.csv'
$source = Join-Path $repo 'outputs\three_lane_momentum_profit_ratchet_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
$expectedManifestHash = 'ED1C0015AD31C4AAD2B6102E40045DA260A39B52ED4FB570A817D5A37E8B92D6'
...
foreach($required in @($repoLock,$outerLock,$runner,$manifest,$source) + $roots) {
   if(!(Test-Path -LiteralPath $required)) { throw "Controlled momentum profit-ratchet recovery prerequisite missing: $required" }
}
if((Get-FileHash -LiteralPath $manifest -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedManifestHash) {
   throw 'Momentum profit-ratchet recovery manifest identity changed.'
}
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Momentum profit-ratchet package source identity changed.'
}
...
if((Test-Path -LiteralPath $unlockFile) -or (Test-Path -LiteralPath $focusAck) -or
   ...
   throw 'Controlled momentum profit-ratchet recovery refuses a pre-existing partial unlock state.'
}
if(@(Get-Process -Name terminal,terminal64,metatester,metatester64,MetaEditor,metaeditor64 -ErrorAction SilentlyContinue).Count -ne 0) {
   throw 'Controlled momentum profit-ratchet recovery requires zero pre-existing MT5-family processes.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Momentum profit-ratchet recovery $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Momentum profit-ratchet recovery focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
   ...
   & $runner -ManifestPath $manifest -PortableRoots $roots -UserAuthorizedFocusRisk `
      -OutputPrefix $(if($SingleWorkerRecovery) { 'THREE_LANE_MOMENTUM_PROFIT_RATCHET_RETRY_SINGLE_WORKER' } else { 'THREE_LANE_MOMENTUM_PROFIT_RATCHET_RETRY_WORKER' }) `
...
if(!$completed) { throw 'Controlled momentum profit-ratchet recovery did not complete.' }
if(!(Test-Path -LiteralPath $repoLock) -or !(Test-Path -LiteralPath $outerLock) -or
   ...
   throw 'Controlled momentum profit-ratchet recovery did not restore hard-lock state.'
}
