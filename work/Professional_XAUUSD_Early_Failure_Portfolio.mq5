...
input ulong  InpPortfolioMagic = 26071783;
...
input group "Closed-Bar No-Follow-Through Exits"
input bool   InpRVUseNoFollowThroughExit = true;
input int    InpRVNoFollowThroughBars = 3;
input double InpRVNoFollowThroughMaximumR = 0.00;
input bool   InpMOUseNoFollowThroughExit = true;
input int    InpMONoFollowThroughBars = 4;
input double InpMONoFollowThroughMaximumR = 0.00;

input group "H1 Band VWAP Reversion Lane"
input bool   InpRVEnabled = true;
input ulong  InpRVMagicNumber = 26071724;
...
input ulong  InpMOMagicNumber = 26071764;
...
bool SharedSafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "EFP-LIVE-ACK-v1"))
   {
...
private:
   ...
   bool TryNoFollowThroughExit(const ulong ticket, const bool buy)
   {
      if(!InpRVUseNoFollowThroughExit || !PositionSelectByTicket(ticket))
         return false;
      datetime openTime = (datetime)PositionGetInteger(POSITION_TIME);
      int openShift = iBarShift(_Symbol, InpRVSignalTimeframe, openTime, false);
      if(openShift < InpRVNoFollowThroughBars)
         return false;
      double openPrice = PositionGetDouble(POSITION_PRICE_OPEN);
      double stopPrice = PositionGetDouble(POSITION_SL);
      double current = buy ? SymbolInfoDouble(_Symbol, SYMBOL_BID)
                           : SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      double initialRisk = MathAbs(openPrice - stopPrice);
      if(openPrice <= 0.0 || stopPrice <= 0.0 || current <= 0.0 || initialRisk <= _Point)
         return false;
      double currentR = (buy ? current - openPrice : openPrice - current) / initialRisk;
      if(currentR > InpRVNoFollowThroughMaximumR)
         return false;
      m_trade.SetExpertMagicNumber(InpRVMagicNumber);
      m_trade.SetDeviationInPoints(InpRVDeviationPoints);
   ...
   bool ManagePositionOnBar()
   {
      for(int i = PositionsTotal() - 1; i >= 0; --i)
      {
         ...
         if(TryNoFollowThroughExit(ticket, buy))
            return true;
      }
      return false;
   }
...
private:
   ...
   bool TryNoFollowThroughExit(const ulong ticket, const bool buy)
   {
      if(!InpMOUseNoFollowThroughExit || !PositionSelectByTicket(ticket))
         return false;
      datetime openTime = (datetime)PositionGetInteger(POSITION_TIME);
      int openShift = iBarShift(_Symbol, InpMOSignalTimeframe, openTime, false);
      if(openShift < InpMONoFollowThroughBars)
         return false;
      double openPrice = PositionGetDouble(POSITION_PRICE_OPEN);
      ...
      if(openPrice <= 0.0 || current <= 0.0 || initialRisk <= _Point)
         return false;
      double currentR = (buy ? current - openPrice : openPrice - current) / initialRisk;
      if(currentR > InpMONoFollowThroughMaximumR)
         return false;
      m_trade.SetExpertMagicNumber(InpMOMagicNumber);
      m_trade.SetDeviationInPoints(InpMODeviationPoints);
      return m_trade.PositionClose(ticket);
...
bool InputsValid()
{
   if(InpRVMagicNumber == InpMOMagicNumber ||
      ...
      return false;
   if(InpRVNoFollowThroughBars < 1 || InpMONoFollowThroughBars < 1 ||
      InpRVNoFollowThroughMaximumR < -1.0 || InpRVNoFollowThroughMaximumR > 1.0 ||
      InpMONoFollowThroughMaximumR < -1.0 || InpMONoFollowThroughMaximumR > 1.0)
      return false;
...
void UpdateDashboard()
{
   ...
   Comment("XAUUSD Early Failure Portfolio\n",
