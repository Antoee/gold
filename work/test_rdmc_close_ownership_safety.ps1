...
Add-Check "forced-close marker clears only after disappearance" ($close.Contains('string forcedCloseKey = PostFillForcedCloseKey(ticket, magic);') -and $close.IndexOf('if(closed)', [StringComparison]::Ordinal) -lt $close.IndexOf('DeleteCriticalPersistentState(forcedCloseKey);', [StringComparison]::Ordinal)) "marker cleanup"
