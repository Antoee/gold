[CmdletBinding()]
param(
   [string]$SourcePath = 'work\Professional_XAUUSD_Three_Lane_Momentum_D1_Strength_Gate_Research.mq5',
   ...
   [string]$PackageDir = 'outputs\three_lane_momentum_d1_strength_gate_discovery_model1_package',
   [string]$ManifestPath = 'outputs\THREE_LANE_MOMENTUM_D1_STRENGTH_GATE_DISCOVERY_MODEL1_MANIFEST.csv'
)
...
& (Join-Path $PSScriptRoot 'test_three_lane_momentum_d1_strength_gate_source.ps1') | Out-Null
...
if($sourceHash -ne $expectedSourceHash) { throw "D1 momentum-strength research source identity changed: $sourceHash" }
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_momentum_d1_strength_gate_discovery_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='mds_control';Role='control';Enabled='false';MinimumPercent='0'},
   [pscustomobject]@{Name='mds_02';Role='lower_support';Enabled='true';MinimumPercent='2'},
   [pscustomobject]@{Name='mds_center_04';Role='center';Enabled='true';MinimumPercent='4'},
   [pscustomobject]@{Name='mds_06';Role='upper_support';Enabled='true';MinimumPercent='6'}
)
$windows = @(
...
$stopRule = 'Frozen pre-2021 D1 momentum-strength discovery. Every row must parse exactly and remain profitable. The 4% center must be no worse than control in both disjoint eras; continuous net >=control +2%; CAGR >=control +0.04 point; PF/recovery/return-DD no worse than control; DD <=control +0.05 point; trades >=85% of control. Both 2% and 6% enabled neighbors must remain positive in both eras, improve continuous net >=1%, CAGR >=control +0.02 point, retain >=98% of control PF/recovery/return-DD, DD <=control +0.08 point, retain >=80% of control trades, and change behavior. No post-result rescue.'
...
foreach($variant in $variants) {
   ...
   Set-FixedInput $inputs 'InpMOUseMinimumD1MomentumStrength' $variant.Enabled
   Set-FixedInput $inputs 'InpMOMinimumAbsoluteD1MomentumPercent' $variant.MinimumPercent
   ...
   foreach($window in $windows) {
      ...
      $rows.Add([pscustomobject][ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='momentum_d1_strength_gate_discovery_model1'
         Window=$window.Name;From=$window.From;To=$window.To;Model=1;Deposit=10000
         MinimumD1MomentumStrengthEnabled=$variant.Enabled;MinimumAbsoluteD1MomentumPercent=$variant.MinimumPercent
...
@(
   '# Momentum D1 Strength Gate Discovery Contract','',
   ...
   '- Frozen center: require the absolute 126-bar D1 close return to be at least 4% before a momentum breakout can enter; direction must still agree with the return sign.',
   '- Frozen support ladder: disabled control plus 2%, 4%, and 6% minimum strength. Requested risk, targets, stops, sizing, cooldown, and the 0.75% account-wide exposure cap are unchanged.',
