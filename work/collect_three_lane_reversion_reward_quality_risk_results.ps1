param(
   ...
   [string]$ReportDir = 'outputs\three_lane_reversion_reward_quality_risk_discovery_model1_package\reports_here',
   [string]$ParserManifestPath = 'outputs\THREE_LANE_REVERSION_REWARD_QUALITY_RISK_DISCOVERY_MODEL1_PARSE_MANIFEST.csv',
   [string]$RawResultsPath = 'outputs\THREE_LANE_REVERSION_REWARD_QUALITY_RISK_DISCOVERY_MODEL1_RAW_RESULTS.csv',
   ...
   [string]$SummaryPath = 'outputs\THREE_LANE_REVERSION_REWARD_QUALITY_RISK_DISCOVERY_SUMMARY.csv',
   [string]$MarkdownPath = 'outputs\THREE_LANE_REVERSION_REWARD_QUALITY_RISK_DISCOVERY_MODEL1_METRICS.md'
)
...
$initialRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath 'outputs') -Filter 'THREE_LANE_REVERSION_REWARD_QUALITY_RISK_WORKER_*.csv' |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
$recoveryRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath 'outputs') -Filter 'THREE_LANE_REVERSION_REWARD_QUALITY_RISK_RECOVERY_*.csv' |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
...
$results = foreach($row in $raw) {
   ...
   $q = $queued[0]
   [pscustomobject]@{
      QueueRank=$q.QueueRank;Candidate=$q.Candidate;CandidateRank=$q.CandidateRank;Role=$q.Role
      Phase=$q.Phase;Window=$q.Window;From=$q.From;To=$q.To;Model=$q.Model;Deposit=$q.Deposit
      StrongSignalRiskEnabled=$q.StrongSignalRiskEnabled;RewardQualityEnabled=$q.RewardQualityEnabled
      MinimumRiskReward=$q.MinimumRiskReward;StrongRiskPercent=$q.StrongRiskPercent
      StrongBodyRatio=$q.StrongBodyRatio;StrongLotCap=$q.StrongLotCap
