param(
   [string]$MetricsPath = "outputs\SPREAD_GUARD_PROBE_REPORT_METRICS.csv",
   [string]$OutCsv = "outputs\SPREAD_GUARD_PROBE_DECISION.csv",
   [string]$OutMarkdown = "outputs\SPREAD_GUARD_PROBE_DECISION.md",
...
$pairs = @(
   [pscustomobject]@{ Pair = "baseline_atr_spread_guard"; Candidate = "baseline_promoted_atr_spread_guard"; Baseline = "baseline_promoted" },
   [pscustomobject]@{ Pair = "tp38_sl18_atr_spread_guard"; Candidate = "tp38_sl18_atr_spread_guard"; Baseline = "tp38_sl18" }
)
...
foreach($pair in $pairs) {
   ...
   $reason = "Candidate or paired unguarded report is missing."
   ...
   elseif($candidateStatus -eq "PARSED" -and $baselineStatus -eq "PARSED") {
      if($candidateNet -lt 0) { $decision = "FAIL_CANDIDATE_LOSS"; $reason = "ATR spread-guard candidate lost money." }
      elseif($candidateNet -lt $baselineNet) { $decision = "FAIL_BELOW_UNGUARDED"; $reason = "ATR spread-guard candidate underperformed its unguarded pair." }
      elseif($candidateDd -gt $MaxAllowedDrawdownPercent) { $decision = "REVIEW_DRAWDOWN"; $reason = "ATR spread-guard candidate passed profit comparison but drawdown needs review." }
      else { $decision = "PASS_SPREAD_GUARD_PROBE"; $reason = "ATR spread-guard candidate was profitable and beat its unguarded pair." }
   }
...
$hasPass = @($rows | Where-Object { $_.Decision -eq "PASS_SPREAD_GUARD_PROBE" }).Count -gt 0
...
$overall = if($hasPass) { "HAS_SPREAD_GUARD_CANDIDATE" } elseif($hasRepair) { "REPAIR_REPORTS" } elseif($hasWaiting) { "WAITING_FOR_REPORTS" } elseif($hasReview) { "REVIEW_DRAWDOWN" } elseif($hasFail) { "NO_SPREAD_GUARD_CANDIDATE" } else { "REVIEW_REQUIRED" }

$md = New-Object System.Collections.Generic.List[string]
$md.Add("# ATR Spread Guard Probe Decision") | Out-Null
$md.Add("") | Out-Null
...
$md.Add("") | Out-Null
if($overall -eq "HAS_SPREAD_GUARD_CANDIDATE") { $md.Add("At least one ATR spread-guard candidate deserves broader stress/recent-OOS validation. Do not promote from this probe alone.") | Out-Null }
elseif($overall -eq "NO_SPREAD_GUARD_CANDIDATE") { $md.Add("No tested ATR spread-guard candidate beat its unguarded pair. Keep the current promoted profile unchanged.") | Out-Null }
elseif($overall -eq "REPAIR_REPORTS") { $md.Add("Repair or re-export ATR spread-guard probe reports before making a filter decision.") | Out-Null }
elseif($overall -eq "REVIEW_DRAWDOWN") { $md.Add("An ATR spread-guard candidate may have profit potential, but drawdown needs review before expanding validation.") | Out-Null }
else { $md.Add("Waiting for exported ATR spread-guard probe reports. Keep the current promoted profile unchanged.") | Out-Null }
