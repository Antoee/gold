...
if(!$UserAuthorizedFocusRisk) { throw 'Controlled momentum partial-runner interaction holdout requires explicit focus/window-risk authorization.' }
...
$manifest = Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_PARTIAL_RUNNER_INTERACTION_HOLDOUT_MODEL1_MANIFEST.csv'
$source = Join-Path $repo 'outputs\three_lane_momentum_partial_runner_interaction_holdout_model1_package\source\Professional_XAUUSD_EA.mq5'
$expectedManifestHash = 'C4F01E5DB151660484D3BE6301ABEFB1667D9C2FA05422932610FC2CCD723A72'
...
foreach($required in @($repoLock,$outerLock,$runner,$manifest,$source) + $roots) {
   if(!(Test-Path -LiteralPath $required)) { throw "Controlled momentum partial-runner prerequisite missing: $required" }
}
if((Get-FileHash -LiteralPath $manifest -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedManifestHash) {
   throw 'Momentum partial-runner manifest identity changed.'
}
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Momentum partial-runner package source identity changed.'
}
...
if((Test-Path -LiteralPath $unlockFile) -or (Test-Path -LiteralPath $focusAck) -or
   ...
   throw 'Controlled momentum partial-runner testing refuses a pre-existing partial unlock state.'
}
if(@(Get-Process -Name terminal,terminal64,metatester,metatester64,MetaEditor,metaeditor64 -ErrorAction SilentlyContinue).Count -ne 0) {
   throw 'Controlled momentum partial-runner testing requires zero pre-existing MT5-family processes.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Momentum partial-runner interaction holdout $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Momentum partial-runner interaction holdout focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
   ...
   & $runner -ManifestPath $manifest -PortableRoots $roots -UserAuthorizedFocusRisk `
      -OutputPrefix 'THREE_LANE_MOMENTUM_PARTIAL_RUNNER_INTERACTION_HOLDOUT_WORKER' `
...
if(!$completed) { throw 'Controlled momentum partial-runner interaction holdout did not complete.' }
if(!(Test-Path -LiteralPath $repoLock) -or !(Test-Path -LiteralPath $outerLock) -or
   ...
   throw 'Controlled momentum partial-runner interaction holdout did not restore hard-lock state.'
}
[pscustomobject][ordered]@{
   Status = 'CONTROLLED_INTERACTION_HOLDOUT_COMPLETE';StartedAtUtc = $startedAtUtc
   CompletedAtUtc = [DateTime]::UtcNow.ToString('o');Configurations = 12;Workers = 4
   MaxCpuPercent = $MaxCpuPercent;LaunchLocksRestored = $true;MT5Processes = 0
