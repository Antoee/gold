...
param(
   ...
   [string]$PackageDir = 'outputs\three_lane_reversion_joint_strong_allocation_discovery_model1_package',
   [string]$ManifestPath = 'outputs\THREE_LANE_REVERSION_JOINT_STRONG_ALLOCATION_DISCOVERY_MODEL1_MANIFEST.csv'
)
...
if($sourceHash -ne $expectedSourceHash) { throw "Published leader source identity changed: $sourceHash" }
if($leaderHash -ne $expectedLeaderHash) { throw "Leader profile identity changed: $leaderHash" }

$base = Get-SourceInputs $source
if($base.Count -ne 183) { throw "Expected 183 source inputs, found $($base.Count)." }
$leaderCount = 0
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_reversion_joint_strong_allocation_discovery_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='jsa_control';Role='control';RiskEnabled='false';LotCapEnabled='true';Body='0.250';Risk='0.600'},
   [pscustomobject]@{Name='jsa_risk_only_reference';Role='risk_only_reference';RiskEnabled='true';LotCapEnabled='false';Body='0.250';Risk='0.650'},
   [pscustomobject]@{Name='jsa_r060';Role='risk_lower_neighbor';RiskEnabled='true';LotCapEnabled='true';Body='0.250';Risk='0.600'},
   [pscustomobject]@{Name='jsa_center_r065';Role='center';RiskEnabled='true';LotCapEnabled='true';Body='0.250';Risk='0.650'},
   [pscustomobject]@{Name='jsa_r070';Role='risk_upper_neighbor';RiskEnabled='true';LotCapEnabled='true';Body='0.250';Risk='0.700'},
   [pscustomobject]@{Name='jsa_body0225';Role='body_lower_neighbor';RiskEnabled='true';LotCapEnabled='true';Body='0.225';Risk='0.650'},
   [pscustomobject]@{Name='jsa_body0275';Role='body_upper_neighbor';RiskEnabled='true';LotCapEnabled='true';Body='0.275';Risk='0.650'}
)
$windows = @(
...
$stopRule = 'Sealed pre-2021 joint-allocation discovery. Every report positive. Center no worse than control in both eras; continuous net >= control +3%; CAGR >= control +0.05 point; PF/recovery/return-DD >= control; DD <=1.20% and <=control +0.08 point; trades >=control -2. Center must beat the risk-only reference in net and return/DD. At least 3 of 4 orthogonal risk/body neighbors pass both eras, >=1.5% net improvement, >=0.03 CAGR point, >=97% efficiency, DD <=1.24%, and changed behavior. No post-result rescue.'
...
foreach($variant in $variants) {
   ...
   Set-FixedInput $inputs 'InpRVUseStrongSignalRisk' $variant.RiskEnabled
   Set-FixedInput $inputs 'InpRVUseStrongSignalLotCap' $variant.LotCapEnabled
   Set-FixedInput $inputs 'InpRVStrongSignalMinimumBodyRatio' $variant.Body
   ...
   foreach($window in $windows) {
      ...
      $rows.Add([pscustomobject][ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='joint_strong_allocation_discovery_model1'
         Window=$window.Name;From=$window.From;To=$window.To;Model=1;Deposit=10000;StrongSignalMinimumBodyRatio=$variant.Body
         StrongSignalRiskEnabled=$variant.RiskEnabled;StrongSignalRiskPercent=$variant.Risk
         StrongSignalLotCapEnabled=$variant.LotCapEnabled;StrongSignalMaximumPositionLots='0.15'
...
@(
   '# Joint Strong-Signal Reversion Allocation Discovery Contract','',
   ...
   '- Frozen center: completed-H1 body ratio >= 0.25 keeps the published 0.15-lot ceiling and requests 0.65% risk; broker-valued sizing and the 0.75% account-wide cap remain authoritative.',
   '- Frozen neighborhood: requested risk 0.60/0.70 and body thresholds 0.225/0.275, plus the exact published control and a risk-only 0.65% reference without the 0.15 strong lot cap.',
