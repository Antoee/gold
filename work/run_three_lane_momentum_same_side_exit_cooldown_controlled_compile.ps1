...
$source=Join-Path $repo 'outputs\three_lane_momentum_same_side_exit_cooldown_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'Same-side exit-cooldown research source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Momentum same-side exit-cooldown compile $startedAtUtc",[Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck,"Momentum same-side exit-cooldown focus acknowledgement $startedAtUtc",[Text.Encoding]::ASCII)
