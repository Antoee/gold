...
foreach($candidate in ($queue | Group-Object Candidate)) {
   ...
   foreach($required in @(
      ...
      "InpRVUseTwoBarReclaimConfirmation=", "InpRVRequirePriorCloseOutsideBand=", "InpRVRequireConfirmationProgress=",
      "InpMOUseTwoCloseBreakoutConfirmation=", "InpMORequireConfirmationProgress=",
...
$control = @($queue | Where-Object Candidate -eq "tbc_fixed_control")
Assert-True ($control.Count -eq 3 -and @($control.RVConfirmationEnabled | Sort-Object -Unique)[0] -eq "false" -and @($control.MOConfirmationEnabled | Sort-Object -Unique)[0] -eq "false") "Unchanged control missing."
