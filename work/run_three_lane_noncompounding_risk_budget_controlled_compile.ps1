...
$source = Join-Path $repo 'outputs\three_lane_noncompounding_risk_budget_model1_package\source\Professional_XAUUSD_EA.mq5'
$expectedSourceHash = 'B72F61E0633F5A57C3BC4D5688C8F7F29155B772F7D2BDE3EDC72429A41E9EA8'
...
$auditPath = Join-Path $repo 'outputs\THREE_LANE_NONCOMPOUNDING_RISK_BUDGET_COMPILE_AUDIT.csv'
$roots = @(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Noncompounding risk-budget source identity changed.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Noncompounding risk-budget compile $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Noncompounding risk-budget focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
