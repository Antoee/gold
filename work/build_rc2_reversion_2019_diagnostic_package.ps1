param(
   ...
   [string]$PackageDir = "outputs\rc2_reversion_2019_diagnostic_model1_package",
   [string]$QueueManifestPath = "outputs\RC2_REVERSION_2019_DIAGNOSTIC_MODEL1_QUEUE.csv",
   [string]$PackageManifestPath = "outputs\RC2_REVERSION_2019_DIAGNOSTIC_MODEL1_MANIFEST.csv",
   [string]$MarkdownPath = "outputs\RC2_REVERSION_2019_DIAGNOSTIC_CONTRACT.md"
)
...
Set-PinnedInput $inputs "InpRVLogFileName" "RC2_REVERSION_2019_DIAGNOSTIC_RV.csv" -StringValue
Set-PinnedInput $inputs "InpMOLogFileName" "RC2_REVERSION_2019_DIAGNOSTIC_MO.csv" -StringValue
Set-PinnedInput $inputs "InpEvidenceSourceHash" $sourceHash -StringValue
Set-PinnedInput $inputs "InpEvidenceRunLabel" "rc2_reversion_2019_diagnostic" -StringValue
...
$profileName = "rc2_reversion_2019_diagnostic.set"
$profilePath = Join-Path $profileDir $profileName
@($inputs.Keys | Sort-Object | ForEach-Object { $inputs[$_] }) |
   Set-Content -LiteralPath $profilePath -Encoding ASCII
$profileHash = (Get-FileHash -LiteralPath $profilePath -Algorithm SHA256).Hash

$configName = "001_rc2_reversion_2019_diagnostic_m1.ini"
$reportName = "rc2_reversion_2019_diagnostic_m1"
Write-SeasonalTesterConfig -Path (Join-Path $configDir $configName) -ReportRoot $reportDir `
   -ReportName $reportName -From "2019.01.01" -To "2019.12.31" `
   -Inputs $inputs -Model 1 -Deposit 10000

$stopRule = "Diagnostic only. The two 2019 reversion losses may identify a mechanism, but thresholds must be supported by 2015-2018 telemetry and validated on 2020 without movement."
$queue = [pscustomobject]@{
   QueueRank=1; Candidate="rc2_reversion_2019_diagnostic"; CandidateRank=1
   SourceType="behavior_preserving_telemetry_fork"; SourceRank=1; Phase="diagnostic_model1"
   Set=$profileName; Window="diagnostic_2019"; From="2019.01.01"; To="2019.12.31"
...
$manifest = [pscustomobject]@{
   QueueRank=1; Candidate="rc2_reversion_2019_diagnostic"; Phase="diagnostic_model1"
   PhaseLabel="RC2 reversion 2019 diagnostic Model1"
   Window="diagnostic_2019"; Model=1; Deposit=10000
