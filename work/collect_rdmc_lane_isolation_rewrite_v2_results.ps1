param(
   ...
   [string]$DecisionCsvPath = "outputs\RDMC_LANE_ISOLATION_REWRITE_V2_DECISION_FIXTURE.csv",
   [string]$ManifestPath = "outputs\RDMC_LANE_ISOLATION_REWRITE_V2_WAVE_01_MANIFEST.csv",
   [string]$ReportDir = "outputs\rdmc_lane_isolation_rewrite_v2_package\reports_here",
   [string]$RunnerLedgerGlob = "",
   [string]$ResultsPath = "outputs\RDMC_LANE_ISOLATION_REWRITE_V2_WAVE_01_RESULTS.csv",
   [string]$RunAuditPath = "",
...
if([string]::IsNullOrWhiteSpace($RunnerLedgerGlob)) {
   $RunnerLedgerGlob = "outputs\RDMC_LANE_ISOLATION_REWRITE_V2_WAVE_{0:D2}_WORKER_*.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($RunAuditPath)) {
   $RunAuditPath = "outputs\RDMC_LANE_ISOLATION_REWRITE_V2_WAVE_{0:D2}_RUN_AUDIT.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($RawResultsPath)) {
   $RawResultsPath = "outputs\RDMC_LANE_ISOLATION_REWRITE_V2_WAVE_{0:D2}_RAW_RESULTS.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($SummaryPath)) {
   $SummaryPath = "outputs\RDMC_LANE_ISOLATION_REWRITE_V2_WAVE_{0:D2}_SUMMARY.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($MetricsMarkdownPath)) {
   $MetricsMarkdownPath = "outputs\RDMC_LANE_ISOLATION_REWRITE_V2_WAVE_{0:D2}_METRICS.md" -f $Wave
}
...
if(!$SkipAdmissionRefresh) {
   if((Resolve-Path -LiteralPath $resultsFull).Path -ne (Join-Path $repo "outputs\RDMC_LANE_ISOLATION_REWRITE_V2_WAVE_01_RESULTS.csv")) {
