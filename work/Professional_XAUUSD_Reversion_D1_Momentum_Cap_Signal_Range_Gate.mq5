...
#property description "D1-cap portfolio research fork with optional H1 momentum signal-range gate"
...
input bool   InpMOUseMinimumSignalRangeGate = false;
input double InpMOMinimumSignalRangeATR = 1.25;
...
private:
   ...
   void TryEntry(const double atr)
   {
      ...
      if(close1 <= 0.0 || close2 <= 0.0)
         return;
      if(InpMOUseMinimumSignalRangeGate)
      {
         double signalRange = iHigh(_Symbol, InpMOSignalTimeframe, 1) -
                              iLow(_Symbol, InpMOSignalTimeframe, 1);
         if(signalRange <= 0.0 || signalRange < InpMOMinimumSignalRangeATR * atr)
            return;
      }
...
bool InputsValid()
{
   ...
   if(InpMOMomentumLookbackBars < 20 || InpMOEntryLookbackBars < 3 ||
      ...
      (InpMOUseMinimumSignalRangeGate &&
       (InpMOMinimumSignalRangeATR <= 0.0 || InpMOMinimumSignalRangeATR > 10.0)) ||
