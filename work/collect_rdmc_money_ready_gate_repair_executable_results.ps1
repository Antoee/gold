param(
   ...
   [string]$ReportDir = "outputs\rdmc_money_ready_gate_repair_executable_package\reports_here",
   [string]$RunnerLedgerGlob = "",
   [string]$ResultsPath = "outputs\RDMC_MONEY_READY_GATE_REPAIR_EXECUTABLE_RESULTS.csv",
   [string]$RunAuditPath = "",
...
if([string]::IsNullOrWhiteSpace($RunnerLedgerGlob)) {
   $RunnerLedgerGlob = "outputs\RDMC_MONEY_READY_GATE_REPAIR_EXECUTABLE_WAVE_{0:D2}_WORKER_*.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($RunAuditPath)) {
   $RunAuditPath = "outputs\RDMC_MONEY_READY_GATE_REPAIR_EXECUTABLE_WAVE_{0:D2}_RUN_AUDIT.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($RawResultsPath)) {
   $RawResultsPath = "outputs\RDMC_MONEY_READY_GATE_REPAIR_EXECUTABLE_WAVE_{0:D2}_RAW_RESULTS.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($SummaryPath)) {
   $SummaryPath = "outputs\RDMC_MONEY_READY_GATE_REPAIR_EXECUTABLE_WAVE_{0:D2}_SUMMARY.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($MetricsMarkdownPath)) {
   $MetricsMarkdownPath = "outputs\RDMC_MONEY_READY_GATE_REPAIR_EXECUTABLE_WAVE_{0:D2}_METRICS.md" -f $Wave
}
...
$waveManifestPath = "outputs\RDMC_MONEY_READY_GATE_REPAIR_EXECUTABLE_WAVE_{0:D2}_MANIFEST.csv" -f $Wave
...
if(!$SkipAdmissionRefresh) {
   if((Resolve-Path -LiteralPath $resultsFull).Path -ne (Join-Path $repo "outputs\RDMC_MONEY_READY_GATE_REPAIR_EXECUTABLE_RESULTS.csv")) {
