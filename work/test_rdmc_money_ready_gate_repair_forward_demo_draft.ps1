...
& (Join-Path $PSScriptRoot 'build_rdmc_money_ready_gate_repair_forward_demo_draft.ps1') | Out-Null

$baseProfilePath = Join-Path $repo 'outputs\rdmc_money_ready_gate_repair_package\profiles\rdmc_money_ready_gate_repair_v1.set'
...
$staticCsvPath = Join-Path $repo 'outputs\RDMC_MONEY_READY_GATE_REPAIR_FORWARD_DEMO_STATIC_READINESS.csv'
$manifestPath = Join-Path $repo 'outputs\RDMC_MONEY_READY_GATE_REPAIR_FORWARD_DEMO_DRAFT_MANIFEST.csv'
$testsCsvPath = Join-Path $repo 'outputs\RDMC_MONEY_READY_GATE_REPAIR_FORWARD_DEMO_DRAFT_TESTS.csv'
$testsMarkdownPath = Join-Path $repo 'outputs\RDMC_MONEY_READY_GATE_REPAIR_FORWARD_DEMO_DRAFT_TESTS.md'
...
if($registration.primaryExecutableManifestSha256 -ne 'EB48BDE3D67F9D16BAD427AB5ACC25BC8DFF8D8F29839EB95ADE615F59668972') { throw 'Primary executable manifest identity changed.' }
if($registration.secondBrokerManifestSha256 -ne '30A508459E0C408BFF9A905F5C9AEB01AF9D411C39165734F197CC2928CE6CB5') { throw 'Second-broker manifest identity changed.' }
...
if($failedStatic.Count -ne 0 -or $registration.staticReadinessStatus -ne 'PASS' -or @($registration.staticReadinessBlockers).Count -ne 0) {
   throw "Successor static readiness is not clean: $($failedStatic -join ', ')"
}
...
$testDir = Join-Path ([IO.Path]::GetTempPath()) 'rdmc-money-ready-gate-repair-forward-demo-draft-tests'
...
function Invoke-TestPreflight {
   return & (Join-Path $PSScriptRoot 'check_rdmc_money_ready_gate_repair_forward_demo_activation.ps1') `
...
if($validAccount.FailedGates -match 'static-trade-readiness|static-blocker-identity') { throw "The clean successor profile failed static readiness: $($validAccount.FailedGates)" }
foreach($requiredBlock in @('primary-executable-gate','executable-ledger-stress','distinct-broker-gate','candidate-compiled-binary','sentinel-compiled-binary')) {
...
$scenarios = @(
   [pscustomobject]@{Scenario='clean_10000_but_prerequisites_pending';ReadyToRegister=[bool]$validAccount.ReadyToRegister;ExpectedReady=$false;RequiredFailure='executable/stress/broker/binaries';Pass=$true},
...
$currentPreflight = & (Join-Path $PSScriptRoot 'check_rdmc_money_ready_gate_repair_forward_demo_activation.ps1')
if($currentPreflight.ReadyToRegister -or $currentPreflight.Status -ne 'REFUSED') { throw 'Current successor preflight was not fail-closed.' }
if($currentPreflight.FailedGates -match 'static-trade-readiness|static-blocker-identity') { throw 'Current successor preflight regressed static readiness.' }
