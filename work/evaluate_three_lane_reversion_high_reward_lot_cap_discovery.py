...
MANIFEST = OUTPUTS / "THREE_LANE_REVERSION_HIGH_REWARD_LOT_CAP_DISCOVERY_MODEL1_MANIFEST.csv"
RAW_RESULTS = OUTPUTS / "THREE_LANE_REVERSION_HIGH_REWARD_LOT_CAP_DISCOVERY_MODEL1_RESULTS.csv"
COMPILE_AUDIT = OUTPUTS / "THREE_LANE_REVERSION_HIGH_REWARD_LOT_CAP_COMPILE_AUDIT.csv"
RESULTS = OUTPUTS / "THREE_LANE_REVERSION_HIGH_REWARD_LOT_CAP_DISCOVERY_EVIDENCE.csv"
SUMMARY = OUTPUTS / "THREE_LANE_REVERSION_HIGH_REWARD_LOT_CAP_DISCOVERY_SUMMARY.csv"
DECISION = OUTPUTS / "THREE_LANE_REVERSION_HIGH_REWARD_LOT_CAP_DISCOVERY_DECISION.csv"
DECISION_MD = OUTPUTS / "THREE_LANE_REVERSION_HIGH_REWARD_LOT_CAP_DISCOVERY_DECISION.md"
EXPECTED_MANIFEST = "8CA894BA7C87358057D74B78693BCFA53F7B151B15B86241DB4C0DAD03D43BDC"
EXPECTED_SOURCE = "3CB0574945CD4A7A486408EF5BCE58648392383C43BFEE6EB1F58B424698302F"
EXPECTED_BINARY = "37CDEA025C4C66BD79022335B542E987775DC8CF1FCD7175D2C70C3E6558A8BD"
CONTROL = "hrlc_control"
REFERENCE = "hrlc_strong020_reference"
CENTER = "hrlc_center_rr250_cap020"
NEIGHBORS = ("hrlc_rr200", "hrlc_rr300", "hrlc_cap018", "hrlc_cap022")
...
def main() -> int:
    if sha256(MANIFEST) != EXPECTED_MANIFEST:
        raise ValueError("high-reward discovery manifest identity changed")
    manifest = read_csv(MANIFEST)
    if len(manifest) != 21 or len({row["Candidate"] for row in manifest}) != 7:
        raise ValueError("high-reward discovery topology changed")
    if any(
        ...
        raise ValueError("high-reward discovery source/model/capital/risk contract changed")
    for row in manifest:
    ...
    for path in OUTPUTS.glob("HIGH_REWARD_LOT_CAP_DISCOVERY_M1*.csv"):
    ...
    def neighbor_pass(candidate: str) -> bool:
        ...
        return (
            ...
            and float(candidate_row["NetProfit"]) >= 1.025 * float(control["NetProfit"])
            ...
            and float(candidate_row["MaxDrawdownPercent"]) <= 1.22
        )
    ...
    center_risk = (
        float(center["MaxDrawdownPercent"]) <= 1.18
    ...
    retention = reference_increment > 0 and retention_percent >= 50.0
    improves_reference = (
    ...
    labels = {
        ...
        "hrlc_rr200": "Adjusted RR 2.00 / cap 0.20",
        CENTER: "**Adjusted RR 2.50 / cap 0.20 center**",
        "hrlc_rr300": "Adjusted RR 3.00 / cap 0.20",
        "hrlc_cap018": "Adjusted RR 2.50 / cap 0.18",
        "hrlc_cap022": "Adjusted RR 2.50 / cap 0.22",
    }
    lines = [
    ...
    lines.extend(
        [
            ...
            f"- Center retained at least 50% of broad-reference incremental net: `{retention}` (`{retention_percent:.2f}%`)",
            ...
            "",
            f"The center improved net profit by only `{money(center_increment)}` over the exact leader control in the sealed continuous window. It failed the preregistered growth, CAGR, and neighborhood gates, so no recent-data or Model 4 budget was spent.",
            "",
