...
$source = Join-Path $repo 'outputs\three_lane_reversion_rejection_quality_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath = Join-Path $repo 'outputs\THREE_LANE_REVERSION_REJECTION_QUALITY_MODEL1_COMPILE_AUDIT.csv'
$roots = @(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Reversion rejection-quality source identity changed.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Reversion rejection-quality Model1 compile $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Reversion rejection-quality Model1 focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
