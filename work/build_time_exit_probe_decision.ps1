param(
   [string]$MetricsPath = "outputs\TIME_EXIT_PROBE_REPORT_METRICS.csv",
   [string]$OutCsv = "outputs\TIME_EXIT_PROBE_DECISION.csv",
   [string]$OutMarkdown = "outputs\TIME_EXIT_PROBE_DECISION.md",
...
$pairs = @(
   [pscustomobject]@{ Pair = "baseline_time_exit"; Candidate = "baseline_promoted_time_exit"; Baseline = "baseline_promoted" },
   [pscustomobject]@{ Pair = "tp38_sl18_time_exit"; Candidate = "tp38_sl18_time_exit"; Baseline = "tp38_sl18" }
)
...
foreach($pair in $pairs) {
   ...
   $reason = "Candidate or paired no-time-exit report is missing."
   ...
   elseif($candidateStatus -eq "PARSED" -and $baselineStatus -eq "PARSED") {
      if($candidateNet -lt 0) { $decision = "FAIL_CANDIDATE_LOSS"; $reason = "Time-exit candidate lost money." }
      elseif($candidateNet -lt $baselineNet) { $decision = "FAIL_BELOW_NO_TIME_EXIT"; $reason = "Time-exit candidate underperformed its no-time-exit pair." }
      elseif($candidateDd -gt $MaxAllowedDrawdownPercent) { $decision = "REVIEW_DRAWDOWN"; $reason = "Time-exit candidate passed profit comparison but drawdown needs review." }
      else { $decision = "PASS_TIME_EXIT_PROBE"; $reason = "Time-exit candidate was profitable and beat its no-time-exit pair." }
   }
...
$hasPass = @($rows | Where-Object { $_.Decision -eq "PASS_TIME_EXIT_PROBE" }).Count -gt 0
...
$overall = if($hasPass) { "HAS_TIME_EXIT_CANDIDATE" } elseif($hasRepair) { "REPAIR_REPORTS" } elseif($hasWaiting) { "WAITING_FOR_REPORTS" } elseif($hasReview) { "REVIEW_DRAWDOWN" } elseif($hasFail) { "NO_TIME_EXIT_CANDIDATE" } else { "REVIEW_REQUIRED" }

$md = New-Object System.Collections.Generic.List[string]
$md.Add("# Time Exit Probe Decision") | Out-Null
$md.Add("") | Out-Null
...
$md.Add("") | Out-Null
if($overall -eq "HAS_TIME_EXIT_CANDIDATE") { $md.Add("At least one time-exit candidate deserves broader stress/recent-OOS validation. Do not promote from this probe alone.") | Out-Null }
elseif($overall -eq "NO_TIME_EXIT_CANDIDATE") { $md.Add("No tested time-exit candidate beat its no-time-exit pair. Keep the current promoted profile unchanged.") | Out-Null }
elseif($overall -eq "REPAIR_REPORTS") { $md.Add("Repair or re-export time-exit probe reports before making a module decision.") | Out-Null }
elseif($overall -eq "REVIEW_DRAWDOWN") { $md.Add("A time-exit candidate may have profit potential, but drawdown needs review before expanding validation.") | Out-Null }
else { $md.Add("Waiting for exported time-exit probe reports. Keep the current promoted profile unchanged.") | Out-Null }
