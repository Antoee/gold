...
$source=Join-Path $repo 'outputs\three_lane_momentum_partial_runner_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_PARTIAL_RUNNER_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){
   throw 'Momentum partial-runner source identity changed.'
}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Momentum partial-runner compile $startedAtUtc",[Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck,"Momentum partial-runner focus acknowledgement $startedAtUtc",[Text.Encoding]::ASCII)
...
$binaryRows=@($roots|ForEach-Object{
   ...
   $s=(Get-FileHash -LiteralPath $installedSource -Algorithm SHA256).Hash.ToUpperInvariant()
   $b=(Get-FileHash -LiteralPath $installedBinary -Algorithm SHA256).Hash.ToUpperInvariant()
   [pscustomobject]@{SourceSha256=$s;BinarySha256=$b;IdentityReady=$identity.Count -ge 2 -and $identity[0] -eq $s -and $identity[1] -eq $b}
})
...
if(!(Test-Path -LiteralPath $repoLock) -or !(Test-Path -LiteralPath $outerLock) -or
   ...
   throw 'Controlled compile did not restore hard-lock state.'
}
