...
Assert-True ($queue.Count -eq 18) "Expected 18 discovery rows."
Assert-True (@($queue.Candidate | Sort-Object -Unique).Count -eq 6) "Expected six variants."
...
foreach($candidate in ($queue | Group-Object Candidate)) {
   ...
   foreach($required in @('InpRiskPercent=0.10','InpMaximumSimultaneousPositions=1','InpMaximumDailyLossPercent=0.75','InpMaximumEquityDrawdownPercent=5.00','InpUseRealAccountSafetyLock=true','InpAllowRealAccountTrading=false')) {
      Assert-True ($text.Contains($required)) "Profile safety contract missing: $required"
   }
}
[pscustomobject]@{ Status = 'PASS'; Rows = $queue.Count; Variants = 6; Windows = 3; HoldoutRows = 0 }
