...
#property description "Research fork with momentum-buy-only initial take-profit allocation"
...
input bool   InpMOUseBuyTakeProfitR = false;
input double InpMOBuyTakeProfitR = 2.50;
...
private:
   ...
   bool OpenPosition(const bool buy, const double atr)
   {
      ...
      double takeProfit = NormalizeDouble(buy ? entryPrice + requestedTakeProfitR * stopDistance
                                              : entryPrice - requestedTakeProfitR * stopDistance, _Digits);
      double lots = LotsForRisk(buy, entryPrice, stopPrice,
...
bool InputsValid()
{
   ...
   if(InpMOMomentumLookbackBars < 20 || InpMOEntryLookbackBars < 3 ||
      ...
      InpMOTakeProfitR <= 0.0 ||
      (InpMOUseBuyTakeProfitR &&
       (InpMOBuyTakeProfitR <= InpMOTakeProfitR || InpMOBuyTakeProfitR > 6.0)) ||
      InpMOMaximumHoldBars < 0 ||
