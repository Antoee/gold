param(
   [string]$QueuePath = "outputs\INDEPENDENT_M15_OVERNIGHT_DRIFT_CONTINUATION_DISCOVERY_MODEL1_QUEUE.csv",
   [string]$ReportDir = "outputs\independent_m15_overnight_drift_continuation_discovery_model1_package\reports_here",
   [string]$ResultsPath = "outputs\INDEPENDENT_M15_OVERNIGHT_DRIFT_CONTINUATION_DISCOVERY_MODEL1_RESULTS.csv",
   [string]$SummaryPath = "outputs\INDEPENDENT_M15_OVERNIGHT_DRIFT_CONTINUATION_DISCOVERY_MODEL1_SUMMARY.csv",
   [string]$MetricsPath = "outputs\INDEPENDENT_M15_OVERNIGHT_DRIFT_CONTINUATION_DISCOVERY_MODEL1_METRICS.md",
   [string]$DecisionCsvPath = "outputs\INDEPENDENT_M15_OVERNIGHT_DRIFT_CONTINUATION_DISCOVERY_DECISION.csv",
   [string]$DecisionMarkdownPath = "outputs\INDEPENDENT_M15_OVERNIGHT_DRIFT_CONTINUATION_DISCOVERY_DECISION.md"
)
...
$rawResults = Join-Path $repo "work\M15ODC_RAW_RESULTS.csv"
$rawSummary = Join-Path $repo "work\M15ODC_RAW_SUMMARY.csv"
$rawMarkdown = Join-Path $repo "work\M15ODC_RAW_METRICS.md"
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "collect_validation_results.ps1") `
   ...
   -OutResults "work\M15ODC_RAW_RESULTS.csv" -OutSummary "work\M15ODC_RAW_SUMMARY.csv" -OutMarkdown "work\M15ODC_RAW_METRICS.md" | Out-Null
...
$runnerByRank = @{}
foreach($workerPath in (Get-ChildItem -LiteralPath (Join-Path $repo "outputs") -Filter "INDEPENDENT_M15_OVERNIGHT_DRIFT_CONTINUATION_DISCOVERY_WORKER_*.csv" -ErrorAction SilentlyContinue)) {
...
$adjacency = @{
   odc_center = @('odc_prior020','odc_prior040','odc_priorbody35','odc_priorbody55','odc_asian045','odc_asian065','odc_drift000','odc_driftminus025','odc_entry6','odc_entry8','odc_signal25','odc_signal45','odc_tp125','odc_tp175')
   odc_prior020 = @('odc_center','odc_prior040'); odc_prior040 = @('odc_center','odc_prior020')
   odc_priorbody35 = @('odc_center','odc_priorbody55'); odc_priorbody55 = @('odc_center','odc_priorbody35')
   odc_asian045 = @('odc_center','odc_asian065'); odc_asian065 = @('odc_center','odc_asian045')
   odc_drift000 = @('odc_center','odc_driftminus025'); odc_driftminus025 = @('odc_center','odc_drift000')
   odc_entry6 = @('odc_center','odc_entry8'); odc_entry8 = @('odc_center','odc_entry6')
   odc_signal25 = @('odc_center','odc_signal45'); odc_signal45 = @('odc_center','odc_signal25')
   odc_tp125 = @('odc_center','odc_tp175'); odc_tp175 = @('odc_center','odc_tp125')
}
$numericPass = @{}
$candidateRows = @{}
...
$md.Add('# Independent M15 Overnight-Drift Continuation Discovery Decision')
$md.Add('')
...
$md.Add('- Exported Model 1 reports: `45 / 45`')
$md.Add('- Discovery windows: `2015-2018`, `2019-2020`, and continuous `2015-2020`')
$md.Add("- Numeric gate passes: ``$numericPassCount / 15``")
$md.Add("- Neighbor-supported eligible profiles: ``$($eligibleRows.Count) / 15``")
$md.Add('')
...
if($gatePassed) {
   ...
   $md.Add('- Reject this overnight-drift continuation family without inspecting 2021-2026.')
...
$metrics.Add('# Independent M15 Overnight-Drift Continuation Discovery Metrics')
$metrics.Add('')
...
$metrics.Add('')
$metrics.Add('See `outputs/INDEPENDENT_M15_OVERNIGHT_DRIFT_CONTINUATION_DISCOVERY_DECISION.md` for the gated interpretation.')
