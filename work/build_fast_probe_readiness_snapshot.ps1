...
function Get-Value { param([object]$Row, [string]$Name, [object]$Default = "") if($null -eq $Row) { return $Default }; $property = $Row.PSObject.Properties[$Name]; if($null -eq $property) { return $Default }; return $property.Value }
function Add-Probe {
   param([System.Collections.Generic.List[object]]$Rows,[string]$Gate,[string]$Path,[string[]]$PassDecisions,[string[]]$FailPrefixes,[string]$MissingAction,[string]$PassAction,[string]$FailAction)
   $data = Read-CsvSafe $Path
   if($data.Count -eq 0) { $Rows.Add([pscustomobject]@{ Gate=$Gate; Status="WAITING_FOR_REPORTS"; Rows=0; DecisionCounts="missing=$Path"; NextAction=$MissingAction }) | Out-Null; return }
   ...
   $hasFail = $false
   foreach($row in $data) { $decision = [string](Get-Value $row "Decision" ""); foreach($prefix in $FailPrefixes) { if($decision -like "$prefix*") { $hasFail = $true } } }
   ...
   $Rows.Add([pscustomobject]@{ Gate=$Gate; Status=$status; Rows=$data.Count; DecisionCounts=$decisionCounts; NextAction=$next }) | Out-Null
}

$rows = New-Object System.Collections.Generic.List[object]
Add-Probe $rows "STRESS_SMOKE" "outputs\STRESS_SMOKE_DECISION.csv" @("PASS_WINDOW") @("FAIL_") "Run/import the 2-row stress smoke reports first." "Proceed to STRESS_MICRO only if the smoke pair passed." "Reject or deprioritize the candidate that failed the smoke window."
Add-Probe $rows "STRESS_MICRO" "outputs\MICRO_TEST_DECISION.csv" @("PASS_WINDOW") @("FAIL_") "Run/import stress micro reports after stress smoke passes." "Proceed to RECENT_OOS only if every paired stress window passed." "Reject or deprioritize the candidate that failed a stress window."
...
Add-Probe $rows "NEWS_FILTER_PROBE" "outputs\NEWS_FILTER_PROBE_DECISION.csv" @("PASS_NEWS_FILTER_PROBE", "PASS_RISK_REDUCTION_REVIEW") @("FAIL_") "Run/import news-filter probe reports." "Expand passing news-filter profile into stress/recent-OOS validation." "Keep news filter disabled for failed variants."
...
$report.Add("# Fast Probe Readiness Snapshot") | Out-Null; $report.Add("") | Out-Null; $report.Add("Offline snapshot only. No MT5 process was launched.") | Out-Null; $report.Add("") | Out-Null; $report.Add("- Overall: **$overall**") | Out-Null; $report.Add("") | Out-Null
$report.Add("| Gate | Status | Rows | Decision Counts | Next Action |") | Out-Null; $report.Add("|---|---|---:|---|---|") | Out-Null
foreach($row in $rows) { $counts = ([string]$row.DecisionCounts) -replace '\|', '/'; $next = ([string]$row.NextAction) -replace '\|', '/'; $report.Add("| $($row.Gate) | $($row.Status) | $($row.Rows) | $counts | $next |") | Out-Null }
$report.Add("") | Out-Null; $report.Add("## Bottom Line") | Out-Null; $report.Add("") | Out-Null
if($overall -eq "FAST_GATES_READY_FOR_EXPANSION") { $report.Add("Fast gates have usable evidence for expansion decisions, but no fast probe alone can promote a live/default profile.") | Out-Null } elseif($overall -eq "PARTIAL_FAST_EVIDENCE") { $report.Add("Some fast probes have usable evidence. Expand only passing candidates and keep missing/failed gates out of promotion decisions.") | Out-Null } else { $report.Add("Fast probes are still waiting for exported reports. Keep the current promoted profile unchanged.") | Out-Null }
