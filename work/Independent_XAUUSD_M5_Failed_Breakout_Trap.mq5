...
input ulong  InpMagicNumber = 26071781;
...
input string InpLogFileName = "Independent_XAUUSD_M5_Failed_Breakout_Trap_Trades.csv";
...
{
   return "IM5FBT_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "M5FBT-LIVE-ACK"))
   {
                      ...
                      const double boxLow)
{
   ...
   string comment = buy ? "M5FBT_BUY" : "M5FBT_SELL";
...
{
   PrintFormat("M5FBT_DIAGNOSTIC breaks=%d reclaims=%d signals=%d box_rejects=%d break_rejects=%d reclaim_rejects=%d regime_rejects=%d safety_rejects=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
