...
$source=Join-Path $repo 'outputs\three_lane_reversion_partial_runner_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$repoLock=Join-Path $PSScriptRoot 'MT5_LOCAL_LAUNCH_DISABLED.lock';$outerLock=Join-Path $sharedWork 'MT5_LOCAL_LAUNCH_DISABLED.lock'
$unlockFile=Join-Path $PSScriptRoot 'ALLOW_MT5_LOCAL_LAUNCH.unlock';$focusAck=Join-Path $PSScriptRoot 'ALLOW_MT5_HIDDEN_DESKTOP_ACK.unlock'
$auditPath=Join-Path $repo 'outputs\THREE_LANE_REVERSION_PARTIAL_RUNNER_COMPILE_AUDIT.csv'
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'Partial-runner source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Partial-runner compile $startedAtUtc",[Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck,"Partial-runner focus acknowledgement $startedAtUtc",[Text.Encoding]::ASCII)
   ...
   $candidates=@($compileOutput|Where-Object{$_.PSObject.Properties.Name -contains 'PortableBinarySha256'});if($candidates.Count){$compileResult=$candidates[-1]}
}finally{
   Remove-Item -LiteralPath $unlockFile,$focusAck -Force -ErrorAction SilentlyContinue;Remove-Item Env:ALLOW_MT5_FOCUS_RISK -ErrorAction SilentlyContinue;Remove-Item Env:ALLOW_MT5_HIDDEN_DESKTOP_ACK -ErrorAction SilentlyContinue
...
$binaryRows=@($roots|ForEach-Object{
   $installedSource=Join-Path $_ 'MQL5\Experts\Professional_XAUUSD_EA.mq5';$installedBinary=Join-Path $_ 'MQL5\Experts\Professional_XAUUSD_EA.ex5';$identityPath=Join-Path $_ 'MQL5\Experts\Professional_XAUUSD_EA.compiled_identity.txt'
   $identity=@(Get-Content -LiteralPath $identityPath);$s=(Get-FileHash -LiteralPath $installedSource -Algorithm SHA256).Hash.ToUpperInvariant();$b=(Get-FileHash -LiteralPath $installedBinary -Algorithm SHA256).Hash.ToUpperInvariant()
