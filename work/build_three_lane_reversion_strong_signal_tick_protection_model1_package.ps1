...
param(
   ...
   [string]$PackageDir = 'outputs\three_lane_reversion_strong_signal_tick_protection_model1_package',
   [string]$QueuePath = 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_MODEL1_QUEUE.csv',
   [string]$ManifestPath = 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_MODEL1_MANIFEST.csv',
   [string]$PackageMarkdownPath = 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_MODEL1_PACKAGE.md',
   [string]$ContractPath = 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_MODEL1_CONTRACT.md'
)
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_reversion_strong_signal_tick_protection_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='rvsstp_control';Role='control';Strong='false';Protection='false';TriggerR='1.00';LockR='0.10'},
   [pscustomobject]@{Name='rvsstp_strong_only';Role='strong_control';Strong='true';Protection='false';TriggerR='1.00';LockR='0.10'},
   [pscustomobject]@{Name='rvsstp_lock005';Role='lower_neighbor';Strong='true';Protection='true';TriggerR='1.00';LockR='0.05'},
   [pscustomobject]@{Name='rvsstp_center';Role='center';Strong='true';Protection='true';TriggerR='1.00';LockR='0.10'},
   [pscustomobject]@{Name='rvsstp_trigger125';Role='upper_neighbor';Strong='true';Protection='true';TriggerR='1.25';LockR='0.10'}
)
$windows = @(
...
$stopRule = 'Tick-granular strong-signal protection. Center must change trading metrics, keep every era >= disabled control, continuous net >= control +5% and >=97% of strong-only, CAGR >= control +0.10 points and >= strong-only -0.03 points, PF/recovery/return-DD >= strong-only, DD <= strong-only and <=1.25%, and >=400 trades. Both protection neighbors must change trading metrics and keep every era >= control, net >= control +3%, CAGR >= control +0.05 points, PF/recovery/return-DD >= control, DD <=1.25%, and >=400 trades. No post-result retuning.'
...
foreach($variant in $variants) {
   ...
   foreach($window in $windows) {
      ...
      $common = [ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='reversion_strong_signal_tick_protection_model1'
...
@(
   '# Three-Lane Reversion Strong-Signal Tick Protection Model 1 Package','',
   ...
   '- This separately preregistered follow-up freezes body ratio 0.25 and requested risk 0.70%, then tests tightening-only protection on every executable tick for those strong reversion positions.',
   '- The optional stop manager uses the position open price and the exact stored initial risk; on every tick it can only move an owned stop toward a 0.05R or 0.10R lock after 1.00R or 1.25R favorable movement.',
