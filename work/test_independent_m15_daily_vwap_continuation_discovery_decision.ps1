...
$olderEra = @($results | Where-Object Window -eq "older_2015_2018")
...
if($results.Count -ne 36) { throw "Expected 36 discovery reports, found $($results.Count)." }
if($candidateNames.Count -ne 12) { throw "Expected 12 discovery candidates, found $($candidateNames.Count)." }
if($olderEra.Count -ne 12 -or $laterEra.Count -ne 12 -or $continuous.Count -ne 12) { throw "Each candidate must have all three discovery windows." }
...
if(@($olderEra | Where-Object { [double]$_.NetProfit -ge 0.0 }).Count -ne 0) { throw "An older-era candidate is no longer losing; rebuild and review the decision." }
...
if([int]$decision.Candidates -ne 12 -or [int]$decision.ReportsParsed -ne 36 -or [int]$decision.NumericPasses -ne 0) { throw "Decision counts do not match the discovery evidence." }
if($decision.HoldoutPermitted -ne "False" -or $decision.Model4Opened -ne "False") { throw "Rejected discovery incorrectly permitted holdout or opened Model4 testing." }
