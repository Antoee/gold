...
#property description "Research-only three-lane XAUUSD portfolio with a protected strong-reversion winner add-on"
...
input bool   InpRVUseStrongSignalProtection = false;
input double InpRVStrongSignalProtectionTriggerR = 1.00;
input double InpRVStrongSignalProtectionLockR = 0.10;
input bool   InpRVUseProtectedWinnerAddOn = false;
input ulong  InpRVAddOnMagicNumber = 26071722;
input double InpRVAddOnTriggerR = 1.00;
input double InpRVAddOnPrimaryLockR = 0.50;
input double InpRVAddOnRiskPercent = 0.15;
input double InpRVAddOnLockedProfitCoverage = 1.25;
input double InpRVAddOnMinimumRemainingRR = 1.20;
...
{
   return magic == InpPortfolioMagic || magic == InpRVMagicNumber ||
          magic == InpRVAddOnMagicNumber || magic == InpMOMagicNumber ||
          magic == InpATBMagicNumber;
}
...
bool AccountWideExposureAllowsWithStopOverride(const bool buy,
                                                const double entryPrice,
                                                const double stopPrice,
                                                const double lots,
                                                const ulong overrideTicket,
                                                const double overrideStop,
                                                string &reason)
{
   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   if(equity <= 0.0 || InpMaximumPortfolioOpenRiskPercent <= 0.0)
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
   {
      ...
      if(ticket == 0 || !PositionSelectByTicket(ticket))
         continue;
      positionCount++;
      if(ticket != overrideTicket)
      {
         bool unprotected = false;
         riskMoney += AccountPositionRiskMoney(ticket, unprotected);
         hasUnprotected = hasUnprotected || unprotected;
         continue;
      }
      ...
      double volume = PositionGetDouble(POSITION_VOLUME);
      long positionType = PositionGetInteger(POSITION_TYPE);
      if(StringLen(symbol) <= 0 || openPrice <= 0.0 || volume <= 0.0 || overrideStop <= 0.0)
      {
         hasUnprotected = true;
         continue;
      }
      bool overrideBuy = positionType == POSITION_TYPE_BUY;
      if(positionType != POSITION_TYPE_BUY && positionType != POSITION_TYPE_SELL)
      {
         hasUnprotected = true;
         continue;
      }
      bool lockedWinner = overrideBuy ? overrideStop >= openPrice : overrideStop <= openPrice;
      if(lockedWinner)
         continue;
      double overriddenRisk = 0.0;
      ENUM_ORDER_TYPE overrideType = overrideBuy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
      if(!RiskMoneyForOrder(symbol, overrideType, openPrice, overrideStop, volume, overriddenRisk))
         hasUnprotected = true;
      else
         riskMoney += overriddenRisk;
   }
   if(hasUnprotected && InpBlockUnprotectedAccountExposure)
   {
      reason = "unprotected account exposure";
   ...
   if(100.0 * (riskMoney + addedRiskMoney) / equity > InpMaximumPortfolioOpenRiskPercent)
   {
      reason = "portfolio open risk limit";
      return false;
   }
...
private:
   ...
   bool IsReversionMagic(const ulong magic)
   {
      return magic == InpRVMagicNumber || magic == InpRVAddOnMagicNumber;
   }

   string StrongRiskKey(const ulong ticket)
   {
      return "TLP_RV_STRONG_RISK_" + IntegerToString((long)ticket);
   }

   string AddOnAttemptKey(const ulong ticket)
   {
      return "TLP_RV_ADD_ATTEMPT_" + IntegerToString((long)ticket);
   }

   bool RegisterStrongRisk(const ulong ticket)
   {
      if(!SelectOwnedPosition(ticket, InpRVMagicNumber))
         return false;
      double riskDistance = MathAbs(PositionGetDouble(POSITION_PRICE_OPEN) -
                                    PositionGetDouble(POSITION_SL));
      return riskDistance > _Point && VerifiedGlobalSet(StrongRiskKey(ticket), riskDistance);
   }

   void ImproveStrongProtectiveStop(const ulong ticket)
   {
      string riskKey = StrongRiskKey(ticket);
      if(!InpRVUseStrongSignalProtection || !GlobalVariableCheck(riskKey) ||
         !SelectOwnedPosition(ticket, InpRVMagicNumber))
         return;
      double initialRisk = GlobalVariableGet(riskKey);
      double openPrice = PositionGetDouble(POSITION_PRICE_OPEN);
      double oldSl = PositionGetDouble(POSITION_SL);
      ...
      if(initialRisk <= _Point || openPrice <= 0.0 || oldSl <= 0.0 || current <= 0.0)
         return;
      double favorable = buy ? current - openPrice : openPrice - current;
      if(favorable < InpRVStrongSignalProtectionTriggerR * initialRisk)
         return;
      double newSl = buy ? openPrice + InpRVStrongSignalProtectionLockR * initialRisk
                         : openPrice - InpRVStrongSignalProtectionLockR * initialRisk;
      newSl = NormalizeDouble(newSl, _Digits);
      double minimumDistance = (double)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL) * _Point;
      bool improves = buy ? newSl > oldSl + _Point * 0.5 : newSl < oldSl - _Point * 0.5;
      bool valid = buy ? newSl > 0.0 && newSl < current - minimumDistance
                       : newSl > current + minimumDistance;
      if(!improves || !valid)
         return;
      string modifyReason = "";
      if(ModifyOwnedPosition(m_trade, ticket, InpRVMagicNumber,
                             newSl, takeProfit, modifyReason))
         WriteEvidenceEvent(m_logHandle, "tlp_rv_m12", "protect", ticket,
                            buy ? "buy" : "sell", PositionGetDouble(POSITION_VOLUME),
                            current, newSl, 0.0, "strong-signal stop lock");
   }

   void ManageStrongPositionOnTick()
   {
      if(!InpRVUseStrongSignalProtection)
         return;
      for(int i = PositionsTotal() - 1; i >= 0; --i)
      {
         ulong ticket = PositionGetTicket(i);
         if(ticket != 0)
            ImproveStrongProtectiveStop(ticket);
      }
   }
   ...
   int ManagedPositionCount()
   {
      ...
      for(int i = PositionsTotal() - 1; i >= 0; --i)
      {
         ...
         if(PositionGetString(POSITION_SYMBOL) == _Symbol &&
            IsReversionMagic((ulong)PositionGetInteger(POSITION_MAGIC)))
            count++;
      }
      return count;
   }
   ...
   double ClosedProfitSince(const datetime fromTime)
   {
      ...
      for(int i = 0; i < HistoryDealsTotal(); ++i)
      {
         ulong ticket = HistoryDealGetTicket(i);
         if(ticket == 0 || HistoryDealGetString(ticket, DEAL_SYMBOL) != _Symbol ||
            !IsReversionMagic((ulong)HistoryDealGetInteger(ticket, DEAL_MAGIC)))
            continue;
         long entryType = HistoryDealGetInteger(ticket, DEAL_ENTRY);
         if(entryType != DEAL_ENTRY_OUT && entryType != DEAL_ENTRY_OUT_BY && entryType != DEAL_ENTRY_INOUT)
            continue;
   ...
   bool AddOnSafetyAllows(string &reason)
   {
      if(!SharedSafetyAllows(reason))
         return false;
      double dailyProfit = ClosedProfitSince(g_dayStart);
      ...
      if(CountOwnedMagicPositions(InpRVMagicNumber) != 1 ||
         CountOwnedMagicPositions(InpRVAddOnMagicNumber) != 0 ||
         ManagedPositionCount() != 1)
      {
         reason = "reversion add-on position topology";
         return false;
      }
      if(SpreadPoints() > InpRVMaximumSpreadPoints)
      {
   ...
   bool TryProtectedWinnerAddOnOnTick()
   {
      if(!InpRVUseProtectedWinnerAddOn)
         return false;
      string safetyReason = "";
      if(!AddOnSafetyAllows(safetyReason))
         return false;

      ulong primaryTicket = 0;
      string findReason = "";
      if(!FindOnlyOwnedPosition(InpRVMagicNumber, primaryTicket, findReason) ||
         GlobalVariableCheck(AddOnAttemptKey(primaryTicket)) ||
         !GlobalVariableCheck(StrongRiskKey(primaryTicket)) ||
         !SelectOwnedPosition(primaryTicket, InpRVMagicNumber))
         return false;

      bool buy = PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY;
      double primaryOpen = PositionGetDouble(POSITION_PRICE_OPEN);
      double primaryOldSl = PositionGetDouble(POSITION_SL);
      double primaryTarget = PositionGetDouble(POSITION_TP);
      double primaryVolume = PositionGetDouble(POSITION_VOLUME);
      double initialRisk = GlobalVariableGet(StrongRiskKey(primaryTicket));
      MqlTick tick;
      if(!SymbolInfoTick(_Symbol, tick))
         return false;
      double executablePrice = buy ? tick.ask : tick.bid;
      double favorablePrice = buy ? tick.bid : tick.ask;
      if(primaryOpen <= 0.0 || primaryOldSl <= 0.0 || primaryTarget <= 0.0 ||
         primaryVolume <= 0.0 || initialRisk <= _Point ||
         executablePrice <= 0.0 || favorablePrice <= 0.0)
         return false;
      double favorable = buy ? favorablePrice - primaryOpen : primaryOpen - favorablePrice;
      if(favorable < InpRVAddOnTriggerR * initialRisk)
         return false;

      double desiredLock = buy ? primaryOpen + InpRVAddOnPrimaryLockR * initialRisk
                               : primaryOpen - InpRVAddOnPrimaryLockR * initialRisk;
      desiredLock = NormalizeDouble(desiredLock, _Digits);
      double minimumDistance = (double)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL) * _Point;
      bool validLock = buy ? desiredLock > primaryOpen && desiredLock < tick.bid - minimumDistance
                           : desiredLock < primaryOpen && desiredLock > tick.ask + minimumDistance;
      bool targetAhead = buy ? primaryTarget > executablePrice : primaryTarget < executablePrice;
      bool stopBehind = buy ? desiredLock < executablePrice - minimumDistance
                            : desiredLock > executablePrice + minimumDistance;
      if(!validLock || !targetAhead || !stopBehind)
         return false;

      double addedRiskDistance = MathAbs(executablePrice - desiredLock);
      double remainingReward = MathAbs(primaryTarget - executablePrice);
      if(addedRiskDistance <= _Point ||
         remainingReward / addedRiskDistance < InpRVAddOnMinimumRemainingRR)
         return false;
      double lots = LotsForRisk(buy, executablePrice, desiredLock,
                                InpRVAddOnRiskPercent, InpRVMaximumPositionLots);
      ENUM_ORDER_TYPE orderType = buy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
      double addedRiskMoney = 0.0;
      double lockedProfitMoney = 0.0;
      if(lots <= 0.0 ||
         !RiskMoneyForOrder(_Symbol, orderType, executablePrice, desiredLock,
                            lots, addedRiskMoney) ||
         !OrderCalcProfit(orderType, _Symbol, primaryVolume, primaryOpen,
                          desiredLock, lockedProfitMoney) ||
         lockedProfitMoney <= 0.0 ||
         lockedProfitMoney + 1e-8 < addedRiskMoney * InpRVAddOnLockedProfitCoverage)
         return false;

      string exposureReason = "";
      if(!AccountWideExposureAllowsWithStopOverride(buy, executablePrice, desiredLock,
                                                     lots, primaryTicket, desiredLock,
                                                     exposureReason))
         return false;
      if(!VerifiedGlobalSet(AddOnAttemptKey(primaryTicket), 1.0))
         return false;

      bool needsImprovement = buy ? desiredLock > primaryOldSl + _Point * 0.5
                                  : desiredLock < primaryOldSl - _Point * 0.5;
      if(needsImprovement)
      {
         string modifyReason = "";
         if(!ModifyOwnedPosition(m_trade, primaryTicket, InpRVMagicNumber,
                                 desiredLock, primaryTarget, modifyReason) ||
            !SelectOwnedPosition(primaryTicket, InpRVMagicNumber))
            return false;
      }
      double actualLock = PositionGetDouble(POSITION_SL);
      bool protectedWinner = buy ? actualLock >= desiredLock - _Point * 0.5
                                 : actualLock <= desiredLock + _Point * 0.5;
      if(!protectedWinner ||
         !OrderCalcProfit(orderType, _Symbol, primaryVolume, primaryOpen,
                          actualLock, lockedProfitMoney) ||
         lockedProfitMoney <= 0.0 ||
         lockedProfitMoney + 1e-8 < addedRiskMoney * InpRVAddOnLockedProfitCoverage ||
         !AccountWideExposureAllows(buy, executablePrice, desiredLock, lots, exposureReason))
         return false;

      m_trade.SetExpertMagicNumber(InpRVAddOnMagicNumber);
      m_trade.SetDeviationInPoints(InpRVDeviationPoints);
      m_trade.SetAsyncMode(false);
      m_trade.SetTypeFillingBySymbol(_Symbol);
      string comment = buy ? "RRO_ADD_BUY" : "RRO_ADD_SELL";
      bool opened = buy
                    ? m_trade.Buy(lots, _Symbol, 0.0, desiredLock,
                                  NormalizeDouble(primaryTarget, _Digits), comment)
                    : m_trade.Sell(lots, _Symbol, 0.0, desiredLock,
                                   NormalizeDouble(primaryTarget, _Digits), comment);
      if(!opened || !TradeResultAllows(m_trade, false))
      {
         string rejectReason = "";
         return RejectPostFill(m_trade, InpRVAddOnMagicNumber,
                               "reversion add-on entry result", rejectReason);
      }
      ulong addOnTicket = 0;
      string postFillReason = "";
      if(!PostFillReconcile(m_trade, InpRVAddOnMagicNumber, buy,
                            InpRVAddOnRiskPercent, InpRVMaximumPositionLots,
                            addOnTicket, postFillReason))
         return false;
      WriteEvidenceEvent(m_logHandle, "tlp_rv_protected_addon", "entry", addOnTicket,
                         buy ? "buy" : "sell", lots, executablePrice, desiredLock, 0.0,
                         "protected strong-reversion winner add-on to original VWAP target");
      return true;
   }
                     ...
                     const bool strongSignal)
   {
      ...
      if(!PostFillReconcile(m_trade, InpRVMagicNumber, buy, requestedRiskPercent,
         ...
         return false;
      if(strongSignal &&
         (InpRVUseStrongSignalProtection || InpRVUseProtectedWinnerAddOn) &&
         !RegisterStrongRisk(ticket))
      {
         string rejectReason = "";
         return RejectPostFill(m_trade, InpRVMagicNumber,
                               "strong reversion risk registration", rejectReason);
      }
   ...
   void TryEntry()
   {
      ...
      OpenPosition(buy, entryPrice, stopPrice, targetPrice, diEdge,
                   requestedRiskPercent, signalBodyRatio, strongSignal);
   }
...
public:
   ...
   void OnTradeTransaction(const MqlTradeTransaction &transaction)
   {
      ...
      if(HistoryDealGetString(transaction.deal, DEAL_SYMBOL) != _Symbol ||
         !IsReversionMagic(magic))
         return;
      long entryType = HistoryDealGetInteger(transaction.deal, DEAL_ENTRY);
      if(entryType != DEAL_ENTRY_OUT && entryType != DEAL_ENTRY_OUT_BY && entryType != DEAL_ENTRY_INOUT)
         return;
      ulong positionId = (ulong)HistoryDealGetInteger(transaction.deal, DEAL_POSITION_ID);
      if(positionId > 0 && magic == InpRVMagicNumber)
      {
         VerifiedGlobalDelete(StrongRiskKey(positionId));
         VerifiedGlobalDelete(AddOnAttemptKey(positionId));
      }
      ...
      WriteEvidenceEvent(m_logHandle,
                         magic == InpRVAddOnMagicNumber ? "tlp_rv_protected_addon" : "tlp_rv_m12",
                         "exit", transaction.deal, "close",
...
bool InputsValid()
{
   if(InpRVMagicNumber == InpRVAddOnMagicNumber ||
      InpRVMagicNumber == InpMOMagicNumber ||
      InpRVMagicNumber == InpATBMagicNumber ||
      InpRVAddOnMagicNumber == InpMOMagicNumber ||
      InpRVAddOnMagicNumber == InpATBMagicNumber ||
      InpMOMagicNumber == InpATBMagicNumber ||
      InpPortfolioMagic == InpRVMagicNumber ||
      InpPortfolioMagic == InpRVAddOnMagicNumber ||
   ...
   if(InpRVATRPeriod < 2 || InpRVADXPeriod < 2 || InpRVRSIPeriod < 2 ||
      ...
      (InpRVUseStrongSignalProtection &&
       (!InpRVUseStrongSignalRisk ||
        InpRVStrongSignalProtectionTriggerR <= 0.0 ||
        InpRVStrongSignalProtectionLockR < 0.0 ||
        InpRVStrongSignalProtectionLockR >= InpRVStrongSignalProtectionTriggerR)) ||
      (InpRVUseProtectedWinnerAddOn &&
       (!InpRVUseStrongSignalRisk || InpRVUseStrongSignalProtection ||
        InpMaximumAccountPositions < 4 ||
        InpRVAddOnTriggerR <= 0.0 ||
        InpRVAddOnPrimaryLockR <= 0.0 ||
        InpRVAddOnPrimaryLockR >= InpRVAddOnTriggerR ||
        InpRVAddOnRiskPercent <= 0.0 ||
        InpRVAddOnRiskPercent > InpMaximumPortfolioOpenRiskPercent ||
        InpRVAddOnLockedProfitCoverage < 1.0 ||
        InpRVAddOnMinimumRemainingRR <= 0.0 ||
        InpRVStrongSignalRiskPercent * InpRVAddOnPrimaryLockR + 1e-9 <
           InpRVAddOnRiskPercent * InpRVAddOnLockedProfitCoverage ||
        InpMORiskPercent + InpATBRiskPercent + InpRVAddOnRiskPercent >
           InpMaximumPortfolioOpenRiskPercent + 1e-9)) ||
