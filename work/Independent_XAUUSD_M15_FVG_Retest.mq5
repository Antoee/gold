...
input ulong  InpMagicNumber = 26071701;
...
input group "Displacement FVG Retest Engine"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_M15;
input int    InpBreakLookbackBars = 20;
input int    InpMaximumSetupAgeBars = 8;
input double InpBreakBufferATR = 0.05;
input double InpMinimumGapATR = 0.05;
input double InpMinimumImpulseRangeATR = 1.20;
input double InpMinimumImpulseBodyPercent = 60.0;
input double InpMinimumImpulseCloseLocation = 0.70;
input double InpRetestToleranceATR = 0.10;
input double InpRetestHoldFraction = 0.50;
input double InpMinimumRetestCloseLocation = 0.60;
input bool   InpRequireRetestDirectionCandle = true;
input bool   InpAllowBuy = true;
input bool   InpAllowSell = true;
input bool   InpUseImpulseVolumeFilter = false;
input int    InpVolumeLookbackBars = 20;
input double InpMinimumImpulseVolumeRatio = 1.10;
...
input bool   InpUseImpulseExtremeStop = false;
...
input int    InpMaximumHoldBars = 64;
...
input string InpLogFileName = "Independent_XAUUSD_M15_FVG_Retest_Trades.csv";
...
FvgSetup g_setup;
int g_setupsCreated = 0;
int g_setupsExpired = 0;
int g_setupsInvalidated = 0;
...
{
   return "IM15FVG_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "M15FVG-LIVE-ACK"))
   {
...
bool OpenFvgPosition(const bool buy, const double atr, const double rawStop)
{
   MqlTick tick;
   ...
   string comment = buy ? "M15FVG_BUY" : "M15FVG_SELL";
...
bool ImpulseVolumeAllows(const int impulseShift)
{
   if(!InpUseImpulseVolumeFilter)
      return true;
   long impulseVolume = iVolume(_Symbol, InpSignalTimeframe, impulseShift);
   if(impulseVolume <= 0 || InpVolumeLookbackBars < 2)
      return false;
   double total = 0.0;
   int count = 0;
   for(int shift = impulseShift + 1; shift <= impulseShift + InpVolumeLookbackBars; ++shift)
   {
   ...
   if(count <= 0 || total <= 0.0)
      return false;
   return (double)impulseVolume >= (total / count) * InpMinimumImpulseVolumeRatio;
}
...
void ClearSetup()
{
   ...
   g_setup.buy = false;
   g_setup.createdBar = 0;
   g_setup.ageBars = 0;
   g_setup.gapLower = 0.0;
   g_setup.gapUpper = 0.0;
   g_setup.impulseExtreme = 0.0;
}

bool DetectNewFvgSetup(const double atr)
{
   ...
   if(open2 <= 0.0 || close1 <= 0.0 || close2 <= 0.0 || high2 <= low2)
      return false;
   double priorHigh = 0.0;
   double priorLow = 0.0;
   if(!ChannelBounds(3, InpBreakLookbackBars, priorHigh, priorLow))
      return false;
   double impulseRange = high2 - low2;
   if(impulseRange < InpMinimumImpulseRangeATR * atr ||
      CandleBodyPercent(2) < InpMinimumImpulseBodyPercent ||
      !ImpulseVolumeAllows(2))
      return false;
   double breakBuffer = InpBreakBufferATR * atr;
   double minimumGap = InpMinimumGapATR * atr;
   bool bullish = InpAllowBuy && close2 > open2 &&
                  CandleCloseLocation(true, 2) >= InpMinimumImpulseCloseLocation &&
                  close2 > priorHigh + breakBuffer && low1 - high3 >= minimumGap;
   bool bearish = InpAllowSell && close2 < open2 &&
                  CandleCloseLocation(false, 2) >= InpMinimumImpulseCloseLocation &&
                  close2 < priorLow - breakBuffer && low3 - high1 >= minimumGap;
   if(bullish == bearish)
      return false;
   bool buy = bullish;
   ...
   g_setup.active = true;
   g_setup.buy = buy;
   g_setup.createdBar = iTime(_Symbol, InpSignalTimeframe, 1);
   g_setup.ageBars = 0;
   g_setup.gapLower = buy ? high3 : high1;
   g_setup.gapUpper = buy ? low1 : low3;
   g_setup.impulseExtreme = buy ? low2 : high2;
   g_setupsCreated++;
   return true;
}

bool TryFvgRetestEntry(const double atr)
{
   if(!g_setup.active || atr <= 0.0)
      return false;
   g_setup.ageBars++;
   ...
   double high1 = iHigh(_Symbol, InpSignalTimeframe, 1);
   double low1 = iLow(_Symbol, InpSignalTimeframe, 1);
   if(open1 <= 0.0 || close1 <= 0.0 || high1 <= low1)
      return false;
   double tolerance = InpRetestToleranceATR * atr;
   bool invalidated = buy ? close1 < g_setup.gapLower - tolerance
                          : close1 > g_setup.gapUpper + tolerance;
   if(invalidated)
   {
      g_setupsInvalidated++;
      ClearSetup();
   ...
   bool touched = low1 <= g_setup.gapUpper + tolerance &&
                  high1 >= g_setup.gapLower - tolerance;
   double holdLevel = buy ? g_setup.gapLower + InpRetestHoldFraction * (g_setup.gapUpper - g_setup.gapLower)
                          : g_setup.gapUpper - InpRetestHoldFraction * (g_setup.gapUpper - g_setup.gapLower);
   bool held = buy ? close1 >= holdLevel : close1 <= holdLevel;
   bool directionCandle = !InpRequireRetestDirectionCandle || (buy ? close1 > open1 : close1 < open1);
   if(!touched || !held || !directionCandle ||
      CandleCloseLocation(buy, 1) < InpMinimumRetestCloseLocation)
      return false;
   string safetyReason = "";
   if(!SafetyAllows(safetyReason) || !RegimeAllows(buy, close1, atr))
   {
      ClearSetup();
      return false;
   }
   g_retestSignals++;
   double rawStop = buy ? MathMin(low1, g_setup.gapLower) - InpStopBufferATR * atr
                        : MathMax(high1, g_setup.gapUpper) + InpStopBufferATR * atr;
   if(InpUseImpulseExtremeStop)
      rawStop = buy ? MathMin(rawStop, g_setup.impulseExtreme - InpStopBufferATR * atr)
                    : MathMax(rawStop, g_setup.impulseExtreme + InpStopBufferATR * atr);
   bool opened = OpenFvgPosition(buy, atr, rawStop);
   ClearSetup();
   return opened;
}
...
int OnInit()
{
   if(InpBreakLookbackBars < 5 || InpMaximumSetupAgeBars < 1 ||
      InpMinimumGapATR < 0.0 || InpMinimumImpulseRangeATR <= 0.0 ||
      InpMinimumImpulseBodyPercent < 0.0 || InpMinimumImpulseBodyPercent > 100.0 ||
      InpMinimumImpulseCloseLocation < 0.0 || InpMinimumImpulseCloseLocation > 1.0 ||
      InpRetestToleranceATR < 0.0 || InpRetestHoldFraction < 0.0 || InpRetestHoldFraction > 1.0 ||
      InpMinimumRetestCloseLocation < 0.0 || InpMinimumRetestCloseLocation > 1.0 ||
      InpVolumeLookbackBars < 2 || InpMinimumImpulseVolumeRatio <= 0.0 ||
...
{
   PrintFormat("M15FVG_DIAGNOSTIC setups=%d expired=%d invalidated=%d retest_signals=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
               g_setupsCreated, g_setupsExpired, g_setupsInvalidated, g_retestSignals,
               g_ordersOpened, g_stopShapeRejects, g_minimumLotRejects, g_exposureRejects, g_orderFailures);
...
void OnTick()
{
   ...
   bool opened = TryFvgRetestEntry(atr);
   if(!opened && ManagedPositionCount() == 0)
      DetectNewFvgSetup(atr);
}
