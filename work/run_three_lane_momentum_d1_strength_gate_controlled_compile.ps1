...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_D1_STRENGTH_GATE_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'D1 momentum-strength research source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Momentum D1-strength compile $startedAtUtc",[Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck,"Momentum D1-strength focus acknowledgement $startedAtUtc",[Text.Encoding]::ASCII)
