...
#property description "Dedicated-account hardened XAUUSD reversion and multiscale-momentum portfolio EA"
...
bool InitialAccountContractAllows(string &reason)
{
   ...
   if(InpUseInitialBalanceContract && !balanceContractExists &&
      !GlobalVariableSet(InitialBalanceContractKey(), InpExpectedInitialBalance))
   {
...
void AuditManagedPositionProtection()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
   {
      ...
      if(ticket == 0 || !PositionSelectByTicket(ticket) ||
         ...
         continue;
      if(!g_guardTrade.PositionClose(ticket))
         Print("SAFETY: failed to close unprotected managed position; retcode=",
               g_guardTrade.ResultRetcode());
      else
...
private:
   ...
   double EfficiencyRatio(const ENUM_TIMEFRAMES timeframe,
                          const int startShift,
                          const int segments)
   {
      if(startShift < 1 || segments < 2)
         return -1.0;
      double closes[];
      ArraySetAsSeries(closes, true);
      int required = segments + 1;
      if(CopyClose(_Symbol, timeframe, startShift, required, closes) != required)
         return -1.0;
      double path = 0.0;
      ...
      if(path <= 0.0)
         return 0.0;
      return MathAbs(closes[0] - closes[segments]) / path;
   }

   double AverageClose(const ENUM_TIMEFRAMES timeframe,
                       const int startShift,
                       const int count)
   {
      ...
      double closes[];
      ArraySetAsSeries(closes, true);
      if(CopyClose(_Symbol, timeframe, startShift, count, closes) != count)
         return -1.0;
      double total = 0.0;
   ...
   double AlignedMomentumPercent(const ENUM_TIMEFRAMES timeframe,
                                 const int lookback,
                                 const bool buy)
   {
      double recentClose = iClose(_Symbol, timeframe, 1);
      double pastClose = iClose(_Symbol, timeframe, 1 + lookback);
      if(recentClose <= 0.0 || pastClose <= 0.0)
         return -999.0;
      double direction = buy ? 1.0 : -1.0;
      return direction * 100.0 * (recentClose - pastClose) / pastClose;
   }

   string FeatureTelemetry(const bool buy,
                           const double atr,
                           const double adx,
                           const double rsi,
                           const double plusDI,
                           ...
                           const double diEdge)
   {
      ...
      double bodyRatio = range1 > 0.0 ? MathMax(0.0, directionalBody) / range1 : -1.0;
      double rejectionWick = buy ? MathMin(open1, close1) - low1
                                 : high1 - MathMax(open1, close1);
      double wickRatio = range1 > 0.0 ? rejectionWick / range1 : -1.0;
      double closeLocation = range1 > 0.0
                             ? (buy ? (close1 - low1) / range1
                                    : (high1 - close1) / range1)
                             : -1.0;
      double penetration = buy ? lower - low1 : high1 - upper;
      ...
      double average200 = AverageClose(InpRVSignalTimeframe, 1, 200);
      double longDistanceATR = average200 > 0.0 && atr > 0.0
                               ? direction * (close1 - average200) / atr
                               : -999.0;
      double volumeSum = 0.0;
      int volumeCount = 0;
      for(int shift = 2; shift <= 21; ++shift)
      {
         long volume = iVolume(_Symbol, InpRVSignalTimeframe, shift);
         if(volume > 0)
         {
      ...
      double currentVolume = (double)iVolume(_Symbol, InpRVSignalTimeframe, 1);
      double volumeRatio = volumeCount > 0 && volumeSum > 0.0
                           ? currentVolume / (volumeSum / volumeCount)
                           : -1.0;

      string telemetry = StringFormat("d1_aligned_momentum_pct=%.6f;d1_efficiency=%.6f;h1_aligned_momentum48_pct=%.6f;h1_efficiency48=%.6f;h1_long_distance_atr=%.6f",
                                      AlignedMomentumPercent(PERIOD_D1, 126, buy),
                                      EfficiencyRatio(PERIOD_D1, 1, 126),
                                      AlignedMomentumPercent(InpRVSignalTimeframe, 48, buy),
                                      EfficiencyRatio(InpRVSignalTimeframe, 1, 48),
                                      longDistanceATR);
      telemetry += StringFormat(";adx=%.6f;di_edge=%.6f;plus_di=%.6f;minus_di=%.6f;rsi=%.6f;atr_pct=%.6f;band_width_atr=%.6f",
                                adx, diEdge, plusDI, minusDI, rsi,
                                close1 > 0.0 ? 100.0 * atr / close1 : -1.0,
                                atr > 0.0 ? (upper - lower) / atr : -1.0);
      telemetry += StringFormat(";body_ratio=%.6f;wick_ratio=%.6f;close_location=%.6f;range_atr=%.6f;penetration_atr=%.6f;middle_distance_atr=%.6f;vwap_distance_atr=%.6f",
                                bodyRatio, wickRatio, closeLocation,
                                atr > 0.0 ? range1 / atr : -1.0,
                                atr > 0.0 ? penetration / atr : -1.0,
                                atr > 0.0 ? direction * (close1 - middle) / atr : -1.0,
                                atr > 0.0 ? direction * (vwap - entryPrice) / atr : -1.0);
      telemetry += StringFormat(";stop_atr=%.6f;target_atr=%.6f;raw_rr=%.6f;volume_ratio=%.6f",
                                atr > 0.0 ? stopDistance / atr : -1.0,
                                atr > 0.0 ? targetDistance / atr : -1.0,
                                stopDistance > 0.0 ? targetDistance / stopDistance : -1.0,
                                volumeRatio);
      return telemetry;
   }
   ...
   void TryEntry()
   {
      ...
      OpenPosition(buy, entryPrice, stopPrice, targetPrice, diEdge,
                   FeatureTelemetry(buy, atr, adx, rsi, plusDI, minusDI,
                                    middle, upper, lower, high1, low1, open1,
                                    close1, vwap, entryPrice, stopPrice,
                                    targetPrice, diEdge));
   }
...
bool InputsValid()
{
   if(InpRVMagicNumber == InpMOMagicNumber ||
      ...
      InpRVRiskPercent + InpMORiskPercent > InpMaximumPortfolioOpenRiskPercent + 1e-9)
      return false;
...
int OnInit()
{
   ...
   g_guardTrade.SetDeviationInPoints((ulong)MathMax(InpRVDeviationPoints, InpMODeviationPoints));
   if(!g_reversion.Init())
      return INIT_FAILED;
   if(!g_momentum.Init())
   {
