...
input ulong  InpMagicNumber = 26071785;
...
input group "Daily Anchored VWAP Continuation Engine"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_M15;
input int    InpPullbackLookbackBars = 3;
input int    InpMinimumVWAPBars = 4;
input double InpVWAPTouchToleranceATR = 0.05;
input double InpMaximumCloseDistanceATR = 0.75;
input bool   InpRequirePriorCloseBeyondVWAP = true;
...
input string InpLogFileName = "Independent_XAUUSD_M15_Daily_VWAP_Continuation_Trades.csv";
...
int g_vwapCandidates = 0;
int g_vwapCalculationRejects = 0;
...
{
   return "IM15DVC_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "M15DVC-LIVE-ACK-v1"))
   {
...
bool OpenDailyVWAPContinuationPosition(const bool buy, const double atr, const double rawStop)
{
   MqlTick tick;
   ...
   string comment = buy ? "M15DVC_BUY" : "M15DVC_SELL";
   ...
   LogEvent("entry", trade.ResultOrder(), buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
             "H1 trend M15 daily VWAP reclaim continuation");
   return true;
}
...
bool TryManagedExit(const ulong ticket, const bool buy)
{
   ...
   if(InpUseTrendFailureExit)
   {
      ...
      {
         bool failed = buy ? closePrice < vwap : closePrice > vwap;
         if(failed)
            return trade.PositionClose(ticket);
      }
   }
...
bool DailyAnchoredVWAP(const int signalShift, double &vwap)
{
   ...
   if(dayOpen <= 0)
      return false;
   int dayOpenShift = iBarShift(_Symbol, InpSignalTimeframe, dayOpen, false);
   int count = dayOpenShift - signalShift + 1;
   if(dayOpenShift < signalShift || count < InpMinimumVWAPBars)
      return false;
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   if(CopyRates(_Symbol, InpSignalTimeframe, signalShift, count, rates) != count)
      return false;
   ...
   for(int i = 0; i < count; ++i)
   {
      ...
      if(volume <= 0.0 || rates[i].high <= 0.0 || rates[i].low <= 0.0 || rates[i].close <= 0.0)
         continue;
...
bool VWAPPullbackAllows(const bool buy, const double atr, double &swingExtreme)
{
   swingExtreme = buy ? DBL_MAX : -DBL_MAX;
   bool touched = false;
   bool closedBeyond = false;
   double tolerance = InpVWAPTouchToleranceATR * atr;
   for(int shift = 2; shift <= 1 + InpPullbackLookbackBars; ++shift)
   {
      double vwap = 0.0;
      if(!DailyAnchoredVWAP(shift, vwap))
         continue;
      double close = iClose(_Symbol, InpSignalTimeframe, shift);
      double low = iLow(_Symbol, InpSignalTimeframe, shift);
      double high = iHigh(_Symbol, InpSignalTimeframe, shift);
      ...
      if((buy && low <= vwap + tolerance) || (!buy && high >= vwap - tolerance))
         touched = true;
      if((buy && close < vwap) || (!buy && close > vwap))
         closedBeyond = true;
   }
   if(!touched || swingExtreme == DBL_MAX || swingExtreme == -DBL_MAX)
      return false;
   return !InpRequirePriorCloseBeyondVWAP || closedBeyond;
}

bool DailyVWAPContinuationSignal(const bool buy, const double atr, double &rawStop)
{
   rawStop = 0.0;
   ...
   if(open1 <= 0.0 || close1 <= 0.0 || high1 <= low1 || high2 <= low2 ||
      !DailyAnchoredVWAP(1, vwap1))
   {
      g_vwapCalculationRejects++;
      return false;
   }
   ...
   if(!VWAPPullbackAllows(buy, atr, swingExtreme))
      return false;
   g_vwapCandidates++;

   double signalRangeATR = (high1 - low1) / atr;
   bool directionCandle = buy ? close1 > open1 : close1 < open1;
   bool vwapReclaim = buy ? close1 > vwap1 : close1 < vwap1;
   bool notChasing = MathAbs(close1 - vwap1) <= InpMaximumCloseDistanceATR * atr;
   bool priorBreak = !InpRequirePreviousHighLowBreak || (buy ? high1 > high2 : low1 < low2);
   bool shape = directionCandle && vwapReclaim && notChasing && priorBreak &&
...
bool TryDailyVWAPContinuationEntry(const double atr)
{
   if(atr <= 0.0)
      return false;
   string safetyReason = "";
   ...
   double buyStop = 0.0;
   double sellStop = 0.0;
   bool buy = InpAllowBuy && DailyVWAPContinuationSignal(true, atr, buyStop);
   bool sell = InpAllowSell && DailyVWAPContinuationSignal(false, atr, sellStop);
   if(buy == sell)
      return false;
   g_entrySignals++;
   return OpenDailyVWAPContinuationPosition(buy, atr, buy ? buyStop : sellStop);
}

int OnInit()
{
   if(InpPullbackLookbackBars < 1 || InpMinimumVWAPBars < 2 ||
      InpVWAPTouchToleranceATR < 0.0 || InpMaximumCloseDistanceATR <= 0.0 ||
   ...
   if(g_atrHandle == INVALID_HANDLE || g_trendFastEmaHandle == INVALID_HANDLE ||
      g_trendSlowEmaHandle == INVALID_HANDLE ||
...
{
   PrintFormat("M15DVC_DIAGNOSTIC vwap_candidates=%d vwap_calculation_rejects=%d shape_rejects=%d regime_rejects=%d volume_rejects=%d entry_signals=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
               g_vwapCandidates, g_vwapCalculationRejects, g_signalShapeRejects, g_regimeRejects, g_volumeRejects,
...
void OnTick()
{
   ...
   if(ManagedPositionCount() > 0)
      return;
   TryDailyVWAPContinuationEntry(atr);
}
