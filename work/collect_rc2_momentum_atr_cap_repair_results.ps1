param(
   ...
   [string]$ReportDir = "outputs\rc2_momentum_atr_cap_repair_model1_package\reports_here",
   [string]$RawResultsPath = "outputs\RC2_MOMENTUM_ATR_CAP_REPAIR_MODEL1_RAW_RESULTS.csv",
   [string]$ResultsPath = "outputs\RC2_MOMENTUM_ATR_CAP_REPAIR_MODEL1_RESULTS.csv",
   [string]$RunPath = "outputs\RC2_MOMENTUM_ATR_CAP_REPAIR_MODEL1_RUN.csv",
   [string]$SummaryPath = "outputs\RC2_MOMENTUM_ATR_CAP_REPAIR_MODEL1_SUMMARY.csv",
   [string]$MarkdownPath = "outputs\RC2_MOMENTUM_ATR_CAP_REPAIR_MODEL1_METRICS.md"
)
...
$initialRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath "outputs") -Filter "RC2_MOMENTUM_ATR_CAP_REPAIR_PORTABLE_*.csv" |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
$retryRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath "outputs") -Filter "RC2_MOMENTUM_ATR_CAP_REPAIR_RERUN_*.csv" |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
...
$results = foreach($queued in $queue) {
   ...
   [pscustomobject]@{
      ...
      MaximumATRPercent=$queued.MaximumATRPercent;InitialDeposit=$row.InitialDeposit
