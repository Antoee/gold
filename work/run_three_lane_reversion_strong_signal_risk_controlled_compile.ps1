...
$source = Join-Path $repo 'outputs\three_lane_reversion_strong_signal_risk_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath = Join-Path $repo 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_RISK_DISCOVERY_COMPILE_AUDIT.csv'
$roots = @(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Strong-signal risk source identity changed.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Reversion strong-signal risk discovery compile $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Reversion strong-signal risk discovery focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
