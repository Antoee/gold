...
$source = Join-Path $repo 'outputs\three_lane_momentum_breakout_failure_exit_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath = Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_BREAKOUT_FAILURE_EXIT_COMPILE_AUDIT.csv'
$roots = @(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Momentum breakout-failure source identity changed.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Momentum breakout-failure compile $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Momentum breakout-failure focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
...
$binaryRows = @($roots | ForEach-Object {
   ...
   [pscustomobject]@{
      SourceSha256=$sourceHash;BinarySha256=$binaryHash
      IdentityReady=$identity.Count -ge 2 -and $identity[0] -eq $sourceHash -and $identity[1] -eq $binaryHash
   }
})
