...
input group "Overnight Drift V2 Signal"
...
input int    InpStopLookbackBars = 4;
input double InpStopBufferATR = 0.05;
input double InpMinimumStopATR = 0.05;
input double InpMaximumStopATR = 0.35;
...
input string InpLogFileName = "Independent_XAUUSD_M15_Overnight_Drift_Structure_V2_Trades.csv";
...
{
   return "IM15ODS2_RISK_" + IntegerToString((long)ticket);
}
...
bool EntrySafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
        InpRealAccountApprovalCode != "M15ODS2-LIVE-ACK"))
   {
                       ...
                       const double rawStop)
{
   ...
   string comment = buy ? "M15ODS2_BUY" : "M15ODS2_SELL";
...
int OnInit()
{
   ...
   {
      Print("M15ODS2 account contract failure: ", accountReason);
      return INIT_FAILED;
   }
...
{
   PrintFormat("M15ODS2_DIAGNOSTIC daily_contexts=%d drift_candidates=%d signal_rejects=%d geometry_rejects=%d safety_rejects=%d minimum_lot_rejects=%d opened=%d order_failures=%d",
