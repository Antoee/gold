...
input group "Compression Expansion Engine"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_M30;
input int    InpBoxLookbackBars = 10;
input double InpMinimumBoxRangeATR = 0.40;
input double InpMaximumBoxRangeATR = 1.80;
input double InpMaximumAverageBoxBarRangeATR = 0.45;
input double InpBreakBufferATR = 0.03;
input double InpMinimumBreakRangeATR = 0.45;
input double InpMaximumBreakRangeATR = 1.80;
input double InpMinimumExpansionRatio = 1.40;
...
input double InpMinimumADX = 14.0;
...
input string InpLogFileName = "Independent_XAUUSD_M30_Compression_Expansion_Trades.csv";
...
int g_compressionCandidates = 0;
int g_expansionSignals = 0;
...
{
   return "IM30CE_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "M30CE-LIVE-ACK"))
   {
...
bool OpenExpansionPosition(const bool buy,
                           const double atr,
                           const double rawStop)
{
   MqlTick tick;
   ...
   string comment = buy ? "M30CE_BUY" : "M30CE_SELL";
   ...
   LogEvent("entry", trade.ResultOrder(), buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
            "M30 compression expansion continuation");
   return true;
}
...
bool TryCompressionExpansionEntry(const double atr)
{
   if(atr <= 0.0)
      return false;
   ...
   double boxHigh = 0.0;
   double boxLow = 0.0;
   if(!ChannelBounds(2, InpBoxLookbackBars, boxHigh, boxLow))
      return false;
   double boxRange = boxHigh - boxLow;
   double averageBoxBarRange = AverageBarRange(2, InpBoxLookbackBars);
   if(boxRange < InpMinimumBoxRangeATR * atr ||
      boxRange > InpMaximumBoxRangeATR * atr ||
      averageBoxBarRange <= 0.0 ||
      averageBoxBarRange > InpMaximumAverageBoxBarRangeATR * atr)
   {
      g_boxShapeRejects++;
      return false;
   }
   g_compressionCandidates++;

   double buffer = InpBreakBufferATR * atr;
   bool buy = InpAllowBuy && close1 > boxHigh + buffer;
   bool sell = InpAllowSell && close1 < boxLow - buffer;
   if(buy == sell)
      return false;

   bool directionCandle = !InpRequireDirectionCandle || (buy ? close1 > open1 : close1 < open1);
   bool breakShapeAllows = directionCandle &&
                           range1 >= InpMinimumBreakRangeATR * atr &&
                           range1 <= InpMaximumBreakRangeATR * atr &&
                           range1 >= InpMinimumExpansionRatio * averageBoxBarRange &&
                           CandleBodyPercent(1) >= InpMinimumBreakBodyPercent &&
                           CandleCloseLocation(buy, 1) >= InpMinimumBreakCloseLocation &&
                           TickVolumeAllows(1, InpUseBreakoutTickVolumeFilter);
   if(!breakShapeAllows)
   {
      g_breakShapeRejects++;
      return false;
   ...
   return OpenExpansionPosition(buy, atr, rawStop);
}

int OnInit()
{
   if(InpBoxLookbackBars < 4 ||
      InpMinimumBoxRangeATR <= 0.0 || InpMaximumBoxRangeATR <= InpMinimumBoxRangeATR ||
      InpMaximumAverageBoxBarRangeATR <= 0.0 || InpBreakBufferATR < 0.0 ||
      InpMinimumBreakRangeATR <= 0.0 || InpMaximumBreakRangeATR < InpMinimumBreakRangeATR ||
      InpMinimumExpansionRatio <= 0.0 ||
...
{
   PrintFormat("M30CE_DIAGNOSTIC compressions=%d signals=%d box_rejects=%d break_rejects=%d regime_rejects=%d safety_rejects=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
               g_compressionCandidates, g_expansionSignals, g_boxShapeRejects,
               g_breakShapeRejects, g_regimeRejects,
...
void OnTick()
{
   ...
   if(ManagedPositionCount() > 0)
      return;
   TryCompressionExpansionEntry(atr);
}
