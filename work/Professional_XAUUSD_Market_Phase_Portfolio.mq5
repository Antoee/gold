...
input ulong  InpPortfolioMagic = 26071782;
...
input group "Closed-Bar Market Phase Allocation"
input bool   InpUseMarketPhaseAllocation = true;
input ENUM_TIMEFRAMES InpMarketPhaseTimeframe = PERIOD_H1;
input int    InpMarketPhaseEfficiencyLookbackBars = 24;
input double InpMarketPhaseRangeEfficiency = 0.20;
input double InpMarketPhaseTrendEfficiency = 0.45;
input double InpMarketPhaseHostileRiskScale = 0.50;

input group "H1 Band VWAP Reversion Lane"
input bool   InpRVEnabled = true;
input ulong  InpRVMagicNumber = 26071723;
...
input ulong  InpMOMagicNumber = 26071763;
...
bool ClosedBarEfficiencyRatio(double &efficiency)
{
   efficiency = 0.0;
   int lookback = InpMarketPhaseEfficiencyLookbackBars;
   if(lookback < 4)
      return false;
   double newest = iClose(_Symbol, InpMarketPhaseTimeframe, 1);
   double oldest = iClose(_Symbol, InpMarketPhaseTimeframe, 1 + lookback);
   ...
   {
      double current = iClose(_Symbol, InpMarketPhaseTimeframe, shift);
      if(current <= 0.0)
         return false;
      path += MathAbs(previous - current);
      previous = current;
   }
   if(path <= _Point)
      return false;
   efficiency = MathAbs(newest - oldest) / path;
   return MathIsValidNumber(efficiency) && efficiency >= 0.0 && efficiency <= 1.0 + 1e-9;
}

double MarketPhaseLaneRiskScale(const bool reversionLane, double &efficiency)
{
   efficiency = -1.0;
   if(!InpUseMarketPhaseAllocation)
      return 1.0;
   if(!ClosedBarEfficiencyRatio(efficiency))
      return 0.0;
   double hostileScale = MathMax(0.0, MathMin(1.0, InpMarketPhaseHostileRiskScale));
   if(efficiency <= InpMarketPhaseRangeEfficiency)
      return reversionLane ? 1.0 : hostileScale;
   if(efficiency >= InpMarketPhaseTrendEfficiency)
      return reversionLane ? hostileScale : 1.0;
   return 1.0;
}
...
bool SharedSafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "MPP-LIVE-ACK-v1"))
   {
...
private:
                     ...
                     const double diEdge)
   {
      ...
      double phaseScale = MarketPhaseLaneRiskScale(true, efficiency);
      if(phaseScale <= 0.0)
         return false;
      double lots = LotsForRisk(buy, entryPrice, stopPrice,
                                InpRVRiskPercent * phaseScale, InpRVMaximumPositionLots);
      ...
      eventReason += "; phase ER " + DoubleToString(efficiency, 3) +
                     "; risk scale " + DoubleToString(phaseScale, 2);
...
private:
   ...
   bool OpenPosition(const bool buy, const double atr)
   {
      ...
      double efficiency = -1.0;
      double phaseScale = MarketPhaseLaneRiskScale(false, efficiency);
      if(phaseScale <= 0.0)
         return false;
      double lots = LotsForRisk(buy, entryPrice, stopPrice,
                                InpMORiskPercent * phaseScale, InpMOMaximumPositionLots);
      string exposureReason = "";
      if(lots <= 0.0 ||
         !AccountWideExposureAllows(buy, entryPrice, stopPrice, lots, exposureReason))
         return false;
      ...
      WriteEvidenceEvent(m_logHandle, "tlp_mom_e20", "entry", m_trade.ResultOrder(),
                         buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
                         "daily momentum plus fresh H1 breakout; phase ER " +
                         DoubleToString(efficiency, 3) + "; risk scale " +
                         DoubleToString(phaseScale, 2));
      return true;
   }
...
bool InputsValid()
{
   if(InpRVMagicNumber == InpMOMagicNumber ||
      ...
      return false;
   if(InpMarketPhaseEfficiencyLookbackBars < 4 ||
      InpMarketPhaseRangeEfficiency < 0.0 ||
      InpMarketPhaseTrendEfficiency > 1.0 ||
      InpMarketPhaseRangeEfficiency >= InpMarketPhaseTrendEfficiency ||
      InpMarketPhaseHostileRiskScale <= 0.0 || InpMarketPhaseHostileRiskScale > 1.0)
      return false;
...
void UpdateDashboard()
{
   ...
   double phaseEfficiency = -1.0;
   ClosedBarEfficiencyRatio(phaseEfficiency);
   Comment("XAUUSD Market Phase Portfolio\n",
           ...
           "Closed-bar phase ER: ", DoubleToString(phaseEfficiency, 3));
}
