param(
   ...
   [string]$ReportDir = "outputs\rc2_di_repair_portfolio_discovery_model1_package\reports_here",
   [string]$RawResultsPath = "outputs\RC2_DI_REPAIR_PORTFOLIO_DISCOVERY_MODEL1_RAW_RESULTS.csv",
   ...
   [string]$SummaryPath = "outputs\RC2_DI_REPAIR_PORTFOLIO_DISCOVERY_MODEL1_SUMMARY.csv",
   [string]$MarkdownPath = "outputs\RC2_DI_REPAIR_PORTFOLIO_DISCOVERY_MODEL1_METRICS.md"
)
...
if($queue.Count -ne 24) { throw "Expected 24 frozen queue rows, found $($queue.Count)." }
...
$initialRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath 'outputs') -Filter 'RC2_DI_REPAIR_PORTABLE_*.csv' |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
$retryRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath 'outputs') -Filter 'RC2_DI_REPAIR_RETRY_*.csv' |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
...
if($raw.Count -ne 24) { throw "Expected 24 parsed result rows, found $($raw.Count)." }
$results = foreach($row in $raw) {
   ...
   [pscustomobject]@{
      ...
      RVMinimumDIEdge=$q.RVMinimumDIEdge;InitialDeposit=$row.InitialDeposit;CalendarDays=$row.CalendarDays
