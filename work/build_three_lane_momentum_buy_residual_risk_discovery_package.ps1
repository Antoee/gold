[CmdletBinding()]
param(
   [string]$SourcePath = 'work\Professional_XAUUSD_Three_Lane_Momentum_Buy_Residual_Risk_Research.mq5',
   ...
   [string]$PackageDir = 'outputs\three_lane_momentum_buy_residual_risk_discovery_model1_package',
...
if($sourceHash -ne $expectedSourceHash) { throw "Buy residual-risk research source identity changed: $sourceHash" }
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_momentum_buy_residual_risk_discovery_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='mbr_control';Role='control';Enabled='false';BuyRisk='0.15'},
   [pscustomobject]@{Name='mbr_buy016';Role='lower_boundary';Enabled='true';BuyRisk='0.16'},
   [pscustomobject]@{Name='mbr_buy017';Role='conservative_support';Enabled='true';BuyRisk='0.17'},
   [pscustomobject]@{Name='mbr_buy018';Role='middle_support';Enabled='true';BuyRisk='0.18'},
   [pscustomobject]@{Name='mbr_buy019';Role='upper_support';Enabled='true';BuyRisk='0.19'},
   [pscustomobject]@{Name='mbr_center_buy020';Role='center';Enabled='true';BuyRisk='0.20'}
)
$windows = @(
...
$stopRule = 'Frozen pre-2021 buy residual-risk discovery. Center positive and >=98% control in both eras; continuous net >=control +4%; CAGR >=control +0.06 point; PF/recovery/return-DD >=97% control; DD <=min(1.20%, control +0.10 point); trades equal control. At least 3 of 4 lower enabled rungs retain 98% of both eras, improve net >=2%, retain 97% efficiency, meet the same DD ceiling, and preserve trades. No post-result rescue.'
...
foreach($variant in $variants) {
   ...
   Set-FixedInput $inputs 'InpMOUseBuyResidualRisk' $variant.Enabled
   Set-FixedInput $inputs 'InpMOBuyResidualRiskPercent' $variant.BuyRisk
   ...
   foreach($window in $windows) {
      ...
      $rows.Add([pscustomobject][ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='momentum_buy_residual_risk_discovery_model1'
         Window=$window.Name;From=$window.From;To=$window.To;Model=1;Deposit=10000
         BuyResidualRiskEnabled=$variant.Enabled;MomentumBuyRiskPercent=$variant.BuyRisk;MomentumSellRiskPercent='0.15';BaseLotEligibility='required'
...
@(
   '# Momentum Buy Residual-Risk Discovery Contract','',
   ...
   '- Frozen center: base-eligible momentum buys request 0.20% while momentum sells remain 0.15%; the authoritative broker-valued account-wide exposure check remains capped at 0.75%.',
   '- Frozen support ladder: disabled control plus buy risk 0.16%, 0.17%, 0.18%, 0.19%, and 0.20%.',
