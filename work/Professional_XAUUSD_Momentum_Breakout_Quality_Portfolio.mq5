...
#property description "Research fork: date-independent H1 momentum breakout quality filter"
...
input bool   InpMOUseBreakoutCandleQuality = false;
input double InpMOMinimumBreakoutBodyPercent = 35.0;
input double InpMOMinimumBreakoutCloseLocation = 0.65;
input double InpMOMinimumBreakoutRangeATR = 0.50;
input bool   InpMOUseTickVolumeConfirmation = false;
input int    InpMOTickVolumeLookbackBars = 20;
input double InpMOMinimumTickVolumeRatio = 1.00;
...
private:
   ...
   {
      if(!InpMOUseBreakoutCandleQuality && !InpMOUseTickVolumeConfirmation)
         return true;
      if(atr <= 0.0)
         return false;

      int barsNeeded = InpMOUseTickVolumeConfirmation
                       ? InpMOTickVolumeLookbackBars + 1
                       : 1;
      MqlRates rates[];
      ArraySetAsSeries(rates, true);
      if(CopyRates(_Symbol, InpMOSignalTimeframe, 1, barsNeeded, rates) != barsNeeded)
         return false;

      double range = rates[0].high - rates[0].low;
      if(range <= 0.0)
         return false;
      if(InpMOUseBreakoutCandleQuality)
      {
         bool directional = buy ? rates[0].close > rates[0].open
                                : rates[0].close < rates[0].open;
         double bodyPercent = 100.0 * MathAbs(rates[0].close - rates[0].open) / range;
         double closeLocation = buy ? (rates[0].close - rates[0].low) / range
                                    : (rates[0].high - rates[0].close) / range;
         if(!directional || bodyPercent < InpMOMinimumBreakoutBodyPercent ||
            closeLocation < InpMOMinimumBreakoutCloseLocation ||
            range / atr < InpMOMinimumBreakoutRangeATR)
            return false;
      }

      if(InpMOUseTickVolumeConfirmation)
      {
         double priorVolume = 0.0;
         for(int i = 1; i <= InpMOTickVolumeLookbackBars; ++i)
            priorVolume += (double)rates[i].tick_volume;
         double averageVolume = priorVolume / (double)InpMOTickVolumeLookbackBars;
         if(averageVolume <= 0.0 ||
            (double)rates[0].tick_volume / averageVolume < InpMOMinimumTickVolumeRatio)
            return false;
      }
      return true;
   }
   ...
   void TryEntry(const double atr)
   {
      ...
      if(buyBreak && RegimeAllows(true, close1, atr) && BreakoutQualityAllows(true, atr))
         OpenPosition(true, atr);
      else if(sellBreak && RegimeAllows(false, close1, atr) && BreakoutQualityAllows(false, atr))
...
bool InputsValid()
{
   ...
   if(InpMOMomentumLookbackBars < 20 || InpMOEntryLookbackBars < 3 ||
       ...
       InpMOMinimumBreakoutBodyPercent < 0.0 || InpMOMinimumBreakoutBodyPercent > 100.0 ||
       InpMOMinimumBreakoutCloseLocation < 0.0 || InpMOMinimumBreakoutCloseLocation > 1.0 ||
       InpMOMinimumBreakoutRangeATR < 0.0 || InpMOTickVolumeLookbackBars < 2 ||
       InpMOMinimumTickVolumeRatio < 0.0 ||
