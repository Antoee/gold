...
if(!$UserAuthorizedFocusRisk) { throw 'Controlled momentum profit-ratchet testing requires explicit focus/window-risk authorization.' }
...
$manifest = Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_PROFIT_RATCHET_DISCOVERY_MODEL1_MANIFEST.csv'
...
$expectedManifestHash = '31B7B958F81B480D0E64139AED4EF88B84BAB757F22ACA6B8A07661FD4957B3D'
...
foreach($required in @($repoLock,$outerLock,$runner,$manifest,$source) + $roots) {
   if(!(Test-Path -LiteralPath $required)) { throw "Controlled momentum profit-ratchet prerequisite missing: $required" }
}
if((Get-FileHash -LiteralPath $manifest -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedManifestHash) {
   throw 'Momentum profit-ratchet manifest identity changed.'
}
...
if((Test-Path -LiteralPath $unlockFile) -or (Test-Path -LiteralPath $focusAck) -or
   ...
   throw 'Controlled momentum profit-ratchet testing refuses a pre-existing partial unlock state.'
}
if(@(Get-Process -Name terminal,terminal64,metatester,metatester64,MetaEditor,metaeditor64 -ErrorAction SilentlyContinue).Count -ne 0) {
   throw 'Controlled momentum profit-ratchet testing requires zero pre-existing MT5-family processes.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Momentum profit-ratchet run $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Momentum profit-ratchet focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
   ...
   & $runner -ManifestPath $manifest -PortableRoots $roots -UserAuthorizedFocusRisk `
      -OutputPrefix $(if($SingleWorkerRecovery) { 'THREE_LANE_MOMENTUM_PROFIT_RATCHET_SINGLE_WORKER' } else { 'THREE_LANE_MOMENTUM_PROFIT_RATCHET_WORKER' }) `
...
if(!$completed) { throw 'Controlled momentum profit-ratchet run did not complete.' }
if(!(Test-Path -LiteralPath $repoLock) -or !(Test-Path -LiteralPath $outerLock) -or
   ...
   throw 'Controlled momentum profit-ratchet run did not restore hard-lock state.'
}
