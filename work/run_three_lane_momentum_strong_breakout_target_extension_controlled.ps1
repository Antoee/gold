...
$manifest = Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_STRONG_BREAKOUT_TARGET_EXTENSION_DISCOVERY_MODEL1_MANIFEST.csv'
...
$expectedManifestHash = '446ECF95B9FBCA28C062BC6387D2CA16C3B6807CB87D07DBD3F12DB2890D4CCA'
...
try {
   ...
   & $runner -ManifestPath $manifest -PortableRoots $roots -UserAuthorizedFocusRisk `
      -OutputPrefix $(if($SingleWorkerRecovery) { 'THREE_LANE_MOMENTUM_STRONG_BREAKOUT_TARGET_EXTENSION_SINGLE_WORKER' } else { 'THREE_LANE_MOMENTUM_STRONG_BREAKOUT_TARGET_EXTENSION_WORKER' }) `
