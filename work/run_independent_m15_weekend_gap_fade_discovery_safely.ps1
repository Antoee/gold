...
param(
   ...
   [string]$OutputPrefix = "INDEPENDENT_M15_WEEKEND_GAP_FADE_PORTABLE"
)
...
try {
   ...
   Set-Content -LiteralPath $unlock -Value "User-authorized isolated weekend-gap discovery." -Encoding ASCII
   ...
   Set-Content -LiteralPath $hardLock -Value "Restored after isolated weekend-gap discovery." -Encoding ASCII
}
