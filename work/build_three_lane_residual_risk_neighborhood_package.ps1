...
param(
   ...
   [string]$PackageDir = 'outputs\three_lane_residual_risk_neighborhood_model1_package',
   [string]$QueuePath = 'outputs\THREE_LANE_RESIDUAL_RISK_NEIGHBORHOOD_MODEL1_QUEUE.csv',
   [string]$ManifestPath = 'outputs\THREE_LANE_RESIDUAL_RISK_NEIGHBORHOOD_MODEL1_PACKAGE_MANIFEST.csv',
   [string]$PackageMarkdownPath = 'outputs\THREE_LANE_RESIDUAL_RISK_NEIGHBORHOOD_MODEL1_PACKAGE.md',
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_residual_risk_neighborhood_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='rr_atb250';Enabled='true';Reserve='0.05';RVMax='0.45';MOMax='0.20';ATBMax='0.25'},
   [pscustomobject]@{Name='rr_atb275';Enabled='true';Reserve='0.05';RVMax='0.45';MOMax='0.20';ATBMax='0.275'},
   [pscustomobject]@{Name='rr_atb325';Enabled='true';Reserve='0.05';RVMax='0.45';MOMax='0.20';ATBMax='0.325'},
   [pscustomobject]@{Name='rr_atb350';Enabled='true';Reserve='0.05';RVMax='0.45';MOMax='0.20';ATBMax='0.35'},
   [pscustomobject]@{Name='rr_atb300_reserve025';Enabled='true';Reserve='0.025';RVMax='0.45';MOMax='0.20';ATBMax='0.30'},
   [pscustomobject]@{Name='rr_atb300_reserve075';Enabled='true';Reserve='0.075';RVMax='0.45';MOMax='0.20';ATBMax='0.30'}
)
$windows = @(
...
$stopRule = 'Neighborhood calibration only. Compare against the prior exact rr_control and rr_adaptive_focus evidence. Require both eras positive and at least three adjacent ceiling/reserve profiles to preserve the selected point qualitative gain before any 2021+ historical cross-period check.'
...
foreach($variant in $variants) {
   ...
   foreach($window in $windows) {
      ...
      $common = [ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Phase='neighborhood_model1'
...
@(
   '# Three-Lane Residual-Risk Neighborhood Package','',
   '**Status: LOCAL PARAMETER-NEIGHBORHOOD CALIBRATION ONLY. THE ATB150 CHAMPION AND FORWARD CANDIDATE ARE UNCHANGED.**','',
   "- Research source SHA-256: ``$sourceHash``",
   "- Frozen champion profile SHA-256: ``$championHash``",
   "- Variants: ``$($variants.Count)``",
   "- Configurations: ``$ordinal``",
   '- The prior rr_adaptive_focus point is fixed at RV 0.45%, momentum 0.20%, adaptive trend 0.30%, and reserve 0.05%.',
   '- This package varies only the adaptive-trend ceiling or residual reserve around that point.',
