...
param(
   ...
   [string]$OutputPrefix = "RC2_DI_REPAIR_PORTABLE"
)
...
try {
   ...
   Set-Content -LiteralPath $unlock -Value "User-authorized isolated RC2 DI-repair discovery." -Encoding ASCII
   ...
   Set-Content -LiteralPath $hardLock -Value "Restored after isolated RC2 DI-repair discovery." -Encoding ASCII
}
