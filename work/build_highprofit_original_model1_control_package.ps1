param(
   [string]$SourcePackageDir = "outputs\range_elite_dgf_lossblock_highprofit_continuous_model4_package",
   [string]$BaseProfilePath = "outputs\CANDIDATE_RANGE_ELITE_HIGHPROFIT_PEAKTRAIL_OFF_CONTINUOUS_PROFILE.set",
   [string]$PackageDir = "outputs\highprofit_original_model1_control_package",
   [string]$QueueManifestPath = "outputs\HIGHPROFIT_ORIGINAL_MODEL1_CONTROL_QUEUE.csv",
   [string]$PackageManifestPath = "outputs\HIGHPROFIT_ORIGINAL_MODEL1_CONTROL_PACKAGE_MANIFEST.csv"
)
...
if(!(Test-Path -LiteralPath $source)) { throw "Original F254 source missing: $source" }
if(!(Test-Path -LiteralPath $baseProfile)) { throw "High-profit profile missing: $baseProfile" }
...
if($sourceHash -ne "F254FDF07B932FD8009E1ABFD761D1C9195568596A559F0DCB73A8CD29157D8F") {
   throw "Original high-profit source hash changed: $sourceHash"
}
...
$candidate = "highprofit_original_f254_model1_control"
...
Set-InputLine -Inputs $inputs -Name "InpEvidenceRunLabel" -Value "highprofit_original_f254_model1_control"
...
$stopRule = "Compare only with the common-executable high-profit Model1 control; mismatch rejects source portability."

@([pscustomobject]@{
   QueueRank = 1; Candidate = $candidate; CandidateRank = 1; SourceType = "highprofit_original_control"
   SourceRank = 1; Phase = "original_f254_model1_control"; Set = $profileName; Window = "continuous_2019_2026"
...
@([pscustomobject]@{
   QueueRank = 1; Candidate = $candidate; Phase = "original_f254_model1_control"
   PhaseLabel = "Original F254 high-profit Model1 control"; Window = "continuous_2019_2026"; Model = 1; Deposit = 10000
