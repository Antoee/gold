[CmdletBinding()]
param(
   [string]$SourcePath = 'work\Professional_XAUUSD_Three_Lane_Reversion_Strong_Reward_Quality_Research.mq5',
   [string]$ChampionProfilePath = 'release\three-lane-trade-ready-rc2-atb150\THREE_LANE_TRADE_READY_RC2_ATB150.set',
   [string]$PackageDir = 'outputs\three_lane_reversion_strong_reward_quality_model4_package',
   [string]$QueuePath = 'outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL4_QUEUE.csv',
   [string]$ManifestPath = 'outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL4_MANIFEST.csv',
   [string]$PackageMarkdownPath = 'outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL4_PACKAGE.md',
   [string]$ContractPath = 'outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL4_CONTRACT.md'
)
...
$expectedSourceHash = '0AF69BFE66200C6CB6F1D4C83A3F4BF3989DFA79E15596A27F384787991A3186'
...
& (Join-Path $PSScriptRoot 'test_three_lane_reversion_strong_reward_quality_source.ps1') | Out-Null
...
if($sourceHash -ne $expectedSourceHash) { throw "Strong-reward quality source identity changed: $sourceHash" }
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_reversion_strong_reward_quality_model4' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='rvsrq_control';Role='control';Enabled='false';Quality='false';MinimumRR='1.20'},
   [pscustomobject]@{Name='rvsrq_rr135';Role='lower_neighbor';Enabled='true';Quality='true';MinimumRR='1.35'},
   [pscustomobject]@{Name='rvsrq_rr150';Role='center';Enabled='true';Quality='true';MinimumRR='1.50'},
   [pscustomobject]@{Name='rvsrq_rr165';Role='upper_neighbor';Enabled='true';Quality='true';MinimumRR='1.65'}
)
$windows = @(
...
$stopRule = 'Strong-reward quality allocation. Center RR1.50 must keep every era >= control, continuous net >= control +5%, CAGR >= control +0.10 points, PF/recovery/return-DD >= control, DD <=1.25% and <=control +0.08 points, and >=400 trades. RR1.35 and RR1.65 neighbors must keep every era >= control, net >= control +3%, CAGR >= control +0.05 points, PF/recovery/return-DD >= control, DD <=1.25%, and >=400 trades. No post-result retuning.'
...
foreach($variant in $variants) {
   ...
   Set-FixedInput $inputs 'InpRVUseStrongRewardQuality' $variant.Quality
   Set-FixedInput $inputs 'InpRVStrongSignalMinimumAdjustedRiskReward' $variant.MinimumRR
   ...
   foreach($window in $windows) {
      ...
      $common = [ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='reversion_strong_reward_quality_model4'
         Window=$window.Name;From=$window.From;To=$window.To;Model=4;Deposit=10000
         Enabled=$variant.Enabled;RewardQualityEnabled=$variant.Quality;StrongSignalMinimumBodyRatio='0.25';StrongSignalRiskPercent='0.70';StrongSignalMinimumAdjustedRiskReward=$variant.MinimumRR
...
@(
   '# Three-Lane Reversion Strong-Reward Quality Model 4 Package','',
   '**Status: FIXED REAL-TICK VALIDATION AFTER MODEL 1 PASS. ATB150 AND THE FORWARD CANDIDATE ARE UNCHANGED.**','',
   "- Research source SHA-256: ``$sourceHash``",
   "- Frozen champion profile SHA-256: ``$championHash``",
   "- Variants: ``$($variants.Count)``",
   "- Configurations: ``$ordinal``",
   '- This separately preregistered follow-up freezes body ratio 0.25 and requested risk 0.70%, then tests whether spread-adjusted pre-entry reward/risk can qualify the extra allocation more efficiently.',
   '- The optional allocator may raise requested reversion risk only when the already-valid completed H1 signal candle meets the body threshold and the already-computed spread-adjusted reward/risk meets the frozen quality threshold.',
...
@(
   '# Reversion Strong-Reward Quality Model 4 Contract','',
   ...
   '- Freeze completed-H1 directional body ratio at 0.25 and requested strong risk at 0.70%. Test only disabled control and the Model 1 passing adjusted reward/risk thresholds 1.35, 1.50, and 1.65.',
   '- Use only completed signal-bar OHLC and spread-adjusted target/stop geometry known before entry. Never use current-bar data, future data, prior outcomes, drawdown, loss streaks, account profit, or calendar exclusions to qualify stronger risk.',
   ...
   '- Require the 1.50 center to match or beat control net in every era, continuous net at least 5% above control, CAGR at least 0.10 points above control, PF/recovery/return-drawdown no worse than control, drawdown at most 1.25% and no more than 0.08 points above control, and at least 400 continuous trades.',
   '- Require both 1.35 and 1.65 neighbors to match or beat control in every era, continuous net at least 3% above control, CAGR at least 0.05 points above control, PF/recovery/return-drawdown no worse than control, drawdown at most 1.25%, and at least 400 trades.',
