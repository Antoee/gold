...
param(
   ...
   [string]$PackageDir = 'outputs\three_lane_residual_risk_holdout_model1_package',
   [string]$QueuePath = 'outputs\THREE_LANE_RESIDUAL_RISK_HOLDOUT_MODEL1_QUEUE.csv',
   [string]$ManifestPath = 'outputs\THREE_LANE_RESIDUAL_RISK_HOLDOUT_MODEL1_PACKAGE_MANIFEST.csv',
   [string]$PackageMarkdownPath = 'outputs\THREE_LANE_RESIDUAL_RISK_HOLDOUT_MODEL1_PACKAGE.md',
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_residual_risk_holdout_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='rr_holdout_control';Enabled='false';Reserve='0.05';RVMax='0.45';MOMax='0.15';ATBMax='0.15'},
   [pscustomobject]@{Name='rr_holdout_selected';Enabled='true';Reserve='0.05';RVMax='0.45';MOMax='0.20';ATBMax='0.30'}
)
$windows = @(
...
$stopRule = 'Historical cross-period check only, not pristine out-of-sample evidence. Require both disjoint periods positive, selected continuous net/CAGR/return-DD above paired control, PF >= 1.50, DD <= 2.50%, and no material recent-period degradation before Model 4 opens.'
...
foreach($variant in $variants) {
   ...
   foreach($window in $windows) {
      ...
      $common = [ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Phase='holdout_model1'
...
@(
   '# Three-Lane Residual-Risk Historical Cross-Period Package','',
   '**Status: 2021-2026 MODEL 1 CHECK ONLY. THE ATB150 CHAMPION AND FORWARD CANDIDATE ARE UNCHANGED.**','',
   "- Research source SHA-256: ``$sourceHash``",
   "- Frozen champion profile SHA-256: ``$championHash``",
   "- Variants: ``$($variants.Count)``",
   "- Configurations: ``$ordinal``",
   '- The selected point is frozen at RV 0.45%, momentum 0.20%, adaptive trend 0.30%, and reserve 0.05%.',
   '- The paired control is exact ATB150 behavior with residual allocation disabled.',
