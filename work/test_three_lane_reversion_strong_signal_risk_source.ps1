...
$forkPath = Join-Path $repo 'work\Professional_XAUUSD_Three_Lane_Reversion_Strong_Signal_Risk_Research.mq5'
$expectedBaseHash = '2F1C1C74067DA6173EB4133DB75C0B0DB4DE7BE46F2BB7A453AEE044536B2158'
$expectedForkHash = '36300BA97B4384C1860ED7754495C5EFC74D2C75603BF0CDCD24BC31D9EAB1DF'
...
if($forkHash -ne $expectedForkHash) { throw "Strong-signal risk source identity changed: $forkHash" }
...
foreach($token in $required) {
   if($fork.IndexOf($token, [StringComparison]::Ordinal) -lt 0) {
      throw "Strong-signal risk source is missing required token: $token"
   }
}
...
if(!$allocator.Success) { throw 'Strong-signal allocator block could not be isolated.' }
