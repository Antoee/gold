...
input ulong  InpMagicNumber = 26071721;
...
input group "Previous-Day Liquidity Sweep Engine"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_M15;
input double InpMinimumSweepATR = 0.05;
input double InpMinimumSweepPoints = 10.0;
input double InpMaximumSweepATR = 1.25;
...
input bool   InpUsePreviousDayMidpointTarget = false;
...
input string InpLogFileName = "Independent_XAUUSD_M15_PrevDay_Sweep_Trades.csv";
...
{
   return "IM15PDS_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "M15PDS-LIVE-ACK"))
   {
...
bool OpenSweepPosition(const bool buy,
                       const double atr,
                       const double rawStop,
                       const double previousDayHigh,
                       const double previousDayLow)
{
   MqlTick tick;
   ...
   if(InpUsePreviousDayMidpointTarget)
   {
      double midpoint = 0.5 * (previousDayHigh + previousDayLow);
   ...
   string comment = buy ? "M15PDS_BUY" : "M15PDS_SELL";
   ...
   LogEvent("entry", trade.ResultOrder(), buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
            "previous-day liquidity sweep reclaim");
   return true;
}
...
bool PreviousDayLevels(double &previousDayHigh, double &previousDayLow)
{
   ...
   return previousDayHigh > previousDayLow && previousDayLow > 0.0;
}

bool TryPreviousDaySweepEntry(const double atr)
{
   if(atr <= 0.0)
      return false;
   double previousDayHigh = 0.0;
   double previousDayLow = 0.0;
   if(!PreviousDayLevels(previousDayHigh, previousDayLow))
      return false;
   ...
   double buySweepDepth = previousDayLow - low1;
   double sellSweepDepth = high1 - previousDayHigh;
   ...
   {
      buy = buy && low2 > previousDayLow - minimumSweep;
      sell = sell && high2 < previousDayHigh + minimumSweep;
   }
   if(!buy && !sell)
      return false;
   g_sweepCandidates++;
   ...
   return OpenSweepPosition(buy, atr, rawStop, previousDayHigh, previousDayLow);
}

int OnInit()
{
   if(InpMinimumSweepATR < 0.0 || InpMinimumSweepPoints < 0.0 ||
      ...
      (InpUsePreviousDayMidpointTarget && InpMinimumMidpointTargetR <= 0.0) ||
...
{
   PrintFormat("M15PDS_DIAGNOSTIC candidates=%d signals=%d rejection_shape_rejects=%d regime_rejects=%d safety_rejects=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
