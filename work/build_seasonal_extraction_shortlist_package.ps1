param(
   [string]$PackageDir = "work\local_mt5_seasonal_extraction_shortlist_package",
...
foreach($profile in $profiles) {
   foreach($window in $windows) {
      ...
      $reportName = "seasonal_extraction_shortlist_{0}_{1}" -f $profile.Name, $window.Window
...
$expected | Export-Csv -LiteralPath "outputs\SEASONAL_EXTRACTION_SHORTLIST_MANIFEST.csv" -NoTypeInformation
"Built $rank seasonal extraction shortlist configs in $PackageDir"
