param(
   ...
   [string]$DecisionCsvPath = "outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_DECISION_FIXTURE.csv",
   [string]$ManifestPath = "outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_GATE_MANIFEST.csv",
   [string]$ReportDir = "outputs\rdmc_momentum_retest_hold_v6_package\reports_here",
   [string]$RunnerLedgerGlob = "",
   [string]$ResultsPath = "outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_GATE_RESULTS.csv",
   [string]$RunAuditPath = "",
...
$evaluator = Join-Path $PSScriptRoot "evaluate_rdmc_momentum_retest_hold_v6.py"
$expectedManifestHash = "EB636293E3A74D260B82D1720D57EB11EBEEC2B1FCF2C55FD091E0A60AFA9B02"
$expectedSourceHash = "B99BEFF28BB0D28596F4D7786C65BDA11001B0BC4924F269F534817EADC6CDCC"
$expectedProfileHash = "9E8AC3C528860799CB152245CF1FFB084A8E9FD07A2A33F70FD927EFA077D390"
...
if([string]::IsNullOrWhiteSpace($RunnerLedgerGlob)) {
   $RunnerLedgerGlob = "outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_WAVE_{0:D2}_WORKER_*.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($RunAuditPath)) {
   $RunAuditPath = "outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_WAVE_{0:D2}_RUN_AUDIT.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($RawResultsPath)) {
   $RawResultsPath = "outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_WAVE_{0:D2}_RAW_RESULTS.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($SummaryPath)) {
   $SummaryPath = "outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_WAVE_{0:D2}_SUMMARY.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($MetricsMarkdownPath)) {
   $MetricsMarkdownPath = "outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_WAVE_{0:D2}_METRICS.md" -f $Wave
}
...
$waveManifestTemp = Join-Path $repo ("outputs\.rdmc_mrh6_wave_{0:D2}_{1}.csv" -f $Wave,[guid]::NewGuid().ToString("N"))
try {
...
if(!$SkipAdmissionRefresh) {
   if((Resolve-Path -LiteralPath $resultsFull).Path -ne (Join-Path $repo "outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_GATE_RESULTS.csv")) {
