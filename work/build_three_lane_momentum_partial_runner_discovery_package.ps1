...
param(
   ...
   [string]$PackageDir = 'outputs\three_lane_momentum_partial_runner_discovery_model1_package',
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_momentum_partial_runner_discovery_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='mopr_control';Role='control';Enabled='false';Close='60';Trigger='2.00';Target='4.00';Lock='1.25'},
   [pscustomobject]@{Name='mopr_center';Role='center';Enabled='true';Close='60';Trigger='2.00';Target='4.00';Lock='1.25'},
   [pscustomobject]@{Name='mopr_close50';Role='close_neighbor_low';Enabled='true';Close='50';Trigger='2.00';Target='4.00';Lock='1.25'},
   [pscustomobject]@{Name='mopr_close70';Role='close_neighbor_high';Enabled='true';Close='70';Trigger='2.00';Target='4.00';Lock='1.25'},
   [pscustomobject]@{Name='mopr_target300';Role='target_neighbor_low';Enabled='true';Close='60';Trigger='2.00';Target='3.00';Lock='1.25'},
   [pscustomobject]@{Name='mopr_target500';Role='target_neighbor_high';Enabled='true';Close='60';Trigger='2.00';Target='5.00';Lock='1.25'},
   [pscustomobject]@{Name='mopr_lock100';Role='lock_neighbor_low';Enabled='true';Close='60';Trigger='2.00';Target='4.00';Lock='1.00'},
   [pscustomobject]@{Name='mopr_lock150';Role='lock_neighbor_high';Enabled='true';Close='60';Trigger='2.00';Target='4.00';Lock='1.50'}
)
$windows = @(
...
$stopRule = 'Frozen pre-2021 discovery. The disabled control must exactly reproduce the leader. Every report must be profitable. Both center eras must retain >=98% of control; center continuous net must improve >=5%, CAGR >=control +0.08 point, PF >=95% of control, DD <=min(1.25%, control +0.15 point), and recovery/return-DD >=95% of control. Partial activity must be confirmed. At least four of six neighbors must retain both-era floors and improve continuous net >=2% with the same quality and drawdown limits. Initial risk, entries, portfolio cap, and all account protections remain fixed. No post-result rescue.'
...
foreach($variant in $variants) {
   ...
   foreach($window in $windows) {
      ...
      $rows.Add([pscustomobject][ordered]@{
         ...
         Phase='momentum_partial_runner_discovery_model1';Window=$window.Name
...
@(
   '# Momentum Partial-Runner Discovery Contract','',
   ...
   '- Frozen center: split-capable momentum positions bank 60% at +2.00R, first lock the remainder at +1.25R, and target +4.00R. Broker-volume normalization applies; unsplittable positions keep the original +2.00R exit.',
   '- Frozen neighborhood: close 50/70%, target +3.00/+5.00R, and stop lock +1.00/+1.50R.',
