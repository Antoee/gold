...
foreach($candidate in ($queue | Group-Object Candidate)) {
   ...
   foreach($required in @(
      ...
      "InpVolatilityRiskTimeframe=16385", "InpVolatilityRiskATRPeriod=20", "InpVolatilityRiskBaselineBars=",
      "InpVolatilityRiskMinimumScale=", "InpVolatilityRiskMaximumScale=",
...
$control = @($queue | Where-Object Candidate -eq "avp_fixed_control")
Assert-True ($control.Count -eq 3 -and @($control.AdaptiveEnabled | Sort-Object -Unique)[0] -eq "false") "Fixed-risk control missing."
