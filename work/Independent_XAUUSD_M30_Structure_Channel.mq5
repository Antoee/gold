...
input ulong  InpMagicNumber = 26071661;
...
input int    InpEntryLookbackBars = 48;
input int    InpExitLookbackBars = 24;
...
input string InpLogFileName = "Independent_XAUUSD_M30_Structure_Channel_Trades.csv";
...
{
   return "IM30SC_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "M30SC-LIVE-ACK"))
   {
...
bool OpenTrendPosition(const bool buy, const double atr)
{
   ...
   string comment = buy ? "M30SC_BUY" : "M30SC_SELL";
   ...
   LogEvent("entry", trade.ResultOrder(), buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
            "fresh M30 channel breakout with structure stop");
   return true;
}
...
{
   PrintFormat("M30SC_DIAGNOSTIC signals=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
