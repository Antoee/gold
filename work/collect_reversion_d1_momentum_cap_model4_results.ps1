param(
   ...
   [string]$ReportDir = "outputs\reversion_d1_momentum_cap_model4_package\reports_here",
   [string]$RawResultsPath = "outputs\REVERSION_D1_MOMENTUM_CAP_MODEL4_RAW_RESULTS.csv",
   [string]$ResultsPath = "outputs\REVERSION_D1_MOMENTUM_CAP_MODEL4_RESULTS.csv",
   [string]$RunPath = "outputs\REVERSION_D1_MOMENTUM_CAP_MODEL4_RUN.csv",
   [string]$SummaryPath = "outputs\REVERSION_D1_MOMENTUM_CAP_MODEL4_SUMMARY.csv",
   [string]$MarkdownPath = "outputs\REVERSION_D1_MOMENTUM_CAP_MODEL4_METRICS.md"
)
...
if($queue.Count -ne 15) { throw "Expected fifteen frozen Model4 queue rows." }
if(@($queue | Where-Object { $_.Model -ne "4" -or $_.To -gt "2026.07.16" }).Count -gt 0) { throw "Unexpected Model4 boundary." }
...
$initialRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath "outputs") -Filter "REVERSION_D1_MOMENTUM_CAP_MODEL4_PORTABLE_*.csv" |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
$retryRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath "outputs") -Filter "REVERSION_D1_MOMENTUM_CAP_MODEL4_RERUN_*.csv" |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
...
if($raw.Count -ne 15) { throw "Expected fifteen parsed Model4 results." }
$results = foreach($queued in $queue) {
   ...
   [pscustomobject]@{
      ...
      HoldoutContractSha256=$queued.HoldoutContractSha256
      Model4ContractSha256=$queued.Model4ContractSha256;InitialDeposit=$row.InitialDeposit
