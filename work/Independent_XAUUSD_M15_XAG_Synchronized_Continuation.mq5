...
input double InpMinimumXAUMoveATR = 0.50;
input double InpMinimumXAGMoveATR = 0.50;
...
input string InpLogFileName = "Independent_XAUUSD_M15_XAG_Synchronized_Continuation_Trades.csv";
...
struct CrossMetalFeatures
{
   ...
   double breakoutHigh;
   double breakoutLow;
   double priorBreakoutHigh;
   double priorBreakoutLow;
   double signalClose;
   double priorClose;
   double bodyPercent;
...
int      g_synchronizedCandidates = 0;
...
{
   return "IM15XMSC_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "XMSC-LIVE-ACK"))
   {
...
bool ChannelBoundsFromRates(const MqlRates &rates[],
                            const int startIndex,
                            const int count,
                            double &channelHigh,
                            double &channelLow)
{
   if(startIndex < 0 || count < 2 || ArraySize(rates) < startIndex + count)
      return false;
   channelHigh = -DBL_MAX;
   channelLow = DBL_MAX;
   ...
   return channelLow > 0.0 && channelHigh > channelLow;
}

bool BuildFeatures(CrossMetalFeatures &features)
{
   int required = InpMoveLookbackBars + 2;
   if(InpBreakoutLookbackBars + 2 > required)
      required = InpBreakoutLookbackBars + 2;
   ...
   if(!ChannelBoundsFromRates(xauRates, 1, InpBreakoutLookbackBars,
                              features.breakoutHigh, features.breakoutLow) ||
      !ChannelBoundsFromRates(xauRates, 2, InpBreakoutLookbackBars,
                              features.priorBreakoutHigh, features.priorBreakoutLow))
      return false;
...
bool OpenSynchronizedContinuationPosition(const bool buy, const CrossMetalFeatures &features)
{
   MqlTick tick;
   ...
   string comment = buy ? "M15XMSC_BUY" : "M15XMSC_SELL";
   ...
   LogEvent("entry", trade.ResultOrder(), buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
            "cross-metal synchronized channel breakout", features.xauMoveAtr,
...
bool TrySynchronizedContinuationEntry()
{
   string safetyReason = "";
   ...
   bool buy = InpAllowBuy && features.xauMoveAtr >= InpMinimumXAUMoveATR &&
              features.xagMoveAtr >= InpMinimumXAGMoveATR;
   bool sell = InpAllowSell && features.xauMoveAtr <= -InpMinimumXAUMoveATR &&
               features.xagMoveAtr <= -InpMinimumXAGMoveATR;
   if(!buy && !sell)
      return false;
   g_synchronizedCandidates++;

   double breakoutBuffer = InpBreakoutBufferATR * features.xauAtr;
   buy = buy && features.signalClose > features.breakoutHigh + breakoutBuffer;
   sell = sell && features.signalClose < features.breakoutLow - breakoutBuffer;
   if(InpRequireFreshBreakout)
   {
      buy = buy && features.priorClose <= features.priorBreakoutHigh + breakoutBuffer;
      sell = sell && features.priorClose >= features.priorBreakoutLow - breakoutBuffer;
   }
   if(!buy && !sell)
   {
      g_breakoutRejects++;
   ...
   return OpenSynchronizedContinuationPosition(buy, features);
}

int OnInit()
{
   if(StringLen(InpReferenceSymbol) <= 0 || InpMoveLookbackBars < 2 ||
      InpBreakoutLookbackBars < 4 ||
      InpCorrelationLookbackBars < 3 || InpMinimumCorrelation < -1.0 || InpMinimumCorrelation > 1.0 ||
      InpMinimumXAUMoveATR <= 0.0 || InpMinimumXAGMoveATR <= 0.0 ||
      InpBreakoutBufferATR < 0.0 || InpMinimumSignalBodyPercent < 0.0 ||
...
{
   PrintFormat("M15XMSC_DIAGNOSTIC feature_bars=%d feature_failures=%d alignment_failures=%d synchronized_candidates=%d correlation_rejects=%d breakout_rejects=%d candle_rejects=%d safety_rejects=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
               g_featureBars, g_featureFailures, g_alignmentFailures, g_synchronizedCandidates,
               g_correlationRejects, g_breakoutRejects, g_candleRejects, g_safetyRejects,
...
void OnTick()
{
   ...
   if(closed || ManagedPositionCount() > 0)
      return;
   TrySynchronizedContinuationEntry();
}
