param(
   ...
   [string]$ReportDir = "outputs\independent_m15_weekend_gap_fade_discovery_model1_package\reports_here",
   [string]$ParserManifestPath = "outputs\INDEPENDENT_M15_WEEKEND_GAP_FADE_DISCOVERY_MODEL1_PARSE_MANIFEST.csv",
   [string]$RawResultsPath = "outputs\INDEPENDENT_M15_WEEKEND_GAP_FADE_DISCOVERY_MODEL1_RAW_RESULTS.csv",
   ...
   [string]$SummaryPath = "outputs\INDEPENDENT_M15_WEEKEND_GAP_FADE_DISCOVERY_MODEL1_SUMMARY.csv",
   [string]$MarkdownPath = "outputs\INDEPENDENT_M15_WEEKEND_GAP_FADE_DISCOVERY_MODEL1_METRICS.md"
)
...
if($queue.Count -ne 21) { throw "Expected 21 frozen queue rows, found $($queue.Count)." }
if(@($queue | Where-Object { $_.To -gt '2020.12.31' }).Count -gt 0) {
   throw "Post-2020 data leaked into the collector."
}

$initialRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath 'outputs') -Filter 'INDEPENDENT_M15_WEEKEND_GAP_FADE_PORTABLE_*.csv' |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
$retryRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath 'outputs') -Filter 'INDEPENDENT_M15_WEEKEND_GAP_FADE_RETRY_*.csv' |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
...
$results = foreach($row in $raw) {
   ...
   [pscustomobject]@{
      ...
      RunLabel = $q.RunLabel
      MinimumGapATR = $q.MinimumGapATR
      MaximumGapATR = $q.MaximumGapATR
      MinimumConfirmationFraction = $q.MinimumConfirmationFraction
