...
| local-reproducibility-freeze | PASS | Local reproducibility bundle manifest exists, zip exists, required checks pass, and every bundle row is PASS | rows=72; pass=72; pending=0; fail=0; zipExists=True; missingRequiredChecks=0 | outputs\TRADE_READY_REPRODUCIBILITY_BUNDLE_MANIFEST.csv + outputs\trade_ready_reproducibility_bundle.zip | Keep rebuilding the bundle after every source/profile/status change; this local freeze does not replace GitHub/source-publication sync. |
