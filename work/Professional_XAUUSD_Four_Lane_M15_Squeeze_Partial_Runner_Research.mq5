...
#property description "Research fork adding a restart-safe partial runner to the M15 squeeze lane"
...
private:
   ...
   void ManagePartialRunner()
   {
      ...
      for(int i = PositionsTotal() - 1; i >= 0; --i)
      {
         ...
         WriteEvidenceEvent(m_logHandle, "m15_sq_pr", "partial_runner", ticket,
   ...
   bool OpenPosition(const bool buy, const double atr, const double rawStop)
   {
      MqlTick tick;
      if(!SymbolInfoTick(_Symbol, tick) || atr <= 0.0)
         return false;
      ...
      WriteEvidenceEvent(m_logHandle, runnerRequested ? "m15_sq_pr" : "m15_sq_b8", "entry", ticket,
                         buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
                         runnerRequested
                         ? "M15 squeeze breakout with protected partial runner"
                         : "M15 Bollinger-Keltner squeeze breakout with H1 trend alignment");
      return true;
   }
...
public:
   ...
   void OnTradeTransaction(const MqlTradeTransaction &transaction)
   {
      ...
      double profit = HistoryDealGetDouble(transaction.deal, DEAL_PROFIT) +
                      HistoryDealGetDouble(transaction.deal, DEAL_SWAP) +
                      HistoryDealGetDouble(transaction.deal, DEAL_COMMISSION);
      WriteEvidenceEvent(m_logHandle, "m15_sq_b8", "exit", transaction.deal, "close",
                         HistoryDealGetDouble(transaction.deal, DEAL_VOLUME),
                         HistoryDealGetDouble(transaction.deal, DEAL_PRICE), 0.0, profit,
                         HistoryDealGetString(transaction.deal, DEAL_COMMENT));
   }
