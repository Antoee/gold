...
param(
   ...
   [string]$OutputPrefix = "RC2_MOMENTUM_RISK_PORTABLE"
)
...
try {
   ...
   Set-Content -LiteralPath $unlock -Value "User-authorized isolated RC2 momentum-risk Model1 screen." -Encoding ASCII
   Set-Content -LiteralPath $hiddenAck -Value "Hidden portable-worker launch acknowledged." -Encoding ASCII
   & (Join-Path $PSScriptRoot "run_mt5_portable_parallel_manifest.ps1") `
      ...
      -MaxCpuPercent $MaxCpuPercent -TimeoutMinutesPerConfig $TimeoutMinutesPerConfig -ProgressIntervalSeconds 10
}
finally {
   Stop-MT5Family
   ...
   if([string]::IsNullOrWhiteSpace($oldFocus)) { Remove-Item Env:\ALLOW_MT5_FOCUS_RISK -ErrorAction SilentlyContinue }
   else { $env:ALLOW_MT5_FOCUS_RISK = $oldFocus }
   if([string]::IsNullOrWhiteSpace($oldHidden)) { Remove-Item Env:\ALLOW_MT5_HIDDEN_DESKTOP_ACK -ErrorAction SilentlyContinue }
   else { $env:ALLOW_MT5_HIDDEN_DESKTOP_ACK = $oldHidden }
   Set-Content -LiteralPath $hardLock -Value "Restored after isolated RC2 momentum-risk Model1 screen." -Encoding ASCII
}
