...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_D1_EXTREME_GUARD_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'D1 momentum extreme-guard research source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Momentum D1 extreme-guard compile $startedAtUtc",[Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck,"Momentum D1 extreme-guard focus acknowledgement $startedAtUtc",[Text.Encoding]::ASCII)
