...
#property description "Research fork with momentum/adaptive agreement allocation"
...
input bool   InpMOUseAdaptiveAgreementRisk = false;
input double InpMOAdaptiveAgreementRiskPercent = 0.25;
...
private:
   ...
   double EntryRiskPercent(const bool buy, bool &adaptiveAgreement)
   {
      adaptiveAgreement = false;
      if(!InpMOUseAdaptiveAgreementRisk)
         return InpMORiskPercent;
      long requiredType = buy ? POSITION_TYPE_BUY : POSITION_TYPE_SELL;
      for(int i = PositionsTotal() - 1; i >= 0; --i)
      {
         ulong ticket = PositionGetTicket(i);
         if(ticket == 0 || !PositionSelectByTicket(ticket))
         ...
         {
            adaptiveAgreement = true;
            return InpMOAdaptiveAgreementRiskPercent;
         }
      }
      return InpMORiskPercent;
   }
   ...
   bool OpenPosition(const bool buy, const double atr)
   {
      ...
      string comment = adaptiveAgreement
                       ? (buy ? "MTSM_XA_BUY" : "MTSM_XA_SELL")
      ...
      ulong ticket = 0;
      string postFillReason = "";
      if(!PostFillReconcile(m_trade, InpMOMagicNumber, buy, requestedRiskPercent,
      ...
      WriteEvidenceEvent(m_logHandle, "tlp_mom_e20", "entry", ticket,
                         buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
                         adaptiveAgreement
                         ? "daily momentum plus fresh H1 breakout; adaptive agreement allocation"
