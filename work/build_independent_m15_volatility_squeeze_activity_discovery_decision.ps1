param(
   [string]$QueuePath = "outputs\INDEPENDENT_M15_VOLATILITY_SQUEEZE_ACTIVITY_DISCOVERY_MODEL1_QUEUE.csv",
   [string]$ReportDir = "outputs\independent_m15_volatility_squeeze_activity_discovery_model1_package\reports_here",
   [string]$ResultsPath = "outputs\INDEPENDENT_M15_VOLATILITY_SQUEEZE_ACTIVITY_DISCOVERY_MODEL1_RESULTS.csv",
   [string]$SummaryPath = "outputs\INDEPENDENT_M15_VOLATILITY_SQUEEZE_ACTIVITY_DISCOVERY_MODEL1_SUMMARY.csv",
   [string]$MetricsPath = "outputs\INDEPENDENT_M15_VOLATILITY_SQUEEZE_ACTIVITY_DISCOVERY_MODEL1_METRICS.md",
   [string]$DecisionCsvPath = "outputs\INDEPENDENT_M15_VOLATILITY_SQUEEZE_ACTIVITY_DISCOVERY_DECISION.csv",
   [string]$DecisionMarkdownPath = "outputs\INDEPENDENT_M15_VOLATILITY_SQUEEZE_ACTIVITY_DISCOVERY_DECISION.md"
)
...
$rawResults = Join-Path $repo "work\M15SQA_RAW_RESULTS.csv"
$rawSummary = Join-Path $repo "work\M15SQA_RAW_SUMMARY.csv"
$rawMarkdown = Join-Path $repo "work\M15SQA_RAW_METRICS.md"
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "collect_validation_results.ps1") `
   ...
   -OutResults "work\M15SQA_RAW_RESULTS.csv" -OutSummary "work\M15SQA_RAW_SUMMARY.csv" `
   -OutMarkdown "work\M15SQA_RAW_METRICS.md" | Out-Null
...
$reproducedRanks = @(3, 4, 5, 12, 14, 34)
...
foreach($item in ($queue | Sort-Object { [int]$_.QueueRank })) {
   ...
   $identityPass = $reportText.IndexOf($sourceHash,[StringComparison]::OrdinalIgnoreCase) -ge 0 -and
                   $reportText.IndexOf("Volatility Squeeze Engine=",[StringComparison]::Ordinal) -ge 0
   ...
   $results.Add([pscustomobject]@{
      ...
      SqueezeBars=$item.SqueezeBars; KeltnerATRMultiplier=$item.KeltnerATRMultiplier
      BreakoutLookbackBars=$item.BreakoutLookbackBars; MaximumBreakoutChannelATR=$item.MaximumBreakoutChannelATR
      MinimumExpansionRatio=$item.MinimumExpansionRatio; TakeProfitR=$item.TakeProfitR
      UseTrend=$item.UseTrend; TrendEMAPeriod=$item.TrendEMAPeriod
      SessionStartHour=$item.SessionStartHour; SessionEndHour=$item.SessionEndHour; StopRule=$item.StopRule
...
$adjacency = @{
   m15sqa_center_b6=@('m15sqa_b4','m15sqa_b8','m15sqa_b10','m15sqa_sq2','m15sqa_sq4','m15sqa_kc140','m15sqa_kc160','m15sqa_kc170','m15sqa_exp090','m15sqa_exp100','m15sqa_exp120','m15sqa_trend50','m15sqa_trend200','m15sqa_session420')
   m15sqa_b4=@('m15sqa_center_b6'); m15sqa_b8=@('m15sqa_center_b6'); m15sqa_b10=@('m15sqa_center_b6')
   m15sqa_sq2=@('m15sqa_center_b6'); m15sqa_sq4=@('m15sqa_center_b6')
   m15sqa_kc140=@('m15sqa_center_b6'); m15sqa_kc160=@('m15sqa_center_b6'); m15sqa_kc170=@('m15sqa_center_b6')
   m15sqa_exp090=@('m15sqa_center_b6'); m15sqa_exp100=@('m15sqa_center_b6'); m15sqa_exp120=@('m15sqa_center_b6')
   m15sqa_trend50=@('m15sqa_center_b6'); m15sqa_trend200=@('m15sqa_center_b6'); m15sqa_session420=@('m15sqa_center_b6')
}
...
$md.Add("# Independent M15 Volatility-Squeeze Activity Discovery Decision")
$md.Add("")
...
$md.Add($headline)
$md.Add("")
$md.Add('This second and final pre-holdout activity screen kept the source byte-identical and centered the neighborhood on a six-bar fresh breakout after the first screen found a profitable but underactive eight-bar clue. Squeeze duration, Keltner width, four/eight/ten-bar breakouts, expansion, H1 trend length, and a wider liquid-hours session were isolated neighbors. Stops remained capped at `$6`, risk remained `0.10%`, and no post-2020 data was opened.')
$md.Add("")
...
$md.Add("- Empty M0 exports reproduced unchanged: ``$($reproducedRanks.Count)``; all final reports contain the correct source identity")
...
@(
   "# Independent M15 Volatility-Squeeze Activity Discovery Metrics", "",
   ...
   'See `outputs/INDEPENDENT_M15_VOLATILITY_SQUEEZE_ACTIVITY_DISCOVERY_DECISION.md` for the gated interpretation.'
