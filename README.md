...
- Newest test result: better research lead, still not trade-ready. The latest default-off DGF-liquidity signal rejection compiles on current source hash `69478904BB4073F48F8F963ED13D789BFE378456D4C054CAB16A8368F4065D92` and passed focused Model4 real-tick testing with `30 / 30` exported reports parsed. `re_may140_late15_dgf_liq_reject1` made `+$3,218.26` with worst window `-$40.20`, but 2019, 2021, and 2025 are still red and worst DD is `24.72%`, so it is a range-elite research lead only. The current stability lead remains unchanged.
