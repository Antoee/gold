param(
   [string]$QueuePath = "outputs\INDEPENDENT_M15_VOLUME_CLIMAX_REVERSAL_DISCOVERY_MODEL1_QUEUE.csv",
   [string]$ReportDir = "outputs\independent_m15_volume_climax_reversal_discovery_model1_package\reports_here",
   [string]$ResultsPath = "outputs\INDEPENDENT_M15_VOLUME_CLIMAX_REVERSAL_DISCOVERY_MODEL1_RESULTS.csv",
   [string]$SummaryPath = "outputs\INDEPENDENT_M15_VOLUME_CLIMAX_REVERSAL_DISCOVERY_MODEL1_SUMMARY.csv",
   [string]$MetricsPath = "outputs\INDEPENDENT_M15_VOLUME_CLIMAX_REVERSAL_DISCOVERY_MODEL1_METRICS.md",
   [string]$DecisionCsvPath = "outputs\INDEPENDENT_M15_VOLUME_CLIMAX_REVERSAL_DISCOVERY_DECISION.csv",
   [string]$DecisionMarkdownPath = "outputs\INDEPENDENT_M15_VOLUME_CLIMAX_REVERSAL_DISCOVERY_DECISION.md"
)
...
$rawResults = Join-Path $repo "work\M15VCR_RAW_RESULTS.csv"
$rawSummary = Join-Path $repo "work\M15VCR_RAW_SUMMARY.csv"
$rawMarkdown = Join-Path $repo "work\M15VCR_RAW_METRICS.md"
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "collect_validation_results.ps1") `
   ...
   -OutResults "work\M15VCR_RAW_RESULTS.csv" -OutSummary "work\M15VCR_RAW_SUMMARY.csv" `
   -OutMarkdown "work\M15VCR_RAW_METRICS.md" | Out-Null
...
foreach($item in ($queue | Sort-Object { [int]$_.QueueRank })) {
   ...
   $results.Add([pscustomobject]@{
      ...
      ProfileSnapshot=$item.ProfileSnapshot; ProfileSha256=$item.ProfileSha256; SourceSha256=$item.SourceSha256
       ...
       MinimumWickPercent=$item.MinimumWickPercent; MinimumVWAPDeviationATR=$item.MinimumVWAPDeviationATR
       MaximumADX=$item.MaximumADX; ExtremeLookbackBars=$item.ExtremeLookbackBars
       MinimumRiskReward=$item.MinimumRiskReward; MaximumTargetR=$item.MaximumTargetR
       UseRangePhaseFilter=$item.UseRangePhaseFilter; StopRule=$item.StopRule
...
$adjacency = @{
   m15vcr_center=@('m15vcr_vol130','m15vcr_vol170','m15vcr_range090','m15vcr_range130','m15vcr_wick35','m15vcr_wick55','m15vcr_dev030','m15vcr_dev060','m15vcr_adx22','m15vcr_adx34','m15vcr_ext4','m15vcr_ext12','m15vcr_rr125','m15vcr_nophase')
   m15vcr_vol130=@('m15vcr_center'); m15vcr_vol170=@('m15vcr_center')
   m15vcr_range090=@('m15vcr_center'); m15vcr_range130=@('m15vcr_center')
   m15vcr_wick35=@('m15vcr_center'); m15vcr_wick55=@('m15vcr_center')
   m15vcr_dev030=@('m15vcr_center'); m15vcr_dev060=@('m15vcr_center')
   m15vcr_adx22=@('m15vcr_center'); m15vcr_adx34=@('m15vcr_center')
   m15vcr_ext4=@('m15vcr_center'); m15vcr_ext12=@('m15vcr_center')
   m15vcr_rr125=@('m15vcr_center'); m15vcr_nophase=@('m15vcr_center')
}
...
$md.Add("# Independent M15 Volume-Climax Reversal Discovery Decision")
$md.Add("")
...
$md.Add('- Stale portable exports reproduced unchanged on alternate workers: `5`; all final reports contain the correct source identity')
...
@(
   "# Independent M15 Volume-Climax Reversal Discovery Metrics", "",
   ...
   'See `outputs/INDEPENDENT_M15_VOLUME_CLIMAX_REVERSAL_DISCOVERY_DECISION.md` for the gated interpretation.'
