param(
   ...
   [string]$ReportDir = "outputs\three_lane_reversion_strong_reward_quality_model4_package\reports_here",
   [string]$ResultsPath = "outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL4_RESULTS.csv",
   [string]$SummaryPath = "outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL4_SUMMARY.csv",
   [string]$DecisionCsvPath = "outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL4_DECISION.csv",
   [string]$DecisionMarkdownPath = "outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL4_DECISION.md",
   [string]$RunAttestationPath = "outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL4_RUN_ATTESTATION.csv"
)
...
if($manifest.Count -ne 16) { throw "Expected sixteen frozen Model4 manifest rows." }
...
$rawResults = "work\RVSRQM4_RAW_RESULTS.csv"
$rawSummary = "work\RVSRQM4_RAW_SUMMARY.csv"
$rawMetrics = "work\RVSRQM4_RAW_METRICS.md"
...
if($raw.Count -ne 16 -or @($raw | Where-Object Status -ne "PARSED").Count -ne 0) {
   throw "Expected sixteen parsed Model4 reports."
}
$rawByReport = @{}
foreach($row in $raw) { $rawByReport[[string]$row.ExpectedReportName] = $row }

$workerRows = [Collections.Generic.List[object]]::new()
$workerFiles = @(Get-ChildItem (Join-Path $repo "outputs") -Filter "THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL4_EXACT_?.csv" -File) | Sort-Object Name
...
if($workerRows.Count -ne 16 -or @($workerRows | Where-Object {
   ...
   $_.PortableBinarySha256 -ne $expectedBinarySha256
}).Count -ne 0) {
...
if(@($workerRows | Where-Object Status -eq "ERROR").Count -ne 0) {
   throw "Model4 runner evidence contains an unexpected error."
}
...
$lines.Add("# Three-Lane Reversion Strong-Reward Quality Model 4 Decision")
$lines.Add("")
$lines.Add($(if($passed) {
   "**Decision: MODEL 4 GATE PASSED. Annual, cost, and Monte Carlo validation may open; no promotion or forward change is authorized.**"
} else {
...
$lines.Add("- Attempts: ``16``; identity-only retries: ``0``")
...
$lines.Add("")
$lines.Add("The center changes no entry or exit rule. It allocates extra risk only when the already-valid completed-bar reversion also has spread-adjusted reward/risk of at least 1.50. Continuous real-tick Model 4 net improved by ``$('{0:N2}' -f $netImprovementPercent)%`` and CAGR by ``$($candidate.CagrPercent - $control.CagrPercent)`` point, but recovery, return-to-drawdown, and relative drawdown failed their frozen gates.")
$lines.Add("")
$lines.Add($(if($passed) {
   "The center and both reward-quality neighbors passed the preregistered Model 4 growth and efficiency gates. Annual, cost, and Monte Carlo validation may open, but this is not a promotion and the forward candidate remains unchanged."
} else {
   "The center added reward, but failed recovery, return-to-drawdown, and relative-drawdown requirements. The RR 1.35 lower neighbor retained higher return-to-drawdown but missed the no-worse recovery gate by ``0.0020``. The frozen gate is not relaxed after observation; annual and stress testing stay closed, and ATB150 remains the historical champion. Center recovery difference versus control: ``$('{0:F4}' -f $recoveryDifference)``."
}))
$lines.Add("")
