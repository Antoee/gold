...
param(
   ...
   [string]$PackageDir = 'outputs\three_lane_momentum_profit_ratchet_discovery_model1_package',
...
& (Join-Path $PSScriptRoot 'test_three_lane_momentum_profit_ratchet_source.ps1') | Out-Null
...
if($sourceHash -ne $expectedSourceHash) { throw "Momentum profit-ratchet source identity changed: $sourceHash" }
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_momentum_profit_ratchet_discovery_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='mpr_control';Role='disabled_control';Enabled=$false;Trigger='1.50';Lock='0.75'},
   [pscustomobject]@{Name='mpr_trigger125';Role='lower_trigger';Enabled=$true;Trigger='1.25';Lock='0.75'},
   [pscustomobject]@{Name='mpr_center';Role='center';Enabled=$true;Trigger='1.50';Lock='0.75'},
   [pscustomobject]@{Name='mpr_trigger175';Role='upper_trigger';Enabled=$true;Trigger='1.75';Lock='0.75'},
   [pscustomobject]@{Name='mpr_lock050';Role='lower_lock';Enabled=$true;Trigger='1.50';Lock='0.50'},
   [pscustomobject]@{Name='mpr_lock100';Role='upper_lock';Enabled=$true;Trigger='1.50';Lock='1.00'},
   [pscustomobject]@{Name='mpr_conservative';Role='conservative_corner';Enabled=$true;Trigger='1.75';Lock='0.50'},
   [pscustomobject]@{Name='mpr_aggressive';Role='aggressive_corner';Enabled=$true;Trigger='1.25';Lock='1.00'}
)
$windows = @(
...
foreach($variant in $variants) {
   ...
   Set-FixedInput $inputs 'InpMOUseProfitRatchet' $variant.Enabled.ToString().ToLowerInvariant()
   Set-FixedInput $inputs 'InpMOProfitRatchetTriggerR' $variant.Trigger
   Set-FixedInput $inputs 'InpMOProfitRatchetLockR' $variant.Lock
   ...
   foreach($window in $windows) {
      ...
      $rows.Add([pscustomobject][ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='momentum_profit_ratchet_discovery_model1'
         ...
         FeatureEnabled=$variant.Enabled;RatchetTriggerR=$variant.Trigger;RatchetLockR=$variant.Lock
...
@(
   '# Momentum Profit Ratchet Package','',
   ...
   '- The default-off fork adds one second-stage stop ratchet evaluated only on the existing completed-H1 management cycle.',
   '- Entry signals, initial stops, take-profit distance, risk sizing, portfolio exposure, and all safety locks are unchanged.',
