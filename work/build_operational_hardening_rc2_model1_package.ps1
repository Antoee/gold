param(
   ...
   [string]$PackageDir = "outputs\operational_hardening_rc2_model1_package",
   [string]$QueueManifestPath = "outputs\OPERATIONAL_HARDENING_RC2_MODEL1_QUEUE.csv",
   [string]$PackageManifestPath = "outputs\OPERATIONAL_HARDENING_RC2_MODEL1_PACKAGE_MANIFEST.csv",
   [string]$MarkdownPath = "outputs\OPERATIONAL_HARDENING_RC2_MODEL1_PACKAGE.md"
)
...
Set-PinnedInput -Inputs $inputs -Name "InpRVLogFileName" -Value "OPERATIONAL_HARDENING_RC2_RV_MODEL1_EVENTS.csv" -StringValue
Set-PinnedInput -Inputs $inputs -Name "InpMOLogFileName" -Value "OPERATIONAL_HARDENING_RC2_MO_MODEL1_EVENTS.csv" -StringValue
Set-PinnedInput -Inputs $inputs -Name "InpEvidenceSourceHash" -Value $sourceHash -StringValue
Set-PinnedInput -Inputs $inputs -Name "InpEvidenceRunLabel" -Value "operational_hardening_rc2_model1" -StringValue
...
$profileName = "operational_hardening_rc2_rv045_mo015_model1.set"
$profilePath = Join-Path $profileDir $profileName
@($inputs.Keys | Sort-Object | ForEach-Object { $inputs[$_] }) |
   Set-Content -LiteralPath $profilePath -Encoding ASCII
$profileHash = (Get-FileHash -LiteralPath $profilePath -Algorithm SHA256).Hash

$configName = "001_operational_hardening_rc2_continuous_2015_2026_m1.ini"
$reportName = "operational_hardening_rc2_continuous_2015_2026_m1"
...
$stopRule = "Require zero signal drift versus v0.1 and v0.2-rc1: identical trade count, lane, side, entry time, exit time, and profit within report precision. Any historical divergence rejects rc2."
$queue = [pscustomobject]@{
   QueueRank=1; Candidate="operational_hardening_rc2_rv045_mo015"; CandidateRank=1
   SourceType="dedicated_account_safety_fork"; SourceRank=1; Phase="continuous_model1_fidelity"
...
$manifest = [pscustomobject]@{
   QueueRank=1; Candidate="operational_hardening_rc2_rv045_mo015"; Phase="continuous_model1_fidelity"
   PhaseLabel="Operational-hardening rc2 Model1 schedule fidelity"
   Window="continuous_2015_2026"; Model=1; Deposit=10000
