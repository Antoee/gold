...
param(
   ...
   [string]$PackageDir = 'outputs\three_lane_residual_risk_broad_model4_package',
   [string]$QueuePath = 'outputs\THREE_LANE_RESIDUAL_RISK_BROAD_MODEL4_QUEUE.csv',
   [string]$ManifestPath = 'outputs\THREE_LANE_RESIDUAL_RISK_BROAD_MODEL4_PACKAGE_MANIFEST.csv',
   [string]$PackageMarkdownPath = 'outputs\THREE_LANE_RESIDUAL_RISK_BROAD_MODEL4_PACKAGE.md',
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_residual_risk_broad_model4' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='rr_model4_control';Enabled='false';Reserve='0.05';RVMax='0.45';MOMax='0.15';ATBMax='0.15'},
   [pscustomobject]@{Name='rr_model4_selected';Enabled='true';Reserve='0.05';RVMax='0.45';MOMax='0.20';ATBMax='0.30'}
)
$windows = @(
...
$stopRule = 'Model 4 real-tick adjudication. Require all three disjoint eras positive, exact disabled-feature control parity with ATB150, selected continuous net/CAGR/return-DD above paired control, PF >= 1.50, DD <= 2.50%, and no material middle/recent degradation before annual and stress gates open.'
...
foreach($variant in $variants) {
   ...
   foreach($window in $windows) {
      ...
      $common = [ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Phase='broad_model4'
...
@(
   '# Three-Lane Residual-Risk Broad Model 4 Package','',
   '**Status: PAIRED REAL-TICK ADJUDICATION ONLY. THE ATB150 CHAMPION AND FORWARD CANDIDATE ARE UNCHANGED.**','',
