...
#property description "Date-independent XAUUSD multiscale time-series momentum research EA"
...
input group "Multiscale Momentum Engine"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_H1;
input ENUM_TIMEFRAMES InpMomentumTimeframe = PERIOD_D1;
input int    InpMomentumLookbackBars = 126;
input int    InpEntryLookbackBars = 10;
...
input bool   InpUseChannelExit = true;
input int    InpExitLookbackBars = 5;
input bool   InpUseMomentumFailureExit = true;
...
input string InpLogFileName = "Independent_XAUUSD_Multiscale_Momentum_Trades.csv";
...
{
   return "IMSM_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "MTSM-LIVE-ACK"))
   {
...
{
   if(InpMomentumLookbackBars < 2)
      return 0;
   double recentClose = iClose(_Symbol, InpMomentumTimeframe, 1);
   double pastClose = iClose(_Symbol, InpMomentumTimeframe, 1 + InpMomentumLookbackBars);
...
bool OpenTrendPosition(const bool buy, const double atr)
{
   ...
   if(!ChannelBounds(1, InpStopLookbackBars, structureHigh, structureLow))
      return false;
   ...
   double minimumDistance = MathMax(InpMinimumStopATR * atr, stopLevel + _Point);
   if(stopDistance < minimumDistance)
      stopDistance = minimumDistance;
   if(stopDistance <= 0.0 || stopDistance > InpMaximumStopATR * atr ||
...
int OnInit()
{
   if(InpMomentumLookbackBars < 20 || InpEntryLookbackBars < 3 ||
      InpStopLookbackBars < 2 || InpExitLookbackBars < 2 ||
      InpATRPeriod < 2 || InpStopBufferATR < 0.0 ||
      InpMinimumStopATR <= 0.0 || InpMaximumStopATR < InpMinimumStopATR ||
      InpTakeProfitR <= 0.0 || InpMaximumHoldBars < 0 ||
