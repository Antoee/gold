...
if(!$UserAuthorizedFocusRisk) { throw 'Controlled same-side exit-cooldown recent confirmation requires explicit focus/window-risk authorization.' }
...
$manifest = Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_RECENT_MODEL1_MANIFEST.csv'
$source = Join-Path $repo 'outputs\three_lane_momentum_same_side_exit_cooldown_recent_model1_package\source\Professional_XAUUSD_EA.mq5'
$expectedManifestHash = '51568706540034336D1623B95FAA934D4E594895DD4D738A266ADE4CB736A81F'
...
try {
   ...
   & $runner -ManifestPath $manifest -PortableRoots $roots -UserAuthorizedFocusRisk `
      -OutputPrefix $(if($SingleWorkerRecovery) { 'THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_RECENT_RECOVERY_WORKER' } else { 'THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_RECENT_WORKER' }) `
