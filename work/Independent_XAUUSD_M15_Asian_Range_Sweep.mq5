...
input ulong  InpMagicNumber = 26071931;
...
input group "Asian-Range Liquidity Sweep Engine"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_M15;
input int    InpAsianRangeStartHour = 0;
input int    InpAsianRangeEndHour = 6;
input int    InpMinimumAsianRangeBars = 16;
input double InpMinimumAsianRangeATR = 1.00;
input double InpMaximumAsianRangeATR = 8.00;
input double InpMinimumSweepATR = 0.10;
input double InpMinimumSweepPoints = 20.0;
input double InpMaximumSweepATR = 0.75;
input double InpMinimumReclaimATR = 0.00;
input double InpMinimumWickToBodyRatio = 1.00;
input double InpMinimumBodyPercent = 10.0;
...
input bool InpUseMaximumADXFilter = false;
input int  InpADXPeriod = 14;
input double InpMaximumADX = 28.0;
...
input bool   InpUseAsianMidpointTarget = false;
input double InpMinimumMidpointTargetR = 1.00;
...
input string InpLogFileName = "Independent_XAUUSD_M15_Asian_Range_Sweep_Trades.csv";
...
int g_sweepCandidates = 0;
int g_sweepSignals = 0;
...
{
   return "IM15ARS_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "M15ARS-LIVE-ACK"))
   {
...
bool OpenSweepPosition(const bool buy,
                       const double atr,
                       const double rawStop,
                       const double asianRangeHigh,
                       const double asianRangeLow,
                       ...
                       const double rangeATR)
{
   ...
   if(InpUseAsianMidpointTarget)
   {
      double midpoint = 0.5 * (asianRangeHigh + asianRangeLow);
      double targetDistance = buy ? midpoint - entryPrice : entryPrice - midpoint;
      if(midpoint <= 0.0 || targetDistance <= 0.0 ||
         targetDistance / stopDistance < InpMinimumMidpointTargetR)
         return false;
      takeProfit = NormalizeDouble(midpoint, _Digits);
   }
   ...
   string comment = buy ? "M15ARS_BUY" : "M15ARS_SELL";
   ...
   string reason = "Asian-range sweep reclaim; sweep ATR " + DoubleToString(sweepDepth / atr, 2) +
                   "; range ATR " + DoubleToString(rangeATR, 2);
...
{
   MqlDateTime day;
   TimeToStruct(signalBarTime, day);
   day.hour = InpAsianRangeStartHour;
   day.min = 0;
   day.sec = 0;
   datetime rangeStart = StructToTime(day);
   day.hour = InpAsianRangeEndHour;
   datetime rangeEnd = StructToTime(day);
   if(rangeStart <= 0 || rangeEnd <= rangeStart || signalBarTime < rangeEnd)
      return false;

   MqlRates rates[];
   int copied = CopyRates(_Symbol, InpSignalTimeframe, rangeStart, rangeEnd - 1, rates);
   if(copied < MathMax(2, InpMinimumAsianRangeBars))
      return false;
   rangeHigh = -DBL_MAX;
   rangeLow = DBL_MAX;
...
bool TryAsianRangeSweepEntry(const double atr)
{
   if(atr <= 0.0)
      return false;
   datetime signalBarTime = iTime(_Symbol, InpSignalTimeframe, 1);
   double asianRangeHigh = 0.0;
   double asianRangeLow = 0.0;
   int asianRangeBars = 0;
   if(signalBarTime <= 0 ||
      !AsianRangeLevels(signalBarTime, asianRangeHigh, asianRangeLow, asianRangeBars))
      return false;
   double rangeATR = (asianRangeHigh - asianRangeLow) / atr;
   if(rangeATR < InpMinimumAsianRangeATR || rangeATR > InpMaximumAsianRangeATR)
   {
      g_regimeRejects++;
      return false;
   }
   ...
   double buySweepDepth = asianRangeLow - low1;
   double sellSweepDepth = high1 - asianRangeHigh;
   ...
   {
      buy = buy && low2 > asianRangeLow - minimumSweep;
      sell = sell && high2 < asianRangeHigh + minimumSweep;
   }
   if(!buy && !sell)
      return false;
   g_sweepCandidates++;
   ...
   return OpenSweepPosition(buy, atr, rawStop, asianRangeHigh, asianRangeLow,
                            sweepDepth, rangeATR);
}

int OnInit()
{
   if(InpAsianRangeStartHour < 0 || InpAsianRangeStartHour > 23 ||
      InpAsianRangeEndHour < 1 || InpAsianRangeEndHour > 23 ||
      InpAsianRangeStartHour >= InpAsianRangeEndHour ||
      InpMinimumAsianRangeBars < 2 || InpMinimumAsianRangeATR <= 0.0 ||
...
{
   PrintFormat("M15ARS_DIAGNOSTIC candidates=%d signals=%d rejection_shape_rejects=%d regime_rejects=%d safety_rejects=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
               g_sweepCandidates, g_sweepSignals, g_rejectionShapeRejects, g_regimeRejects,
