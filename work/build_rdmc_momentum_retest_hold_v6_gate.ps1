...
$package = Join-Path $repo 'outputs\rdmc_momentum_retest_hold_v6_package'
...
$expectedSource = 'B99BEFF28BB0D28596F4D7786C65BDA11001B0BC4924F269F534817EADC6CDCC'
$expectedProfile = '9E8AC3C528860799CB152245CF1FFB084A8E9FD07A2A33F70FD927EFA077D390'
...
if($inputs.Keys.Count -ne 612) { throw "Expected 612 rewrite profile inputs, found $($inputs.Keys.Count)." }
...
$rows = foreach($spec in $specs) {
   ...
   $reportName = 'rdmc_mrh6_w{0:D2}_m1_{1}_{2}' -f ([int]$spec.Wave),$spec.Role,$spec.Window
   ...
   [pscustomobject][ordered]@{
      ...
      Wave = $spec.Wave
      Role = $spec.Role
      Candidate = 'rdmc_momentum_retest_hold_v6'
      Profile = 'rdmc_momentum_retest_hold_v6'
