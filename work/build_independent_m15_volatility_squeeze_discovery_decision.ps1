param(
   [string]$QueuePath = "outputs\INDEPENDENT_M15_VOLATILITY_SQUEEZE_DISCOVERY_MODEL1_QUEUE.csv",
   [string]$ReportDir = "outputs\independent_m15_volatility_squeeze_discovery_model1_package\reports_here",
   [string]$ResultsPath = "outputs\INDEPENDENT_M15_VOLATILITY_SQUEEZE_DISCOVERY_MODEL1_RESULTS.csv",
   [string]$SummaryPath = "outputs\INDEPENDENT_M15_VOLATILITY_SQUEEZE_DISCOVERY_MODEL1_SUMMARY.csv",
   [string]$MetricsPath = "outputs\INDEPENDENT_M15_VOLATILITY_SQUEEZE_DISCOVERY_MODEL1_METRICS.md",
   [string]$DecisionCsvPath = "outputs\INDEPENDENT_M15_VOLATILITY_SQUEEZE_DISCOVERY_DECISION.csv",
   [string]$DecisionMarkdownPath = "outputs\INDEPENDENT_M15_VOLATILITY_SQUEEZE_DISCOVERY_DECISION.md"
)
...
$rawResults = Join-Path $repo "work\M15SQ_RAW_RESULTS.csv"
$rawSummary = Join-Path $repo "work\M15SQ_RAW_SUMMARY.csv"
$rawMarkdown = Join-Path $repo "work\M15SQ_RAW_METRICS.md"
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "collect_validation_results.ps1") `
   ...
   -OutResults "work\M15SQ_RAW_RESULTS.csv" -OutSummary "work\M15SQ_RAW_SUMMARY.csv" `
   -OutMarkdown "work\M15SQ_RAW_METRICS.md" | Out-Null
...
foreach($item in ($queue | Sort-Object { [int]$_.QueueRank })) {
   ...
   $results.Add([pscustomobject]@{
      ...
      UseTrend=$item.UseTrend; TrendEMAPeriod=$item.TrendEMAPeriod; StopRule=$item.StopRule
...
$adjacency = @{
   m15sq_center=@('m15sq_sq2','m15sq_sq4','m15sq_kc140','m15sq_kc160','m15sq_break8','m15sq_break16','m15sq_channel275','m15sq_exp090','m15sq_exp130','m15sq_tp125','m15sq_tp200','m15sq_trend50','m15sq_trend200','m15sq_notrend')
   m15sq_sq2=@('m15sq_center'); m15sq_sq4=@('m15sq_center')
   m15sq_kc140=@('m15sq_center'); m15sq_kc160=@('m15sq_center')
   m15sq_break8=@('m15sq_center'); m15sq_break16=@('m15sq_center')
   m15sq_channel275=@('m15sq_center'); m15sq_exp090=@('m15sq_center'); m15sq_exp130=@('m15sq_center')
   m15sq_tp125=@('m15sq_center'); m15sq_tp200=@('m15sq_center')
   m15sq_trend50=@('m15sq_center'); m15sq_trend200=@('m15sq_center'); m15sq_notrend=@('m15sq_center')
}
...
$md.Add("# Independent M15 Volatility-Squeeze Discovery Decision")
$md.Add("")
...
@(
   "# Independent M15 Volatility-Squeeze Discovery Metrics", "",
   ...
   'See `outputs/INDEPENDENT_M15_VOLATILITY_SQUEEZE_DISCOVERY_DECISION.md` for the gated interpretation.'
