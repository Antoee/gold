...
$source=Join-Path $repo 'outputs\three_lane_reversion_joint_strong_allocation_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_REVERSION_JOINT_STRONG_ALLOCATION_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'Published leader source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Joint strong-allocation compile $startedAtUtc",[Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck,"Joint strong-allocation focus acknowledgement $startedAtUtc",[Text.Encoding]::ASCII)
