...
#property description "Research-only three-lane XAUUSD portfolio with selective adaptive-trend payoff extension"
...
input bool   InpATBUseSelectiveTargetExtension = false;
input double InpATBExtendedTakeProfitR = 3.00;
input double InpATBExtensionMinimumADX = 22.0;
input double InpATBExtensionMinimumBodyPercent = 55.0;
input double InpATBExtensionMinimumCloseLocationPercent = 75.0;
input double InpATBExtensionMinimumRangeATR = 0.75;
...
private:
   ...
   {
      if(!InpATBUseSelectiveTargetExtension || atr <= 0.0)
         return false;
      double open1 = iOpen(_Symbol, InpATBSignalTimeframe, 1);
      double high1 = iHigh(_Symbol, InpATBSignalTimeframe, 1);
      double low1 = iLow(_Symbol, InpATBSignalTimeframe, 1);
      ...
      double range = high1 - low1;
      double adx = 0.0;
      if(open1 <= 0.0 || low1 <= 0.0 || close1 <= 0.0 || range <= 0.0 ||
         !BufferValue(m_adxHandle, 0, 1, adx))
         return false;
      double bodyPercent = 100.0 * MathAbs(close1 - open1) / range;
      double closeLocation = buy ? 100.0 * (close1 - low1) / range
                                 : 100.0 * (high1 - close1) / range;
      double rangeAtr = range / atr;
      return (buy ? close1 > open1 : close1 < open1) &&
             adx >= InpATBExtensionMinimumADX &&
             bodyPercent >= InpATBExtensionMinimumBodyPercent &&
             closeLocation >= InpATBExtensionMinimumCloseLocationPercent &&
             rangeAtr >= InpATBExtensionMinimumRangeATR;
   }
...
bool InputsValid()
{
   ...
   if(InpMOMomentumLookbackBars < 20 || InpMOEntryLookbackBars < 3 ||
      ...
      return false;
   if(InpATBExtendedTakeProfitR <= InpATBTakeProfitR ||
      InpATBExtendedTakeProfitR > 10.0 ||
      InpATBExtensionMinimumADX < 0.0 || InpATBExtensionMinimumADX > 100.0 ||
      InpATBExtensionMinimumBodyPercent < 0.0 || InpATBExtensionMinimumBodyPercent > 100.0 ||
      InpATBExtensionMinimumCloseLocationPercent < 0.0 ||
      InpATBExtensionMinimumCloseLocationPercent > 100.0 ||
      InpATBExtensionMinimumRangeATR < 0.0 || InpATBExtensionMinimumRangeATR > 10.0)
      return false;
