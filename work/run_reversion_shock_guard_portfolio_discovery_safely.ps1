...
param(
   ...
   [string]$OutputPrefix = "REVERSION_SHOCK_GUARD_PORTABLE"
)
...
try {
   ...
   Set-Content -LiteralPath $unlock -Value "User-authorized isolated reversion shock-guard discovery." -Encoding ASCII
   ...
   Set-Content -LiteralPath $hardLock -Value "Restored after isolated reversion shock-guard discovery." -Encoding ASCII
}
