[CmdletBinding()]
param(
   [string]$SourcePath = 'work\Professional_XAUUSD_Three_Lane_Selective_Payoff_Research.mq5',
   [string]$ChampionProfilePath = 'release\three-lane-trade-ready-rc2-atb150\THREE_LANE_TRADE_READY_RC2_ATB150.set',
   [string]$PackageDir = 'outputs\three_lane_selective_payoff_discovery_model1_package',
   [string]$QueuePath = 'outputs\THREE_LANE_SELECTIVE_PAYOFF_DISCOVERY_MODEL1_QUEUE.csv',
   [string]$ManifestPath = 'outputs\THREE_LANE_SELECTIVE_PAYOFF_DISCOVERY_MODEL1_PACKAGE_MANIFEST.csv',
   [string]$PackageMarkdownPath = 'outputs\THREE_LANE_SELECTIVE_PAYOFF_DISCOVERY_MODEL1_PACKAGE.md',
   [string]$ContractPath = 'outputs\THREE_LANE_SELECTIVE_PAYOFF_CONTRACT.md'
)
...
$expectedSourceHash = '56B674D2C85A879212350F944838FDCE7AF91E320D799FEA1EDAB5BF9A0D5C02'
...
& (Join-Path $PSScriptRoot 'test_three_lane_selective_payoff_source.ps1') | Out-Null
...
if($base.Count -ne 184) { throw "Expected 184 selective-payoff inputs, found $($base.Count)." }
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_selective_payoff_discovery_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='sp_control';Enabled='false';Target='3.00';ADX='22.0';Body='55.0';Close='75.0';Range='0.75'},
   [pscustomobject]@{Name='sp_center';Enabled='true';Target='3.00';ADX='22.0';Body='55.0';Close='75.0';Range='0.75'},
   [pscustomobject]@{Name='sp_target250';Enabled='true';Target='2.50';ADX='22.0';Body='55.0';Close='75.0';Range='0.75'},
   [pscustomobject]@{Name='sp_target400';Enabled='true';Target='4.00';ADX='22.0';Body='55.0';Close='75.0';Range='0.75'},
   [pscustomobject]@{Name='sp_adx18';Enabled='true';Target='3.00';ADX='18.0';Body='55.0';Close='75.0';Range='0.75'},
   [pscustomobject]@{Name='sp_adx26';Enabled='true';Target='3.00';ADX='26.0';Body='55.0';Close='75.0';Range='0.75'},
   [pscustomobject]@{Name='sp_body45';Enabled='true';Target='3.00';ADX='22.0';Body='45.0';Close='75.0';Range='0.75'},
   [pscustomobject]@{Name='sp_body65';Enabled='true';Target='3.00';ADX='22.0';Body='65.0';Close='75.0';Range='0.75'},
   [pscustomobject]@{Name='sp_range050';Enabled='true';Target='3.00';ADX='22.0';Body='55.0';Close='75.0';Range='0.50'},
   [pscustomobject]@{Name='sp_range100';Enabled='true';Target='3.00';ADX='22.0';Body='55.0';Close='75.0';Range='1.00'}
)
$windows = @(
...
$stopRule = 'Selective-payoff discovery only. Require both disjoint eras positive, at least three extended entries, continuous CAGR at least 0.20 points above exact disabled control, PF >= 1.60, DD <= 1.50%, return/DD and recovery no worse than control, and at least two one-factor neighbors preserving the qualitative gain before any 2021+ check.'
...
foreach($variant in $variants) {
   ...
   Set-FixedInput $inputs 'InpATBUseSelectiveTargetExtension' $variant.Enabled
   Set-FixedInput $inputs 'InpATBExtendedTakeProfitR' $variant.Target
   Set-FixedInput $inputs 'InpATBExtensionMinimumADX' $variant.ADX
   Set-FixedInput $inputs 'InpATBExtensionMinimumBodyPercent' $variant.Body
   Set-FixedInput $inputs 'InpATBExtensionMinimumCloseLocationPercent' $variant.Close
   Set-FixedInput $inputs 'InpATBExtensionMinimumRangeATR' $variant.Range
   ...
   foreach($window in $windows) {
      ...
      $common = [ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Phase='selective_payoff_discovery_model1'
         Window=$window.Name;From=$window.From;To=$window.To;Model=1;Deposit=10000
         Enabled=$variant.Enabled;ExtendedTakeProfitR=$variant.Target;MinimumADX=$variant.ADX
         MinimumBodyPercent=$variant.Body;MinimumCloseLocationPercent=$variant.Close;MinimumRangeATR=$variant.Range
...
@(
   '# Three-Lane Selective Payoff Discovery Package','',
   '**Status: SELECTIVE PAYOFF DISCOVERY ONLY. THE ATB150 CHAMPION AND FORWARD CANDIDATE ARE UNCHANGED.**','',
   "- Research source SHA-256: ``$sourceHash``",
   "- Frozen champion profile SHA-256: ``$championHash``",
   "- Variants: ``$($variants.Count)``",
   "- Configurations: ``$ordinal``",
   '- The control is exact ATB150 with the extension disabled.',
   '- Candidates widen only the adaptive-trend target when the completed entry H4 bar passes fixed ADX, body, close-location, and ATR-range thresholds. Entry eligibility, stops, risk, other exits, loss limits, and the 0.75% open-risk cap remain unchanged.',
...
@(
   '# Three-Lane Selective Payoff Contract','',
   '**Status: RESEARCH ONLY. NO PROMOTION OR REAL TRADING IS AUTHORIZED.**','',
   '- Keep every entry signal, initial stop, requested risk, lot cap, session, and account guard unchanged.',
   '- Use only completed entry-bar H4 values and the existing completed-bar ADX handle; never use future prices or outcome state.',
   '- Widen only the adaptive-trend take-profit for qualified entries; existing channel, momentum-failure, break-even, and time exits remain active.',
   '- Require measurable extension activity and a broad risk-normalized improvement rather than a larger-target headline.',
