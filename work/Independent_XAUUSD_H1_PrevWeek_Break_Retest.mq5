...
input ulong  InpMagicNumber = 26071731;
...
input group "Previous-Week Break-And-Retest Engine"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_H1;
input int    InpMaximumSetupAgeBars = 8;
input double InpBreakBufferATR = 0.08;
input double InpMinimumBreakRangeATR = 0.70;
input double InpMinimumBreakBodyPercent = 40.0;
input double InpMinimumBreakCloseLocation = 0.65;
input bool   InpRequireFreshBreak = true;
input double InpRetestToleranceATR = 0.20;
input double InpMaximumRetestPenetrationATR = 0.35;
input double InpMinimumRetestReclaimATR = 0.02;
input double InpMinimumRetestBodyPercent = 15.0;
input double InpMinimumRetestCloseLocation = 0.55;
input double InpMaximumEntryDistanceATR = 0.60;
input bool   InpRequireRetestDirectionCandle = true;
input bool   InpAllowBuy = true;
input bool   InpAllowSell = true;
input bool   InpUseBreakoutVolumeFilter = false;
input bool   InpUseRetestVolumeFilter = false;
...
input bool InpUseMinimumADXFilter = false;
...
input int    InpStopLookbackBars = 2;
...
input int    InpMaximumHoldBars = 48;
...
input string InpLogFileName = "Independent_XAUUSD_H1_PrevWeek_Break_Retest_Trades.csv";
...
BreakRetestSetup g_setup;
int g_breakCandidates = 0;
int g_breakSetups = 0;
int g_breakShapeRejects = 0;
int g_retestCandidates = 0;
int g_retestSignals = 0;
int g_retestShapeRejects = 0;
int g_setupInvalidations = 0;
int g_setupExpirations = 0;
...
{
   return "IH1PWBR_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "H1PWBR-LIVE-ACK"))
   {
...
bool OpenBreakRetestPosition(const bool buy,
                             const double atr,
                             const double rawStop)
{
   MqlTick tick;
   ...
   string comment = buy ? "H1PWBR_BUY" : "H1PWBR_SELL";
   ...
   LogEvent("entry", trade.ResultOrder(), buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
            "previous-week break and retest");
   return true;
}
...
bool PreviousWeekLevels(double &previousWeekHigh, double &previousWeekLow)
{
   ...
   return previousWeekHigh > previousWeekLow && previousWeekLow > 0.0;
}

bool TryActiveRetest(const double atr)
{
   if(!g_setup.active || atr <= 0.0)
      return false;

   g_setup.ageBars++;
   if(g_setup.ageBars > InpMaximumSetupAgeBars)
   {
      g_setupExpirations++;
      ResetSetup();
      return false;
   ...
   datetime barTime = iTime(_Symbol, InpSignalTimeframe, 1);
   ...
   if(barTime <= g_setup.breakoutBarTime || open1 <= 0.0 || close1 <= 0.0 || high1 <= low1)
      return false;

   double level = g_setup.level;
   double maxPenetration = InpMaximumRetestPenetrationATR * atr;
   bool invalidated = g_setup.buy ? close1 < level - maxPenetration
                                  : close1 > level + maxPenetration;
   if(invalidated)
   {
      g_setupInvalidations++;
      ResetSetup();
      return false;
   }

   double tolerance = InpRetestToleranceATR * atr;
   double reclaim = InpMinimumRetestReclaimATR * atr;
   bool touched = g_setup.buy ? low1 <= level + tolerance
                              : high1 >= level - tolerance;
   bool depthAllows = g_setup.buy ? low1 >= level - maxPenetration
                                  : high1 <= level + maxPenetration;
   bool reclaimed = g_setup.buy ? close1 >= level + reclaim
                                : close1 <= level - reclaim;
   double entryDistance = MathAbs(close1 - level);
   if(!touched || !depthAllows || !reclaimed || entryDistance > InpMaximumEntryDistanceATR * atr)
      return false;

   g_retestCandidates++;
   bool directionCandle = !InpRequireRetestDirectionCandle ||
                          (g_setup.buy ? close1 > open1 : close1 < open1);
   bool shapeAllows = CandleBodyPercent(1) >= InpMinimumRetestBodyPercent &&
                      CandleCloseLocation(g_setup.buy, 1) >= InpMinimumRetestCloseLocation &&
                      directionCandle && TickVolumeAllows(1, InpUseRetestVolumeFilter);
   if(!shapeAllows)
   {
      g_retestShapeRejects++;
      return false;
   }
   if(!RegimeAllows(g_setup.buy, close1, atr))
   {
      g_regimeRejects++;
      return false;
   }
   ...
   double channelHigh = 0.0;
   double channelLow = 0.0;
   if(!ChannelBounds(1, MathMax(1, InpStopLookbackBars), channelHigh, channelLow))
   {
      g_stopShapeRejects++;
      ResetSetup();
      return false;
   ...
   bool buy = g_setup.buy;
   double rawStop = buy ? channelLow - InpStopBufferATR * atr
                        : channelHigh + InpStopBufferATR * atr;
   g_retestSignals++;
   ResetSetup();
   return OpenBreakRetestPosition(buy, atr, rawStop);
}
...
bool RegisterPreviousWeekBreak(const double atr)
{
   ...
   double previousWeekHigh = 0.0;
   double previousWeekLow = 0.0;
   if(!PreviousWeekLevels(previousWeekHigh, previousWeekLow))
      return false;
   ...
   if(open1 <= 0.0 || close1 <= 0.0 || close2 <= 0.0 || range1 <= 0.0)
      return false;

   double breakBuffer = InpBreakBufferATR * atr;
   bool buy = InpAllowBuy && close1 >= previousWeekHigh + breakBuffer;
   bool sell = InpAllowSell && close1 <= previousWeekLow - breakBuffer;
   if(InpRequireFreshBreak)
   {
      buy = buy && close2 < previousWeekHigh + breakBuffer;
      sell = sell && close2 > previousWeekLow - breakBuffer;
   }
   if(!buy && !sell)
      return false;
   ...
   bool shapeAllows = range1 >= InpMinimumBreakRangeATR * atr &&
                      CandleBodyPercent(1) >= InpMinimumBreakBodyPercent &&
                      CandleCloseLocation(buy, 1) >= InpMinimumBreakCloseLocation &&
                      directionCandle && TickVolumeAllows(1, InpUseBreakoutVolumeFilter);
   if(!shapeAllows)
   {
      g_breakShapeRejects++;
      return false;
   ...
   g_setup.active = true;
   g_setup.buy = buy;
   g_setup.level = buy ? previousWeekHigh : previousWeekLow;
   g_setup.breakoutBarTime = iTime(_Symbol, InpSignalTimeframe, 1);
   g_setup.ageBars = 0;
   g_breakSetups++;
   return true;
}

bool TryPreviousWeekBreakRetestEntry(const double atr)
{
   if(TryActiveRetest(atr))
      return true;
   if(!g_setup.active)
      RegisterPreviousWeekBreak(atr);
   return false;
}

int OnInit()
{
   if(InpMaximumSetupAgeBars < 1 || InpBreakBufferATR < 0.0 ||
      InpMinimumBreakRangeATR <= 0.0 ||
      InpMinimumBreakBodyPercent < 0.0 || InpMinimumBreakBodyPercent > 100.0 ||
      InpMinimumBreakCloseLocation < 0.0 || InpMinimumBreakCloseLocation > 1.0 ||
      InpRetestToleranceATR < 0.0 || InpMaximumRetestPenetrationATR < 0.0 ||
      InpMinimumRetestReclaimATR < 0.0 ||
      InpMinimumRetestBodyPercent < 0.0 || InpMinimumRetestBodyPercent > 100.0 ||
      InpMinimumRetestCloseLocation < 0.0 || InpMinimumRetestCloseLocation > 1.0 ||
      InpMaximumEntryDistanceATR <= 0.0 ||
...
{
   PrintFormat("H1PWBR_DIAGNOSTIC break_candidates=%d break_setups=%d break_shape_rejects=%d retest_candidates=%d retest_signals=%d retest_shape_rejects=%d invalidations=%d expirations=%d regime_rejects=%d safety_rejects=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
               g_breakCandidates, g_breakSetups, g_breakShapeRejects, g_retestCandidates,
               g_retestSignals, g_retestShapeRejects, g_setupInvalidations, g_setupExpirations,
               g_regimeRejects, g_safetyRejects, g_ordersOpened, g_stopShapeRejects,
               g_minimumLotRejects, g_exposureRejects, g_orderFailures);
...
void OnTick()
{
   ...
   if(ManagedPositionCount() > 0)
      return;
   TryPreviousWeekBreakRetestEntry(atr);
}
...
double OnTester()
{
   ...
   if(trades < 40.0)
      return profit - (40.0 - trades) * 1000.0;
