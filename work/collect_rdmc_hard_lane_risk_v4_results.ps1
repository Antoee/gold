param(
   ...
   [string]$DecisionCsvPath = "outputs\RDMC_HARD_LANE_RISK_V4_DECISION_FIXTURE.csv",
   [string]$ManifestPath = "outputs\RDMC_HARD_LANE_RISK_V4_WAVE_01_MANIFEST.csv",
   [string]$ReportDir = "outputs\rdmc_hard_lane_risk_v4_package\reports_here",
   [string]$RunnerLedgerGlob = "",
   [string]$ResultsPath = "outputs\RDMC_HARD_LANE_RISK_V4_WAVE_01_RESULTS.csv",
   [string]$RunAuditPath = "",
...
$evaluator = Join-Path $PSScriptRoot "evaluate_rdmc_hard_lane_risk_v4.py"
$expectedManifestHash = "636CCDBB66A43BFEDA99BBE7007A363CF666E92FB8AF2B5CB8B23A9B0C024E1C"
$expectedSourceHash = "7A6CA3C9E9644656A0CDC64A6D078B446FB1A9981B16CDE727E65B13A5C06831"
$expectedProfileHash = "224801DB7A02F2C54C3070ABB190187EA8C73FC7181CDD07BA33040025A2922B"
...
if([string]::IsNullOrWhiteSpace($RunnerLedgerGlob)) {
   $RunnerLedgerGlob = "outputs\RDMC_HARD_LANE_RISK_V4_WAVE_{0:D2}_WORKER_*.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($RunAuditPath)) {
   $RunAuditPath = "outputs\RDMC_HARD_LANE_RISK_V4_WAVE_{0:D2}_RUN_AUDIT.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($RawResultsPath)) {
   $RawResultsPath = "outputs\RDMC_HARD_LANE_RISK_V4_WAVE_{0:D2}_RAW_RESULTS.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($SummaryPath)) {
   $SummaryPath = "outputs\RDMC_HARD_LANE_RISK_V4_WAVE_{0:D2}_SUMMARY.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($MetricsMarkdownPath)) {
   $MetricsMarkdownPath = "outputs\RDMC_HARD_LANE_RISK_V4_WAVE_{0:D2}_METRICS.md" -f $Wave
}
...
if(!$SkipAdmissionRefresh) {
   if((Resolve-Path -LiteralPath $resultsFull).Path -ne (Join-Path $repo "outputs\RDMC_HARD_LANE_RISK_V4_WAVE_01_RESULTS.csv")) {
