param(
   ...
   [string]$ReportDir = "outputs\independent_m15_trend_liquidity_reclaim_discovery_model1_package\reports_here",
   [string]$ParserManifestPath = "outputs\TREND_LIQUIDITY_RECLAIM_DISCOVERY_MODEL1_PARSE_MANIFEST.csv",
   [string]$RawResultsPath = "outputs\TREND_LIQUIDITY_RECLAIM_DISCOVERY_MODEL1_RAW_RESULTS.csv",
   ...
   [string]$SummaryPath = "outputs\TREND_LIQUIDITY_RECLAIM_DISCOVERY_MODEL1_SUMMARY.csv",
   [string]$MarkdownPath = "outputs\TREND_LIQUIDITY_RECLAIM_DISCOVERY_MODEL1_METRICS.md"
)
...
if($queue.Count -ne 28) { throw "Expected 28 frozen queue rows, found $($queue.Count)." }
if(@($queue | Where-Object { $_.To -gt '2020.12.31' }).Count -gt 0) {
   throw "Post-2020 data leaked into the collector."
}

$initialRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath 'outputs') -Filter 'TREND_LIQUIDITY_RECLAIM_PORTABLE_*.csv' |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
$retryRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath 'outputs') -Filter 'TREND_LIQUIDITY_RECLAIM_RETRY_*.csv' |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
...
if($raw.Count -ne 28) { throw "Expected 28 parsed result rows, found $($raw.Count)." }
...
$results = foreach($row in $raw) {
   ...
   [pscustomobject]@{
      ...
      RunLabel = $q.RunLabel
      Role = $q.Role
      LiquidityLookbackBars = $q.LiquidityLookbackBars
      MinimumBodyPercent = $q.MinimumBodyPercent
      UsePostLossQuarantine = $q.UsePostLossQuarantine
      PostLossQuarantineDays = $q.PostLossQuarantineDays
      RiskPercent = $q.RiskPercent
      EntryStartHour = $q.EntryStartHour
