param(
   [string]$QueuePath = "outputs\TWO_BAR_CONFIRMATION_PORTFOLIO_DISCOVERY_MODEL1_QUEUE.csv",
   [string]$ReportDir = "outputs\two_bar_confirmation_portfolio_discovery_model1_package\reports_here",
   [string]$SourcePath = "work\Professional_XAUUSD_Two_Bar_Confirmation_Portfolio.mq5",
   [string]$ResultsPath = "outputs\TWO_BAR_CONFIRMATION_PORTFOLIO_DISCOVERY_MODEL1_RESULTS.csv",
   [string]$SummaryPath = "outputs\TWO_BAR_CONFIRMATION_PORTFOLIO_DISCOVERY_MODEL1_SUMMARY.csv",
   [string]$DecisionCsvPath = "outputs\TWO_BAR_CONFIRMATION_PORTFOLIO_DISCOVERY_DECISION.csv",
   [string]$DecisionMarkdownPath = "outputs\TWO_BAR_CONFIRMATION_PORTFOLIO_DISCOVERY_DECISION.md",
   [string]$MetricsPath = "outputs\TWO_BAR_CONFIRMATION_PORTFOLIO_DISCOVERY_MODEL1_METRICS.md"
)
...
$rawResults = Join-Path $repo "work\TBC_RAW_RESULTS.csv"
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "collect_validation_results.ps1") `
   ...
   -OutResults "work\TBC_RAW_RESULTS.csv" -OutSummary "work\TBC_RAW_SUMMARY.csv" `
   -OutMarkdown "work\TBC_RAW_METRICS.md" | Out-Null
...
foreach($item in ($queue | Sort-Object { [int]$_.QueueRank })) {
   ...
   $identityPass = $reportText.IndexOf($sourceHash,[StringComparison]::OrdinalIgnoreCase) -ge 0 -and
                   $reportText.IndexOf("Closed-Bar Two-Step Entry Confirmation=",[StringComparison]::Ordinal) -ge 0
   ...
   $results.Add([pscustomobject]@{
      ...
      RVConfirmationEnabled=$item.RVConfirmationEnabled; RVRequirePriorCloseOutside=$item.RVRequirePriorCloseOutside
      RVRequireProgress=$item.RVRequireProgress; MOConfirmationEnabled=$item.MOConfirmationEnabled
      MORequireProgress=$item.MORequireProgress
...
$control = $candidateRows['tbc_fixed_control'].Continuous
...
$adjacency = @{
   tbc_fixed_control=@()
   tbc_rv_wick=@('tbc_rv_close','tbc_both_wick_hold')
   tbc_rv_close=@('tbc_rv_wick','tbc_both_close_hold')
   tbc_mo_hold=@('tbc_mo_progress','tbc_both_wick_hold')
   tbc_mo_progress=@('tbc_mo_hold','tbc_both_wick_progress')
   tbc_both_wick_hold=@('tbc_both_close_hold','tbc_both_wick_progress','tbc_rv_wick','tbc_mo_hold')
   tbc_both_close_hold=@('tbc_both_wick_hold','tbc_rv_close')
   tbc_both_wick_progress=@('tbc_both_wick_hold','tbc_mo_progress')
}
$basicPass = @{}; $qualityPass = @{}
foreach($candidate in $candidateRows.Keys) {
   $set = $candidateRows[$candidate]
   $basicPass[$candidate] = $candidate -ne 'tbc_fixed_control' -and
...
foreach($candidate in ($candidateRows.Keys | Sort-Object)) {
   ...
   $summary.Add([pscustomobject]@{
      Candidate=$candidate; RVConfirmationEnabled=$set.Continuous.RVConfirmationEnabled
      MOConfirmationEnabled=$set.Continuous.MOConfirmationEnabled
      ...
      Decision=$(if($eligible){"DISCOVERY_ELIGIBLE"}elseif($candidate -eq 'tbc_fixed_control'){"CONTROL_ONLY"}else{"REJECT_BEFORE_HOLDOUT"})
   }) | Out-Null
}
...
$md.Add("# Two-Bar Confirmation Portfolio Discovery Decision")
$md.Add("")
$md.Add($(if($eligibleRows.Count -gt 0) { "**Decision: freeze eligible two-bar confirmation profiles for untouched holdout; Model 4 and promotion remain closed.**" } else { "**Decision: rejected in pre-2021 discovery; holdout, Model 4, and promotion remain closed.**" }))
$md.Add("")
$md.Add("Released risk, stops, targets, and position management were retained. Reversion entries could require extension then reclaim; momentum entries could require a breakout then a second close that held beyond the same channel.")
$md.Add("")
...
$md.Add("- Eligible two-bar confirmation profiles: ``$($eligibleRows.Count) / 7``")
$md.Add("")
...
@(
   "# Two-Bar Confirmation Portfolio Discovery Metrics", "",
