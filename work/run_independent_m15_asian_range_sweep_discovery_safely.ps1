...
param(
   ...
   [string]$OutputPrefix = "INDEPENDENT_M15_ASIAN_RANGE_SWEEP_PORTABLE"
)
...
try {
   ...
   Set-Content -LiteralPath $unlock -Value "User-authorized isolated Asian-range discovery." -Encoding ASCII
   ...
   Set-Content -LiteralPath $hardLock -Value "Restored after isolated Asian-range discovery." -Encoding ASCII
}
