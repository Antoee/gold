...
#property description "Behavior-neutral M15 squeeze entry-feature telemetry on the restart-safe partial-runner fork"
...
input bool   InpRVUseStrongSignalRisk = false;
input double InpRVStrongSignalMinimumBodyRatio = 0.15;
input double InpRVStrongSignalRiskPercent = 0.60;
input bool   InpRVUseStrongSignalLotCap = false;
...
input bool   InpMOUseSameSideExitCooldown = false;
input int    InpMOSameSideExitCooldownMinutes = 60;
...
input group "M15 Volatility-Squeeze Diversifier Lane"
input bool   InpSQEnabled = false;
input ulong  InpSQMagicNumber = 26071772;
input double InpSQRiskPercent = 0.10;
input ENUM_TIMEFRAMES InpSQSignalTimeframe = PERIOD_M15;
...
input bool   InpSQAllowBuy = true;
input bool   InpSQAllowSell = true;
...
input ENUM_TIMEFRAMES InpSQTrendTimeframe = PERIOD_H1;
input int    InpSQTrendEMAPeriod = 100;
input int    InpSQTrendEMASlopeBars = 3;
input bool   InpSQRequireTrendAlignment = true;
input bool   InpSQUseADXFilter = false;
input int    InpSQADXPeriod = 14;
input double InpSQMinimumADX = 14.0;
input double InpSQMaximumADX = 45.0;
input bool   InpSQUseVolatilityFilter = true;
input double InpSQMinimumATRPercent = 0.03;
input double InpSQMaximumATRPercent = 2.50;
input int    InpSQATRPeriod = 20;
input double InpSQStopBufferATR = 0.10;
input double InpSQMinimumStopATR = 0.25;
input double InpSQMaximumStopATR = 1.25;
input double InpSQMaximumStopPriceDistance = 6.00;
input double InpSQTakeProfitR = 1.50;
input bool   InpSQUsePartialRunner = false;
input double InpSQPartialClosePercent = 80.0;
input double InpSQPartialTriggerR = 1.50;
input double InpSQPartialTargetR = 4.00;
input double InpSQPartialStopLockR = 1.25;
input bool   InpSQUseBreakEven = true;
input double InpSQBreakEvenTriggerR = 0.80;
input double InpSQBreakEvenLockR = 0.10;
input bool   InpSQUseChandelierTrail = false;
input int    InpSQChandelierLookbackBars = 8;
input double InpSQChandelierATR = 2.50;
input bool   InpSQUseTrendFailureExit = false;
input int    InpSQMaximumHoldBars = 32;
input bool   InpSQUseSessionFilter = true;
input int    InpSQSessionStartHour = 6;
input int    InpSQSessionEndHour = 18;
input bool   InpSQDisableFridayAfterHour = true;
input int    InpSQFridayCutoffHour = 18;
input double InpSQMaximumPositionLots = 1.00;
input int    InpSQMaximumTradesPerDay = 2;
input double InpSQMaximumDailyLossPercent = 0.75;
input int    InpSQMaximumConsecutiveLosses = 4;
input int    InpSQLossCooldownHours = 24;
input double InpSQMaximumSpreadPoints = 50.0;
input int    InpSQDeviationPoints = 20;
input string InpSQLogFileName = "FOUR_LANE_SQ_EVENTS.csv";
...
bool IsPortfolioMagic(const ulong magic)
{
   return magic == InpPortfolioMagic || magic == InpRVMagicNumber || magic == InpMOMagicNumber ||
          magic == InpATBMagicNumber || magic == InpSQMagicNumber;
}
                               ...
                               string &reason)
{
   ...
   double alignmentTolerance = MathMax(step * 1e-6, 1e-8);
   double normalizedClose = MathFloor(closeVolume / step + 1e-8) * step;
   double expectedVolume = MathFloor((previousVolume - closeVolume) / step + 1e-8) * step;
   if(MathAbs(currentVolume - previousVolume) > tolerance ||
      MathAbs(normalizedClose - closeVolume) > alignmentTolerance ||
      normalizedClose < minimum - alignmentTolerance ||
      closeVolume >= currentVolume - tolerance ||
      expectedVolume < minimum - alignmentTolerance)
   {
      reason = "partial volume contract";
      return false;
   }
   if(!trade.PositionClosePartial(ticket, closeVolume) || !TradeResultAllows(trade, false))
   {
   ...
   if(!SelectOwnedPosition(ticket, magic))
   {
      reason = "partial position disappeared";
      return false;
   }
   double resultingVolume = PositionGetDouble(POSITION_VOLUME);
   if(resultingVolume >= previousVolume - tolerance ||
      MathAbs(resultingVolume - expectedVolume) > tolerance)
   {
      reason = "partial remainder not confirmed";
      return false;
   }
   reason = "partial closed";
   return true;
}
...
bool PositionIdentifierOpen(const ulong positionIdentifier)
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
   {
      ulong ticket = PositionGetTicket(i);
      if(ticket > 0 && PositionSelectByTicket(ticket) &&
         (ulong)PositionGetInteger(POSITION_IDENTIFIER) == positionIdentifier)
         return true;
   }
   return false;
}
...
private:
   ...
   bool OpenPosition(const bool buy,
                     ...
                     const double diEdge,
                     const double requestedRiskPercent,
                     const double requestedMaximumPositionLots,
                     const double signalBodyRatio)
   {
      ...
      if(!PostFillReconcile(m_trade, InpRVMagicNumber, buy, requestedRiskPercent,
      ...
      eventReason += "; body ratio " + DoubleToString(signalBodyRatio, 3);
      eventReason += "; requested risk " + DoubleToString(requestedRiskPercent, 3);
      eventReason += "; requested lot cap " + DoubleToString(requestedMaximumPositionLots, 2);
   ...
   void TryEntry()
   {
      ...
      OpenPosition(buy, entryPrice, stopPrice, targetPrice, diEdge,
                   requestedRiskPercent, requestedMaximumPositionLots, signalBodyRatio);
   }
...
private:
   ...
   datetime LastExitTimeForSide(const bool buy)
   {
      if(!HistorySelect(0, TimeCurrent()))
         return 0;
      ...
      for(int i = HistoryDealsTotal() - 1; i >= 0; --i)
      {
         ...
         if(entryType != DEAL_ENTRY_OUT && entryType != DEAL_ENTRY_OUT_BY &&
            ...
            continue;
         if((ENUM_DEAL_TYPE)HistoryDealGetInteger(ticket, DEAL_TYPE) != closingDealType)
            continue;
         return (datetime)HistoryDealGetInteger(ticket, DEAL_TIME);
      }
      return 0;
   ...
   bool ExitCooldownAllows(const bool buy)
   {
      if(!InpMOUseSameSideExitCooldown || InpMOSameSideExitCooldownMinutes <= 0)
         return true;
      datetime lastExit = LastExitTimeForSide(buy);
      return lastExit <= 0 ||
             TimeCurrent() - lastExit >= InpMOSameSideExitCooldownMinutes * 60;
   }
   ...
   bool OpenPosition(const bool buy, const double atr)
   {
      ...
      ulong ticket = 0;
      string postFillReason = "";
      if(!PostFillReconcile(m_trade, InpMOMagicNumber, buy, InpMORiskPercent,
...
private:
   ...
   bool OpenPosition(const bool buy, const double atr)
   {
      ...
      ulong ticket = 0;
      string postFillReason = "";
      if(!PostFillReconcile(m_trade, InpATBMagicNumber, buy, InpATBRiskPercent,
      ...
      WriteEvidenceEvent(m_logHandle, "atb_h4_d1", "entry", ticket,
                         buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
                         "adaptive H4 Donchian breakout with D1 trend alignment");
      return true;
   }
...
private:
   ...
   string RiskKey(const ulong ticket)
   {
      return "TLP_SQ_RISK_" + IntegerToString((long)ticket);
   }

   string RunnerKeyPrefix()
   {
      return "TLP_SQ_PR_" + IntegerToString((long)AccountInfoInteger(ACCOUNT_LOGIN)) +
             "_" + IntegerToString((long)InpSQMagicNumber) + "_";
   }

   string RunnerVolumeKey(const ulong ticket)
   {
      return RunnerKeyPrefix() + "V_" + IntegerToString((long)ticket);
   }

   string RunnerDoneKey(const ulong ticket)
   {
      return RunnerKeyPrefix() + "D_" + IntegerToString((long)ticket);
   }

   void DeleteRunnerState(const ulong ticket)
   {
      VerifiedGlobalDelete(RunnerVolumeKey(ticket));
      VerifiedGlobalDelete(RunnerDoneKey(ticket));
   }

   bool PartialVolumes(const double totalVolume,
                       double &closeVolume,
                       double &remainingVolume)
   {
      ...
      double step = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
      double minimum = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
      if(totalVolume <= 0.0 || step <= 0.0 || minimum <= 0.0)
         return false;
      closeVolume = MathFloor(totalVolume * InpSQPartialClosePercent /
                              (100.0 * step) + 1e-8) * step;
      remainingVolume = MathFloor((totalVolume - closeVolume) / step + 1e-8) * step;
      return closeVolume >= minimum - 1e-8 &&
             remainingVolume >= minimum - 1e-8 &&
             closeVolume < totalVolume - step * 0.5;
   }

   bool RegisterRunnerState(const ulong ticket, const double initialVolume)
   {
      if(!SelectOwnedPosition(ticket, InpSQMagicNumber) ||
         !GlobalVariableCheck(RiskKey(ticket)) || initialVolume <= 0.0 ||
         !VerifiedGlobalSet(RunnerVolumeKey(ticket), initialVolume) ||
         !VerifiedGlobalSet(RunnerDoneKey(ticket), 0.0))
      {
         DeleteRunnerState(ticket);
         return false;
      }
      return true;
   }

   void CleanupOrphanedRunnerState()
   {
      string marker = RunnerKeyPrefix() + "V_";
      for(int i = GlobalVariablesTotal() - 1; i >= 0; --i)
      {
         string key = GlobalVariableName(i);
         if(StringFind(key, marker) != 0)
            continue;
         ulong ticket = (ulong)StringToInteger(StringSubstr(key, StringLen(marker)));
         if(ticket == 0 || !SelectOwnedPosition(ticket, InpSQMagicNumber))
            DeleteRunnerState(ticket);
      }
   }

   bool ProtectRunnerPosition(const ulong ticket, const bool buy)
   {
      if(!SelectOwnedPosition(ticket, InpSQMagicNumber) ||
         !GlobalVariableCheck(RiskKey(ticket)))
         return false;
      double riskDistance = GlobalVariableGet(RiskKey(ticket));
      double openPrice = PositionGetDouble(POSITION_PRICE_OPEN);
      double oldSl = PositionGetDouble(POSITION_SL);
      double takeProfit = PositionGetDouble(POSITION_TP);
      double protectedStop = NormalizeDouble(buy
                                             ? openPrice + InpSQPartialStopLockR * riskDistance
                                             : openPrice - InpSQPartialStopLockR * riskDistance,
                                             _Digits);
      double tolerance = MathMax(_Point, SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE)) * 0.5;
      bool alreadyProtected = oldSl > 0.0 &&
                              (buy ? oldSl >= protectedStop - tolerance
                                   : oldSl <= protectedStop + tolerance);
      if(alreadyProtected)
         return true;
      string modifyReason = "";
      if(ModifyOwnedPosition(m_trade, ticket, InpSQMagicNumber,
                             protectedStop, takeProfit, modifyReason))
         return true;
      string closeReason = "";
      CloseOwnedPosition(m_trade, ticket, InpSQMagicNumber, closeReason);
      return false;
   }

   void ManagePartialRunner()
   {
      if(!InpSQUsePartialRunner)
         return;
      for(int i = PositionsTotal() - 1; i >= 0; --i)
      {
         ulong ticket = PositionGetTicket(i);
         if(ticket == 0 || !SelectOwnedPosition(ticket, InpSQMagicNumber) ||
            !GlobalVariableCheck(RiskKey(ticket)) ||
            !GlobalVariableCheck(RunnerVolumeKey(ticket)) ||
            !GlobalVariableCheck(RunnerDoneKey(ticket)))
            continue;
         bool buy = PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY;
         double initialVolume = GlobalVariableGet(RunnerVolumeKey(ticket));
         double currentVolume = PositionGetDouble(POSITION_VOLUME);
         double step = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
         double tolerance = MathMax(step * 0.5, 1e-8);
         bool partialDone = GlobalVariableGet(RunnerDoneKey(ticket)) > 0.5 ||
                            currentVolume < initialVolume - tolerance;
         if(partialDone)
         {
            if(GlobalVariableGet(RunnerDoneKey(ticket)) <= 0.5 &&
               !VerifiedGlobalSet(RunnerDoneKey(ticket), 1.0))
            {
               string closeReason = "";
               CloseOwnedPosition(m_trade, ticket, InpSQMagicNumber, closeReason);
               continue;
            }
            ProtectRunnerPosition(ticket, buy);
            continue;
         }
         double openPrice = PositionGetDouble(POSITION_PRICE_OPEN);
         double riskDistance = GlobalVariableGet(RiskKey(ticket));
         double triggerPrice = buy ? openPrice + InpSQPartialTriggerR * riskDistance
                                   : openPrice - InpSQPartialTriggerR * riskDistance;
         double currentPrice = buy ? SymbolInfoDouble(_Symbol, SYMBOL_BID)
                                   : SymbolInfoDouble(_Symbol, SYMBOL_ASK);
         bool reached = buy ? currentPrice >= triggerPrice : currentPrice <= triggerPrice;
         if(!reached)
            continue;
         ...
         double closeVolume = 0.0, remainingVolume = 0.0;
         if(!PartialVolumes(currentVolume, closeVolume, remainingVolume))
         {
            string closeReason = "";
            CloseOwnedPosition(m_trade, ticket, InpSQMagicNumber, closeReason);
            continue;
         ...
         bool partialConfirmed = CloseOwnedPositionPartial(m_trade, ticket, InpSQMagicNumber,
                                                           closeVolume, currentVolume, partialReason);
         if(!partialConfirmed)
         {
            if(!SelectOwnedPosition(ticket, InpSQMagicNumber))
               DeleteRunnerState(ticket);
            else if(PositionGetDouble(POSITION_VOLUME) < currentVolume - tolerance &&
                    !VerifiedGlobalSet(RunnerDoneKey(ticket), 1.0))
            {
               string closeReason = "";
               CloseOwnedPosition(m_trade, ticket, InpSQMagicNumber, closeReason);
         ...
         if(!VerifiedGlobalSet(RunnerDoneKey(ticket), 1.0))
         {
            string closeReason = "";
            CloseOwnedPosition(m_trade, ticket, InpSQMagicNumber, closeReason);
            continue;
         }
         WriteEvidenceEvent(m_logHandle, "m15_sq_pr_telemetry", "partial_runner", ticket,
                            buy ? "buy" : "sell", closeVolume, currentPrice,
                            PositionGetDouble(POSITION_SL), 0.0,
                            "1.5R trigger reached; protected remainder retained");
      }
   }
   ...
   bool SessionAllows()
   {
      ...
      if(!TimeToStruct(TimeCurrent(), now))
         return false;
      if(InpSQDisableFridayAfterHour && now.day_of_week == 5 &&
         now.hour >= InpSQFridayCutoffHour)
         return false;
      if(!InpSQUseSessionFilter || InpSQSessionStartHour == InpSQSessionEndHour)
         return true;
      if(InpSQSessionStartHour < InpSQSessionEndHour)
         return now.hour >= InpSQSessionStartHour && now.hour < InpSQSessionEndHour;
      return now.hour >= InpSQSessionStartHour || now.hour < InpSQSessionEndHour;
   }
   ...
   int ManagedPositionCount()
   {
      ...
      for(int i = PositionsTotal() - 1; i >= 0; --i)
      {
         ...
         if(ticket == 0 || !PositionSelectByTicket(ticket))
            continue;
         if(PositionGetString(POSITION_SYMBOL) == _Symbol &&
            (ulong)PositionGetInteger(POSITION_MAGIC) == InpSQMagicNumber)
            count++;
      }
      return count;
   }
   ...
   double ClosedProfitSince(const datetime fromTime)
   {
      ...
      for(int i = 0; i < HistoryDealsTotal(); ++i)
      {
         ulong ticket = HistoryDealGetTicket(i);
         if(ticket == 0 || HistoryDealGetString(ticket, DEAL_SYMBOL) != _Symbol ||
            (ulong)HistoryDealGetInteger(ticket, DEAL_MAGIC) != InpSQMagicNumber)
            continue;
         long entryType = HistoryDealGetInteger(ticket, DEAL_ENTRY);
         if(entryType != DEAL_ENTRY_OUT && entryType != DEAL_ENTRY_OUT_BY && entryType != DEAL_ENTRY_INOUT)
            continue;
   ...
   bool SafetyAllows(string &reason)
   {
      ...
      double dailyProfit = ClosedProfitSince(g_dayStart);
      double dayStartBalance = AccountInfoDouble(ACCOUNT_BALANCE) - dailyProfit;
      if(InpSQMaximumDailyLossPercent > 0.0 && dayStartBalance > 0.0 &&
         dailyProfit <= -dayStartBalance * InpSQMaximumDailyLossPercent / 100.0)
      {
         reason = "squeeze daily loss limit";
         return false;
      }
      if(InpSQMaximumTradesPerDay > 0 && EntriesSince(g_dayStart) >= InpSQMaximumTradesPerDay)
      {
         reason = "squeeze daily trade limit";
         return false;
      }
      if(ManagedPositionCount() >= 1)
      {
         reason = "squeeze position limit";
         return false;
      }
      if(SpreadPoints() > InpSQMaximumSpreadPoints)
      {
         reason = "squeeze spread limit";
         return false;
      }
      if(!SessionAllows())
      {
         reason = "squeeze session filter";
         return false;
      }
      int streak = 0;
      datetime lastLoss = 0;
      LossStreak(streak, lastLoss);
      if(InpSQMaximumConsecutiveLosses > 0 && streak >= InpSQMaximumConsecutiveLosses &&
         lastLoss > 0 && TimeCurrent() - lastLoss < InpSQLossCooldownHours * 3600)
      {
         reason = "squeeze loss cooldown";
         return false;
      }
      return true;
   }
                      ...
                      double &channelLow)
   {
      ...
      MqlRates rates[];
      ArraySetAsSeries(rates, true);
      if(CopyRates(_Symbol, InpSQSignalTimeframe, startShift, count, rates) != count)
         return false;
      channelHigh = -DBL_MAX;
      channelLow = DBL_MAX;
      for(int i = 0; i < count; ++i)
   ...
   bool RegimeAllows(const bool buy, const double closePrice, const double atr)
   {
      if(closePrice <= 0.0 || atr <= 0.0)
         return false;
      if(InpSQUseVolatilityFilter)
      {
         double atrPercent = 100.0 * atr / closePrice;
         if(atrPercent < InpSQMinimumATRPercent || atrPercent > InpSQMaximumATRPercent)
            return false;
      }
      if(InpSQUseTrendEMAFilter)
      {
         double emaNow = 0.0, emaPast = 0.0;
         if(!BufferValue(m_trendEmaHandle, 0, 1, emaNow) ||
            !BufferValue(m_trendEmaHandle, 0, 1 + MathMax(1, InpSQTrendEMASlopeBars), emaPast))
            return false;
      ...
      {
         double adx = 0.0;
         if(!BufferValue(m_adxHandle, 0, 1, adx) || adx < InpSQMinimumADX ||
   ...
   {
      double openPrice = iOpen(_Symbol, InpSQSignalTimeframe, shift);
      double closePrice = iClose(_Symbol, InpSQSignalTimeframe, shift);
      double high = iHigh(_Symbol, InpSQSignalTimeframe, shift);
      double low = iLow(_Symbol, InpSQSignalTimeframe, shift);
      double range = high - low;
      if(range <= 0.0)
         return 0.0;
      return 100.0 * MathAbs(closePrice - openPrice) / range;
   }
   ...
   double CandleCloseLocation(const bool buy, const int shift)
   {
      ...
      if(range <= 0.0)
         return 0.0;
      return buy ? (closePrice - low) / range : (high - closePrice) / range;
   }
   ...
   bool TickVolumeAllows(const int signalShift)
   {
      if(!InpSQUseBreakoutTickVolumeFilter)
         return true;
      long signalVolume = iVolume(_Symbol, InpSQSignalTimeframe, signalShift);
      ...
      {
         long volume = iVolume(_Symbol, InpSQSignalTimeframe, shift);
         if(volume <= 0)
            continue;
         total += (double)volume;
         count++;
      ...
      return count > 0 && total > 0.0 &&
             (double)signalVolume >= (total / count) * InpSQMinimumVolumeRatio;
   }
   ...
   bool SqueezeBarAllows(const int shift)
   {
      double atr = 0.0, basis = 0.0, upperBand = 0.0, lowerBand = 0.0;
      if(!BufferValue(m_atrHandle, 0, shift, atr) || atr <= 0.0 ||
         !BufferValue(m_keltnerEmaHandle, 0, shift, basis) || basis <= 0.0 ||
         !BufferValue(m_bollingerHandle, 1, shift, upperBand) ||
         !BufferValue(m_bollingerHandle, 2, shift, lowerBand) || upperBand <= lowerBand)
         return false;
      double keltnerOffset = InpSQKeltnerATRMultiplier * atr;
      return upperBand <= basis + keltnerOffset && lowerBand >= basis - keltnerOffset;
   }

   bool SqueezeWindowAllows(const int startShift, const int count)
   {
      for(int shift = startShift; shift < startShift + count; ++shift)
      {
         if(!SqueezeBarAllows(shift))
            return false;
      }
      return true;
   }
   ...
   double EntryTickVolumeRatio(const int signalShift)
   {
      ...
      double total = 0.0;
      int count = 0;
      for(int shift = signalShift + 1;
          shift <= signalShift + InpSQVolumeLookbackBars;
          ++shift)
      {
         long volume = iVolume(_Symbol, InpSQSignalTimeframe, shift);
         if(volume <= 0)
      ...
      return count > 0 && total > 0.0
             ? (double)signalVolume / (total / count)
             : -1.0;
   }
   ...
   string EntryTelemetry(const bool buy,
                         ...
                         const double averageSqueezeBarRange)
   {
      double highPrice = iHigh(_Symbol, InpSQSignalTimeframe, 1);
      double lowPrice = iLow(_Symbol, InpSQSignalTimeframe, 1);
      double range = highPrice - lowPrice;
      double breakoutATR = atr > 0.0
                           ? (buy ? (closePrice - channelHigh) / atr
                                  : (channelLow - closePrice) / atr)
                           : -1.0;
      double bodyRatio = CandleBodyPercent(1) / 100.0;
      double closeLocation = CandleCloseLocation(buy, 1);
      double rangeATR = atr > 0.0 ? range / atr : -1.0;
      double expansionRatio = averageSqueezeBarRange > 0.0
                              ? range / averageSqueezeBarRange
                              : -1.0;
      double channelWidthATR = atr > 0.0 ? (channelHigh - channelLow) / atr : -1.0;
      double squeezeRangeATR = atr > 0.0 ? averageSqueezeBarRange / atr : -1.0;
      double atrPercent = closePrice > 0.0 ? 100.0 * atr / closePrice : -1.0;
      double tickVolumeRatio = EntryTickVolumeRatio(1);

      double adx = -1.0;
      BufferValue(m_adxHandle, 0, 1, adx);
      double emaNow = 0.0, emaPast = 0.0;
      double trendDistanceATR = -1.0, trendSlopeATR = -1.0;
      if(atr > 0.0 &&
         BufferValue(m_trendEmaHandle, 0, 1, emaNow) &&
         BufferValue(m_trendEmaHandle, 0,
                     1 + MathMax(1, InpSQTrendEMASlopeBars), emaPast))
      {
         trendDistanceATR = buy ? (closePrice - emaNow) / atr
                                : (emaNow - closePrice) / atr;
         trendSlopeATR = buy ? (emaNow - emaPast) / atr
                             : (emaPast - emaNow) / atr;
      }

      double squeezeRatioTotal = 0.0;
      double squeezeRatioMaximum = -1.0;
      int squeezeRatioCount = 0;
      for(int shift = 2; shift < 2 + InpSQSqueezeBars; ++shift)
      {
         double barATR = 0.0, basis = 0.0, upperBand = 0.0, lowerBand = 0.0;
         if(!BufferValue(m_atrHandle, 0, shift, barATR) || barATR <= 0.0 ||
            !BufferValue(m_keltnerEmaHandle, 0, shift, basis) || basis <= 0.0 ||
            !BufferValue(m_bollingerHandle, 1, shift, upperBand) ||
            !BufferValue(m_bollingerHandle, 2, shift, lowerBand) ||
            upperBand <= lowerBand)
            continue;
         double keltnerWidth = 2.0 * InpSQKeltnerATRMultiplier * barATR;
         if(keltnerWidth <= 0.0)
            continue;
         double ratio = (upperBand - lowerBand) / keltnerWidth;
         squeezeRatioTotal += ratio;
         squeezeRatioMaximum = MathMax(squeezeRatioMaximum, ratio);
         squeezeRatioCount++;
      }
      double squeezeRatioMean = squeezeRatioCount > 0
                                ? squeezeRatioTotal / squeezeRatioCount
                                : -1.0;

      return StringFormat("breakout_atr=%.6f;body_ratio=%.6f;close_location=%.6f;range_atr=%.6f;expansion_ratio=%.6f;channel_width_atr=%.6f;squeeze_range_atr=%.6f;tick_volume_ratio=%.6f;atr_pct=%.6f;adx=%.6f;trend_distance_atr=%.6f;trend_slope_atr=%.6f;squeeze_ratio_mean=%.6f;squeeze_ratio_max=%.6f",
                          breakoutATR, bodyRatio, closeLocation, rangeATR,
                          expansionRatio, channelWidthATR, squeezeRangeATR,
                          tickVolumeRatio, atrPercent, adx, trendDistanceATR,
                          trendSlopeATR, squeezeRatioMean, squeezeRatioMaximum);
   }

   bool OpenPosition(const bool buy,
                     const double atr,
                     const double rawStop,
                     const string entryTelemetry)
   {
      MqlTick tick;
      if(!SymbolInfoTick(_Symbol, tick) || atr <= 0.0)
         return false;
      ...
      if(stopDistance <= 0.0)
         return false;
      double stopLevel = (double)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL) * _Point;
      double minimumDistance = MathMax(InpSQMinimumStopATR * atr, stopLevel + _Point);
      if(stopDistance < minimumDistance)
         stopDistance = minimumDistance;
      if(stopDistance > InpSQMaximumStopATR * atr ||
         (InpSQMaximumStopPriceDistance > 0.0 && stopDistance > InpSQMaximumStopPriceDistance))
         return false;
      double stopPrice = NormalizeDouble(buy ? entryPrice - stopDistance
                                             : entryPrice + stopDistance, _Digits);
      double lots = LotsForRisk(buy, entryPrice, stopPrice,
                                InpSQRiskPercent, InpSQMaximumPositionLots);
      double closeVolume = 0.0, remainingVolume = 0.0;
      bool runnerRequested = InpSQUsePartialRunner &&
                             PartialVolumes(lots, closeVolume, remainingVolume);
      double targetR = runnerRequested ? InpSQPartialTargetR : InpSQTakeProfitR;
      double takeProfit = NormalizeDouble(buy ? entryPrice + targetR * stopDistance
                                              : entryPrice - targetR * stopDistance, _Digits);
      string exposureReason = "";
      if(lots <= 0.0 || !AccountWideExposureAllows(buy, entryPrice, stopPrice, lots, exposureReason))
         return false;
      m_trade.SetExpertMagicNumber(InpSQMagicNumber);
      m_trade.SetDeviationInPoints(InpSQDeviationPoints);
      string comment = buy ? "M15SQ_BUY" : "M15SQ_SELL";
      bool opened = buy ? m_trade.Buy(lots, _Symbol, 0.0, stopPrice, takeProfit, comment)
                        : m_trade.Sell(lots, _Symbol, 0.0, stopPrice, takeProfit, comment);
      if(!opened || !TradeResultAllows(m_trade, false))
      {
         string rejectReason = "";
         return RejectPostFill(m_trade, InpSQMagicNumber, "squeeze entry result", rejectReason);
      }
      ulong ticket = 0;
      string postFillReason = "";
      if(!PostFillReconcile(m_trade, InpSQMagicNumber, buy, InpSQRiskPercent,
                            InpSQMaximumPositionLots, ticket, postFillReason))
         return false;
      if(!RegisterRiskForPosition(ticket))
      {
         string rejectReason = "";
         return RejectPostFill(m_trade, InpSQMagicNumber, "squeeze risk registration", rejectReason);
      }
      if(runnerRequested)
      {
         double actualVolume = PositionGetDouble(POSITION_VOLUME);
         if(!PartialVolumes(actualVolume, closeVolume, remainingVolume) ||
            !RegisterRunnerState(ticket, actualVolume))
         {
            string rejectReason = "";
            return RejectPostFill(m_trade, InpSQMagicNumber,
                                  "squeeze partial-runner registration", rejectReason);
         }
      }
      ulong positionIdentifier = (ulong)PositionGetInteger(POSITION_IDENTIFIER);
      if(positionIdentifier == 0)
         positionIdentifier = ticket;
      string evidenceReason = entryTelemetry +
                              ";stop_atr=" + DoubleToString(stopDistance / atr, 6);
      WriteEvidenceEvent(m_logHandle,
                         runnerRequested ? "m15_sq_pr_telemetry" : "m15_sq_b8_telemetry",
                         "entry", positionIdentifier,
                         buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
                         evidenceReason);
      return true;
   }
   ...
   {
      if(InpSQMaximumHoldBars > 0)
      {
         datetime openTime = (datetime)PositionGetInteger(POSITION_TIME);
         int openShift = iBarShift(_Symbol, InpSQSignalTimeframe, openTime, false);
         if(openShift >= InpSQMaximumHoldBars)
         {
            string closeReason = "";
            return CloseOwnedPosition(m_trade, ticket, InpSQMagicNumber, closeReason);
         }
      }
      if(InpSQUseTrendFailureExit)
      {
         double ema = 0.0;
         double closePrice = iClose(_Symbol, InpSQSignalTimeframe, 1);
         if(BufferValue(m_trendEmaHandle, 0, 1, ema) && closePrice > 0.0)
         {
   ...
   void ImproveProtectiveStop(const ulong ticket, const bool buy, const double atr)
   {
      ...
      double r = favorable / initialRisk;
      double newSl = oldSl;
      if(InpSQUseBreakEven && r >= InpSQBreakEvenTriggerR)
      {
         double breakEven = buy ? openPrice + InpSQBreakEvenLockR * initialRisk
                                : openPrice - InpSQBreakEvenLockR * initialRisk;
         if((buy && breakEven > newSl) || (!buy && breakEven < newSl))
            newSl = breakEven;
      }
      if(InpSQUseChandelierTrail)
      {
         double channelHigh = 0.0, channelLow = 0.0;
         if(ChannelBounds(1, InpSQChandelierLookbackBars, channelHigh, channelLow))
         {
            double chandelier = buy ? channelHigh - InpSQChandelierATR * atr
                                     : channelLow + InpSQChandelierATR * atr;
   ...
   void TryEntry(const double atr)
   {
      double open1 = iOpen(_Symbol, InpSQSignalTimeframe, 1);
      double close1 = iClose(_Symbol, InpSQSignalTimeframe, 1);
      double high1 = iHigh(_Symbol, InpSQSignalTimeframe, 1);
      double low1 = iLow(_Symbol, InpSQSignalTimeframe, 1);
      double range1 = high1 - low1;
      if(atr <= 0.0 || open1 <= 0.0 || close1 <= 0.0 || range1 <= 0.0 ||
         !SqueezeWindowAllows(2, InpSQSqueezeBars))
         return;
      double channelHigh = 0.0, channelLow = 0.0;
      ...
      if(channelRange <= 0.0 || channelRange > InpSQMaximumBreakoutChannelATR * atr ||
         averageSqueezeBarRange <= 0.0)
         return;
      double buffer = InpSQBreakBufferATR * atr;
      bool buy = InpSQAllowBuy && close1 > channelHigh + buffer;
      bool sell = InpSQAllowSell && close1 < channelLow - buffer;
      if(buy == sell)
         return;
      bool directionCandle = !InpSQRequireDirectionCandle || (buy ? close1 > open1 : close1 < open1);
      if(!directionCandle || range1 < InpSQMinimumBreakRangeATR * atr ||
         ...
         !TickVolumeAllows(1) || !RegimeAllows(buy, close1, atr))
         return;
      string safetyReason = "";
      if(!SafetyAllows(safetyReason))
         return;
      ...
      string telemetry = EntryTelemetry(buy, close1, atr, channelHigh, channelLow,
                                        averageSqueezeBarRange);
      OpenPosition(buy, atr, rawStop, telemetry);
   }

public:
   CSqueezeDiversifierLane()
   {
      m_atrHandle = INVALID_HANDLE;
      m_bollingerHandle = INVALID_HANDLE;
      m_keltnerEmaHandle = INVALID_HANDLE;
   ...
   bool Init()
   {
      if(!InpSQEnabled)
         return true;
      m_atrHandle = iATR(_Symbol, InpSQSignalTimeframe, InpSQATRPeriod);
      m_bollingerHandle = iBands(_Symbol, InpSQSignalTimeframe, InpSQBollingerPeriod, 0,
                                 InpSQBollingerDeviation, PRICE_CLOSE);
      m_keltnerEmaHandle = iMA(_Symbol, InpSQSignalTimeframe, InpSQKeltnerEMAPeriod, 0,
                               MODE_EMA, PRICE_TYPICAL);
      m_trendEmaHandle = iMA(_Symbol, InpSQTrendTimeframe, InpSQTrendEMAPeriod, 0,
                             MODE_EMA, PRICE_CLOSE);
      m_adxHandle = iADX(_Symbol, InpSQSignalTimeframe, InpSQADXPeriod);
      if(m_atrHandle == INVALID_HANDLE || m_bollingerHandle == INVALID_HANDLE ||
         m_keltnerEmaHandle == INVALID_HANDLE || m_trendEmaHandle == INVALID_HANDLE ||
         m_adxHandle == INVALID_HANDLE)
         return false;
      m_trade.SetExpertMagicNumber(InpSQMagicNumber);
      m_trade.SetDeviationInPoints(InpSQDeviationPoints);
      m_trade.SetAsyncMode(false);
      m_trade.SetTypeFillingBySymbol(_Symbol);
      m_logHandle = OpenEvidenceFile(InpSQLogFileName);
      CleanupOrphanedRunnerState();
      return !InpLogTrades || m_logHandle != INVALID_HANDLE;
   }
   ...
   void Deinit()
   {
      ...
      if(m_keltnerEmaHandle != INVALID_HANDLE) IndicatorRelease(m_keltnerEmaHandle);
   ...
   void OnTick()
   {
      if(!InpSQEnabled)
         return;
      ManagePartialRunner();
      datetime currentBar = iTime(_Symbol, InpSQSignalTimeframe, 0);
      if(currentBar <= 0 || currentBar == m_lastSignalBar)
         return;
      m_lastSignalBar = currentBar;
      double atr = 0.0;
   ...
   void OnTradeTransaction(const MqlTradeTransaction &transaction)
   {
      if(transaction.type != TRADE_TRANSACTION_DEAL_ADD || !HistoryDealSelect(transaction.deal))
         return;
      if(HistoryDealGetString(transaction.deal, DEAL_SYMBOL) != _Symbol ||
         (ulong)HistoryDealGetInteger(transaction.deal, DEAL_MAGIC) != InpSQMagicNumber)
         return;
      long entryType = HistoryDealGetInteger(transaction.deal, DEAL_ENTRY);
      if(entryType != DEAL_ENTRY_OUT && entryType != DEAL_ENTRY_OUT_BY && entryType != DEAL_ENTRY_INOUT)
         return;
      bool positionOpen = transaction.position > 0 &&
                          PositionIdentifierOpen(transaction.position);
      if(transaction.position > 0 && !positionOpen)
      {
         VerifiedGlobalDelete(RiskKey(transaction.position));
         DeleteRunnerState(transaction.position);
      }
      double profit = HistoryDealGetDouble(transaction.deal, DEAL_PROFIT) +
                      HistoryDealGetDouble(transaction.deal, DEAL_SWAP) +
                      HistoryDealGetDouble(transaction.deal, DEAL_COMMISSION);
      string exitReason = HistoryDealGetString(transaction.deal, DEAL_COMMENT) +
                          ";position_id=" + IntegerToString((long)transaction.position) +
                          ";position_open_after_exit=" + (positionOpen ? "1" : "0");
      WriteEvidenceEvent(m_logHandle,
                         InpSQUsePartialRunner ? "m15_sq_pr_telemetry" : "m15_sq_b8_telemetry",
                         "exit", transaction.position, "close",
                         HistoryDealGetDouble(transaction.deal, DEAL_VOLUME),
                         HistoryDealGetDouble(transaction.deal, DEAL_PRICE), 0.0, profit,
                         exitReason);
   }
...
bool InputsValid()
{
   if(InpRVMagicNumber == InpMOMagicNumber ||
      ...
      InpSQRiskPercent <= 0.0 || InpSQRiskPercent > InpMaximumPortfolioOpenRiskPercent ||
   ...
   if(InpRVATRPeriod < 2 || InpRVADXPeriod < 2 || InpRVRSIPeriod < 2 ||
      ...
      ((InpRVUseStrongSignalRisk || InpRVUseStrongSignalLotCap) &&
       (InpRVStrongSignalMinimumBodyRatio < 0.0 ||
        InpRVStrongSignalMinimumBodyRatio > 1.0)) ||
      (InpRVUseStrongSignalRisk &&
       (InpRVStrongSignalRiskPercent <= InpRVRiskPercent ||
      ...
      (InpRVUseStrongSignalLotCap &&
       (InpRVStrongSignalMaximumPositionLots <= InpRVMaximumPositionLots ||
        InpRVStrongSignalMaximumPositionLots > 1.0)) ||
   ...
   if(InpMOMomentumLookbackBars < 20 || InpMOEntryLookbackBars < 3 ||
      ...
      InpMOSameSideExitCooldownMinutes < 0 || InpMOSameSideExitCooldownMinutes > 1440 ||
      (InpMOUseSameSideExitCooldown && InpMOSameSideExitCooldownMinutes < 1) ||
   ...
   if(InpATBMomentumLookbackBars < 20 || InpATBEntryLookbackBars < 3 ||
      ...
      return false;
   if(InpSQSqueezeBars < 2 || InpSQSqueezeBars > 12 ||
      InpSQBollingerPeriod < 5 || InpSQBollingerDeviation <= 0.0 ||
      InpSQKeltnerEMAPeriod < 5 || InpSQKeltnerATRMultiplier <= 0.0 ||
      InpSQBreakoutLookbackBars < InpSQSqueezeBars ||
      InpSQMaximumBreakoutChannelATR <= 0.0 || InpSQBreakBufferATR < 0.0 ||
      InpSQMinimumBreakRangeATR <= 0.0 ||
      InpSQMaximumBreakRangeATR < InpSQMinimumBreakRangeATR ||
      InpSQMinimumExpansionRatio <= 0.0 ||
      ...
      InpSQTrendEMAPeriod < 2 || InpSQTrendEMASlopeBars < 1 ||
      InpSQADXPeriod < 2 || InpSQMinimumADX < 0.0 ||
      (InpSQMaximumADX > 0.0 && InpSQMaximumADX < InpSQMinimumADX) ||
      InpSQMinimumATRPercent < 0.0 ||
      InpSQMaximumATRPercent < InpSQMinimumATRPercent ||
      InpSQATRPeriod < 2 || InpSQStopBufferATR < 0.0 ||
      InpSQMinimumStopATR <= 0.0 || InpSQMaximumStopATR < InpSQMinimumStopATR ||
      InpSQTakeProfitR <= 0.0 ||
      (InpSQUsePartialRunner &&
       (InpSQPartialClosePercent <= 0.0 || InpSQPartialClosePercent >= 100.0 ||
        InpSQPartialTriggerR < InpSQTakeProfitR ||
        InpSQPartialTargetR <= InpSQPartialTriggerR || InpSQPartialTargetR > 10.0 ||
        InpSQPartialStopLockR < 0.0 || InpSQPartialStopLockR >= InpSQPartialTriggerR)) ||
      InpSQBreakEvenTriggerR < 0.0 ||
      InpSQBreakEvenLockR < 0.0 || InpSQChandelierLookbackBars < 2 ||
      InpSQChandelierATR <= 0.0 || InpSQMaximumHoldBars < 0 ||
      InpSQSessionStartHour < 0 || InpSQSessionStartHour > 23 ||
      InpSQSessionEndHour < 0 || InpSQSessionEndHour > 23 ||
      InpSQFridayCutoffHour < 0 || InpSQFridayCutoffHour > 23 ||
      InpSQMaximumPositionLots <= 0.0 || InpSQMaximumTradesPerDay < 0 ||
      InpSQMaximumDailyLossPercent < 0.0 || InpSQMaximumConsecutiveLosses < 0 ||
      InpSQLossCooldownHours < 0 || InpSQMaximumSpreadPoints < 0.0 ||
      InpSQDeviationPoints < 0)
      return false;
   return true;
}
...
void UpdateDashboard()
{
   ...
   Comment("XAUUSD Transferable Portfolio\n",
           ...
           "  Margin: ", DoubleToString(AccountInfoDouble(ACCOUNT_MARGIN_LEVEL), 1), "%\n",
            ...
            "% + SQ ", DoubleToString(InpSQRiskPercent, 2), "%");
}
...
int OnInit()
{
   ...
   g_guardTrade.SetDeviationInPoints((ulong)MathMax(MathMax(MathMax(InpRVDeviationPoints,
                                                                    InpMODeviationPoints),
                                                            InpATBDeviationPoints),
                                                    InpSQDeviationPoints));
                        ...
                        const MqlTradeResult &result)
{
   ...
   g_squeeze.OnTradeTransaction(transaction);
}
