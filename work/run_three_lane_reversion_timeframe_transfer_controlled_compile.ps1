...
if(!$UserAuthorizedFocusRisk){throw 'Controlled timeframe-transfer compile requires explicit focus/window-risk authorization.'}
...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_REVERSION_TIMEFRAME_TRANSFER_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'Timeframe-transfer source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Timeframe-transfer controlled compile $startedAtUtc",[Text.Encoding]::ASCII)
