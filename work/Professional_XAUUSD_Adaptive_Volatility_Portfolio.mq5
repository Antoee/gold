...
input group "Adaptive Volatility Risk Controller"
input bool   InpUseAdaptiveVolatilityRisk = true;
input ENUM_TIMEFRAMES InpVolatilityRiskTimeframe = PERIOD_H1;
input int    InpVolatilityRiskATRPeriod = 20;
input int    InpVolatilityRiskBaselineBars = 126;
input double InpVolatilityRiskMinimumScale = 0.75;
input double InpVolatilityRiskMaximumScale = 1.25;

input group "H1 Band VWAP Reversion Lane"
input bool   InpRVEnabled = true;
input ulong  InpRVMagicNumber = 26071722;
...
input ulong  InpMOMagicNumber = 26071762;
...
input string InpRVLogFileName = "ADAPTIVE_VOLATILITY_PORTFOLIO_RV_EVENTS.csv";
input string InpMOLogFileName = "ADAPTIVE_VOLATILITY_PORTFOLIO_MO_EVENTS.csv";
...
int g_volatilityRiskAtrHandle = INVALID_HANDLE;
datetime g_lastVolatilityRiskBar = 0;
double g_volatilityRiskScale = 1.0;
double g_volatilityRiskScaleSum = 0.0;
double g_volatilityRiskScaleMinimum = DBL_MAX;
double g_volatilityRiskScaleMaximum = 0.0;
int g_volatilityRiskScaleSamples = 0;
...
{
   return "XAU_AVP_PEAK_" + IntegerToString((long)AccountInfoInteger(ACCOUNT_LOGIN)) +
...
bool SharedSafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "AVP-LIVE-ACK-v1"))
   {
   ...
   if(g_peakEquity <= 0.0 || equity > g_peakEquity)
   {
      g_peakEquity = equity;
      if(!MQLInfoInteger(MQL_TESTER))
         GlobalVariableSet(PeakEquityKey(), g_peakEquity);
   }
   ...
   double dailyProfit = ClosedPortfolioProfitSince(g_dayStart);
...
double AdaptiveVolatilityRiskScale()
{
   if(!InpUseAdaptiveVolatilityRisk)
      return 1.0;
   datetime currentBar = iTime(_Symbol, InpVolatilityRiskTimeframe, 0);
   if(currentBar > 0 && currentBar == g_lastVolatilityRiskBar &&
      g_volatilityRiskScaleSamples > 0)
      return g_volatilityRiskScale;

   int baselineBars = MathMax(20, InpVolatilityRiskBaselineBars);
   int required = baselineBars + 1;
   double atrValues[];
   double closeValues[];
   if(g_volatilityRiskAtrHandle == INVALID_HANDLE ||
      CopyBuffer(g_volatilityRiskAtrHandle, 0, 1, required, atrValues) != required ||
      CopyClose(_Symbol, InpVolatilityRiskTimeframe, 1, required, closeValues) != required)
      return g_volatilityRiskScaleSamples > 0 ? g_volatilityRiskScale : 1.0;

   int currentIndex = required - 1;
   if(atrValues[currentIndex] <= 0.0 || closeValues[currentIndex] <= 0.0)
      return g_volatilityRiskScaleSamples > 0 ? g_volatilityRiskScale : 1.0;
   double currentAtrPercent = atrValues[currentIndex] / closeValues[currentIndex];
   double baselineSum = 0.0;
   int valid = 0;
   for(int i = 0; i < currentIndex; ++i)
   {
      if(atrValues[i] <= 0.0 || closeValues[i] <= 0.0)
         continue;
      baselineSum += atrValues[i] / closeValues[i];
      valid++;
   }
   if(valid < baselineBars || currentAtrPercent <= 0.0)
      return g_volatilityRiskScaleSamples > 0 ? g_volatilityRiskScale : 1.0;

   double baselineAtrPercent = baselineSum / valid;
   double rawScale = baselineAtrPercent / currentAtrPercent;
   double scale = MathMax(InpVolatilityRiskMinimumScale,
                          MathMin(InpVolatilityRiskMaximumScale, rawScale));
   if(!MathIsValidNumber(scale) || scale <= 0.0)
      return g_volatilityRiskScaleSamples > 0 ? g_volatilityRiskScale : 1.0;
   g_volatilityRiskScale = scale;
   g_lastVolatilityRiskBar = currentBar;
   g_volatilityRiskScaleSum += scale;
   g_volatilityRiskScaleMinimum = MathMin(g_volatilityRiskScaleMinimum, scale);
   g_volatilityRiskScaleMaximum = MathMax(g_volatilityRiskScaleMaximum, scale);
   g_volatilityRiskScaleSamples++;
   return g_volatilityRiskScale;
}
                   ...
                   const double maximumLots)
{
   ...
   if(!RiskMoneyForOrder(_Symbol, orderType, entryPrice, stopPrice, 1.0, lossPerLot))
      return 0.0;
   double effectiveRiskPercent = riskPercent * AdaptiveVolatilityRiskScale();
...
private:
                     ...
                     const double diEdge)
   {
      ...
      WriteEvidenceEvent(m_logHandle, "tlp_rv_m12", "entry", m_trade.ResultOrder(),
...
private:
   ...
   {
      return "AVP_MO_RISK_" + IntegerToString((long)ticket);
   }
   ...
   bool OpenPosition(const bool buy, const double atr)
   {
      ...
      WriteEvidenceEvent(m_logHandle, "tlp_mom_e20", "entry", m_trade.ResultOrder(),
                         buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
...
bool InputsValid()
{
   double maximumRiskScale = InpUseAdaptiveVolatilityRisk ? InpVolatilityRiskMaximumScale : 1.0;
   if(InpRVMagicNumber == InpMOMagicNumber ||
      ...
      (InpRVRiskPercent + InpMORiskPercent) * maximumRiskScale >
         InpMaximumPortfolioOpenRiskPercent + 1e-9)
      return false;
   if(InpUseAdaptiveVolatilityRisk &&
      (InpVolatilityRiskATRPeriod < 2 || InpVolatilityRiskBaselineBars < 20 ||
       InpVolatilityRiskMinimumScale <= 0.0 || InpVolatilityRiskMinimumScale > 1.0 ||
       InpVolatilityRiskMaximumScale < 1.0 ||
       InpVolatilityRiskMaximumScale < InpVolatilityRiskMinimumScale))
      return false;
...
void UpdateDashboard()
{
   ...
   double riskScale = AdaptiveVolatilityRiskScale();
   Comment("XAUUSD Adaptive Volatility Portfolio\n",
           ...
           "%  Vol scale: ", DoubleToString(riskScale, 3), "x");
}
...
int OnInit()
{
   ...
   if(InpUseAdaptiveVolatilityRisk)
   {
      g_volatilityRiskAtrHandle = iATR(_Symbol, InpVolatilityRiskTimeframe,
                                       InpVolatilityRiskATRPeriod);
      if(g_volatilityRiskAtrHandle == INVALID_HANDLE)
         return INIT_FAILED;
   }
   ...
   {
      if(g_volatilityRiskAtrHandle != INVALID_HANDLE)
         IndicatorRelease(g_volatilityRiskAtrHandle);
      return INIT_FAILED;
   }
   if(!g_momentum.Init())
   {
...
void OnDeinit(const int reason)
{
   ...
   if(g_volatilityRiskAtrHandle != INVALID_HANDLE)
      IndicatorRelease(g_volatilityRiskAtrHandle);
   double minimumScale = g_volatilityRiskScaleSamples > 0 ? g_volatilityRiskScaleMinimum : 1.0;
   double maximumScale = g_volatilityRiskScaleSamples > 0 ? g_volatilityRiskScaleMaximum : 1.0;
   double averageScale = g_volatilityRiskScaleSamples > 0 ?
                         g_volatilityRiskScaleSum / g_volatilityRiskScaleSamples : 1.0;
   PrintFormat("AVP_DIAGNOSTIC adaptive=%s samples=%d min_scale=%.4f max_scale=%.4f average_scale=%.4f",
               InpUseAdaptiveVolatilityRisk ? "true" : "false",
               g_volatilityRiskScaleSamples, minimumScale, maximumScale, averageScale);
   Comment("");
}
