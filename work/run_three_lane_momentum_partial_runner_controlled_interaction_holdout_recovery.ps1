...
$manifest = Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_PARTIAL_RUNNER_INTERACTION_HOLDOUT_MODEL1_RECOVERY_MANIFEST.csv'
$expectedManifestHash = '7DDE4275D4C9CF74E7E1F29F8727E892E0EF9CF848CE0E8821E3C56AAB2F16C8'
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile,"Momentum partial-runner interaction holdout recovery $startedAtUtc",[Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck,"Momentum partial-runner interaction holdout recovery focus acknowledgement $startedAtUtc",[Text.Encoding]::ASCII)
   ...
   & $runner -ManifestPath $manifest -PortableRoots $roots -UserAuthorizedFocusRisk `
      -OutputPrefix 'THREE_LANE_MOMENTUM_PARTIAL_RUNNER_INTERACTION_HOLDOUT_RECOVERY' `
...
[pscustomobject][ordered]@{
   Status='CONTROLLED_INTERACTION_HOLDOUT_RECOVERY_COMPLETE';StartedAtUtc=$startedAtUtc;CompletedAtUtc=[DateTime]::UtcNow.ToString('o')
