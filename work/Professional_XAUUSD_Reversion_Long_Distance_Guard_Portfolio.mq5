...
#property description "Research fork with a long-distance guard on XAUUSD reversion entries"
...
input bool   InpRVUseLongDistanceGate = false;
input int    InpRVLongDistanceLookbackBars = 200;
input double InpRVMinimumLongDistanceATR = -10.0;
...
private:
   ...
   double AverageClose(const int startShift, const int count)
   {
      if(startShift < 1 || count < 2)
         return -1.0;
      double closes[];
      ArraySetAsSeries(closes, true);
      if(CopyClose(_Symbol, InpRVSignalTimeframe, startShift, count, closes) != count)
         return -1.0;
      double total = 0.0;
   ...
   void TryEntry()
   {
      ...
      if(InpRVUseDIEdgeGate && diEdge < InpRVMinimumDIEdge)
         return;
      if(InpRVUseLongDistanceGate)
      {
         double longMean = AverageClose(1, InpRVLongDistanceLookbackBars);
         if(longMean <= 0.0)
            return;
         double alignedDistanceATR = direction * (close1 - longMean) / atr;
         if(alignedDistanceATR < InpRVMinimumLongDistanceATR)
            return;
      }
...
bool InputsValid()
{
   ...
   if(InpRVATRPeriod < 2 || InpRVADXPeriod < 2 || InpRVRSIPeriod < 2 ||
      ...
      (InpRVUseLongDistanceGate &&
       (InpRVLongDistanceLookbackBars < 20 || InpRVMinimumLongDistanceATR > 0.0 ||
        InpRVMinimumLongDistanceATR < -50.0)))
      return false;
