...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_STRONG_BREAKOUT_TARGET_EXTENSION_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'Strong-breakout target-extension source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Strong-breakout target-extension compile $startedAtUtc",[Text.Encoding]::ASCII)
