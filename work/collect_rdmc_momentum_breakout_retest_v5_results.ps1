param(
   ...
   [string]$DecisionCsvPath = "outputs\RDMC_MOMENTUM_BREAKOUT_RETEST_V5_DECISION_FIXTURE.csv",
   [string]$ManifestPath = "outputs\RDMC_MOMENTUM_BREAKOUT_RETEST_V5_GATE_MANIFEST.csv",
   [string]$ReportDir = "outputs\rdmc_momentum_breakout_retest_v5_package\reports_here",
   [string]$RunnerLedgerGlob = "",
   [string]$ResultsPath = "outputs\RDMC_MOMENTUM_BREAKOUT_RETEST_V5_GATE_RESULTS.csv",
   [string]$RunAuditPath = "",
...
if([string]::IsNullOrWhiteSpace($RunnerLedgerGlob)) {
   $RunnerLedgerGlob = "outputs\RDMC_MOMENTUM_BREAKOUT_RETEST_V5_WAVE_{0:D2}_WORKER_*.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($RunAuditPath)) {
   $RunAuditPath = "outputs\RDMC_MOMENTUM_BREAKOUT_RETEST_V5_WAVE_{0:D2}_RUN_AUDIT.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($RawResultsPath)) {
   $RawResultsPath = "outputs\RDMC_MOMENTUM_BREAKOUT_RETEST_V5_WAVE_{0:D2}_RAW_RESULTS.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($SummaryPath)) {
   $SummaryPath = "outputs\RDMC_MOMENTUM_BREAKOUT_RETEST_V5_WAVE_{0:D2}_SUMMARY.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($MetricsMarkdownPath)) {
   $MetricsMarkdownPath = "outputs\RDMC_MOMENTUM_BREAKOUT_RETEST_V5_WAVE_{0:D2}_METRICS.md" -f $Wave
}
...
$waveManifestTemp = Join-Path $repo ("outputs\.rdmc_mbr5_wave_{0:D2}_{1}.csv" -f $Wave,[guid]::NewGuid().ToString("N"))
try {
...
if(!$SkipAdmissionRefresh) {
   if((Resolve-Path -LiteralPath $resultsFull).Path -ne (Join-Path $repo "outputs\RDMC_MOMENTUM_BREAKOUT_RETEST_V5_GATE_RESULTS.csv")) {
