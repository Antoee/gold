...
if(!$UserAuthorizedFocusRisk) { throw 'Controlled momentum-adaptive risk-reallocation testing requires explicit focus/window-risk authorization.' }
...
$manifest = Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_ADAPTIVE_RISK_REALLOCATION_DISCOVERY_MODEL1_MANIFEST.csv'
$source = Join-Path $repo 'outputs\three_lane_momentum_adaptive_risk_reallocation_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
$expectedManifestHash = '1A22157C6C27F4DF07B7FF82B4AC8100A5D785DC212169EBD9A8E3137D1CC938'
$expectedSourceHash = 'B6810B305549968E2273DAAF736A63759FE5C16F3B416F5C69E39840FBE5173E'
$expectedBinaryHash = '0BF7AEEE1D5F9496A6C7A88012D7059A8D894B2475097C158254670E1A189883'
$roots = @(
...
foreach($required in @($repoLock,$outerLock,$runner,$manifest,$source) + $roots) {
   if(!(Test-Path -LiteralPath $required)) { throw "Controlled risk-reallocation prerequisite missing: $required" }
}
if((Get-FileHash -LiteralPath $manifest -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedManifestHash) {
   throw 'Risk-reallocation manifest identity changed.'
}
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Risk-reallocation package source identity changed.'
}
...
if((Test-Path -LiteralPath $unlockFile) -or (Test-Path -LiteralPath $focusAck) -or
   ...
   throw 'Controlled risk-reallocation testing refuses a pre-existing partial unlock state.'
}
if(@(Get-Process -Name terminal,terminal64,metatester,metatester64,MetaEditor,metaeditor64 -ErrorAction SilentlyContinue).Count -ne 0) {
   throw 'Controlled risk-reallocation testing requires zero pre-existing MT5-family processes.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Momentum-adaptive risk-reallocation discovery $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Momentum-adaptive risk-reallocation focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
   ...
   & $runner -ManifestPath $manifest -PortableRoots $roots -UserAuthorizedFocusRisk `
      -OutputPrefix $(if($SingleWorkerRecovery) { 'THREE_LANE_MOMENTUM_ADAPTIVE_RISK_REALLOCATION_RECOVERY_WORKER' } else { 'THREE_LANE_MOMENTUM_ADAPTIVE_RISK_REALLOCATION_WORKER' }) `
...
if(!$completed) { throw 'Controlled risk-reallocation discovery did not complete.' }
if(!(Test-Path -LiteralPath $repoLock) -or !(Test-Path -LiteralPath $outerLock) -or
   ...
   throw 'Controlled risk-reallocation discovery did not restore hard-lock state.'
}
