...
$source=Join-Path $repo 'outputs\three_lane_reversion_strong_signal_lot_cap_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_LOT_CAP_COMPILE_AUDIT.csv'
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'Selective lot-cap source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Selective lot-cap compile $startedAtUtc",[Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck,"Selective lot-cap focus acknowledgement $startedAtUtc",[Text.Encoding]::ASCII)
