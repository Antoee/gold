...
$manifest = Join-Path $repo 'outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_GATE_MANIFEST.csv'
$source = Join-Path $repo 'outputs\rdmc_momentum_retest_hold_v6_package\source\Professional_XAUUSD_EA.mq5'
$profile = Join-Path $repo 'outputs\rdmc_momentum_retest_hold_v6_package\profiles\rdmc_momentum_retest_hold_v6.set'
$reportDir = Join-Path $repo 'outputs\rdmc_momentum_retest_hold_v6_package\reports_here'
$decision = Join-Path $repo 'outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_DECISION_FIXTURE.csv'
$runner = Join-Path $PSScriptRoot 'run_mt5_portable_parallel_manifest.ps1'
$collector = Join-Path $PSScriptRoot 'collect_rdmc_momentum_retest_hold_v6_results.ps1'
...
$expectedBinaryHash = 'DAEB452BB696A13D1DC2E0F292FB2AD6D316521BE44C3CDBC64A264F76F5890F'
...
$waveManifest = Join-Path $env:TEMP ('rdmc_mrh6_wave_{0:D2}_{1}.csv' -f $Wave,[guid]::NewGuid().ToString('N'))
$workerPrefix = 'RDMC_MOMENTUM_RETEST_HOLD_V6_WAVE_{0:D2}_WORKER' -f $Wave
try {
   ...
   & $collector -Wave $Wave -DecisionCsvPath $decision -ManifestPath $manifest -ReportDir $reportDir `
      ...
      -ResultsPath 'outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_GATE_RESULTS.csv' -SkipAdmissionRefresh
   & python $evaluator
