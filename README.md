...
Last updated: 2026-07-14 UTC after hidden local MT5 first-pass testing of `trade_ready_conservative`, `money_ready`, several locked LowATR risk-shape candidates, and a 32-variant LowATR exit/risk sweep. There is no newly validated trade-ready best profile. The current research best remains LowATR OrderFlow. The raw locked LowATR fast screen produced the big number, `+$8,437.54` on `Model=1` from `2024.01.01` to `2026.07.12`, but it also had `25.94%` drawdown, so it is research-only and not promoted. The exit sweep found higher headline profit (`+$9,274.07`) and one under-6% drawdown candidate (`lowatr_exit_peak_r20`), but the under-6% candidate failed yearly quality because 2025 was too weak.
...
Plain English: the bot can produce the kind of big backtest number you were asking about, but the big version is not safe enough yet. The latest hidden local raw LowATR locked screen made `+$8,437.54` in the 2024-2026 Model1 continuous test, which is the new "big money" research clue, but it came with `25.94%` drawdown. The safer risk20/risk23 variants reduced risk, but either made much less, failed recovery, or still exceeded the `6%` first-pass drawdown cap. So the old `$866 in 2.5 years` baseline is clearly outdated, but there is still no live-ready/high-profit profile. The next useful work is strategy/risk-shape improvement, not risk inflation.
...
2. Rework the LowATR risk shape or strategy logic before the next first-pass build. The raw locked LowATR screen can make `+$8,437.54`, but at `25.94%` drawdown; the safer variants did not clear the first-pass path.
3. Consider a targeted strategy-code change before the next first-pass build: preserve LowATR trade frequency while reducing the 2025 recovery weakness and continuous drawdown, or test a separate FMLR/activity-blend candidate. Do not simply increase risk to make a failed profile look profitable.
