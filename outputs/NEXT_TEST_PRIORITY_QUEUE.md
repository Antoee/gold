...
| 5 | ADX_FILTER_PROBE | ADX 18 trend-strength filter probe | 4 | Expand only passing ADX candidates into stress/recent-OOS validation. | Stop spending tester time on failed ADX variants. |
| 6 | SPREAD_GUARD_PROBE | ATR-relative spread guard probe | 4 | Expand only passing spread-guard candidates into stress/recent-OOS validation. | Stop spending tester time on failed spread-guard variants. |
| 7 | TIME_EXIT_PROBE | 240-minute stale-trade exit probe | 4 | Expand only passing time-exit candidates into stress/recent-OOS validation. | Stop spending tester time on failed time-exit variants. |
| 8 | MTF_TREND_PROBE | H1 200 EMA trend filter probe | 4 | Expand only passing MTF candidates into stress/recent-OOS validation. | Stop spending tester time on failed MTF variants. |
| 9 | STRUCTURE_TRAIL_PROBE | Structure trailing stop probe | 4 | Expand only passing structure-trailing candidates into stress/recent-OOS validation. | Stop spending tester time on failed structure-trailing variants. |
| 10 | SESSION_PROBE | `tp38_sl18` session variants vs unfiltered baseline | 6 | Expand only passing session candidates into stress/recent-OOS validation. | Stop spending tester time on failed session variants. |
| 11 | FULL_HANDOFF | Full candidate validation | 24 | Build promotion packet only if all gates pass. | Keep current promoted baseline. |
