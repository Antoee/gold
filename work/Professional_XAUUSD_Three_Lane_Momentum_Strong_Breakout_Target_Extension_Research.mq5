...
#property description "Default-off strong-breakout target extension for sealed research"
...
private:
                         ...
                         const double channelLow)
   {
      ...
      return StringFormat("d1_pct=%.6f;breakout_atr=%.6f;body_ratio=%.6f;close_location=%.6f;range_atr=%.6f;channel_width_atr=%.6f;atr_pct=%.6f;tick_volume_ratio=%.6f",
   ...
   double BreakoutTargetR(const bool buy, const double closePrice)
   {
      ...
      double lowPrice = iLow(_Symbol, InpMOSignalTimeframe, 1);
      double range = highPrice - lowPrice;
      if(openPrice <= 0.0 || closePrice <= 0.0 || range <= 0.0)
         return InpMOTakeProfitR;
      double bodyRatio = MathAbs(closePrice - openPrice) / range;
      double closeLocation = buy ? (closePrice - lowPrice) / range
                                 : (highPrice - closePrice) / range;
      if(bodyRatio < InpMOStrongBreakoutMinimumBodyRatio ||
         closeLocation < InpMOStrongBreakoutMinimumCloseLocation)
         return InpMOTakeProfitR;
                     ...
                     const string entryTelemetry)
   {
      ...
      string evidenceReason = "daily momentum plus fresh H1 breakout;" + entryTelemetry +
                              ";target_r=" + DoubleToString(targetR, 6) +
      ...
      if(buyBreak && RegimeAllows(true, close1, atr))
      {
         if(ExitCooldownAllows(true))
            OpenPosition(true, atr, BreakoutTargetR(true, close1),
                         EntryTelemetry(true, close1, atr, channelHigh, channelLow));
      }
      else if(sellBreak && RegimeAllows(false, close1, atr))
      {
         if(ExitCooldownAllows(false))
            OpenPosition(false, atr, BreakoutTargetR(false, close1),
                         EntryTelemetry(false, close1, atr, channelHigh, channelLow));
      }
   }
...
bool InputsValid()
{
   ...
   if(InpMOMomentumLookbackBars < 20 || InpMOEntryLookbackBars < 3 ||
      ...
      InpMOStrongBreakoutMinimumBodyRatio < 0.0 ||
      InpMOStrongBreakoutMinimumBodyRatio > 1.0 ||
      InpMOStrongBreakoutMinimumCloseLocation < 0.0 ||
      InpMOStrongBreakoutMinimumCloseLocation > 1.0 ||
      (InpMOUseStrongBreakoutTargetExtension &&
       InpMOStrongBreakoutTakeProfitR <= InpMOTakeProfitR) ||
