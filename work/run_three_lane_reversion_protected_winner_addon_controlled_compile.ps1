...
$source = Join-Path $repo 'outputs\three_lane_reversion_protected_winner_addon_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath = Join-Path $repo 'outputs\THREE_LANE_REVERSION_PROTECTED_WINNER_ADDON_COMPILE_AUDIT.csv'
$roots = @(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Protected reversion add-on source identity changed.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Protected reversion winner add-on compile $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Protected reversion winner add-on focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
