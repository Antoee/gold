[CmdletBinding()]
param(
   [string]$SourcePath = 'work\Professional_XAUUSD_Three_Lane_Reversion_Partial_Runner_Research.mq5',
   ...
   [string]$PackageDir = 'outputs\three_lane_reversion_partial_runner_discovery_model1_package',
   [string]$ManifestPath = 'outputs\THREE_LANE_REVERSION_PARTIAL_RUNNER_DISCOVERY_MODEL1_MANIFEST.csv'
)
...
$expectedSourceHash = '614DCF5B0C55DF25DABDCF903C3193A0CE248AA2671788A400B5C39A4209F719'
...
& (Join-Path $PSScriptRoot 'test_three_lane_reversion_partial_runner_source.ps1') | Out-Null
...
if($sourceHash -ne $expectedSourceHash) { throw "Partial-runner research source identity changed: $sourceHash" }
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_reversion_partial_runner_discovery_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='rvpr_control';Role='control';Enabled='false';Close='80';Target='2.00';Lock='0.50'},
   [pscustomobject]@{Name='rvpr_center';Role='center';Enabled='true';Close='80';Target='2.00';Lock='0.50'},
   [pscustomobject]@{Name='rvpr_close75';Role='close_neighbor_low';Enabled='true';Close='75';Target='2.00';Lock='0.50'},
   [pscustomobject]@{Name='rvpr_close85';Role='close_neighbor_high';Enabled='true';Close='85';Target='2.00';Lock='0.50'},
   [pscustomobject]@{Name='rvpr_target175';Role='target_neighbor_low';Enabled='true';Close='80';Target='1.75';Lock='0.50'},
   [pscustomobject]@{Name='rvpr_target225';Role='target_neighbor_high';Enabled='true';Close='80';Target='2.25';Lock='0.50'},
   [pscustomobject]@{Name='rvpr_lock025';Role='lock_neighbor_low';Enabled='true';Close='80';Target='2.00';Lock='0.25'},
   [pscustomobject]@{Name='rvpr_lock075';Role='lock_neighbor_high';Enabled='true';Close='80';Target='2.00';Lock='0.75'}
)
$windows = @(
...
$stopRule = 'Frozen pre-2021 discovery. Both eras profitable and no more than 2% below same-era control. Center continuous net >= control +4%; CAGR >= control +0.06 point; PF >= control; DD <= min(1.20%, control +0.10 point); recovery and return/DD >=97% control. At least 4 of 6 neighbors pass both eras, continuous net >=control +2%, same PF/DD/97% efficiency floors. Confirmed behavior change required. No post-result rescue.'
...
foreach($variant in $variants) {
   ...
   Set-FixedInput $inputs 'InpRVUseStrongSignalPartialRunner' $variant.Enabled
   Set-FixedInput $inputs 'InpRVPartialRunnerClosePercent' $variant.Close
   Set-FixedInput $inputs 'InpRVPartialRunnerTargetMultiplier' $variant.Target
   Set-FixedInput $inputs 'InpRVPartialRunnerStopLockR' $variant.Lock
   ...
   foreach($window in $windows) {
      ...
      $rows.Add([pscustomobject][ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='reversion_partial_runner_discovery_model1'
         Window=$window.Name;From=$window.From;To=$window.To;Model=1;Deposit=10000
         PartialRunnerEnabled=$variant.Enabled;ClosePercent=$variant.Close;TargetMultiplier=$variant.Target;StopLockR=$variant.Lock
         BaseReversionRiskPercent='0.45';StrongSignalBodyRatio='0.25';StrongSignalLotCap='0.15';MomentumRiskPercent='0.15';AdaptiveRiskPercent='0.15'
...
@(
   '# Reversion Partial Runner Discovery Contract','',
   ...
   '- Frozen center: strong-signal reversion closes 80% at its original VWAP target after locking +0.50R, while the protected remainder targets 2.00x the original distance.',
   '- Frozen neighborhood: close fractions 75/85%, target multipliers 1.75/2.25x, and stop locks +0.25/+0.75R.',
