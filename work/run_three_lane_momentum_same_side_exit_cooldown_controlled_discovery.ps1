...
if(!$UserAuthorizedFocusRisk) { throw 'Controlled same-side exit-cooldown testing requires explicit focus/window-risk authorization.' }
...
$manifest = Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_DISCOVERY_MODEL1_MANIFEST.csv'
...
$expectedManifestHash = 'FCB5D72AB5BAFD5C5EE38FBECA03046ACED40F468DD3864BFF8DB482FED92057'
...
try {
   ...
   & $runner -ManifestPath $manifest -PortableRoots $roots -UserAuthorizedFocusRisk `
      -OutputPrefix $(if($SingleWorkerRecovery) { 'THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_RECOVERY_WORKER' } else { 'THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_WORKER' }) `
