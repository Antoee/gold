...
#property description "Behavior-neutral momentum market-phase telemetry for sealed research"
...
private:
   ...
   int m_h4AdxHandle;
   int m_h1AdxHandle;
   int m_d1FastEmaHandle;
   int m_d1SlowEmaHandle;
   int m_logHandle;
   datetime m_lastSignalBar;
   ...
   double DirectionalEfficiency(const ENUM_TIMEFRAMES timeframe,
                                const bool buy,
                                const int lookback)
   {
      double recent = iClose(_Symbol, timeframe, 1);
      double oldest = iClose(_Symbol, timeframe, 1 + lookback);
      if(recent <= 0.0 || oldest <= 0.0)
         return -2.0;
      double path = 0.0;
      double prior = recent;
      for(int shift = 2; shift <= 1 + lookback; ++shift)
      {
         double value = iClose(_Symbol, timeframe, shift);
         if(value <= 0.0)
            return -2.0;
         path += MathAbs(prior - value);
         prior = value;
      }
      if(path <= 0.0)
         return -2.0;
      double direction = buy ? 1.0 : -1.0;
      return direction * (recent - oldest) / path;
   }

   double DirectionalRangePosition(const ENUM_TIMEFRAMES timeframe,
                                   const bool buy,
                                   const int lookback)
   {
      ...
      {
         double highPrice = iHigh(_Symbol, timeframe, shift);
         double lowPrice = iLow(_Symbol, timeframe, shift);
         if(highPrice <= 0.0 || lowPrice <= 0.0 || highPrice < lowPrice)
            return -1.0;
         highest = MathMax(highest, highPrice);
         lowest = MathMin(lowest, lowPrice);
      }
      double width = highest - lowest;
      if(width <= 0.0)
         return -1.0;
      return buy ? (closePrice - lowest) / width : (highest - closePrice) / width;
   }

   double H1CompressionRatio()
   {
      double recentRange = 0.0;
      double priorRange = 0.0;
      for(int shift = 1; shift <= 25; ++shift)
      {
         double highPrice = iHigh(_Symbol, InpMOSignalTimeframe, shift);
         double lowPrice = iLow(_Symbol, InpMOSignalTimeframe, shift);
         if(highPrice <= 0.0 || lowPrice <= 0.0 || highPrice < lowPrice)
            return -1.0;
         if(shift <= 5)
            recentRange += highPrice - lowPrice;
         else
            priorRange += highPrice - lowPrice;
      }
      recentRange /= 5.0;
      priorRange /= 20.0;
      return priorRange > 0.0 ? recentRange / priorRange : -1.0;
   }
   ...
   {
      double openPrice = iOpen(_Symbol, InpMOSignalTimeframe, 1);
      double highPrice = iHigh(_Symbol, InpMOSignalTimeframe, 1);
      double lowPrice = iLow(_Symbol, InpMOSignalTimeframe, 1);
      double range = highPrice - lowPrice;
      double bodyRatio = range > 0.0 ? MathAbs(closePrice - openPrice) / range : -1.0;
      double closeLocation = range > 0.0
                             ? (buy ? (closePrice - lowPrice) / range
                                    : (highPrice - closePrice) / range)
                             : -1.0;
      ...
      double recentD1 = iClose(_Symbol, InpMOMomentumTimeframe, 1);
      double pastD1 = iClose(_Symbol, InpMOMomentumTimeframe,
                             1 + InpMOMomentumLookbackBars);
      double d1MomentumPercent = recentD1 > 0.0 && pastD1 > 0.0
                                 ? 100.0 * MathAbs(recentD1 / pastD1 - 1.0)
                                 : -1.0;

      double averageTickVolume = 0.0;
      int volumeSamples = 0;
      for(int shift = 2; shift <= 21; ++shift)
      {
         long value = iVolume(_Symbol, InpMOSignalTimeframe, shift);
         if(value <= 0)
            continue;
         averageTickVolume += (double)value;
         volumeSamples++;
      }
      if(volumeSamples > 0)
         averageTickVolume /= volumeSamples;
      long signalTickVolume = iVolume(_Symbol, InpMOSignalTimeframe, 1);
      double tickVolumeRatio = averageTickVolume > 0.0
                               ? (double)signalTickVolume / averageTickVolume
                               : -1.0;

      double d1Adx = -1.0, h4Adx = -1.0, h1Adx = -1.0;
      BufferValue(m_d1AdxHandle, 0, 1, d1Adx);
      BufferValue(m_h4AdxHandle, 0, 1, h4Adx);
      BufferValue(m_h1AdxHandle, 0, 1, h1Adx);
      double d1Atr = -1.0, d1Fast = -1.0, d1Slow = -1.0, d1FastPast = -1.0;
      double d1EmaGapATR = -999.0, d1PriceGapATR = -999.0, d1EmaSlopeATR = -999.0;
      if(BufferValue(m_d1AtrHandle, 0, 1, d1Atr) && d1Atr > 0.0 &&
         BufferValue(m_d1FastEmaHandle, 0, 1, d1Fast) &&
         BufferValue(m_d1SlowEmaHandle, 0, 1, d1Slow) &&
         BufferValue(m_d1FastEmaHandle, 0, 21, d1FastPast))
      {
         double direction = buy ? 1.0 : -1.0;
         d1EmaGapATR = direction * (d1Fast - d1Slow) / d1Atr;
         d1PriceGapATR = direction * (recentD1 - d1Slow) / d1Atr;
         d1EmaSlopeATR = direction * (d1Fast - d1FastPast) / d1Atr;
      }
      double d1Efficiency20 = DirectionalEfficiency(PERIOD_D1, buy, 20);
      double d1RangePosition60 = DirectionalRangePosition(PERIOD_D1, buy, 60);
      double h4Efficiency20 = DirectionalEfficiency(PERIOD_H4, buy, 20);
      double h1Efficiency20 = DirectionalEfficiency(InpMOSignalTimeframe, buy, 20);
      double h1Compression = H1CompressionRatio();

      return StringFormat("d1_pct=%.6f;breakout_atr=%.6f;body_ratio=%.6f;close_location=%.6f;range_atr=%.6f;channel_width_atr=%.6f;atr_pct=%.6f;tick_volume_ratio=%.6f;market_d1_adx=%.6f;market_h4_adx=%.6f;market_h1_adx=%.6f;d1_ema_gap_atr=%.6f;d1_price_gap_atr=%.6f;d1_ema_slope_atr=%.6f;d1_efficiency20=%.6f;d1_range_position60=%.6f;h4_efficiency20=%.6f;h1_efficiency20=%.6f;h1_compression5_20=%.6f",
                          d1MomentumPercent, breakoutATR, bodyRatio, closeLocation,
                          rangeATR, channelWidthATR, atrPercent, tickVolumeRatio,
                          d1Adx, h4Adx, h1Adx, d1EmaGapATR, d1PriceGapATR,
                          d1EmaSlopeATR, d1Efficiency20, d1RangePosition60,
                          h4Efficiency20, h1Efficiency20, h1Compression);
   }
   ...
   bool OpenPosition(const bool buy, const double atr, const string entryTelemetry)
   {
      MqlTick tick;
      ...
      string evidenceReason = "daily momentum plus fresh H1 breakout;" + entryTelemetry +
...
public:
   ...
   {
      m_atrHandle = INVALID_HANDLE;
      m_d1AtrHandle = INVALID_HANDLE;
      m_d1AdxHandle = INVALID_HANDLE;
      m_h4AdxHandle = INVALID_HANDLE;
      m_h1AdxHandle = INVALID_HANDLE;
      m_d1FastEmaHandle = INVALID_HANDLE;
      m_d1SlowEmaHandle = INVALID_HANDLE;
      m_logHandle = INVALID_HANDLE;
      m_lastSignalBar = 0;
   }
   ...
   bool Init()
   {
      ...
      m_d1AtrHandle = iATR(_Symbol, PERIOD_D1, 14);
      m_d1AdxHandle = iADX(_Symbol, PERIOD_D1, 14);
      m_h4AdxHandle = iADX(_Symbol, PERIOD_H4, 14);
      m_h1AdxHandle = iADX(_Symbol, InpMOSignalTimeframe, 14);
      ...
      if(m_atrHandle == INVALID_HANDLE || m_d1AtrHandle == INVALID_HANDLE ||
         m_d1AdxHandle == INVALID_HANDLE || m_h4AdxHandle == INVALID_HANDLE ||
         m_h1AdxHandle == INVALID_HANDLE || m_d1FastEmaHandle == INVALID_HANDLE ||
         m_d1SlowEmaHandle == INVALID_HANDLE)
         return false;
      m_trade.SetExpertMagicNumber(InpMOMagicNumber);
      m_trade.SetDeviationInPoints(InpMODeviationPoints);
   ...
   {
      if(m_atrHandle != INVALID_HANDLE) IndicatorRelease(m_atrHandle);
      if(m_d1AtrHandle != INVALID_HANDLE) IndicatorRelease(m_d1AtrHandle);
      if(m_d1AdxHandle != INVALID_HANDLE) IndicatorRelease(m_d1AdxHandle);
      if(m_h4AdxHandle != INVALID_HANDLE) IndicatorRelease(m_h4AdxHandle);
      if(m_h1AdxHandle != INVALID_HANDLE) IndicatorRelease(m_h1AdxHandle);
      if(m_d1FastEmaHandle != INVALID_HANDLE) IndicatorRelease(m_d1FastEmaHandle);
      if(m_d1SlowEmaHandle != INVALID_HANDLE) IndicatorRelease(m_d1SlowEmaHandle);
      if(m_logHandle != INVALID_HANDLE) FileClose(m_logHandle);
   }
