...
- Fresh current-source continuous Model4 is only `+1,195.69` (`+119.57%` total, `+36.51%/yr` CAGR); the historical `+4,507.51` continuous headline needs reproduction before it should be used as the current headline.
...
- Money-ready demo/forward-test candidate: `outputs/CANDIDATE_MONEY_READY_PROFILE.set`, SHA-256 `2F2B757768AA14CC864F5A161305E1A4E05C1B4228B880BEBA6CD5C50D8D6231`. The alias `outputs/CANDIDATE_TRADE_READINESS_PROFILE.set` has the same hash. This candidate enables the EA symbol and trade-readiness safety gates, caps risk at `0.50%`, caps open risk at `0.75%`, caps lots at `0.05`, allows one position, blocks real-account trading by default, keeps Adaptive Reverse/FMLR/tick-speed experiments off, stamps evidence identity, keeps live approval identity disabled, and requires spread, trading-cost, margin, loss, profit-giveback, break-even, ATR trailing, and MFE protection. This is demo/forward-test only and does not replace the current research-best profile.
- Conservative trade-ready candidate: `outputs/CANDIDATE_TRADE_READY_CONSERVATIVE_PROFILE.set`, SHA-256 `0A97B46D7E3A3C3566EF4E787BCB63E2138D114C2F0F898F9A8B1A10F842BF90`. The first-pass validation queue, next-run package, and parallel lanes were rebuilt on 2026-07-14 from this current profile hash.
- Source-level trade-readiness safety gate: `InpUseTradeReadinessSafetyGate` refuses initialization when enabled if the profile is loosened past configured risk/spread/margin/exit caps. `SymbolSafetyLockAllows()` blocks wrong-symbol initialization, and `RealAccountSafetyLockAllows()` blocks real-account initialization unless explicit approval code, approval profile id, approval source hash, evidence profile id/source hash, evidence run label, and trade-readiness gate requirements are all satisfied. Current source hash: `46770EACA60826F90E1E9A9B7425356F96F7C8F83CF8F8C1FBE271632866933E`.
- Evidence identity refinement: trade logs now include `profile_id`, `source_hash`, and `run_label`; conservative returned logs are expected to prove `profile_id=trade_ready_conservative` and `source_hash=46770EACA60826F90E1E9A9B7425356F96F7C8F83CF8F8C1FBE271632866933E` before trade-quality or Monte Carlo evidence is trusted.
