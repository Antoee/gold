...
$report.Add("") | Out-Null
$report.Add("| Rank | Profile | Phase | Set | Window | Model | Role | Reason | Config |") | Out-Null
$report.Add("|---:|---|---|---|---|---:|---|---|---|") | Out-Null
foreach($item in $ranked) {
   $report.Add("| $($item.Rank) | ``$($item.Profile)`` | $($item.Phase) | $($item.Set) | $($item.Window) | $($item.Model) | $($item.Role) | $($item.Reason) | ``$($item.Config)`` |") | Out-Null
}
$report.Add("") | Out-Null
$report.Add("## Rule") | Out-Null
$report.Add("") | Out-Null
$report.Add("Use this list to spend limited MT5 time on the highest-information windows first. Phase 1 is pruning only; no profile can replace the current default without complete phase-2 real-tick evidence and the promotion gate passing.") | Out-Null
