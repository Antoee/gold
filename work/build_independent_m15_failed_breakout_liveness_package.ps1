param(
   ...
   [string]$PackageManifestPath = "outputs\INDEPENDENT_M15_FAILED_BREAKOUT_LIVENESS_MODEL1_PACKAGE_MANIFEST.csv",
   [string]$MarkdownPath = "outputs\INDEPENDENT_M15_FAILED_BREAKOUT_LIVENESS_MODEL1_PACKAGE.md"
)
...
$variants = @(
   (New-Variant "fbt_b14_struct_r075" @{ BoxBars="14"; MinTargetR="0.75" }),
   (New-Variant "fbt_b16_struct_r075" @{ BoxBars="16"; MinTargetR="0.75" }),
   (New-Variant "fbt_b18_struct_r075" @{ BoxBars="18"; MinTargetR="0.75" }),
   (New-Variant "fbt_b14_fixed_r125" @{ BoxBars="14"; BoxTarget="false"; TPR="1.25" }),
   (New-Variant "fbt_b14_fixed_r150" @{ BoxBars="14"; BoxTarget="false"; TPR="1.50" }),
   (New-Variant "fbt_b14_fixed_r200" @{ BoxBars="14"; BoxTarget="false"; TPR="2.00" }),
   (New-Variant "fbt_b16_fixed_r125" @{ BoxBars="16"; BoxTarget="false"; TPR="1.25" }),
   (New-Variant "fbt_b16_fixed_r150" @{ BoxBars="16"; BoxTarget="false"; TPR="1.50" }),
   (New-Variant "fbt_b16_fixed_r200" @{ BoxBars="16"; BoxTarget="false"; TPR="2.00" }),
   (New-Variant "fbt_b18_fixed_r125" @{ BoxBars="18"; BoxTarget="false"; TPR="1.25" }),
   (New-Variant "fbt_b18_fixed_r150" @{ BoxBars="18"; BoxTarget="false"; TPR="1.50" }),
   (New-Variant "fbt_b18_fixed_r200" @{ BoxBars="18"; BoxTarget="false"; TPR="2.00" })
)
...
$rank = 0
$candidateRank = 0
$stopRule = "Liveness follow-up only through 2020: both disjoint eras must be profitable; continuous PF at least 1.20; at least 60 continuous trades; DD at or below 5%; and neighboring 14/16/18-box plus exit shapes must support the result before opening 2021-2026."
...
foreach($variant in $variants) {
   ...
   Set-InputLine -Inputs $inputs -Name "InpEvidenceRunLabel" -Value "independent_m15_failed_breakout_liveness_model1"
   ...
   foreach($window in $windows) {
      ...
      $queueRows.Add([pscustomobject]@{
         ...
         SourceType="independent_m15_failed_breakout_trap"; SourceRank=1; Phase="liveness_model1"
      ...
      $runRows.Add([pscustomobject]@{
         QueueRank=$rank; Candidate=$variant.Name; Phase="liveness_model1"
         PhaseLabel="Independent M15 failed-breakout liveness Model1"; Window=$window.Name; Model=1; Deposit=10000
...
$md.Add("# Independent M15 Failed-Breakout Liveness Package")
$md.Add("")
$md.Add("Bounded 14/16/18-box liveness follow-up after the first discovery screen. No configuration includes data after 2020.")
$md.Add("")
