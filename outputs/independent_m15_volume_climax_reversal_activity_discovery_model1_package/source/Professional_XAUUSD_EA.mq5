...
input ulong  InpMagicNumber = 26071773;
...
input double InpMinimumRangeATR = 1.10;
input double InpMaximumRangeATR = 3.00;
input double InpMinimumWickPercent = 45.0;
input double InpMaximumBodyPercent = 55.0;
input double InpMinimumCloseLocation = 0.55;
input double InpMinimumVWAPDeviationATR = 0.45;
input double InpMaximumVWAPDeviationATR = 2.50;
input double InpMinimumRiskReward = 1.10;
input double InpMaximumTargetR = 1.75;
...
input double InpMaximumTrendDistanceATR = 2.50;
...
input string InpLogFileName = "Independent_XAUUSD_M15_Volume_Climax_Reversal_Trades.csv";
...
{
   return "IM15VCR_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "M15VCR-LIVE-ACK"))
   {
                        ...
                        const double vwapTarget)
{
   ...
   string comment = buy ? "M15VCR_BUY" : "M15VCR_SELL";
...
bool TryVolumeClimaxEntry(const double atr)
{
   ...
   if(volumeRatio < InpMinimumVolumeRatio)
   {
      g_volumeRejects++;
      return false;
   }
...
int OnInit()
{
   if(InpVolumeLookbackBars < 2 || InpMinimumVolumeRatio <= 0.0 ||
...
{
   PrintFormat("M15VCR_DIAGNOSTIC climaxes=%d signals=%d volume_rejects=%d shape_rejects=%d vwap_rejects=%d regime_rejects=%d safety_rejects=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
               g_climaxCandidates, g_reversalSignals, g_volumeRejects,
