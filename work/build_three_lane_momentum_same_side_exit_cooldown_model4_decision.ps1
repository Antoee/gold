...
param(
   ...
   [string]$ReportDir='outputs\three_lane_momentum_same_side_exit_cooldown_model4_package\reports_here',
   [string]$CompileAuditPath='outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_COMPILE_AUDIT.csv',
   [string]$ResultsPath='outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_MODEL4_RESULTS.csv',
   [string]$SummaryPath='outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_MODEL4_SUMMARY.csv',
   [string]$DecisionCsvPath='outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_MODEL4_DECISION.csv',
   [string]$DecisionMarkdownPath='outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_MODEL4_DECISION.md',
   [string]$RunAttestationPath='outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_MODEL4_RUN_ATTESTATION.csv'
)
$ErrorActionPreference='Stop'
Set-StrictMode -Version Latest
$repo=(Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$sourceHash='B6810B305549968E2273DAAF736A63759FE5C16F3B416F5C69E39840FBE5173E'
$controlName='msec_control';$centerName='msec_center_060';$continuous='continuous_2015_2026'
...
foreach($pattern in @('THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_MODEL4_WORKER_?.csv','THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_MODEL4_RECOVERY_WORKER_?.csv')){foreach($file in Get-ChildItem (Join-Path $repo 'outputs') -Filter $pattern -File){foreach($row in @(Import-Csv $file.FullName)){$workers.Add($row)|Out-Null}}}
...
$finalRuns=@(Import-Csv (Resolve-P 'outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_MODEL4_RECOVERY_WORKER_1.csv'))
if($finalRuns.Count-ne 8-or@($finalRuns|Where-Object{$_.Status-ne'REPORT_FOUND'}).Count){throw 'Canonical recovery evidence is incomplete.'}
...
$growth=[double]$center.NetProfit-gt[double]$control.NetProfit;$cagr=[double]$center.CagrPercent-ge[double]$control.CagrPercent
...
$risk=[double]$center.MaxDrawdownPercent-le[double]$control.MaxDrawdownPercent+0.03;$trades=[int]$center.TotalTrades-ge[math]::Ceiling(0.98*[int]$control.TotalTrades)
...
$lines.Add('# Momentum Same-Side Exit Cooldown Model 4 Decision')
$lines.Add('')
$lines.Add($(if($passed){'**Decision: MODEL 4 GATE PASSED. Annual and stress validation may open; promotion remains closed.**'}else{'**Decision: REJECTED IN MODEL 4. Annual/stress expansion, promotion, forward change, and live approval remain closed.**'}))
$lines.Add('')
$lines.Add("- Reports: ``$($results.Count) / 8``; attempts: ``$($workers.Count)``; refusals: ``$(@($workers|Where-Object{$_.Status-eq'ERROR'}).Count)``")
$lines.Add("- Source: ``$sourceHash``; EX5: ``$binary``")
$lines.Add('- XAUUSD, `$10,000`, Model 4 real ticks through `2026-07-12`; real trading disabled')
$lines.Add('')
$lines.Add('| Profile | 2015-18 | 2019-22 | 2023-26 | Continuous | CAGR | PF | Trades | DD | Recovery | Return/DD |')
$lines.Add('|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|')
foreach($row in $summary){
   $label=if($row.Candidate-eq$centerName){'**60-minute center**'}else{'Control'}
   $lines.Add("| $label | $(Money([double]$row.OlderNetProfit)) | $(Money([double]$row.MiddleNetProfit)) | $(Money([double]$row.RecentNetProfit)) | $(Money([double]$row.ContinuousNetProfit)) | $($row.CagrPercent)%/yr | $($row.ProfitFactor) | $($row.TotalTrades) | $($row.MaxDrawdownPercent)% | $($row.RecoveryFactor) | $($row.ReturnDrawdown) |")
}
$lines.Add('')
$lines.Add('## Frozen Gate')
$lines.Add('')
$lines.Add("- Center positive/no worse every era: ``$allCenterPositive / $noWorse`` ($(PassText($allCenterPositive-and$noWorse)))")
$lines.Add("- Growth/CAGR: ``$growth / $cagr`` ($(PassText($growth-and$cagr)))")
$lines.Add("- PF/recovery/return-DD: ``$efficiency`` ($(PassText $efficiency))")
$lines.Add("- Drawdown/trades: ``$risk / $trades`` ($(PassText($risk-and$trades)))")
$lines.Add('')
$lines.Add($(if($passed){'This exact center must still pass annual restarts, execution-cost stress, Monte Carlo, hard-risk auditing, and broker variation before historical promotion can be considered.'}else{'No threshold or cap rescue is permitted from this result.'}))
$lines.Add('')
$lines.Add('The published leader and registered forward candidate were unchanged during this gate; real trading remained disabled.')
