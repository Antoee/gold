...
#property description "Research fork with completed-bar momentum profit ratchet"
...
private:
   ...
   void ImproveProtectiveStop(const ulong ticket, const bool buy)
   {
      ...
      if(InpMOUseProfitRatchet && r >= InpMOProfitRatchetTriggerR)
      {
         ...
         if((buy && ratchet > newSl) || (!buy && ratchet < newSl))
            newSl = ratchet;
      }
...
bool InputsValid()
{
   ...
   if(InpMOMomentumLookbackBars < 20 || InpMOEntryLookbackBars < 3 ||
      ...
      InpMOProfitRatchetTriggerR <= 0.0 ||
      InpMOProfitRatchetTriggerR >= InpMOTakeProfitR ||
      InpMOProfitRatchetLockR < 0.0 ||
      InpMOProfitRatchetLockR >= InpMOProfitRatchetTriggerR ||
