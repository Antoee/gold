...
if(!$UserAuthorizedFocusRisk) { throw 'Controlled same-side exit-cooldown Model 4 validation requires explicit focus/window-risk authorization.' }
...
$manifest = Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_MODEL4_MANIFEST.csv'
$source = Join-Path $repo 'outputs\three_lane_momentum_same_side_exit_cooldown_model4_package\source\Professional_XAUUSD_EA.mq5'
$expectedManifestHash = '8A0CD5AD2CF9E6730E2398550E5186F94138BA79F22BD78D64C20CDD604BD15A'
...
try {
   ...
   & $runner -ManifestPath $manifest -PortableRoots $roots -UserAuthorizedFocusRisk `
      -OutputPrefix $(if($SingleWorkerRecovery) { 'THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_MODEL4_RECOVERY_WORKER' } else { 'THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_MODEL4_WORKER' }) `
