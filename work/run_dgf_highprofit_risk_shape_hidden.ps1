param(
   [int]$TimeoutMinutesPerConfig = 20,
   [ValidateRange(1,100)][int]$MaxCpuPercent = 80,
   [string]$OutCsv = "outputs\DGF_HIGHPROFIT_RISK_SHAPE_CURRENT_SOURCE_STATUS.csv",
   [string]$OutMarkdown = "outputs\DGF_HIGHPROFIT_RISK_SHAPE_CURRENT_SOURCE_STATUS.md"
)
...
try {
   ...
   Set-Content -LiteralPath $unlockFile -Value "Controlled DGF high-profit risk-shape probe allowed at $($started.ToString('s'))." -Encoding ASCII
   ...
   & (Join-Path $PSScriptRoot "build_dgf_highprofit_risk_shape_probe_package.ps1")

   & (Join-Path $PSScriptRoot "run_first_pass_package_hidden.ps1") `
      -ManifestPath "outputs\DGF_HIGHPROFIT_RISK_SHAPE_PACKAGE_MANIFEST.csv" `
      -TimeoutMinutesPerConfig $TimeoutMinutesPerConfig `
      -MaxCpuPercent $MaxCpuPercent `
      -Run `
      -OutCsv "outputs\DGF_HIGHPROFIT_RISK_SHAPE_RUN.csv" `
      -OutMarkdown "outputs\DGF_HIGHPROFIT_RISK_SHAPE_RUN.md"

   & (Join-Path $PSScriptRoot "import_first_pass_hidden_log_results.ps1") `
      -RunCsv "outputs\DGF_HIGHPROFIT_RISK_SHAPE_RUN.csv" `
      -QueueManifestPath "outputs\DGF_HIGHPROFIT_RISK_SHAPE_QUEUE.csv" `
      -OutResults "outputs\DGF_HIGHPROFIT_RISK_SHAPE_RESULTS.csv" `
      -OutSummary "outputs\DGF_HIGHPROFIT_RISK_SHAPE_SUMMARY.csv" `
      -OutMarkdown "outputs\DGF_HIGHPROFIT_RISK_SHAPE_METRICS.md"
   ...
   Set-Content -LiteralPath $hardLockFile -Value "Restored after controlled DGF high-profit risk-shape probe." -Encoding ASCII
}

$finished = Get-Date
$runRows = @()
if(Test-Path -LiteralPath (Resolve-RepoPath "outputs\DGF_HIGHPROFIT_RISK_SHAPE_RUN.csv")) {
   $runRows = @(Import-Csv -LiteralPath (Resolve-RepoPath "outputs\DGF_HIGHPROFIT_RISK_SHAPE_RUN.csv"))
}
...
$md.Add("# DGF High-Profit Risk-Shape Current-Source Status")
$md.Add("")
