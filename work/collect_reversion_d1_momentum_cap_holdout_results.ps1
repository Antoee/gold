param(
   ...
   [string]$ReportDir = "outputs\reversion_d1_momentum_cap_holdout_model1_package\reports_here",
   [string]$RawResultsPath = "outputs\REVERSION_D1_MOMENTUM_CAP_HOLDOUT_MODEL1_RAW_RESULTS.csv",
   [string]$ResultsPath = "outputs\REVERSION_D1_MOMENTUM_CAP_HOLDOUT_MODEL1_RESULTS.csv",
   [string]$RunPath = "outputs\REVERSION_D1_MOMENTUM_CAP_HOLDOUT_MODEL1_RUN.csv",
   [string]$SummaryPath = "outputs\REVERSION_D1_MOMENTUM_CAP_HOLDOUT_MODEL1_SUMMARY.csv",
   [string]$MarkdownPath = "outputs\REVERSION_D1_MOMENTUM_CAP_HOLDOUT_MODEL1_METRICS.md"
)
...
if($queue.Count -ne 3) { throw "Expected three frozen holdout queue rows." }
if(@($queue | Where-Object { $_.From -lt "2021.01.01" -or $_.To -gt "2026.07.16" }).Count -gt 0) { throw "Unexpected holdout date boundary." }
...
$initialRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath "outputs") -Filter "REVERSION_D1_MOMENTUM_CAP_HOLDOUT_PORTABLE_*.csv" |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
$retryRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath "outputs") -Filter "REVERSION_D1_MOMENTUM_CAP_HOLDOUT_RERUN_*.csv" |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
...
if($raw.Count -ne 3) { throw "Expected three parsed holdout results." }
$results = foreach($queued in $queue) {
   ...
   [pscustomobject]@{
      ...
      DiscoveryContractSha256=$queued.DiscoveryContractSha256
      HoldoutContractSha256=$queued.HoldoutContractSha256;InitialDeposit=$row.InitialDeposit
