...
input group "Weekend-Gap Signal"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_M15;
input ENUM_TIMEFRAMES InpATRTimeframe = PERIOD_D1;
input int    InpATRPeriod = 14;
input double InpMinimumGapATR = 0.08;
input double InpMaximumGapATR = 0.50;
input double InpMinimumGapPoints = 50.0;
input double InpMinimumConfirmationFraction = 0.15;
input double InpMaximumConfirmationFraction = 0.80;
input bool   InpRequireDirectionalFirstBar = true;
input bool   InpAllowBuy = true;
input bool   InpAllowSell = true;

input group "Stops and Position Management"
input double InpStopBufferATR = 0.03;
input double InpMaximumStopATR = 0.35;
input double InpMinimumRewardRisk = 1.15;
input int    InpMaximumHoldBars = 48;
input bool   InpUseBreakEven = true;
input double InpBreakEvenTriggerR = 0.75;
...
input double InpMaximumSpreadPoints = 100.0;
input double InpMaximumSpreadGapPercent = 15.0;
...
input string InpLogFileName = "Independent_XAUUSD_M15_Weekend_Gap_Fade_Trades.csv";
...
datetime g_lastEvaluatedWeekendOpen = 0;
double g_peakEquity = 0.0;
int g_weekendEvents = 0;
int g_gapCandidates = 0;
int g_confirmationRejects = 0;
...
{
   return "IM15WGF_RISK_" + IntegerToString((long)ticket);
}
...
{
   if(InpEnforceAccountCurrency && AccountInfoString(ACCOUNT_CURRENCY) != InpExpectedAccountCurrency)
   {
...
bool EntrySafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "M15WGF-LIVE-ACK"))
   {
...
{
   MqlTick tick;
   if(!SymbolInfoTick(_Symbol, tick) || atr <= 0.0 || rawStop <= 0.0 || rawTarget <= 0.0)
      return false;
   entryPrice = buy ? tick.ask : tick.bid;
   stopDistance = buy ? entryPrice - rawStop : rawStop - entryPrice;
   double targetDistance = buy ? rawTarget - entryPrice : entryPrice - rawTarget;
   if(entryPrice <= 0.0 || stopDistance <= 0.0 || targetDistance <= 0.0)
      return false;
   ...
   if(stopDistance / atr > InpMaximumStopATR || targetDistance < brokerMinimum ||
   ...
   targetPrice = NormalizeDouble(rawTarget, _Digits);
...
bool IsFirstBarAfterWeekend(MqlRates &firstBar, MqlRates &priorBar)
{
   ...
   if(CopyRates(_Symbol, InpSignalTimeframe, 1, 2, rates) != 2)
      return false;
   firstBar = rates[0];
   priorBar = rates[1];
   MqlDateTime firstTime;
   MqlDateTime priorTime;
   if(!TimeToStruct(firstBar.time, firstTime) || !TimeToStruct(priorBar.time, priorTime))
      return false;
   bool weekdayTransition = priorTime.day_of_week == 5 &&
                            (firstTime.day_of_week == 0 || firstTime.day_of_week == 1);
   return weekdayTransition && firstBar.time - priorBar.time >= 24 * 3600;
}

bool OpenGapFadePosition(const bool buy,
                         const double atr,
                         const double gapDistance,
                         const double rawStop,
                         const double priorClose)
{
   string safetyReason = "";
   ...
   if(SpreadPoints() * _Point > gapDistance * InpMaximumSpreadGapPercent / 100.0)
   {
      g_safetyRejects++;
      return false;
   }
   ...
   if(!FinalizeGeometry(buy, atr, rawStop, priorClose,
   ...
   string comment = buy ? "M15WGF_BUY" : "M15WGF_SELL";
   ...
   LogEvent("entry", trade.ResultOrder(), buy ? "buy" : "sell", lots,
            entryPrice, stopPrice, targetPrice, 0.0, "weekend gap fade");
   return true;
}

bool TryWeekendGapEntry(const double atr)
{
   MqlRates firstBar;
   MqlRates priorBar;
   if(!IsFirstBarAfterWeekend(firstBar, priorBar))
      return false;
   if(firstBar.time == g_lastEvaluatedWeekendOpen)
      return false;
   g_lastEvaluatedWeekendOpen = firstBar.time;
   g_weekendEvents++;
   if(atr <= 0.0 || firstBar.open <= 0.0 || firstBar.close <= 0.0 ||
      priorBar.close <= 0.0 || firstBar.high <= firstBar.low)
      return false;
   double signedGap = firstBar.open - priorBar.close;
   double gapDistance = MathAbs(signedGap);
   double minimumGap = MathMax(InpMinimumGapATR * atr, InpMinimumGapPoints * _Point);
   if(gapDistance < minimumGap || gapDistance > InpMaximumGapATR * atr)
      return false;
   g_gapCandidates++;
   bool buy = signedGap < 0.0;
   if((buy && !InpAllowBuy) || (!buy && !InpAllowSell))
      return false;
   double movementTowardClose = buy ? firstBar.close - firstBar.open
                                    : firstBar.open - firstBar.close;
   double confirmationFraction = movementTowardClose / gapDistance;
   bool directionAllows = !InpRequireDirectionalFirstBar || movementTowardClose > 0.0;
   bool targetNotFilled = buy ? firstBar.close < priorBar.close : firstBar.close > priorBar.close;
   if(!directionAllows || !targetNotFilled ||
      confirmationFraction < InpMinimumConfirmationFraction ||
      confirmationFraction > InpMaximumConfirmationFraction)
   {
      g_confirmationRejects++;
      return false;
   }
   double rawStop = buy ? firstBar.low - InpStopBufferATR * atr
                        : firstBar.high + InpStopBufferATR * atr;
   return OpenGapFadePosition(buy, atr, gapDistance, rawStop, priorBar.close);
}
...
int OnInit()
{
   if(InpATRPeriod < 2 || InpMinimumGapATR < 0.0 ||
      InpMaximumGapATR <= InpMinimumGapATR || InpMinimumGapPoints < 0.0 ||
      InpMinimumConfirmationFraction < 0.0 ||
      InpMaximumConfirmationFraction <= InpMinimumConfirmationFraction ||
      InpMaximumConfirmationFraction > 1.0 || InpStopBufferATR < 0.0 ||
      InpMaximumStopATR <= 0.0 || InpMinimumRewardRisk <= 0.0 ||
      ...
      InpMaximumSpreadPoints <= 0.0 || InpMaximumSpreadGapPercent <= 0.0 ||
      InpMaximumSpreadGapPercent > 100.0 || InpAccountWideMaxOpenRiskPercent < InpRiskPercent ||
   ...
   {
      Print("M15WGF account contract failure: ", accountReason);
      return INIT_FAILED;
   }
...
{
   PrintFormat("M15WGF_DIAGNOSTIC weekend_events=%d gap_candidates=%d confirmation_rejects=%d geometry_rejects=%d safety_rejects=%d minimum_lot_rejects=%d opened=%d order_failures=%d",
               g_weekendEvents, g_gapCandidates, g_confirmationRejects, g_geometryRejects,
