...
if(Test-Path -LiteralPath $tempRoot) {
   ...
   if(!$resolvedTemp.StartsWith($resolvedParent, [System.StringComparison]::OrdinalIgnoreCase)) { throw "Refusing to clean unexpected parser smoke path: $resolvedTemp" }
...
$manifestRows = @(
   [pscustomobject]@{ Priority = 1; Phase = "phase1_fast_triage"; Profile = "fixture_profile"; Set = "stress"; Window = "2024_Q1"; From = "2024.01.01"; To = "2024.03.31" },
   [pscustomobject]@{ Priority = 2; Phase = "phase2_real_tick_validation"; Profile = "balance_only"; Set = "split"; Window = "full"; From = "2024.01.01"; To = "2026.07.02" },
   [pscustomobject]@{ Priority = 3; Phase = "phase1_fast_triage"; Profile = "bad_report"; Set = "stress"; Window = "missing_profit"; From = "2025.01.01"; To = "2025.03.31" }
)
...
& powershell -NoProfile -ExecutionPolicy Bypass -File $collector -RepoRoot $tempRoot -ManifestPath "manifest.csv" -ReportDir "reports" -ReportNameTemplate "validation_{Profile}_{Set}_{Window}" -OutResults $outResults -OutSummary $outSummary -OutMarkdown $outMarkdown | Out-Null
...
if($null -eq $first -or $null -eq $second -or $null -eq $bad) { throw "Expected parser smoke rows were not produced." }
...
if($null -eq $badSummary) { throw "Expected bad-report summary row was not produced." }
...
if(Test-Path -LiteralPath $tempRoot) {
   $resolvedTemp = (Resolve-Path -LiteralPath $tempRoot).Path
   if(!$resolvedTemp.StartsWith($resolvedParent, [System.StringComparison]::OrdinalIgnoreCase)) { throw "Refusing to clean unexpected parser smoke path after test: $resolvedTemp" }
   Remove-Item -LiteralPath $tempRoot -Recurse -Force
}
