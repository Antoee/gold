...
#property description "Research fork with a restart-safe strong-signal reversion partial runner"
...
input bool   InpRVUseStrongSignalPartialRunner = false;
input double InpRVPartialRunnerClosePercent = 80.0;
input double InpRVPartialRunnerTargetMultiplier = 2.00;
input double InpRVPartialRunnerStopLockR = 0.50;
...
private:
   ...
   {
      return "TLP_RV_PR_" + IntegerToString((long)AccountInfoInteger(ACCOUNT_LOGIN)) +
             "_" + IntegerToString((long)InpRVMagicNumber) + "_";
   }

   string RunnerRiskKey(const ulong ticket)
   {
      return RunnerKeyPrefix() + "R_" + IntegerToString((long)ticket);
   }

   string RunnerTriggerKey(const ulong ticket)
   {
      return RunnerKeyPrefix() + "T_" + IntegerToString((long)ticket);
   }
   ...
   {
      VerifiedGlobalDelete(RunnerRiskKey(ticket));
      VerifiedGlobalDelete(RunnerTriggerKey(ticket));
                       ...
                       double &remainingVolume)
   {
      ...
      if(totalVolume <= 0.0 || step <= 0.0 || minimum <= 0.0)
         return false;
      closeVolume = MathFloor(totalVolume * InpRVPartialRunnerClosePercent /
   ...
   bool RegisterRunnerState(const ulong ticket,
                            ...
                            const double initialVolume)
   {
      ...
      if(riskDistance <= 0.0 || triggerPrice <= 0.0 || initialVolume <= 0.0 ||
         !VerifiedGlobalSet(RunnerRiskKey(ticket), riskDistance) ||
         !VerifiedGlobalSet(RunnerTriggerKey(ticket), triggerPrice) ||
   ...
   {
      string marker = RunnerKeyPrefix() + "T_";
      for(int i = GlobalVariablesTotal() - 1; i >= 0; --i)
      {
         ...
         if(ticket == 0 || !SelectOwnedPosition(ticket, InpRVMagicNumber))
            DeleteRunnerState(ticket);
      }
   }
   ...
   {
      if(!SelectOwnedPosition(ticket, InpRVMagicNumber) ||
         !GlobalVariableCheck(RunnerRiskKey(ticket)))
         return false;
      double riskDistance = GlobalVariableGet(RunnerRiskKey(ticket));
      double openPrice = PositionGetDouble(POSITION_PRICE_OPEN);
      double oldSl = PositionGetDouble(POSITION_SL);
      double takeProfit = PositionGetDouble(POSITION_TP);
      double protectedStop = NormalizeDouble(buy
                                             ? openPrice + InpRVPartialRunnerStopLockR * riskDistance
                                             : openPrice - InpRVPartialRunnerStopLockR * riskDistance,
                                             _Digits);
      ...
      if(ModifyOwnedPosition(m_trade, ticket, InpRVMagicNumber,
         ...
         return true;
      string closeReason = "";
      CloseOwnedPosition(m_trade, ticket, InpRVMagicNumber, closeReason);
      return false;
   }

   void ManagePartialRunner()
   {
      if(!InpRVUseStrongSignalPartialRunner)
         return;
      for(int i = PositionsTotal() - 1; i >= 0; --i)
      {
         ulong ticket = PositionGetTicket(i);
         if(ticket == 0 || !SelectOwnedPosition(ticket, InpRVMagicNumber) ||
            !GlobalVariableCheck(RunnerTriggerKey(ticket)) ||
            !GlobalVariableCheck(RunnerRiskKey(ticket)) ||
            !GlobalVariableCheck(RunnerVolumeKey(ticket)) ||
            !GlobalVariableCheck(RunnerDoneKey(ticket)))
            continue;
         bool buy = PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY;
         double triggerPrice = GlobalVariableGet(RunnerTriggerKey(ticket));
         ...
         bool partialConfirmed = CloseOwnedPositionPartial(m_trade, ticket, InpRVMagicNumber,
         ...
         WriteEvidenceEvent(m_logHandle, "tlp_rv_pr", "partial_runner", ticket,
                            buy ? "buy" : "sell", closeVolume, currentPrice,
                            PositionGetDouble(POSITION_SL), 0.0,
                            "original VWAP reached; protected remainder retained");
      }
   }
                     ...
                     const double signalBodyRatio)
   {
      ...
      double closeVolume = 0.0, remainingVolume = 0.0;
      bool runnerRequested = InpRVUseStrongSignalPartialRunner &&
                             signalBodyRatio >= InpRVStrongSignalMinimumBodyRatio &&
                             PartialVolumes(lots, closeVolume, remainingVolume);
      double orderTargetPrice = targetPrice;
      if(runnerRequested)
      {
         double targetDistance = MathAbs(targetPrice - entryPrice);
         orderTargetPrice = buy
                            ? entryPrice + targetDistance * InpRVPartialRunnerTargetMultiplier
                            : entryPrice - targetDistance * InpRVPartialRunnerTargetMultiplier;
      }
      ...
      string comment = buy ? "RRO;Band VWAP reversion;Lower" : "RRO;Band VWAP reversion;Upper";
       bool opened = buy
                    ? m_trade.Buy(lots, _Symbol, 0.0, NormalizeDouble(stopPrice, _Digits),
                                  NormalizeDouble(orderTargetPrice, _Digits), comment)
      ...
      if(runnerRequested)
      {
            ...
            !RegisterRunnerState(ticket, targetPrice, PositionGetDouble(POSITION_VOLUME)))
         {
            ...
            return RejectPostFill(m_trade, InpRVMagicNumber,
                                  "reversion partial-runner registration", rejectReason);
         }
      }
      ...
      eventReason += runnerRequested ? "; partial runner eligible" : "; standard VWAP exit";
...
public:
   ...
   void OnTradeTransaction(const MqlTradeTransaction &transaction)
   {
      ...
      WriteEvidenceEvent(m_logHandle, "tlp_rv_pr", "exit", transaction.deal, "close",
   ...
   if(InpRVATRPeriod < 2 || InpRVADXPeriod < 2 || InpRVRSIPeriod < 2 ||
      ...
      InpRVVWAPLookbackBars < 5 || InpRVStopLookbackBars < 2 ||
       ((InpRVUseStrongSignalRisk || InpRVUseStrongSignalLotCap ||
         InpRVUseStrongSignalPartialRunner) &&
      ...
      (InpRVUseStrongSignalRisk &&
       ...
       (InpRVUseStrongSignalPartialRunner &&
        (InpRVPartialRunnerClosePercent <= 0.0 ||
         InpRVPartialRunnerClosePercent >= 100.0 ||
         InpRVPartialRunnerTargetMultiplier <= 1.0 ||
         InpRVPartialRunnerTargetMultiplier > 5.0 ||
         InpRVPartialRunnerStopLockR < 0.0 ||
         InpRVPartialRunnerStopLockR >= InpRVMinimumRiskReward)) ||
