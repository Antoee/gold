...
param(
   ...
   [string]$PackageDir = 'outputs\three_lane_residual_risk_v2_model1_package',
   [string]$QueuePath = 'outputs\THREE_LANE_RESIDUAL_RISK_V2_MODEL1_QUEUE.csv',
   [string]$ManifestPath = 'outputs\THREE_LANE_RESIDUAL_RISK_V2_MODEL1_PACKAGE_MANIFEST.csv',
   [string]$PackageMarkdownPath = 'outputs\THREE_LANE_RESIDUAL_RISK_V2_MODEL1_PACKAGE.md',
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_residual_risk_v2_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='rr2_control';Enabled='false';Reserve='0.05';RVMax='0.45';MOMax='0.15';ATBMax='0.15'},
   [pscustomobject]@{Name='rr2_at250';Enabled='true';Reserve='0.05';RVMax='0.45';MOMax='0.15';ATBMax='0.25'},
   [pscustomobject]@{Name='rr2_at300';Enabled='true';Reserve='0.05';RVMax='0.45';MOMax='0.15';ATBMax='0.30'},
   [pscustomobject]@{Name='rr2_pair_low';Enabled='true';Reserve='0.05';RVMax='0.45';MOMax='0.175';ATBMax='0.25'},
   [pscustomobject]@{Name='rr2_pair_center';Enabled='true';Reserve='0.05';RVMax='0.45';MOMax='0.175';ATBMax='0.30'},
   [pscustomobject]@{Name='rr2_pair_high';Enabled='true';Reserve='0.05';RVMax='0.45';MOMax='0.20';ATBMax='0.30'}
)
$windows = @(
...
$stopRule = 'V2 Model 1 screen. Require all three disjoint eras positive, the exact control trade count, continuous CAGR at least 0.35 points above control, PF >= 1.55, DD <= 2.25%, and return/DD plus recovery no worse than control before Model 4 opens.'
...
foreach($variant in $variants) {
   ...
   foreach($window in $windows) {
      ...
      $common = [ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Phase='v2_model1'
...
@(
   '# Three-Lane Base-Eligible Residual-Risk V2 Model 1 Package','',
   '**Status: FAST SCREEN ONLY. THE ATB150 CHAMPION AND FORWARD CANDIDATE ARE UNCHANGED.**','',
   ...
   '- Candidate ceilings vary only momentum and adaptive-trend expansion; reversion remains at its base 0.45% ceiling.',
