...
$source = Join-Path $repo 'outputs\three_lane_residual_risk_v2_model4_characterization_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath = Join-Path $repo 'outputs\THREE_LANE_RESIDUAL_RISK_V2_MODEL4_CHARACTERIZATION_COMPILE_AUDIT.csv'
$roots = @(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Residual-risk V2 characterization source identity changed.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Residual-risk V2 characterization compile $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Residual-risk V2 characterization focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
