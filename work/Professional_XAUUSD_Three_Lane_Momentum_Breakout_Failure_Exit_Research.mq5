...
#property description "Research-only three-lane XAUUSD portfolio with an early momentum breakout-failure exit"
...
input bool   InpMOUseBreakoutFailureExit = false;
input int    InpMOBreakoutFailureMaximumBars = 3;
input double InpMOBreakoutFailureBufferATR = 0.05;
...
private:
   ...
   string BreakoutFailureLevelKey(const ulong ticket)
   {
      return "TLP_MO_FAIL_LEVEL_" + IntegerToString((long)ticket);
   }

   string BreakoutFailureATRKey(const ulong ticket)
   {
      return "TLP_MO_FAIL_ATR_" + IntegerToString((long)ticket);
   }

   void ClearBreakoutFailureState(const ulong ticket)
   {
      VerifiedGlobalDelete(BreakoutFailureLevelKey(ticket));
      VerifiedGlobalDelete(BreakoutFailureATRKey(ticket));
   }
   ...
   bool RegisterBreakoutFailureState(const ulong ticket,
                                     const double breakoutLevel,
                                     const double atr)
   {
      if(!InpMOUseBreakoutFailureExit)
         return true;
      if(ticket == 0 || breakoutLevel <= 0.0 || atr <= 0.0)
         return false;
      if(!VerifiedGlobalSet(BreakoutFailureLevelKey(ticket), breakoutLevel))
         return false;
      if(VerifiedGlobalSet(BreakoutFailureATRKey(ticket), atr))
         return true;
      VerifiedGlobalDelete(BreakoutFailureLevelKey(ticket));
      return false;
   }
                     ...
                     const double breakoutLevel)
   {
      ...
      if(!RegisterBreakoutFailureState(ticket, breakoutLevel, atr))
      {
         string rejectReason = "";
         return RejectPostFill(m_trade, InpMOMagicNumber,
                               "momentum breakout-failure state registration", rejectReason);
      }
   ...
   bool TryBreakoutFailureExit(const ulong ticket, const bool buy)
   {
      if(!InpMOUseBreakoutFailureExit ||
         !GlobalVariableCheck(BreakoutFailureLevelKey(ticket)) ||
         !GlobalVariableCheck(BreakoutFailureATRKey(ticket)))
         return false;
      datetime openTime = (datetime)PositionGetInteger(POSITION_TIME);
      int openShift = iBarShift(_Symbol, InpMOSignalTimeframe, openTime, false);
      if(openShift < 1 || openShift > InpMOBreakoutFailureMaximumBars)
         return false;
      double closePrice = iClose(_Symbol, InpMOSignalTimeframe, 1);
      double breakoutLevel = GlobalVariableGet(BreakoutFailureLevelKey(ticket));
      double entryATR = GlobalVariableGet(BreakoutFailureATRKey(ticket));
      if(closePrice <= 0.0 || breakoutLevel <= 0.0 || entryATR <= 0.0)
         return false;
      double buffer = InpMOBreakoutFailureBufferATR * entryATR;
      bool failed = buy ? closePrice < breakoutLevel - buffer
                        : closePrice > breakoutLevel + buffer;
      if(!failed)
         return false;
      double volume = PositionGetDouble(POSITION_VOLUME);
      string closeReason = "";
      if(!CloseOwnedPosition(m_trade, ticket, InpMOMagicNumber, closeReason))
         return false;
      WriteEvidenceEvent(m_logHandle, "tlp_mom_breakout_failure", "exit_signal", ticket,
                         buy ? "buy" : "sell", volume, closePrice, 0.0, 0.0,
                         "completed H1 close returned inside stored pre-break channel");
      return true;
   }

   bool TryChannelExit(const ulong ticket, const bool buy)
   {
      if(TryBreakoutFailureExit(ticket, buy))
         return true;
...
bool InputsValid()
{
   ...
   if(InpMOMomentumLookbackBars < 20 || InpMOEntryLookbackBars < 3 ||
      ...
      (InpMOUseBreakoutFailureExit &&
       (InpMOBreakoutFailureMaximumBars < 1 ||
        InpMOBreakoutFailureBufferATR < 0.0 ||
