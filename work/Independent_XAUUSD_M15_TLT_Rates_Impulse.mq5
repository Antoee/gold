...
input string InpTLTSymbol = "TLT";
input ulong  InpMagicNumber = 26072013;
...
input group "TLT Rates-Impulse Signal"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_M15;
input ENUM_TIMEFRAMES InpReferenceTimeframe = PERIOD_D1;
input int    InpReferenceLookbackBars = 1;
input int    InpReferenceATRPeriod = 14;
input int    InpReferenceTrendBars = 10;
input int    InpMaximumAlignmentSeconds = 259200;
input double InpMinimumReferenceMoveATR = 0.20;
input bool   InpRequireReferenceTrend = true;
...
input string InpLogFileName = "Independent_XAUUSD_M15_TLT_Rates_Impulse_Trades.csv";
...
int g_referenceContexts = 0;
int g_alignmentRejects = 0;
int g_referenceCandidates = 0;
...
{
   return "IM15TLTRI_RISK_" + IntegerToString((long)ticket);
}
...
bool EntrySafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
        InpRealAccountApprovalCode != "M15TLTRI-LIVE-ACK"))
   {
...
bool BuildReferenceDirection(const datetime decisionTime,
                             bool &buy,
                             double &normalizedMove)
{
   buy = false;
   normalizedMove = 0.0;
   int timeframeSeconds = PeriodSeconds(InpReferenceTimeframe);
   if(StringLen(InpTLTSymbol) <= 0 || decisionTime <= 0 || timeframeSeconds <= 0)
      return false;

   int shift = iBarShift(InpTLTSymbol, InpReferenceTimeframe, decisionTime, false);
   if(shift < 0)
   {
      g_alignmentRejects++;
      return false;
   }
   datetime referenceOpen = iTime(InpTLTSymbol, InpReferenceTimeframe, shift);
   if(referenceOpen <= 0)
   {
      g_alignmentRejects++;
      return false;
   }
   if(referenceOpen + timeframeSeconds > decisionTime)
   {
      shift++;
      referenceOpen = iTime(InpTLTSymbol, InpReferenceTimeframe, shift);
   }
   datetime referenceClose = referenceOpen + timeframeSeconds;
   long alignmentSeconds = (long)(decisionTime - referenceClose);
   if(referenceOpen <= 0 || referenceClose > decisionTime || alignmentSeconds < 0 ||
      alignmentSeconds > InpMaximumAlignmentSeconds)
   {
      g_alignmentRejects++;
      return false;
   }

   int required = (int)MathMax(InpReferenceLookbackBars + 1,
                               MathMax(InpReferenceATRPeriod + 1, InpReferenceTrendBars + 1)) + 1;
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   if(CopyRates(InpTLTSymbol, InpReferenceTimeframe, shift, required, rates) != required)
   {
      g_alignmentRejects++;
      return false;
   }
   double atrSum = 0.0;
   for(int i = 0; i < InpReferenceATRPeriod; ++i)
   {
   ...
   double referenceATR = atrSum / InpReferenceATRPeriod;
   if(referenceATR <= 0.0 || rates[0].close <= 0.0 ||
      rates[InpReferenceLookbackBars].close <= 0.0)
      return false;

   g_referenceContexts++;
   normalizedMove = (rates[0].close - rates[InpReferenceLookbackBars].close) / referenceATR;
   bool buyCandidate = normalizedMove >= InpMinimumReferenceMoveATR;
   bool sellCandidate = normalizedMove <= -InpMinimumReferenceMoveATR;
   if(!buyCandidate && !sellCandidate)
      return false;
   g_referenceCandidates++;

   if(InpRequireReferenceTrend)
   {
      double priorCloseSum = 0.0;
      for(int i = 1; i <= InpReferenceTrendBars; ++i)
         priorCloseSum += rates[i].close;
      double priorAverage = priorCloseSum / InpReferenceTrendBars;
      bool trendAligned = buyCandidate ? rates[0].close > priorAverage
                                       : rates[0].close < priorAverage;
      if(!trendAligned)
      {
         g_trendRejects++;
         return false;
...
bool OpenRatesImpulsePosition(const bool buy,
                              const double atr,
                              const double rawStop)
{
   string safetyReason = "";
   ...
   string comment = buy ? "M15TLTRI_BUY" : "M15TLTRI_SELL";
   ...
   LogEvent("entry", trade.ResultOrder(), buy ? "buy" : "sell", lots,
             entryPrice, stopPrice, targetPrice, 0.0, "completed TLT D1 impulse plus XAUUSD M15 breakout");
   return true;
}

bool TryRatesImpulseEntry(const double atr)
{
   ...
   if(!SessionAllows(currentBarTime))
      return false;
   bool buy = false;
   double referenceMove = 0.0;
   if(!BuildReferenceDirection(currentBarTime, buy, referenceMove))
      return false;
   ...
   return OpenRatesImpulsePosition(buy, atr, rawStop);
}
...
int OnInit()
{
   if(StringLen(InpTLTSymbol) <= 0 || InpTLTSymbol == _Symbol ||
      InpReferenceTimeframe != PERIOD_D1 || InpReferenceLookbackBars < 1 ||
      InpReferenceATRPeriod < 2 || InpReferenceTrendBars < 2 ||
      InpMaximumAlignmentSeconds < PeriodSeconds(InpReferenceTimeframe) ||
      InpMinimumReferenceMoveATR <= 0.0 ||
   ...
   {
      Print("M15TLTRI account contract failure: ", accountReason);
      return INIT_FAILED;
   }
   if(!SymbolSelect(InpTLTSymbol, true))
   {
      Print("M15TLTRI TLT symbol selection failed code=", GetLastError());
      return INIT_FAILED;
   }
   iClose(InpTLTSymbol, InpReferenceTimeframe, 1);
...
{
   PrintFormat("M15TLTRI_DIAGNOSTIC reference_contexts=%d alignment_rejects=%d reference_candidates=%d trend_rejects=%d breakout_rejects=%d geometry_rejects=%d safety_rejects=%d minimum_lot_rejects=%d opened=%d order_failures=%d",
                g_referenceContexts, g_alignmentRejects, g_referenceCandidates, g_trendRejects,
