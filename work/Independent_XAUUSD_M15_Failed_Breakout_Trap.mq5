...
input ulong  InpMagicNumber = 26071741;
...
input group "Failed-Breakout Trap Engine"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_M15;
input int    InpBoxLookbackBars = 12;
input int    InpMaximumBreakoutAgeBars = 4;
input double InpMinimumBoxRangeATR = 0.80;
input double InpMaximumBoxRangeATR = 2.50;
input double InpBreakBufferATR = 0.05;
input double InpMaximumBreakExtensionATR = 1.20;
input double InpMinimumBreakRangeATR = 0.45;
input double InpMinimumBreakBodyPercent = 35.0;
input double InpMinimumBreakCloseLocation = 0.60;
input double InpMinimumReclaimDepthRatio = 0.15;
input double InpMinimumReclaimRangeATR = 0.30;
input double InpMinimumReclaimBodyPercent = 20.0;
input double InpMinimumReclaimCloseLocation = 0.58;
input bool   InpRequireDirectionCandle = true;
input bool   InpAllowBuy = true;
input bool   InpAllowSell = true;
input bool   InpUseBreakoutTickVolumeFilter = false;
input bool   InpUseReclaimTickVolumeFilter = false;
input int    InpVolumeLookbackBars = 20;
input double InpMinimumVolumeRatio = 1.05;
...
input bool InpUseMaximumADXFilter = true;
input int  InpADXPeriod = 14;
input double InpMaximumADX = 26.0;
...
input bool   InpUseBoxOppositeTarget = true;
input double InpMinimumTargetR = 1.00;
input double InpMaximumTargetATR = 4.00;
input bool   InpUseFixedTakeProfit = false;
...
input string InpLogFileName = "Independent_XAUUSD_M15_Failed_Breakout_Trap_Trades.csv";
...
int g_breakCandidates = 0;
int g_reclaimCandidates = 0;
int g_trapSignals = 0;
int g_boxShapeRejects = 0;
int g_breakShapeRejects = 0;
int g_reclaimShapeRejects = 0;
...
{
   return "IM15FBT_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "M15FBT-LIVE-ACK"))
   {
...
bool RegimeAllows(const bool buy, const double closePrice, const double atr)
{
   ...
   if(InpUseMaximumADXFilter)
   {
      double adx = 0.0;
      if(!BufferValue(g_adxHandle, 0, 1, adx) || adx > InpMaximumADX)
         return false;
   }
   return true;
}
...
bool OpenTrapPosition(const bool buy,
                      const double atr,
                      const double rawStop,
                      ...
                      const double boxLow)
{
   ...
   if(InpUseBoxOppositeTarget)
   {
      double structuralTarget = buy ? boxHigh : boxLow;
      double targetDistance = buy ? structuralTarget - entryPrice : entryPrice - structuralTarget;
      if(structuralTarget <= 0.0 || targetDistance <= 0.0 ||
         targetDistance / stopDistance < InpMinimumTargetR ||
         (InpMaximumTargetATR > 0.0 && targetDistance > InpMaximumTargetATR * atr))
         return false;
      takeProfit = NormalizeDouble(structuralTarget, _Digits);
   }
   else if(InpUseFixedTakeProfit && InpTakeProfitR > 0.0)
   ...
   string comment = buy ? "M15FBT_BUY" : "M15FBT_SELL";
   ...
   LogEvent("entry", trade.ResultOrder(), buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
            "compression failed-breakout snapback");
   return true;
}
...
bool TickVolumeAllows(const int signalShift, const bool enabled)
{
   if(!enabled)
      return true;
...
bool IsFirstReclaimAfterBreak(const bool buy,
                              const int breakShift,
                              const double boxHigh,
                              const double boxLow)
{
   for(int shift = breakShift - 1; shift >= 2; --shift)
   {
      double closePrice = iClose(_Symbol, InpSignalTimeframe, shift);
      if(closePrice <= 0.0)
         return false;
      if((buy && closePrice >= boxLow) || (!buy && closePrice <= boxHigh))
         return false;
   }
   return true;
}
...
bool TryFailedBreakoutTrapEntry(const double atr)
{
   ...
   for(int breakShift = 2; breakShift <= 1 + InpMaximumBreakoutAgeBars; ++breakShift)
   {
      double boxHigh = 0.0;
      double boxLow = 0.0;
      if(!ChannelBounds(breakShift + 1, InpBoxLookbackBars, boxHigh, boxLow))
         continue;
      double boxRange = boxHigh - boxLow;
      if(boxRange < InpMinimumBoxRangeATR * atr || boxRange > InpMaximumBoxRangeATR * atr)
      {
         g_boxShapeRejects++;
         continue;
      }
      ...
      if(breakOpen <= 0.0 || breakClose <= 0.0 || breakRange <= 0.0)
         continue;

      double buffer = InpBreakBufferATR * atr;
      bool buy = InpAllowBuy && breakClose < boxLow - buffer && breakLow < boxLow - buffer &&
                 boxLow - breakLow <= InpMaximumBreakExtensionATR * atr;
      bool sell = InpAllowSell && breakClose > boxHigh + buffer && breakHigh > boxHigh + buffer &&
                  breakHigh - boxHigh <= InpMaximumBreakExtensionATR * atr;
      if(!buy && !sell)
         continue;
      g_breakCandidates++;
      if(buy == sell)
      ...
      bool breakDirection = buy ? breakClose < breakOpen : breakClose > breakOpen;
      bool breakShapeAllows = breakRange >= InpMinimumBreakRangeATR * atr &&
                              CandleBodyPercent(breakShift) >= InpMinimumBreakBodyPercent &&
                              CandleCloseLocation(!buy, breakShift) >= InpMinimumBreakCloseLocation &&
                              breakDirection &&
                              TickVolumeAllows(breakShift, InpUseBreakoutTickVolumeFilter);
      if(!breakShapeAllows || !IsFirstReclaimAfterBreak(buy, breakShift, boxHigh, boxLow))
      {
         g_breakShapeRejects++;
         continue;
      }
      ...
      g_reclaimCandidates++;
      double requiredReclaim = InpMinimumReclaimDepthRatio * boxRange;
      bool reclaimed = buy ? close1 >= boxLow + requiredReclaim && close1 < boxHigh
                           : close1 <= boxHigh - requiredReclaim && close1 > boxLow;
      bool reclaimDirection = !InpRequireDirectionCandle || (buy ? close1 > open1 : close1 < open1);
      bool reclaimShapeAllows = reclaimed &&
                                range1 >= InpMinimumReclaimRangeATR * atr &&
                                CandleBodyPercent(1) >= InpMinimumReclaimBodyPercent &&
                                CandleCloseLocation(buy, 1) >= InpMinimumReclaimCloseLocation &&
                                reclaimDirection &&
                                TickVolumeAllows(1, InpUseReclaimTickVolumeFilter);
      if(!reclaimShapeAllows)
      {
         g_reclaimShapeRejects++;
         continue;
      }
      ...
      double pathHigh = 0.0;
      double pathLow = 0.0;
      if(!ChannelBounds(1, breakShift, pathHigh, pathLow))
         return false;
      g_trapSignals++;
      double rawStop = buy ? pathLow - InpStopBufferATR * atr
                           : pathHigh + InpStopBufferATR * atr;
      return OpenTrapPosition(buy, atr, rawStop, boxHigh, boxLow);
   }
   return false;
}
...
int OnInit()
{
   if(InpBoxLookbackBars < 4 || InpMaximumBreakoutAgeBars < 1 || InpMaximumBreakoutAgeBars > 24 ||
      InpMinimumBoxRangeATR <= 0.0 || InpMaximumBoxRangeATR <= InpMinimumBoxRangeATR ||
      InpBreakBufferATR < 0.0 || InpMaximumBreakExtensionATR <= InpBreakBufferATR ||
      InpMinimumBreakRangeATR <= 0.0 ||
      InpMinimumBreakBodyPercent < 0.0 || InpMinimumBreakBodyPercent > 100.0 ||
      InpMinimumBreakCloseLocation < 0.50 || InpMinimumBreakCloseLocation > 1.0 ||
      InpMinimumReclaimDepthRatio < 0.0 || InpMinimumReclaimDepthRatio >= 1.0 ||
      InpMinimumReclaimRangeATR <= 0.0 ||
      InpMinimumReclaimBodyPercent < 0.0 || InpMinimumReclaimBodyPercent > 100.0 ||
      InpMinimumReclaimCloseLocation < 0.50 || InpMinimumReclaimCloseLocation > 1.0 ||
      InpVolumeLookbackBars < 2 || InpMinimumVolumeRatio <= 0.0 ||
      InpTrendEMAPeriod < 2 || InpTrendEMASlopeBars < 1 ||
      InpADXPeriod < 2 || InpMaximumADX <= 0.0 ||
      InpATRPeriod < 2 || InpStopBufferATR < 0.0 ||
      InpMinimumStopATR <= 0.0 || InpMaximumStopATR < InpMinimumStopATR ||
      (!InpUseBoxOppositeTarget && !InpUseFixedTakeProfit) ||
      (InpUseBoxOppositeTarget && (InpMinimumTargetR <= 0.0 || InpMaximumTargetATR <= 0.0)) ||
...
{
   PrintFormat("M15FBT_DIAGNOSTIC breaks=%d reclaims=%d signals=%d box_rejects=%d break_rejects=%d reclaim_rejects=%d regime_rejects=%d safety_rejects=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
               g_breakCandidates, g_reclaimCandidates, g_trapSignals, g_boxShapeRejects,
               g_breakShapeRejects, g_reclaimShapeRejects, g_regimeRejects,
...
void OnTick()
{
   ...
   if(ManagedPositionCount() > 0)
      return;
   TryFailedBreakoutTrapEntry(atr);
}
