param(
   ...
   [string]$PackageDir = "outputs\independent_m15_overnight_drift_continuation_holdout_model1_package",
   [string]$QueueManifestPath = "outputs\INDEPENDENT_M15_OVERNIGHT_DRIFT_CONTINUATION_HOLDOUT_MODEL1_QUEUE.csv",
   [string]$PackageManifestPath = "outputs\INDEPENDENT_M15_OVERNIGHT_DRIFT_CONTINUATION_HOLDOUT_MODEL1_PACKAGE_MANIFEST.csv",
   [string]$MarkdownPath = "outputs\INDEPENDENT_M15_OVERNIGHT_DRIFT_CONTINUATION_HOLDOUT_MODEL1_PACKAGE.md"
)
...
$windows = @(
   [pscustomobject]@{ Name = "early_2021_2022"; From = "2021.01.01"; To = "2022.12.31" },
   [pscustomobject]@{ Name = "middle_2023_2024"; From = "2023.01.01"; To = "2024.12.31" },
   [pscustomobject]@{ Name = "recent_2025_2026"; From = "2025.01.01"; To = "2026.07.12" },
...
$ordinal = 0
$candidateRank = 0
$stopRule = "Frozen holdout: require all three disjoint post-2020 windows positive, continuous PF at least 1.15, at least 90 continuous trades, DD at or below 3%, positive payoff and return/DD at least 1.0 before Model 4."
foreach($variant in $variants) {
   ...
   Set-InputLine -Inputs $inputs -Name "InpEvidenceRunLabel" -Value "independent_m15_overnight_drift_continuation_holdout_model1"
   ...
   foreach($window in $windows) {
      ...
      $queueRows.Add([pscustomobject]@{
         ...
         SourceType = "independent_m15_overnight_drift_continuation"; SourceRank = 1; Phase = "holdout_model1"
      ...
      $runRows.Add([pscustomobject]@{
         QueueRank = $queueRank; Candidate = $variant.Name; Phase = "holdout_model1"
         PhaseLabel = "Independent M15 overnight-drift continuation holdout Model1"; Window = $window.Name; Model = 1; Deposit = 10000
...
$md.Add("# Independent M15 Overnight-Drift Continuation Holdout Package")
$md.Add("")
$md.Add("Frozen post-2020 holdout for the discovery center and two orthogonal one-factor survivors. Profiles were selected and hashed before these windows were opened.")
$md.Add("")
$md.Add("- Source SHA-256: ``$sourceHash``")
$md.Add("- Variants: ``$($variants.Count)``")
$md.Add("- Holdout windows: ``$($windows.Name -join ', ')``")
