param(
   ...
   [string]$OutDir = "outputs\peak_r20_dfg_spread_risk_yearly_package",
   [string]$OutQueueManifest = "outputs\PEAK_R20_DFG_SPREAD_RISK_YEARLY_QUEUE.csv",
   [string]$OutPackageManifest = "outputs\PEAK_R20_DFG_SPREAD_RISK_YEARLY_PACKAGE_MANIFEST.csv",
   [string]$OutMarkdown = "outputs\PEAK_R20_DFG_SPREAD_RISK_YEARLY_PACKAGE.md",
   [int]$Model = 1
)
...
$candidates = @(
   [pscustomobject]@{
      Name = "r10_a7_dfg_risk_base"
      Rationale = "Control: no diagnostic fallback spread risk scaling."
      Overrides = @{}
   },
   [pscustomobject]@{
      Name = "r10_a7_dfg_risk_25_45_50"
      Rationale = "Scale diagnostic fallback risk from 25 to 45 spread points, floor 0.50."
      Overrides = @{
   ...
   [pscustomobject]@{
      Name = "r10_a7_dfg_risk_25_35_35"
      Rationale = "Scale diagnostic fallback risk from 25 to 35 spread points, floor 0.35."
      Overrides = @{
         InpUseDiagnosticFallbackSpreadRiskScaling = "true"
         InpDiagnosticFallbackSpreadRiskStartPoints = "25.0"
         InpDiagnosticFallbackSpreadRiskFullPoints = "35.0"
         InpDiagnosticFallbackMinSpreadRiskMultiplier = "0.35"
      }
   },
   [pscustomobject]@{
      Name = "r10_a7_dfg_risk_25_31_25"
      Rationale = "Aggressively scale diagnostic fallback risk from 25 to 31 spread points, floor 0.25."
      Overrides = @{
         InpUseDiagnosticFallbackSpreadRiskScaling = "true"
         InpDiagnosticFallbackSpreadRiskStartPoints = "25.0"
         InpDiagnosticFallbackSpreadRiskFullPoints = "31.0"
         InpDiagnosticFallbackMinSpreadRiskMultiplier = "0.25"
      }
   },
   [pscustomobject]@{
      Name = "r10_a7_dfg_risk_20_35_35"
      Rationale = "Scale diagnostic fallback risk from 20 to 35 spread points, floor 0.35."
      Overrides = @{
         InpUseDiagnosticFallbackSpreadRiskScaling = "true"
         InpDiagnosticFallbackSpreadRiskStartPoints = "20.0"
         InpDiagnosticFallbackSpreadRiskFullPoints = "35.0"
         InpDiagnosticFallbackMinSpreadRiskMultiplier = "0.35"
      }
   },
   [pscustomobject]@{
      Name = "r10_a7_dfg_risk_20_31_25"
      Rationale = "Aggressively scale diagnostic fallback risk from 20 to 31 spread points, floor 0.25."
      Overrides = @{
         InpUseDiagnosticFallbackSpreadRiskScaling = "true"
         InpDiagnosticFallbackSpreadRiskStartPoints = "20.0"
         InpDiagnosticFallbackSpreadRiskFullPoints = "31.0"
...
if(!(Test-Path -LiteralPath $baseSet)) {
   throw "Base DFG spread risk profile missing: $baseSet"
}
...
foreach($candidate in $candidates) {
   foreach($window in $windows) {
      ...
      $runLabel = "peak_r20_dfg_spread_risk_{0}_{1}_m{2}" -f $candidate.Name, $window.Window, $Model
      ...
      $reportName = "peak_r20_dfg_spread_risk_{0}_{1}_m{2}" -f $candidate.Name, $window.Window, $Model
      ...
      $stopRule = "Reject unless diagnostic fallback spread risk scaling improves stability without creating red yearly windows."
      $queue.Add([pscustomobject]@{
         QueueRank = $rank
         Candidate = $candidate.Name
         CandidateRank = $rank
         SourceType = "peak_r20_dfg_spread_risk_yearly"
         SourceRank = $rank
         Phase = "phase6_dfg_spread_risk_yearly_model$Model"
         Set = $setName
      ...
      $runRows.Add([pscustomobject]@{
         ...
         Candidate = $candidate.Name
         Phase = "phase6_dfg_spread_risk_yearly_model$Model"
         PhaseLabel = "Peak R20 diagnostic fallback spread risk yearly Model$Model"
         Window = $window.Window
         Model = $Model
...
$md.Add("# Peak R20 Diagnostic Fallback Spread Risk Yearly Package") | Out-Null
$md.Add("") | Out-Null
