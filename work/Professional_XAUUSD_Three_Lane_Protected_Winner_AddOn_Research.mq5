...
#property description "Research-only three-lane XAUUSD portfolio with a protected momentum winner add-on"
...
input group "Protected Momentum Winner Add-On Research"
input bool   InpMOUseProtectedWinnerAddOn = false;
input ulong  InpMOAddOnMagicNumber = 26071762;
input double InpMOAddOnMinimumProfitR = 1.25;
input int    InpMOAddOnMinimumHoldBars = 4;
input int    InpMOAddOnBreakoutLookbackBars = 6;
input double InpMOAddOnBreakoutBufferATR = 0.00;
input double InpMOAddOnRiskMultiplier = 0.50;
input double InpMOAddOnPrimaryLockR = 0.75;
input double InpMOAddOnLockedProfitCoverage = 1.25;
...
bool IsPortfolioMagic(const ulong magic)
{
   return magic == InpPortfolioMagic || magic == InpRVMagicNumber ||
          magic == InpMOMagicNumber || magic == InpMOAddOnMagicNumber ||
...
private:
                     ...
                     const double diEdge)
   {
      ...
      if(!PostFillReconcile(m_trade, InpRVMagicNumber, buy, InpRVRiskPercent,
...
private:
   ...
   bool IsMomentumMagic(const ulong magic)
   {
      return magic == InpMOMagicNumber || magic == InpMOAddOnMagicNumber;
   }
   ...
   int ManagedPositionCount()
   {
      ...
      for(int i = PositionsTotal() - 1; i >= 0; --i)
      {
         ...
         if(ticket == 0 || !PositionSelectByTicket(ticket))
            continue;
         if(PositionGetString(POSITION_SYMBOL) == _Symbol &&
            IsMomentumMagic((ulong)PositionGetInteger(POSITION_MAGIC)))
            count++;
      }
      return count;
   }
   ...
   double ClosedProfitSince(const datetime fromTime)
   {
      ...
      for(int i = 0; i < HistoryDealsTotal(); ++i)
      {
         ulong ticket = HistoryDealGetTicket(i);
         if(ticket == 0 || HistoryDealGetString(ticket, DEAL_SYMBOL) != _Symbol ||
            !IsMomentumMagic((ulong)HistoryDealGetInteger(ticket, DEAL_MAGIC)))
            continue;
         long entryType = HistoryDealGetInteger(ticket, DEAL_ENTRY);
         if(entryType != DEAL_ENTRY_OUT && entryType != DEAL_ENTRY_OUT_BY && entryType != DEAL_ENTRY_INOUT)
            continue;
   ...
   bool SafetyAllows(const int maximumLanePositions, string &reason)
   {
      if(!SharedSafetyAllows(reason))
         return false;
      double dailyProfit = ClosedProfitSince(g_dayStart);
      ...
      if(maximumLanePositions < 1 || ManagedPositionCount() >= maximumLanePositions)
      {
   ...
   bool RegisterRiskForPosition(const ulong ticket, const ulong magic)
   {
      if(!SelectOwnedPosition(ticket, magic) || !IsMomentumMagic(magic))
         return false;
   ...
   bool WinnerAddOnAllows(const bool buy, const double addedRiskMoney)
   {
      if(!InpMOUseProtectedWinnerAddOn || addedRiskMoney <= 0.0 ||
         CountOwnedMagicPositions(InpMOAddOnMagicNumber) > 0)
         return false;
      ulong primaryTicket = 0;
      string findReason = "";
      if(!FindOnlyOwnedPosition(InpMOMagicNumber, primaryTicket, findReason) ||
         !SelectOwnedPosition(primaryTicket, InpMOMagicNumber))
         return false;
      bool primaryBuy = PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY;
      if(primaryBuy != buy)
         return false;
      datetime openTime = (datetime)PositionGetInteger(POSITION_TIME);
      int heldBars = iBarShift(_Symbol, InpMOSignalTimeframe, openTime, false);
      if(heldBars < InpMOAddOnMinimumHoldBars)
         return false;
      double openPrice = PositionGetDouble(POSITION_PRICE_OPEN);
      double oldSl = PositionGetDouble(POSITION_SL);
      double takeProfit = PositionGetDouble(POSITION_TP);
      double volume = PositionGetDouble(POSITION_VOLUME);
      double initialRisk = GlobalVariableCheck(RiskKey(primaryTicket))
                           ? GlobalVariableGet(RiskKey(primaryTicket))
                           : MathAbs(openPrice - oldSl);
      double current = buy ? SymbolInfoDouble(_Symbol, SYMBOL_BID)
                           : SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      if(openPrice <= 0.0 || initialRisk <= 0.0 || current <= 0.0 || volume <= 0.0)
         return false;
      double favorable = buy ? current - openPrice : openPrice - current;
      if(favorable / initialRisk < InpMOAddOnMinimumProfitR)
         return false;

      double desiredSl = NormalizeDouble(buy ? openPrice + InpMOAddOnPrimaryLockR * initialRisk
                                             : openPrice - InpMOAddOnPrimaryLockR * initialRisk, _Digits);
      double minimumDistance = (double)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL) * _Point;
      bool validLock = buy ? desiredSl > openPrice && desiredSl < current - minimumDistance
                           : desiredSl < openPrice && desiredSl > current + minimumDistance;
      if(!validLock)
         return false;
      bool needsImprovement = buy ? desiredSl > oldSl + _Point : desiredSl < oldSl - _Point;
      if(needsImprovement)
      {
         string modifyReason = "";
         if(!ModifyOwnedPosition(m_trade, primaryTicket, InpMOMagicNumber,
                                 desiredSl, takeProfit, modifyReason) ||
            !SelectOwnedPosition(primaryTicket, InpMOMagicNumber))
            return false;
      }
      double lockedSl = PositionGetDouble(POSITION_SL);
      bool protectedWinner = buy ? lockedSl > openPrice : lockedSl < openPrice;
      if(!protectedWinner)
         return false;
      double lockedProfitMoney = 0.0;
      ENUM_ORDER_TYPE orderType = buy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
      if(!OrderCalcProfit(orderType, _Symbol, volume, openPrice, lockedSl, lockedProfitMoney) ||
         lockedProfitMoney <= 0.0)
         return false;
      return lockedProfitMoney + 1e-8 >=
             addedRiskMoney * InpMOAddOnLockedProfitCoverage;
   }

   bool OpenPosition(const bool buy, const double atr, const bool addOn)
   {
      MqlTick tick;
      ...
      double riskPercent = addOn ? InpMORiskPercent * InpMOAddOnRiskMultiplier
                                 : InpMORiskPercent;
      double lots = LotsForRisk(buy, entryPrice, stopPrice,
                                 riskPercent, InpMOMaximumPositionLots);
      double addedRiskMoney = 0.0;
      ENUM_ORDER_TYPE orderType = buy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
      if(lots <= 0.0 ||
         !RiskMoneyForOrder(_Symbol, orderType, entryPrice, stopPrice, lots, addedRiskMoney))
         return false;
      string exposureReason = "";
      if(!AccountWideExposureAllows(buy, entryPrice, stopPrice, lots, exposureReason))
         return false;
      if(addOn && !WinnerAddOnAllows(buy, addedRiskMoney))
         return false;
      ulong magic = addOn ? InpMOAddOnMagicNumber : InpMOMagicNumber;
      m_trade.SetExpertMagicNumber(magic);
      m_trade.SetDeviationInPoints(InpMODeviationPoints);
      string comment = addOn ? (buy ? "MTSM_ADD_BUY" : "MTSM_ADD_SELL")
                             : (buy ? "MTSM_BUY" : "MTSM_SELL");
      ...
      {
         string rejectReason = "";
         return RejectPostFill(m_trade, magic, "momentum entry result", rejectReason);
      }
      ulong ticket = 0;
      string postFillReason = "";
      if(!PostFillReconcile(m_trade, magic, buy, riskPercent,
                            InpMOMaximumPositionLots, ticket, postFillReason))
         return false;
      if(!RegisterRiskForPosition(ticket, magic))
      {
         string rejectReason = "";
         return RejectPostFill(m_trade, magic, "momentum risk registration", rejectReason);
      }
      WriteEvidenceEvent(m_logHandle, addOn ? "tlp_mom_protected_addon" : "tlp_mom_e20", "entry", ticket,
                         buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
                         addOn ? "protected winner plus fresh H1 continuation breakout"
                               : "daily momentum plus fresh H1 breakout");
      return true;
   }

   bool TryChannelExit(const ulong ticket, const bool buy, const ulong magic)
   {
      if(InpMOUseChannelExit)
      {
         ...
         if(ChannelBounds(2, InpMOExitLookbackBars, exitHigh, exitLow))
         {
            ...
            {
               string closeReason = "";
               return CloseOwnedPosition(m_trade, ticket, magic, closeReason);
            }
         }
      }
   ...
   void ImproveProtectiveStop(const ulong ticket, const bool buy, const ulong magic)
   {
      double openPrice = PositionGetDouble(POSITION_PRICE_OPEN);
      double oldSl = PositionGetDouble(POSITION_SL);
      double takeProfit = PositionGetDouble(POSITION_TP);
      ...
      {
         string modifyReason = "";
         ModifyOwnedPosition(m_trade, ticket, magic,
   ...
   bool ManagePositionOnBar()
   {
      ...
      for(int i = PositionsTotal() - 1; i >= 0; --i)
      {
         ...
         if(ticket == 0 || !PositionSelectByTicket(ticket))
            continue;
         ulong magic = (ulong)PositionGetInteger(POSITION_MAGIC);
         if(PositionGetString(POSITION_SYMBOL) != _Symbol || !IsMomentumMagic(magic))
            continue;
         bool buy = PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY;
         if(TryChannelExit(ticket, buy, magic))
         {
            closed = true;
            continue;
         }
         if(PositionSelectByTicket(ticket))
            ImproveProtectiveStop(ticket, buy, magic);
      }
      return closed;
   }

   void TryEntry(const double atr, const bool addOn)
   {
      string safetyReason = "";
      if(!SafetyAllows(addOn ? 2 : 1, safetyReason))
         return;
      int lookback = addOn ? InpMOAddOnBreakoutLookbackBars : InpMOEntryLookbackBars;
      double channelHigh = 0.0, channelLow = 0.0;
      if(!ChannelBounds(2, lookback, channelHigh, channelLow))
         return;
      double close1 = iClose(_Symbol, InpMOSignalTimeframe, 1);
      double close2 = iClose(_Symbol, InpMOSignalTimeframe, 2);
      if(close1 <= 0.0 || close2 <= 0.0)
         return;
      double buffer = (addOn ? InpMOAddOnBreakoutBufferATR : InpMOBreakoutBufferATR) * atr;
      ...
      if(InpMORequireFreshBreakout)
      {
         ...
         if(!ChannelBounds(3, lookback, priorHigh, priorLow))
            return;
      ...
      if(buyBreak && RegimeAllows(true, close1, atr))
         OpenPosition(true, atr, addOn);
      else if(sellBreak && RegimeAllows(false, close1, atr))
         OpenPosition(false, atr, addOn);
   }
...
public:
   ...
   void OnTick()
   {
      ...
      if(closed)
         return;
      int managed = ManagedPositionCount();
      if(managed == 0)
         TryEntry(atr, false);
      else if(InpMOUseProtectedWinnerAddOn && managed == 1 &&
   ...
   void OnTradeTransaction(const MqlTradeTransaction &transaction)
   {
      ...
      WriteEvidenceEvent(m_logHandle,
                         magic == InpMOAddOnMagicNumber ? "tlp_mom_protected_addon" : "tlp_mom_e20",
...
bool InputsValid()
{
   if(InpRVMagicNumber == InpMOMagicNumber ||
      InpRVMagicNumber == InpMOAddOnMagicNumber ||
      InpRVMagicNumber == InpATBMagicNumber ||
      InpMOMagicNumber == InpMOAddOnMagicNumber ||
      InpMOMagicNumber == InpATBMagicNumber ||
      InpMOAddOnMagicNumber == InpATBMagicNumber ||
      InpPortfolioMagic == InpRVMagicNumber ||
      InpPortfolioMagic == InpMOMagicNumber ||
      InpPortfolioMagic == InpMOAddOnMagicNumber ||
      InpPortfolioMagic == InpATBMagicNumber ||
      InpMaximumPortfolioOpenRiskPercent <= 0.0 ||
      InpMaximumAccountPositions < (InpMOUseProtectedWinnerAddOn ? 2 : 1) ||
      ...
      (InpMOUseProtectedWinnerAddOn &&
       InpRVRiskPercent + InpATBRiskPercent + InpMORiskPercent * InpMOAddOnRiskMultiplier >
   ...
   if(InpMOMomentumLookbackBars < 20 || InpMOEntryLookbackBars < 3 ||
      ...
      InpMOAddOnMinimumProfitR <= 0.0 || InpMOAddOnMinimumHoldBars < 1 ||
      InpMOAddOnBreakoutLookbackBars < 2 || InpMOAddOnBreakoutBufferATR < 0.0 ||
      InpMOAddOnRiskMultiplier <= 0.0 || InpMOAddOnRiskMultiplier > 1.0 ||
      InpMOAddOnPrimaryLockR <= 0.0 || InpMOAddOnPrimaryLockR >= InpMOAddOnMinimumProfitR ||
      InpMOAddOnLockedProfitCoverage < 1.0 ||
      InpMOAddOnPrimaryLockR + 1e-9 <
      InpMOAddOnRiskMultiplier * InpMOAddOnLockedProfitCoverage)
      return false;
