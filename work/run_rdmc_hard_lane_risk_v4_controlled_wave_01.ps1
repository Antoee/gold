...
$reportDir = Join-Path $repo 'outputs\rdmc_hard_lane_risk_v4_package\reports_here'
$decision = Join-Path $repo 'outputs\RDMC_HARD_LANE_RISK_V4_DECISION_FIXTURE.csv'
$runner = Join-Path $PSScriptRoot 'run_mt5_portable_parallel_manifest.ps1'
$collector = Join-Path $PSScriptRoot 'collect_rdmc_hard_lane_risk_v4_results.ps1'
...
$expectedBinaryHash = '7E354482E8D69F1E0F878DB4AF34943C04EF3D911FAC48CCC6104BD8B17E5D7C'
...
$allowedResumeNames = @(
   'rdmc_hlr4_w01_m1_critical_2019.htm',
   'rdmc_hlr4_w01_m1_critical_2019.identity.json'
)
...
try {
   Get-ChildItem -Path (Join-Path $repo 'outputs\RDMC_HARD_LANE_RISK_V4_WAVE_01_WORKER_*.csv') -File -ErrorAction SilentlyContinue |
      Remove-Item -Force
   ...
   & $runner -ManifestPath $manifest -PortableRoots $roots -UserAuthorizedFocusRisk `
      -OutputPrefix 'RDMC_HARD_LANE_RISK_V4_WAVE_01_WORKER' -MaxCpuPercent $MaxCpuPercent `
   ...
   & $collector -Wave 1 -DecisionCsvPath $decision -ManifestPath $manifest -ReportDir $reportDir `
      -RunnerLedgerGlob 'outputs\RDMC_HARD_LANE_RISK_V4_WAVE_01_WORKER_*.csv' `
      -ResultsPath 'outputs\RDMC_HARD_LANE_RISK_V4_WAVE_01_RESULTS.csv' -SkipAdmissionRefresh
   & python $evaluator
