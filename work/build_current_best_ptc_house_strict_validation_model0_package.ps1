param(
   [string]$PackageDir = "work\local_mt5_current_best_ptc_house_strict_validation_model0_package",
...
$ptcHouseStrict = @{
   ...
   InpBreakoutContinuationMinScore = "10"
   InpBreakoutContinuationRequireExecution = "true"
   InpBreakoutContinuationRequireOrderFlow = "true"
   InpBreakoutContinuationRequireRegime = "true"
   InpUsePowerTrendContinuation = "true"
   InpPowerTrendContinuationMinScore = "14"
   InpPowerTrendContinuationMinADX = "32.0"
   ...
   InpPowerTrendContinuationStandaloneMinScore = "15"
   InpPowerTrendContinuationRiskMultiplier = "0.35"
   InpPowerTrendContinuationTPMultiplier = "1.45"
   InpAllowFlatMonthMomentumOutsideMonthFilter = "true"
}

$profiles = @(
   [pscustomobject]@{ Name = "base_micro_r035"; Overrides = @{} },
   [pscustomobject]@{ Name = "ptc_house_strict"; Overrides = $ptcHouseStrict }
)
...
foreach($profile in $profiles) {
   foreach($window in $windows) {
      ...
      $reportName = "current_best_ptc_house_strict_validation_model0_{0}_{1}" -f $profile.Name, $window.Window
...
$expected | Export-Csv -LiteralPath "outputs\CURRENT_BEST_PTC_HOUSE_STRICT_VALIDATION_MODEL0_MANIFEST.csv" -NoTypeInformation
"Built $rank PTC house strict validation Model 0 configs in $PackageDir"
