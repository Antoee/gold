...
- Wired the research retest package, report collector, decision gate, smoke coverage, report-import preflight, and local pipeline manifest into the offline refresh.
- The main 20-run external micro package remains the current promotion-path source of truth; this 18-run package is research-only and cannot promote a profile by itself.
...
- Local pipeline manifest: 32 tracked artifacts, 0 missing.
...
- `work/refresh_offline_validation_state.ps1`: PASS, 24 steps, 0 failed.
- `outputs/REPORT_IMPORT_PREFLIGHT.csv`: research retest decision smoke PASS; fully pinned research retest `READY_RESEARCH`; research retest metrics `WAITING_FOR_REPORTS`; research retest decision `WAITING_FOR_REPORTS`; legacy evidence input pins REVIEW; local pipeline manifest PASS; local safety PASS; external report return PASS; external micro decision `COMPILE_REQUIRED`.
- `outputs/LOCAL_PIPELINE_MANIFEST.csv`: PASS, 32 artifacts, 0 missing.
...
No profit is proven by this update. The research retest reports are still missing, so the current promoted profile remains unchanged until fresh external MT5 reports are returned and pass the gates.
