...
$package = Join-Path $repo "outputs\rdmc_second_broker_validation_package"
$source = Join-Path $repo "outputs\rdmc_diversified_repair_executable_gate_package\source\Professional_XAUUSD_EA.mq5"
$profile = Join-Path $repo "outputs\rdmc_diversified_repair_executable_gate_package\profiles\rdmc_diversified_repair_restart_safe_v2.set"
$reports = Join-Path $package "reports_here"
$packageReadme = Join-Path $package "README_RDMC_SECOND_BROKER_VALIDATION.md"
$manifestPath = Join-Path $repo "outputs\RDMC_SECOND_BROKER_VALIDATION_MANIFEST.csv"
$templatePath = Join-Path $repo "outputs\RDMC_SECOND_BROKER_SPECIFICATION_TEMPLATE.csv"
$contractPath = Join-Path $repo "outputs\RDMC_SECOND_BROKER_VALIDATION_CONTRACT.md"
$decisionPath = Join-Path $repo "outputs\RDMC_SECOND_BROKER_VALIDATION_DECISION.csv"
$decisionMarkdownPath = Join-Path $repo "outputs\RDMC_SECOND_BROKER_VALIDATION_DECISION.md"
$builderPath = Join-Path $repo "work\build_rdmc_second_broker_validation_gate.ps1"
$evaluatorPath = Join-Path $repo "work\evaluate_rdmc_second_broker_validation_gate.py"
$collectorPath = Join-Path $repo "work\collect_rdmc_second_broker_validation_results.py"
$testPath = Join-Path $repo "work\test_rdmc_second_broker_validation_gate.py"
...
$expectedManifestHash = "5334471DE730CA25B1028B85BCC7DACF0FC5C23BCDDB8541AED75EC320DDEC04"
...
Add-Check "actual second-broker specification absent" (!(Test-Path (Join-Path $repo 'outputs\RDMC_SECOND_BROKER_SPECIFICATION.csv'))) "not supplied"
Add-Check "actual second-broker results absent" (!(Test-Path (Join-Path $repo 'outputs\RDMC_SECOND_BROKER_VALIDATION_RESULTS.csv'))) "not supplied"
