...
$reportDir = Join-Path $repo 'outputs\rdmc_momentum_loss_decay_v3_package\reports_here'
$decision = Join-Path $repo 'outputs\RDMC_MOMENTUM_LOSS_DECAY_V3_DECISION_FIXTURE.csv'
$runner = Join-Path $PSScriptRoot 'run_mt5_portable_parallel_manifest.ps1'
$collector = Join-Path $PSScriptRoot 'collect_rdmc_momentum_loss_decay_v3_results.ps1'
...
$expectedBinaryHash = 'BCDB0C5A859636645D0AF12A945B138742DBFE726424CBFEC29633D5933F6D24'
...
$allowedResumeNames = @(
   'rdmc_mld3_w01_m1_critical_2019.htm',
   'rdmc_mld3_w01_m1_critical_2019.identity.json'
)
...
try {
   Get-ChildItem -Path (Join-Path $repo 'outputs\RDMC_MOMENTUM_LOSS_DECAY_V3_WAVE_01_WORKER_*.csv') -File -ErrorAction SilentlyContinue |
      Remove-Item -Force
   ...
   & $runner -ManifestPath $manifest -PortableRoots $roots -UserAuthorizedFocusRisk `
      -OutputPrefix 'RDMC_MOMENTUM_LOSS_DECAY_V3_WAVE_01_WORKER' -MaxCpuPercent $MaxCpuPercent `
   ...
   & $collector -Wave 1 -DecisionCsvPath $decision -ManifestPath $manifest -ReportDir $reportDir `
      -RunnerLedgerGlob 'outputs\RDMC_MOMENTUM_LOSS_DECAY_V3_WAVE_01_WORKER_*.csv' `
      -ResultsPath 'outputs\RDMC_MOMENTUM_LOSS_DECAY_V3_WAVE_01_RESULTS.csv' -SkipAdmissionRefresh
   & python $evaluator
