...
Last updated: 2026-07-14 UTC after hidden local MT5 first-pass testing of `trade_ready_conservative`, `money_ready`, and several locked LowATR risk-shape candidates. There is no newly validated trade-ready best profile. The current research best remains LowATR OrderFlow. The raw locked LowATR fast screen produced the big number, `+$8,437.54` on `Model=1` from `2024.01.01` to `2026.07.12`, but it also had `25.94%` drawdown, so it is research-only and not promoted. Safer variants either made too little, failed yearly recovery, or exceeded the `6%` first-pass drawdown cap.
