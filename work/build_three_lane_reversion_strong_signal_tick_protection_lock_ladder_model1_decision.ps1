param(
   ...
   [string]$ReportDir = "outputs\three_lane_reversion_strong_signal_tick_protection_lock_ladder_model1_package\reports_here",
   [string]$ResultsPath = "outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_LOCK_LADDER_MODEL1_RESULTS.csv",
   [string]$SummaryPath = "outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_LOCK_LADDER_MODEL1_SUMMARY.csv",
   [string]$DecisionCsvPath = "outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_LOCK_LADDER_MODEL1_DECISION.csv",
   [string]$DecisionMarkdownPath = "outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_LOCK_LADDER_MODEL1_DECISION.md",
   [string]$RunAttestationPath = "outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_LOCK_LADDER_MODEL1_RUN_ATTESTATION.csv"
)
...
$controlName = "rvsstpll_control"
$strongControlName = "rvsstpll_strong_only"
$referenceName = "rvsstpll_lock010"
$lowerName = "rvsstpll_lock015"
$candidateName = "rvsstpll_lock020"
$upperName = "rvsstpll_lock025"
...
if($manifest.Count -ne 24) { throw "Expected twenty-four frozen Model1 manifest rows." }
...
if(@($manifest.Candidate | Sort-Object -Unique).Count -ne 6 -or
   @($manifest.Window | Sort-Object -Unique).Count -ne 4) {
   throw "Manifest candidate/window topology changed."
}

$rawResults = "work\RVSSTPLLM1_RAW_RESULTS.csv"
$rawSummary = "work\RVSSTPLLM1_RAW_SUMMARY.csv"
$rawMetrics = "work\RVSSTPLLM1_RAW_METRICS.md"
...
if($raw.Count -ne 24 -or @($raw | Where-Object Status -ne "PARSED").Count -ne 0) {
   throw "Expected twenty-four parsed Model1 reports."
}
$rawByReport = @{}
...
$workerFiles = @(
   Get-ChildItem (Join-Path $repo "outputs") -Filter "THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_LOCK_LADDER_MODEL1_EXACT_?.csv" -File
   Get-ChildItem (Join-Path $repo "outputs") -Filter "THREE_LANE_REVERSION_STRONG_SIGNAL_TICK_PROTECTION_LOCK_LADDER_MODEL1_RETRY_?.csv" -File
) | Sort-Object Name
foreach($file in $workerFiles) {
   foreach($row in @(Import-Csv -LiteralPath $file.FullName)) { $workerRows.Add($row) | Out-Null }
}
if($workerRows.Count -ne 27 -or @($workerRows | Where-Object {
...
if(@($workerRows | Where-Object Status -eq "ERROR").Count -ne 3) {
   throw "Expected exactly three preserved identity-only refusals."
}
...
$reference = $byCandidateWindow["$referenceName|$continuousWindow"]
...
function Test-BehaviorChanged([string]$Name) {
   return @($windows | Where-Object {
      $window = $_
      $protected = $byCandidateWindow["$Name|$window"]
...
$behaviorChanged = Test-BehaviorChanged $candidateName
$lowerBehaviorChanged = Test-BehaviorChanged $lowerName
$upperBehaviorChanged = Test-BehaviorChanged $upperName
$tradeCountGate = [int]$candidate.TotalTrades -ge 400
$lowerGate = $lowerBehaviorChanged -and
...
$upperGate = $upperBehaviorChanged -and
...
$summary = foreach($name in @($controlName,$strongControlName,$referenceName,$lowerName,$candidateName,$upperName)) {
   ...
   [pscustomobject][ordered]@{
      ...
      GatePass = if($name -eq $controlName -or $name -eq $strongControlName -or $name -eq $referenceName) { $true } elseif($name -eq $candidateName) { $passed } elseif($name -eq $lowerName) { $lowerGate } else { $upperGate }
   }
}
...
$decision = [pscustomobject][ordered]@{
   ...
   LowerNeighborBehaviorChanged = $lowerBehaviorChanged
   UpperNeighborBehaviorChanged = $upperBehaviorChanged
   ...
   ObservedReferenceNetProfit = $reference.NetProfit
...
$lines.Add("# Three-Lane Reversion Strong-Signal Tick Protection Lock Ladder Model 1 Decision")
$lines.Add("")
$lines.Add($(if($passed) {
...
$lines.Add("")
$lines.Add("- Reports: ``24 / 24`` parsed and exact source/binary identity valid")
$lines.Add("- Attempts: ``27``; identity-only retries: ``3``")
...
$lines.Add("- Fixed 1.00R-trigger lock ladder: observed reference ``0.10R``; new neighborhood ``0.15R / 0.20R / 0.25R``")
...
foreach($row in $summary) {
   $label = switch($row.Candidate) {
      ...
      $referenceName { "Observed reference lock 0.10R" }
      $lowerName { "Lower neighbor lock 0.15R" }
      $candidateName { "Center lock 0.20R" }
      $upperName { "Upper neighbor lock 0.25R" }
   }
...
$lines.Add("| 0.15R lower neighbor independently passes | net ``$(Money ([double]$lower.NetProfit))``; CAGR ``$($lower.CagrPercent)%``; recovery ``$($lower.RecoveryFactor)`` | $(BoolText $lowerGate) |")
$lines.Add("| 0.25R upper neighbor independently passes | net ``$(Money ([double]$upper.NetProfit))``; CAGR ``$($upper.CagrPercent)%``; recovery ``$($upper.RecoveryFactor)`` | $(BoolText $upperGate) |")
$lines.Add("")
$lines.Add("## Interpretation")
$lines.Add("")
$lines.Add("The ladder fixes the every-tick trigger at 1.00R and raises only the profit-lock distance. The 0.20R center fell to ``$(Money ([double]$candidate.NetProfit))``, ``$($candidate.CagrPercent)%`` CAGR, PF ``$($candidate.ProfitFactor)``, ``$($candidate.MaxDrawdownPercent)%`` drawdown, and recovery ``$($candidate.RecoveryFactor)``. Both new neighbors also underperformed disabled control.")
$lines.Add("")
$lines.Add($(if($passed) {
   "The center and both lock neighbors changed behavior and passed the preregistered Model 1 growth and efficiency gates. A fixed Model 4 real-tick comparison may open, but this is not a promotion and the forward candidate remains unchanged."
} else {
   "The higher-lock ladder failed growth, drawdown, recovery, and neighbor support. Its non-monotonic path response contradicts a simple retained-profit improvement, so this protection family closes without another lock or trigger search. Model 4 stays closed and ATB150 remains the historical champion. Center recovery difference versus strong-only: ``$('{0:F4}' -f ([double]$candidate.RecoveryFactor - [double]$strongControl.RecoveryFactor))``."
}))
$lines.Add("")
