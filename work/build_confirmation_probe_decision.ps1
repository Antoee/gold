param(
   [string]$MetricsPath = "outputs\CONFIRMATION_PROBE_REPORT_METRICS.csv",
   [string]$OutCsv = "outputs\CONFIRMATION_PROBE_DECISION.csv",
   [string]$OutMarkdown = "outputs\CONFIRMATION_PROBE_DECISION.md",
...
$pairs = @(
   [pscustomobject]@{ Pair = "baseline_confirm3"; Candidate = "baseline_promoted_confirm3"; Baseline = "baseline_promoted" },
   [pscustomobject]@{ Pair = "tp38_sl18_confirm3"; Candidate = "tp38_sl18_confirm3"; Baseline = "tp38_sl18" }
)
...
foreach($pair in $pairs) {
   ...
   $reason = "Candidate or paired confirmation-2 report is missing."
   ...
   elseif($candidateStatus -eq "PARSED" -and $baselineStatus -eq "PARSED") {
      if($candidateNet -lt 0) { $decision = "FAIL_CANDIDATE_LOSS"; $reason = "Confirmation-3 candidate lost money." }
      elseif($candidateNet -lt $baselineNet) { $decision = "FAIL_BELOW_CONFIRM2"; $reason = "Confirmation-3 candidate underperformed its confirmation-2 pair." }
      elseif($candidateDd -gt $MaxAllowedDrawdownPercent) { $decision = "REVIEW_DRAWDOWN"; $reason = "Confirmation-3 candidate passed profit comparison but drawdown needs review." }
      else { $decision = "PASS_CONFIRMATION_PROBE"; $reason = "Confirmation-3 candidate was profitable and beat its confirmation-2 pair." }
   }
...
$hasPass = @($rows | Where-Object { $_.Decision -eq "PASS_CONFIRMATION_PROBE" }).Count -gt 0
...
$overall = if($hasPass) { "HAS_CONFIRMATION_CANDIDATE" } elseif($hasRepair) { "REPAIR_REPORTS" } elseif($hasWaiting) { "WAITING_FOR_REPORTS" } elseif($hasReview) { "REVIEW_DRAWDOWN" } elseif($hasFail) { "NO_CONFIRMATION_CANDIDATE" } else { "REVIEW_REQUIRED" }

$md = New-Object System.Collections.Generic.List[string]
$md.Add("# Confirmation Strictness Probe Decision") | Out-Null
$md.Add("") | Out-Null
...
$md.Add("") | Out-Null
if($overall -eq "HAS_CONFIRMATION_CANDIDATE") { $md.Add("At least one confirmation-3 candidate deserves broader stress/recent-OOS validation. Do not promote from this probe alone.") | Out-Null }
elseif($overall -eq "NO_CONFIRMATION_CANDIDATE") { $md.Add("No tested confirmation-3 candidate beat its confirmation-2 pair. Keep the current promoted profile unchanged.") | Out-Null }
elseif($overall -eq "REPAIR_REPORTS") { $md.Add("Repair or re-export confirmation probe reports before making a filter decision.") | Out-Null }
elseif($overall -eq "REVIEW_DRAWDOWN") { $md.Add("A confirmation-3 candidate may have profit potential, but drawdown needs review before expanding validation.") | Out-Null }
else { $md.Add("Waiting for exported confirmation probe reports. Keep the current promoted profile unchanged.") | Out-Null }
