param(
   ...
   [string]$DecisionCsvPath = "outputs\RDMC_TIERED_MOMENTUM_V7_DECISION_FIXTURE.csv",
   [string]$ManifestPath = "outputs\RDMC_TIERED_MOMENTUM_V7_GATE_MANIFEST.csv",
   [string]$ReportDir = "outputs\rdmc_tiered_momentum_v7_package\reports_here",
   [string]$RunnerLedgerGlob = "",
   [string]$ResultsPath = "outputs\RDMC_TIERED_MOMENTUM_V7_GATE_RESULTS.csv",
   [string]$RunAuditPath = "",
   [string]$RawResultsPath = "",
   [string]$SummaryPath = "",
   [string]$MetricsMarkdownPath = "",
   [string]$ExpectedManifestHash = "80BD25AE1E3D88B5D758AA70F74FB673EE1FB8A09CC1AEF356CA50F4DC3D4C1D",
   [string]$ExpectedSourceHash = "27CAD37CD903032335DA570CDEC75AC39C2EA6BEF04CA264D1586EDC866F6AF6",
   [string]$ExpectedProfileHash = "6E2EF7B031FF30216876E0232A8CE9D6BFC9F7913A863103DC9B12C1A04A100C",
...
$expectedManifestHash = $ExpectedManifestHash.ToUpperInvariant()
$expectedSourceHash = $ExpectedSourceHash.ToUpperInvariant()
$expectedProfileHash = $ExpectedProfileHash.ToUpperInvariant()
...
if([string]::IsNullOrWhiteSpace($RunnerLedgerGlob)) {
   $RunnerLedgerGlob = "outputs\RDMC_TIERED_MOMENTUM_V7_WAVE_{0:D2}_WORKER_*.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($RunAuditPath)) {
   $RunAuditPath = "outputs\RDMC_TIERED_MOMENTUM_V7_WAVE_{0:D2}_RUN_AUDIT.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($RawResultsPath)) {
   $RawResultsPath = "outputs\RDMC_TIERED_MOMENTUM_V7_WAVE_{0:D2}_RAW_RESULTS.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($SummaryPath)) {
   $SummaryPath = "outputs\RDMC_TIERED_MOMENTUM_V7_WAVE_{0:D2}_SUMMARY.csv" -f $Wave
}
if([string]::IsNullOrWhiteSpace($MetricsMarkdownPath)) {
   $MetricsMarkdownPath = "outputs\RDMC_TIERED_MOMENTUM_V7_WAVE_{0:D2}_METRICS.md" -f $Wave
}
...
$waveManifestTemp = Join-Path $repo ("outputs\.rdmc_tmv7_wave_{0:D2}_{1}.csv" -f $Wave,[guid]::NewGuid().ToString("N"))
try {
   $waveRows | Export-Csv -LiteralPath $waveManifestTemp -NoTypeInformation -Encoding ASCII
   & (Join-Path $PSScriptRoot "collect_validation_results.ps1") `
      -RepoRoot $repo -ManifestPath (Relative-RepoPath $waveManifestTemp) -ReportDir (Relative-RepoPath $reportFull) `
   ...
   $metricsFull = Resolve-RepoPath $MetricsMarkdownPath
   $metricsText = Get-Content -LiteralPath $metricsFull -Raw
   $temporaryManifest = [regex]::Escape((Relative-RepoPath $waveManifestTemp))
   $stableHeader = "- Gate manifest: ``$(Relative-RepoPath $manifestFull)```r`n- Admitted wave: ``$Wave``"
   $metricsText = [regex]::Replace($metricsText, "- Manifest: ``$temporaryManifest``", $stableHeader)
   [IO.File]::WriteAllText($metricsFull, $metricsText, [Text.Encoding]::ASCII)
}
finally {
   Remove-Item -LiteralPath $waveManifestTemp -Force -ErrorAction SilentlyContinue
}
...
if(!$SkipAdmissionRefresh) {
   if((Resolve-Path -LiteralPath $resultsFull).Path -ne (Join-Path $repo "outputs\RDMC_TIERED_MOMENTUM_V7_GATE_RESULTS.csv")) {
