...
input ulong  InpMagicNumber = 26071982;
...
input group "Inside-Day Compression Breakout Engine"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_M15;
input double InpMinimumInsideRangeRatio = 0.25;
input double InpMaximumInsideRangeRatio = 0.75;
input double InpBreakoutBufferATR = 0.05;
input double InpMinimumBodyPercent = 40.0;
...
input int    InpStopLookbackBars = 3;
...
input int    InpChandelierLookbackBars = 12;
...
input string InpLogFileName = "Independent_XAUUSD_M15_Inside_Day_Breakout_Trades.csv";
...
datetime g_lastInsideSetupDay = 0;
double g_peakEquity = 0.0;
int g_entrySignals = 0;
int g_insideDaySetups = 0;
...
{
   return "IM15IDB_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "M15IDB-LIVE-ACK"))
   {
...
bool InsideDayContext(double &insideHigh,
                      double &insideLow,
                      double &rangeRatio)
{
   insideHigh = iHigh(_Symbol, PERIOD_D1, 1);
   insideLow = iLow(_Symbol, PERIOD_D1, 1);
   double motherHigh = iHigh(_Symbol, PERIOD_D1, 2);
   double motherLow = iLow(_Symbol, PERIOD_D1, 2);
   double insideRange = insideHigh - insideLow;
   double motherRange = motherHigh - motherLow;
   if(insideLow <= 0.0 || motherLow <= 0.0 ||
      insideRange <= 0.0 || motherRange <= 0.0)
      return false;
   rangeRatio = insideRange / motherRange;
   return insideHigh < motherHigh && insideLow > motherLow &&
          rangeRatio >= InpMinimumInsideRangeRatio &&
          rangeRatio <= InpMaximumInsideRangeRatio;
}
...
bool OpenInsideDayPosition(const bool buy, const double atr)
{
   MqlTick tick;
   ...
   string comment = buy ? "M15IDB_BUY" : "M15IDB_SELL";
   ...
   LogEvent("entry", trade.ResultOrder(), buy ? "buy" : "sell", lots, entryPrice, stopPrice, 0.0,
            "fresh M15 break of completed inside-day range");
   return true;
}
...
void TryEntry(const double atr)
{
   ...
   if(!SafetyAllows(safetyReason))
      return;
   double insideHigh = 0.0;
   double insideLow = 0.0;
   double insideRangeRatio = 0.0;
   if(!InsideDayContext(insideHigh, insideLow, insideRangeRatio))
      return;
   if(g_lastInsideSetupDay != g_dayStart)
   {
      g_lastInsideSetupDay = g_dayStart;
      g_insideDaySetups++;
   }
   ...
   bool buyBreak = InpAllowBuy && close1 > insideHigh + buffer;
   bool sellBreak = InpAllowSell && close1 < insideLow - buffer;
   if(InpRequireFreshBreakout)
   {
      buyBreak = buyBreak && close2 <= insideHigh + buffer;
      sellBreak = sellBreak && close2 >= insideLow - buffer;
   }
   ...
   if(buyBreak)
   {
      ...
      g_entrySignals++;
      OpenInsideDayPosition(true, atr);
   }
   else if(sellBreak)
   {
      ...
      g_entrySignals++;
      OpenInsideDayPosition(false, atr);
   }
}

int OnInit()
{
   if(InpMinimumInsideRangeRatio <= 0.0 ||
      InpMaximumInsideRangeRatio < InpMinimumInsideRangeRatio ||
      InpMaximumInsideRangeRatio >= 1.0 ||
...
{
   PrintFormat("M15IDB_DIAGNOSTIC inside_setups=%d signals=%d candle_rejects=%d volume_rejects=%d regime_rejects=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
               g_insideDaySetups, g_entrySignals, g_candleRejects, g_volumeRejects,
