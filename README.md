...
**2026-07-18 active repair queue: no new best. The frozen `1.00 / 1.25 / 1.50 ATR` signal-range family remains queued for eight Model1 runs on 2019 and 2022, with exact report admission and preregistered stop rules ready. A read-only offline pre-screen decoded the exact local 100,000-bar H1 cache and reproduced all 135 selection range/ATR values within six-decimal rounding. Simple control-trade exclusion leaves every threshold negative in both failure years; the `1.25 ATR` center projects `-$16.36` in 2019 and `-$76.42` on only 14 trades in 2022. Because removing a trade can expose later signals, this is a low-prior diagnostic rather than the frozen decision. The real run plan remains `LOCKED` with zero reports, and no candidate changed.**
...
The annual failures are now decomposed in an exact 342-trade ledger. The 2019 loss is entirely momentum; in 2022, momentum lost `$71.78` and one reversion trade lost `$21.00`. All 14 Wednesday trades in those two years lost, but deleting Wednesday is rejected because the complete annual sample earns `+$427.11` at PF `1.99` on that weekday and the exclusion still leaves 2022 negative. The active repair uses a causal completed-candle signal-range gate selected only from the one-to-one 2015-2018 telemetry/Model4 join. The read-only [offline pre-screen](outputs/RDMC_SIGNAL_RANGE_GATE_OFFLINE_PRESCREEN.md) validates the local H1 cache and ATR alignment against all 135 joined trades, then shows that simple exclusion leaves every frozen threshold negative in both failure years. The `1.00 ATR` row projects `-$47.01 / -$86.36`, the `1.25` center `-$16.36 / -$76.42`, and `1.50` `-$14.88 / -$53.31`; stricter rows also miss the 18-trade floor. This does not amend the contract because filtered paths can admit later entries. The exact [collector](work/collect_rdmc_signal_range_gate_repair_results.ps1) and [decision builder](work/build_rdmc_signal_range_gate_repair_decision.ps1) remain authoritative for the still-unrun reports. The [diagnosis](outputs/RDMC_ANNUAL_FAILURE_DIAGNOSIS.md), [selection](outputs/RDMC_SIGNAL_RANGE_GATE_SELECTION.md), [contract](outputs/RDMC_SIGNAL_RANGE_GATE_REPAIR_CONTRACT.md), and [locked run plan](outputs/RDMC_SIGNAL_RANGE_GATE_REPAIR_HIDDEN_RUN_PLAN.md) preserve the boundary and confirm that no new best exists.
