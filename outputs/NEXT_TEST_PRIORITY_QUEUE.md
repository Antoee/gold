...
| 4 | SESSION_PROBE | `tp38_sl18` session variants vs unfiltered baseline | 6 | Expand only passing session candidates into stress/recent-OOS validation. | Stop spending tester time on failed session variants. |
| 5 | FULL_HANDOFF | Full candidate validation | 24 | Build promotion packet only if all gates pass. | Keep current promoted baseline. |
