[CmdletBinding()]
param(
   [string]$SourcePath = 'work\Professional_XAUUSD_Three_Lane_Reversion_Strong_Signal_Protection_Research.mq5',
   [string]$ChampionProfilePath = 'release\three-lane-trade-ready-rc2-atb150\THREE_LANE_TRADE_READY_RC2_ATB150.set',
   [string]$PackageDir = 'outputs\three_lane_reversion_strong_signal_protection_model1_package',
   [string]$QueuePath = 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_PROTECTION_MODEL1_QUEUE.csv',
   [string]$ManifestPath = 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_PROTECTION_MODEL1_MANIFEST.csv',
   [string]$PackageMarkdownPath = 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_PROTECTION_MODEL1_PACKAGE.md',
   [string]$ContractPath = 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_PROTECTION_MODEL1_CONTRACT.md'
)
...
$expectedSourceHash = '9810F048C62E93C217FB955DA51A4364055BF7806EFC739B37621E3B251F9206'
...
& (Join-Path $PSScriptRoot 'test_three_lane_reversion_strong_signal_protection_source.ps1') | Out-Null
...
if($sourceHash -ne $expectedSourceHash) { throw "Strong-signal protection source identity changed: $sourceHash" }
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_reversion_strong_signal_protection_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='rvssp_control';Role='control';Strong='false';Protection='false';TriggerR='1.00';LockR='0.10'},
   [pscustomobject]@{Name='rvssp_strong_only';Role='strong_control';Strong='true';Protection='false';TriggerR='1.00';LockR='0.10'},
   [pscustomobject]@{Name='rvssp_lock005';Role='lower_neighbor';Strong='true';Protection='true';TriggerR='1.00';LockR='0.05'},
   [pscustomobject]@{Name='rvssp_center';Role='center';Strong='true';Protection='true';TriggerR='1.00';LockR='0.10'},
   [pscustomobject]@{Name='rvssp_trigger125';Role='upper_neighbor';Strong='true';Protection='true';TriggerR='1.25';LockR='0.10'}
)
$windows = @(
...
$stopRule = 'Strong-signal protection. Center 1.00R/0.10R must keep every era >= disabled control, continuous net >= control +5% and >=97% of strong-only, CAGR >= control +0.10 points and >= strong-only -0.03 points, PF/recovery/return-DD >= strong-only, DD <= strong-only and <=1.25%, and >=400 trades. Both protection neighbors must keep every era >= control, net >= control +3%, CAGR >= control +0.05 points, PF/recovery/return-DD >= control, DD <=1.25%, and >=400 trades. No post-result retuning.'
...
foreach($variant in $variants) {
   ...
   foreach($window in $windows) {
      ...
      $common = [ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='reversion_strong_signal_protection_model1'
...
@(
   '# Three-Lane Reversion Strong-Signal Protection Model 1 Package','',
   ...
   '- This separately preregistered follow-up freezes body ratio 0.25 and requested risk 0.70%, then tests tightening-only protection on those strong reversion positions.',
   '- The optional stop manager uses the position open price and the exact stored initial risk; it can only move an owned stop toward a 0.05R or 0.10R lock after 1.00R or 1.25R favorable movement.',
...
@(
   '# Reversion Strong-Signal Protection Model 1 Contract','',
   ...
   '- Use only completed signal-bar OHLC at entry, exact stored initial risk, and current executable bid/ask while managing the owned position. Never use future data, prior outcomes, drawdown, loss streaks, account profit, or calendar exclusions.',
   ...
   '- Require the 1.00R/0.10R center to match or beat disabled control net in every era, continuous net at least 5% above disabled control and at least 97% of strong-only, CAGR at least 0.10 points above disabled control and no more than 0.03 points below strong-only, PF/recovery/return-drawdown no worse than strong-only, drawdown no worse than strong-only and at most 1.25%, and at least 400 continuous trades.',
   '- Require both protection neighbors to match or beat disabled control in every era, continuous net at least 3% above disabled control, CAGR at least 0.05 points above disabled control, PF/recovery/return-drawdown no worse than disabled control, drawdown at most 1.25%, and at least 400 trades.',
