...
#property description "Research fork with optional tightening-only reversion break-even management"
...
input bool   InpRVUseBreakEven = false;
input double InpRVBreakEvenTriggerR = 0.75;
input double InpRVBreakEvenLockR = 0.00;
...
private:
   ...
   void ImproveProtectiveStop(const ulong ticket, const bool buy)
   {
      ...
      if(openPrice <= 0.0 || oldSl <= 0.0 || initialRisk <= _Point || current <= 0.0)
         return;
      ...
      double newSl = buy ? openPrice + InpRVBreakEvenLockR * initialRisk
                         : openPrice - InpRVBreakEvenLockR * initialRisk;
...
bool InputsValid()
{
   ...
   if(InpRVATRPeriod < 2 || InpRVADXPeriod < 2 || InpRVRSIPeriod < 2 ||
      ...
      (InpRVUseBreakEven &&
       (InpRVBreakEvenTriggerR <= 0.0 ||
        InpRVBreakEvenLockR < 0.0 ||
