...
#property description "Research fork with bounded reversion retracement entries"
...
input bool   InpRVUseRetracementEntry = false;
input double InpRVRetracementEntryOffsetATR = 0.15;
input int    InpRVRetracementEntryLifetimeBars = 1;
...
private:
   ...
   bool m_entryArmed;
   bool m_armedBuy;
   datetime m_armedUntil;
   double m_armedTriggerPrice;
   double m_armedStopPrice;
   double m_armedTargetPrice;
   double m_armedATR;
   double m_armedDIEdge;
   double m_armedRiskPercent;
   double m_armedMaximumPositionLots;
   double m_armedSignalBodyRatio;

   void ClearArmedEntry()
   {
      m_entryArmed = false;
      m_armedBuy = false;
      m_armedUntil = 0;
      m_armedTriggerPrice = 0.0;
      m_armedStopPrice = 0.0;
      m_armedTargetPrice = 0.0;
      m_armedATR = 0.0;
      m_armedDIEdge = 0.0;
      m_armedRiskPercent = 0.0;
      m_armedMaximumPositionLots = 0.0;
      m_armedSignalBodyRatio = 0.0;
   }
                     ...
                     const bool retracementEntry)
   {
      ...
      string comment = retracementEntry
                       ? (buy ? "RRE;Band VWAP retrace;Lower" : "RRE;Band VWAP retrace;Upper")
                       : (buy ? "RRO;Band VWAP reversion;Lower" : "RRO;Band VWAP reversion;Upper");
      bool opened = buy
      ...
      if(retracementEntry)
         eventReason += "; adverse retracement entry";
   ...
   bool ArmRetracementEntry(const bool buy,
                            const double signalEntryPrice,
                            const double stopPrice,
                            const double targetPrice,
                            const double atr,
                            const double diEdge,
                            ...
                            const double signalBodyRatio)
   {
      if(m_entryArmed || atr <= 0.0)
         return false;
      double offset = atr * InpRVRetracementEntryOffsetATR;
      double triggerPrice = buy ? signalEntryPrice - offset : signalEntryPrice + offset;
      if((buy && (triggerPrice <= stopPrice || triggerPrice >= targetPrice)) ||
         (!buy && (triggerPrice >= stopPrice || triggerPrice <= targetPrice)))
         return false;
      datetime currentBar = iTime(_Symbol, InpRVSignalTimeframe, 0);
      int periodSeconds = PeriodSeconds(InpRVSignalTimeframe);
      if(currentBar <= 0 || periodSeconds <= 0)
         return false;

      m_entryArmed = true;
      m_armedBuy = buy;
      m_armedUntil = currentBar + periodSeconds * InpRVRetracementEntryLifetimeBars;
      m_armedTriggerPrice = triggerPrice;
      m_armedStopPrice = stopPrice;
      m_armedTargetPrice = targetPrice;
      m_armedATR = atr;
      m_armedDIEdge = diEdge;
      m_armedRiskPercent = requestedRiskPercent;
      m_armedMaximumPositionLots = requestedMaximumPositionLots;
      m_armedSignalBodyRatio = signalBodyRatio;
      string reason = buy ? "armed buy retracement" : "armed sell retracement";
      reason += "; trigger " + DoubleToString(triggerPrice, _Digits);
      reason += "; expires " + TimeToString(m_armedUntil, TIME_DATE | TIME_MINUTES);
      WriteEvidenceEvent(m_logHandle, "tlp_rv_rre", "armed", 0,
                         buy ? "buy" : "sell", 0.0, signalEntryPrice,
                         stopPrice, 0.0, reason);
      return true;
   }
   ...
   bool ProcessArmedEntry()
   {
      if(!m_entryArmed)
         return false;
      if(ManagedPositionCount() > 0 || TimeCurrent() >= m_armedUntil)
      {
         ClearArmedEntry();
         return false;
      }
      ...
      if(!SymbolInfoTick(_Symbol, tick))
         return false;
      if((m_armedBuy && (tick.bid <= m_armedStopPrice || tick.bid >= m_armedTargetPrice)) ||
         (!m_armedBuy && (tick.ask >= m_armedStopPrice || tick.ask <= m_armedTargetPrice)))
      {
         ClearArmedEntry();
         return false;
      }
      bool triggered = m_armedBuy ? tick.ask <= m_armedTriggerPrice
      ...
      double spreadATRPercent = 100.0 * SpreadPoints() / (m_armedATR / _Point);
      ...
      double entryPrice = m_armedBuy ? tick.ask : tick.bid;
      double stopDistance = m_armedBuy ? entryPrice - m_armedStopPrice
                                       : m_armedStopPrice - entryPrice;
      double targetDistance = m_armedBuy ? m_armedTargetPrice - entryPrice
                                         : entryPrice - m_armedTargetPrice;
      ...
      if(stopDistance <= 0.0 || targetDistance <= 0.0 ||
         (InpRVMaximumStopATR > 0.0 && stopDistance > m_armedATR * InpRVMaximumStopATR) ||
         targetDistance < m_armedATR * MathMax(0.0, InpRVMinimumTargetATR) ||
         adjustedReward <= 0.0 || adjustedRisk <= 0.0 ||
      ...
      bool buy = m_armedBuy;
      double stopPrice = m_armedStopPrice;
      double targetPrice = m_armedTargetPrice;
      double diEdge = m_armedDIEdge;
      double requestedRiskPercent = m_armedRiskPercent;
      double requestedMaximumPositionLots = m_armedMaximumPositionLots;
      double signalBodyRatio = m_armedSignalBodyRatio;
      ClearArmedEntry();
      return OpenPosition(buy, entryPrice, stopPrice, targetPrice, diEdge,
                          requestedRiskPercent, requestedMaximumPositionLots,
                          signalBodyRatio, true);
   }
   ...
   void TryEntry()
   {
      ...
      if(InpRVUseRetracementEntry)
      {
         if(ArmRetracementEntry(buy, entryPrice, stopPrice, targetPrice, atr, diEdge,
                                requestedRiskPercent, requestedMaximumPositionLots,
                                signalBodyRatio))
            ProcessArmedEntry();
      }
...
public:
   ...
   void OnTick()
   {
      ...
      if(ManagedPositionCount() > 0)
         ClearArmedEntry();
      else if(InpRVUseRetracementEntry && ProcessArmedEntry())
         return;
      datetime currentBar = iTime(_Symbol, InpRVSignalTimeframe, 0);
...
bool InputsValid()
{
   ...
   if(InpRVATRPeriod < 2 || InpRVADXPeriod < 2 || InpRVRSIPeriod < 2 ||
      ...
      InpRVRetracementEntryOffsetATR < 0.0 ||
      InpRVRetracementEntryOffsetATR > 1.0 ||
      InpRVRetracementEntryLifetimeBars < 1 ||
      InpRVRetracementEntryLifetimeBars > 8 ||
      (InpRVUseRetracementEntry && InpRVRetracementEntryOffsetATR <= 0.0))
      return false;
