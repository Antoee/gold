param(
   ...
   [string]$ReportDir = "outputs\reversion_d1_momentum_cap_discovery_model1_package\reports_here",
   [string]$RawResultsPath = "outputs\REVERSION_D1_MOMENTUM_CAP_DISCOVERY_MODEL1_RAW_RESULTS.csv",
   [string]$ResultsPath = "outputs\REVERSION_D1_MOMENTUM_CAP_DISCOVERY_MODEL1_RESULTS.csv",
   [string]$RunPath = "outputs\REVERSION_D1_MOMENTUM_CAP_DISCOVERY_MODEL1_RUN.csv",
   [string]$SummaryPath = "outputs\REVERSION_D1_MOMENTUM_CAP_DISCOVERY_MODEL1_SUMMARY.csv",
   [string]$MarkdownPath = "outputs\REVERSION_D1_MOMENTUM_CAP_DISCOVERY_MODEL1_METRICS.md"
)
...
if($queue.Count -ne 35) { throw "Expected thirty-five frozen queue rows." }
...
$initialRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath "outputs") -Filter "REVERSION_D1_MOMENTUM_CAP_PORTABLE_*.csv" |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
$retryRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath "outputs") -Filter "REVERSION_D1_MOMENTUM_CAP_RERUN_*.csv" |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
...
if($raw.Count -ne 35) { throw "Expected thirty-five parsed results." }
$results = foreach($queued in $queue) {
   ...
   [pscustomobject]@{
      ...
      RVMinimumDIEdge=$queued.RVMinimumDIEdge;D1MomentumCapEnabled=$queued.D1MomentumCapEnabled
      D1MomentumLookbackBars=$queued.D1MomentumLookbackBars
      MaximumAbsoluteD1MomentumPercent=$queued.MaximumAbsoluteD1MomentumPercent;InitialDeposit=$row.InitialDeposit
