param(
   ...
   [string]$ReportDir = "outputs\reversion_long_distance_guard_repair_model1_package\reports_here",
   [string]$RawResultsPath = "outputs\REVERSION_LONG_DISTANCE_GUARD_REPAIR_MODEL1_RAW_RESULTS.csv",
   [string]$ResultsPath = "outputs\REVERSION_LONG_DISTANCE_GUARD_REPAIR_MODEL1_RESULTS.csv",
   [string]$RunPath = "outputs\REVERSION_LONG_DISTANCE_GUARD_REPAIR_MODEL1_RUN.csv",
   [string]$SummaryPath = "outputs\REVERSION_LONG_DISTANCE_GUARD_REPAIR_MODEL1_SUMMARY.csv",
   [string]$MarkdownPath = "outputs\REVERSION_LONG_DISTANCE_GUARD_REPAIR_MODEL1_METRICS.md"
)
...
if($queue.Count -ne 8) { throw "Expected eight frozen queue rows." }
if(@($queue | Where-Object { $_.To -gt "2020.12.31" }).Count -gt 0) { throw "Post-2020 data leaked into repair." }
...
$initialRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath "outputs") -Filter "REVERSION_LONG_DISTANCE_GUARD_REPAIR_PORTABLE_*.csv" |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
$retryRuns = @(Get-ChildItem -LiteralPath (Resolve-RepoPath "outputs") -Filter "REVERSION_LONG_DISTANCE_GUARD_REPAIR_RERUN_*.csv" |
   ForEach-Object { Import-Csv -LiteralPath $_.FullName })
...
if($raw.Count -ne 8) { throw "Expected eight parsed results." }
$results = foreach($queued in $queue) {
   ...
   [pscustomobject]@{
      QueueRank=$queued.QueueRank;Candidate=$queued.Candidate;Window=$queued.Window;From=$queued.From;To=$queued.To
      Model=$queued.Model;Deposit=$queued.Deposit;ExpectedReportName=$queued.ExpectedReportName
      Status=$row.Status;ReportPath=$reportPath;ProfileSha256=$queued.ProfileSha256
      SourceSha256=$queued.SourceSha256;ContractSha256=$queued.ContractSha256
      GuardEnabled=$queued.GuardEnabled;LookbackBars=$queued.LookbackBars
