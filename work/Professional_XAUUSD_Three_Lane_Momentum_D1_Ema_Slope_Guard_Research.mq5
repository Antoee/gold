...
#property description "Research fork with momentum D1 EMA-slope overextension guard"
...
private:
   CTrade m_trade;
   int m_atrHandle;
   int m_d1SlopeAtrHandle;
   int m_d1SlopeEmaHandle;
   int m_logHandle;
   datetime m_lastSignalBar;
   ...
   {
      if(!InpMOUseD1EmaSlopeOverextensionGuard)
         return true;
      double d1Atr = 0.0, recentEma = 0.0, pastEma = 0.0;
      if(!BufferValue(m_d1SlopeAtrHandle, 0, 1, d1Atr) || d1Atr <= 0.0 ||
         !BufferValue(m_d1SlopeEmaHandle, 0, 1, recentEma) ||
         !BufferValue(m_d1SlopeEmaHandle, 0, 1 + InpMOD1EmaSlopeLookbackBars, pastEma))
         return false;
      double direction = buy ? 1.0 : -1.0;
   ...
   bool RegimeAllows(const bool buy, const double closePrice, const double atr)
   {
      ...
      bool momentumAligned = buy ? direction > 0 : direction < 0;
...
public:
   ...
   {
      m_atrHandle = INVALID_HANDLE;
      m_d1SlopeAtrHandle = INVALID_HANDLE;
      m_d1SlopeEmaHandle = INVALID_HANDLE;
      m_logHandle = INVALID_HANDLE;
      m_lastSignalBar = 0;
   }
   ...
   bool Init()
   {
      ...
      if(InpMOUseD1EmaSlopeOverextensionGuard)
      {
         m_d1SlopeAtrHandle = iATR(_Symbol, InpMOMomentumTimeframe, InpMOD1EmaSlopeATRPeriod);
         m_d1SlopeEmaHandle = iMA(_Symbol, InpMOMomentumTimeframe,
                                  InpMOD1EmaSlopePeriod, 0, MODE_EMA, PRICE_CLOSE);
      }
      if(m_atrHandle == INVALID_HANDLE ||
         (InpMOUseD1EmaSlopeOverextensionGuard &&
          (m_d1SlopeAtrHandle == INVALID_HANDLE || m_d1SlopeEmaHandle == INVALID_HANDLE)))
         return false;
      m_trade.SetExpertMagicNumber(InpMOMagicNumber);
      m_trade.SetDeviationInPoints(InpMODeviationPoints);
      m_trade.SetAsyncMode(false);
   ...
   {
      if(m_atrHandle != INVALID_HANDLE) IndicatorRelease(m_atrHandle);
      if(m_d1SlopeAtrHandle != INVALID_HANDLE) IndicatorRelease(m_d1SlopeAtrHandle);
      if(m_d1SlopeEmaHandle != INVALID_HANDLE) IndicatorRelease(m_d1SlopeEmaHandle);
      if(m_logHandle != INVALID_HANDLE) FileClose(m_logHandle);
   }
...
bool InputsValid()
{
   ...
   if(InpMOMomentumLookbackBars < 20 || InpMOEntryLookbackBars < 3 ||
      ...
      (InpMOUseD1EmaSlopeOverextensionGuard &&
       (InpMOD1EmaSlopePeriod < 2 || InpMOD1EmaSlopeLookbackBars < 1 ||
        InpMOD1EmaSlopeATRPeriod < 2 || InpMOMaximumD1EmaSlopeATR <= 0.0 ||
        InpMOMaximumD1EmaSlopeATR > 20.0)) ||
