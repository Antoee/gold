...
Future-data answer: 2024 through now is research-seen data, not a guarantee. No profile is expected to work forever without monitoring. A release still requires frozen out-of-sample evidence, walk-forward checks, spread/slippage stress, broker variation, and forward demo monitoring with automatic disable rules.
...
- Current corrected answer: no money-ready bot. The most stable current-source baseline is `sr_m4_sweep_off`: Model4 real ticks `+$211.37` on `$10,000`, `+2.11%` total, `0.28%/yr`, PF `2.12`, `26` trades, and `0.82%` DD. It is stable but economically too weak. All older high-profit headlines are superseded as live-readiness evidence because they predate broker-accurate `OrderCalcProfit` sizing.
