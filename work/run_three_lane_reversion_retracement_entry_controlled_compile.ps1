...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_REVERSION_RETRACEMENT_ENTRY_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'Retracement-entry research source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Reversion retracement-entry compile $startedAtUtc",[Text.Encoding]::ASCII)
