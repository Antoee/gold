...
Last updated: 2026-07-14 UTC after hidden local MT5 first-pass testing of `trade_ready_conservative`, `money_ready`, locked LowATR risk-shape candidates, a 33-variant R20 opportunity sweep, yearly splits, a 22-variant R10 drawdown-reduction sweep, and a 5-profile Model4 real-tick shortlist. There is still no newly validated trade-ready best profile. The strict/stability branch remains `lowatr_exit_peak_r20` style evidence: `+$464.86`, `18.40%/yr`, PF `6.7890`, recovery `5.1411`, and `5.81%` drawdown on the fast continuous screen, but it is too sparse and weak in 2025. The high-profit research frontier is still the no-peak-trail R10 branch: Model4 baseline `+$1,564.01` with `10.64%` drawdown. The drawdown fallback is `r10_profit_guard40`: Model4 `+$1,000.97`, `39.61%/yr`, PF `3.4058`, recovery `8.5240`, and `7.76%` drawdown. Neither is trade-ready because full exported reports, split/stress validation, broker variation, and forward/demo evidence are still missing.
...
Plain English: the old `$866 in 2.5 years` number is outdated. The newest serious branch made `+$1,564.01` on a continuous Model4 real-tick screen and `+$1,716.76` on Model1, with all yearly Model1 splits green. That is a real improvement, but the cost is about `10.6%` drawdown, so it is not the stable/trade-ready answer yet. The drawdown sweep found a lower-risk fallback, `r10_profit_guard40`, at `+$1,000.97` Model4 with `7.76%` drawdown. The current choice is basically: stricter R20 style is safer but too small/sparse; no-peak-trail R10 makes more but swings harder; profit-guard R10 is more balanced but still needs real validation.
...
2. Validate the R10 drawdown fallback path before any promotion attempt: compare `r10_base`, `r10_dailytrail35`, `r10_loss_scale_15`, `r10_loss_scale_25`, and `r10_profit_guard40` on yearly splits, older windows, and stress settings.
3. Consider a targeted strategy-code/risk-shape change before the next first-pass build: keep later-year participation while cutting the 2025 drawdown path. Do not simply increase risk to make a failed profile look profitable.
