...
Add-Check "description advertises tightening-only stops" ($source.Contains('tightening-only protective stops')) "description"
...
Add-Check "wrapper selects exact ticket before request" ($modify.Contains('ticket == 0 || !PositionSelectByTicket(ticket)')) "preselect"
Add-Check "wrapper reads symbol type and current stop" (@('POSITION_SYMBOL','POSITION_TYPE','POSITION_SL').Where({ $modify.Contains($_) }).Count -eq 3) "position state"
Add-Check "wrapper requires existing and requested stops" ($modify.Contains('currentSL <= 0.0 || sl <= 0.0')) "nonzero protection"
Add-Check "wrapper requires executor magic ownership" ($modify.Contains('POSITION_MAGIC') -and $modify.Contains('executor.RequestMagic()')) "magic"
Add-Check "wrapper requires expert-owned position" ($modify.Contains('POSITION_REASON_EXPERT')) "expert reason"
...
Add-Check "wrapper reselects ticket after request" ([regex]::Matches($modify, 'PositionSelectByTicket\(ticket\)').Count -eq 2) "before and after"
