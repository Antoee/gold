[CmdletBinding()]
param(
   [string]$SourcePath = 'work\Professional_XAUUSD_Three_Lane_Momentum_D1_Extreme_Guard_Research.mq5',
   ...
   [string]$PackageDir = 'outputs\three_lane_momentum_d1_extreme_guard_discovery_model1_package',
   [string]$ManifestPath = 'outputs\THREE_LANE_MOMENTUM_D1_EXTREME_GUARD_DISCOVERY_MODEL1_MANIFEST.csv'
)
...
$expectedSourceHash = '6493A292B8126FD03596A0062BBC065144AEE949D63E55E7B4F10D8469989A11'
...
& (Join-Path $PSScriptRoot 'test_three_lane_momentum_d1_extreme_guard_source.ps1') | Out-Null
...
if($sourceHash -ne $expectedSourceHash) { throw "D1 momentum extreme-guard research source identity changed: $sourceHash" }
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_momentum_d1_extreme_guard_discovery_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='mde_control';Role='control';Enabled='false';MaximumPercent='100'},
   [pscustomobject]@{Name='mde_12';Role='lower_support';Enabled='true';MaximumPercent='12'},
   [pscustomobject]@{Name='mde_center_18';Role='center';Enabled='true';MaximumPercent='18'},
   [pscustomobject]@{Name='mde_24';Role='upper_support';Enabled='true';MaximumPercent='24'},
   [pscustomobject]@{Name='mde_30';Role='upper_boundary';Enabled='true';MaximumPercent='30'}
)
$windows = @(
...
$stopRule = 'Frozen pre-2021 D1 momentum extreme-guard discovery. Every row must parse exactly and remain profitable. The 18% center must be no worse than control in both disjoint eras; continuous net >=control +2%; CAGR >=control +0.04 point; PF/recovery/return-DD no worse than control; DD <=control +0.05 point; trades >=85% of control. At least two of the 12%, 24%, and 30% neighbors must remain positive in both eras, improve continuous net >=1%, CAGR >=control +0.02 point, retain >=98% of control PF/recovery/return-DD, DD <=control +0.08 point, retain >=80% of control trades, and change behavior. No post-result rescue.'
...
foreach($variant in $variants) {
   ...
   Set-FixedInput $inputs 'InpMOUseMaximumD1MomentumStrength' $variant.Enabled
   Set-FixedInput $inputs 'InpMOMaximumAbsoluteD1MomentumPercent' $variant.MaximumPercent
   ...
   foreach($window in $windows) {
      ...
      $rows.Add([pscustomobject][ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='momentum_d1_extreme_guard_discovery_model1'
         Window=$window.Name;From=$window.From;To=$window.To;Model=1;Deposit=10000
         MaximumD1MomentumStrengthEnabled=$variant.Enabled;MaximumAbsoluteD1MomentumPercent=$variant.MaximumPercent
...
@(
   '# Momentum D1 Extreme Guard Discovery Contract','',
   ...
   '- Frozen center: block a momentum breakout only when the absolute 126-bar D1 close return exceeds 18%; direction must still agree with the return sign.',
   '- Frozen support ladder: disabled control plus 12%, 18%, 24%, and 30% maximum strength. Requested risk, targets, stops, sizing, cooldown, and the 0.75% account-wide exposure cap are unchanged.',
