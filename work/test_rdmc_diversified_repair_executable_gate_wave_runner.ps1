...
try {
   ...
   foreach($token in @("assert_mt5_launch_allowed.ps1", "prepare_mt5_portable_shared_expert.ps1", "mt5_tick_cache_sync_helpers.ps1", "sync_mt5_portable_xauusd_tick_cache.ps1", "run_mt5_portable_parallel_manifest.ps1", "collect_rdmc_diversified_repair_executable_gate_results.ps1", "MaxParallelism", "currentWave", "PortableBinarySha256", "DISJOINT_THEN_SYNC_THEN_CONTINUOUS")) {
   ...
   $manifest = @(Import-Csv -LiteralPath (Join-Path $repo "outputs\RDMC_DIVERSIFIED_REPAIR_EXECUTABLE_GATE_MANIFEST.csv"))
