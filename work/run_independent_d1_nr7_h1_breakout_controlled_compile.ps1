...
$auditPath=Join-Path $repo 'outputs\INDEPENDENT_D1_NR7_H1_BREAKOUT_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'NR7 research source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"NR7 controlled compile $startedAtUtc",[Text.Encoding]::ASCII)
