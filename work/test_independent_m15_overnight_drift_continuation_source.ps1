...
if(!(Test-Path -LiteralPath $full -PathType Leaf)) { throw "M15 overnight-drift source missing: $full" }
...
foreach($token in $required) {
   if($text.IndexOf($token, [StringComparison]::Ordinal) -lt 0) { throw "Required M15 overnight-drift token missing: $token" }
}
$forbidden = @(
   'martingale',
   'averaging down',
   ...
   'OpenGapFadePosition',
   'TryWeekendGapEntry',
   'InpMaximumSpreadGapPercent'
)
foreach($token in $forbidden) {
   if($text.IndexOf($token, [StringComparison]::OrdinalIgnoreCase) -ge 0) { throw "Forbidden/stale M15 overnight-drift token present: $token" }
}
[pscustomobject]@{
   Status = "PASS"
