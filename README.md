...
**2026-07-18 active repair queue: no new best yet. Exact annual ledgers show that momentum produced all 32 trades in 2019 and `-$71.78` of the `-$92.78` loss in 2022. A weekday explanation was rejected because Wednesday is the strongest weekday over all 342 annual trades. An isolated completed-H1 signal-range gate compiled with `0 errors, 0 warnings`; its frozen `1.00 / 1.25 / 1.50 ATR` family is queued for only eight Model1 early-gate runs on 2019 and 2022. Exact report admission and preregistered stop-rule tooling now pass synthetic collector `4/4`, decision `7/7`, and package `28/28` checks. The real run plan remains `LOCKED` with zero reports, so no performance result or candidate change is being claimed.**
...
The annual failures are now decomposed in an exact 342-trade ledger. The 2019 loss is entirely momentum; in 2022, momentum lost `$71.78` and one reversion trade lost `$21.00`. All 14 Wednesday trades in those two years lost, but deleting Wednesday is rejected because the complete annual sample earns `+$427.11` at PF `1.99` on that weekday and the exclusion still leaves 2022 negative. The active repair therefore uses a causal completed-candle signal-range gate selected only from the one-to-one 2015-2018 telemetry/Model4 join. The `1.00`, `1.25`, and `1.50 ATR` rows remain positive in every selection year; the `1.25 ATR` midpoint is frozen as center. This differs from the earlier breakout-quality screen, which bundled weaker `0.35-0.65 ATR` range thresholds with body and close-location filters. The exact [collector](work/collect_rdmc_signal_range_gate_repair_results.ps1) rejects mismatched runner rows, profile bytes, package paths, and report-embedded source hashes. The [decision builder](work/build_rdmc_signal_range_gate_repair_decision.ps1) opens Model4 only if the center and at least one neighbor are profitable with at least 18 trades in both years and beat the control on combined net. The [diagnosis](outputs/RDMC_ANNUAL_FAILURE_DIAGNOSIS.md), [selection](outputs/RDMC_SIGNAL_RANGE_GATE_SELECTION.md), [contract](outputs/RDMC_SIGNAL_RANGE_GATE_REPAIR_CONTRACT.md), and [locked run plan](outputs/RDMC_SIGNAL_RANGE_GATE_REPAIR_HIDDEN_RUN_PLAN.md) explicitly record that no repair report or new best exists yet.
