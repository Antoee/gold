...
if($queue.Count -ne 12 -or $manifest.Count -ne 12) { throw "Expected six lookbacks across two windows." }
if(@($queue.Candidate | Sort-Object -Unique).Count -ne 6) { throw "Expected six unique lead-lookback variants." }
if(Compare-Object @('xmll_lead2','xmll_lead3','xmll_lead4','xmll_lead5','xmll_lead6','xmll_lead7') @($queue.Candidate | Sort-Object -Unique)) {
   throw "Lead-lookback neighborhood must be exactly 2 through 7 bars."
}
...
foreach($row in $manifest) {
   ...
   foreach($token in @('Expert=Professional_XAUUSD_EA.ex5','Model=1','Visual=0','InpReferenceSymbol=XAGUSD','InpRiskPercent=0.10','InpAllowRealAccountTrading=false','InpRequirePriorRangeReclaim=')) {
...
[pscustomobject]@{ Status="PASS"; Configurations=12; Candidates=6; Windows=2; LatestDate="2020-12-31"; RealTradingDefault=$false }
