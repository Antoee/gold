...
$source = Join-Path $repo 'outputs\three_lane_reversion_break_even_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath = Join-Path $repo 'outputs\THREE_LANE_REVERSION_BREAK_EVEN_DISCOVERY_COMPILE_AUDIT.csv'
$roots = @(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Reversion break-even source identity changed.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Reversion break-even discovery compile $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Reversion break-even discovery focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
