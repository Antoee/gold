...
$source = Join-Path $repo 'outputs\three_lane_reversion_liquidity_sweep_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
$expectedSourceHash = 'C5AB825B9F8BB701D02E4144E0062E79CC5A64FCECE04C6057C0A24755AD1A65'
...
$auditPath = Join-Path $repo 'outputs\THREE_LANE_REVERSION_LIQUIDITY_SWEEP_DISCOVERY_COMPILE_AUDIT.csv'
$roots = @(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Reversion liquidity-sweep source identity changed.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Reversion liquidity-sweep discovery compile $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Reversion liquidity-sweep discovery focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
