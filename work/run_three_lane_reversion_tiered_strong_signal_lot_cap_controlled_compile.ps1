...
$source=Join-Path $repo 'outputs\three_lane_reversion_tiered_strong_signal_lot_cap_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_REVERSION_TIERED_STRONG_SIGNAL_LOT_CAP_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'Tiered source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Tiered selective lot-cap compile $startedAtUtc",[Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck,"Tiered selective lot-cap focus acknowledgement $startedAtUtc",[Text.Encoding]::ASCII)
