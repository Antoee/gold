...
$source = Join-Path $repo 'outputs\independent_m15_dual_regime_normalized_stop_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath = Join-Path $repo 'outputs\INDEPENDENT_M15_DUAL_REGIME_NORMALIZED_STOP_COMPILE_AUDIT.csv'
$roots = @(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Dual-regime normalized-stop source identity changed.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Dual-regime normalized-stop compile $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Dual-regime normalized-stop focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
