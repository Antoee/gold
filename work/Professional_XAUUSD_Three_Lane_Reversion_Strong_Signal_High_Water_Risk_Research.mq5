...
#property description "Research fork with high-water throttled strong-signal reversion risk"
...
input bool   InpRVUseStrongSignalDrawdownThrottle = false;
input double InpRVStrongSignalFullRiskMaximumDrawdownPercent = 0.30;
...
bool CurrentPortfolioDrawdownPercent(double &drawdownPercent)
{
   ...
   if(equity <= 0.0 || peakEquity <= 0.0 || !MathIsValidNumber(equity) ||
      !MathIsValidNumber(peakEquity))
      return false;
   drawdownPercent = MathMax(0.0, 100.0 * (peakEquity - equity) / peakEquity);
   return MathIsValidNumber(drawdownPercent);
}
...
private:
   ...
   bool OpenPosition(const bool buy,
                     ...
                     const double portfolioDrawdownPercent)
   {
      ...
      eventReason += "; peak drawdown " + DoubleToString(portfolioDrawdownPercent, 3);
   ...
   void TryEntry()
   {
      ...
      double portfolioDrawdownPercent = 0.0;
      bool drawdownAvailable = CurrentPortfolioDrawdownPercent(portfolioDrawdownPercent);
      ...
      if(InpRVUseStrongSignalRisk && fullStrongRiskAllowed &&
      ...
      OpenPosition(buy, entryPrice, stopPrice, targetPrice, diEdge,
                   requestedRiskPercent, signalBodyRatio, portfolioDrawdownPercent);
   }
...
bool InputsValid()
{
   ...
   if(InpRVATRPeriod < 2 || InpRVADXPeriod < 2 || InpRVRSIPeriod < 2 ||
      ...
      (InpRVUseStrongSignalDrawdownThrottle &&
       (!InpRVUseStrongSignalRisk ||
        InpRVStrongSignalFullRiskMaximumDrawdownPercent < 0.0 ||
        InpRVStrongSignalFullRiskMaximumDrawdownPercent >
