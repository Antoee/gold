...
$source=Join-Path $repo 'outputs\three_lane_reversion_high_reward_lot_cap_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_REVERSION_HIGH_REWARD_LOT_CAP_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'High-reward source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"High-reward lot-cap compile $startedAtUtc",[Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck,"High-reward lot-cap focus acknowledgement $startedAtUtc",[Text.Encoding]::ASCII)
