param(
   ...
   [string]$PackageDir = "outputs\adaptive_volatility_portfolio_discovery_model1_package",
   [string]$QueueManifestPath = "outputs\ADAPTIVE_VOLATILITY_PORTFOLIO_DISCOVERY_MODEL1_QUEUE.csv",
   [string]$PackageManifestPath = "outputs\ADAPTIVE_VOLATILITY_PORTFOLIO_DISCOVERY_MODEL1_PACKAGE_MANIFEST.csv",
   [string]$MarkdownPath = "outputs\ADAPTIVE_VOLATILITY_PORTFOLIO_DISCOVERY_MODEL1_PACKAGE.md"
)
...
& (Join-Path $PSScriptRoot "test_adaptive_volatility_portfolio_source.ps1") | Out-Null
...
$variants = @(
   [pscustomobject]@{ Name="avp_fixed_control"; Enabled="false"; Timeframe="16385"; Atr="20"; Baseline="126"; Minimum="0.75"; Maximum="1.25" },
   [pscustomobject]@{ Name="avp_center"; Enabled="true"; Timeframe="16385"; Atr="20"; Baseline="126"; Minimum="0.75"; Maximum="1.25" },
   [pscustomobject]@{ Name="avp_bounds80_120"; Enabled="true"; Timeframe="16385"; Atr="20"; Baseline="126"; Minimum="0.80"; Maximum="1.20" },
   [pscustomobject]@{ Name="avp_bounds85_115"; Enabled="true"; Timeframe="16385"; Atr="20"; Baseline="126"; Minimum="0.85"; Maximum="1.15" },
   [pscustomobject]@{ Name="avp_baseline63"; Enabled="true"; Timeframe="16385"; Atr="20"; Baseline="63"; Minimum="0.75"; Maximum="1.25" },
   [pscustomobject]@{ Name="avp_baseline252"; Enabled="true"; Timeframe="16385"; Atr="20"; Baseline="252"; Minimum="0.75"; Maximum="1.25" },
   [pscustomobject]@{ Name="avp_downside_only"; Enabled="true"; Timeframe="16385"; Atr="20"; Baseline="126"; Minimum="0.75"; Maximum="1.00" },
   [pscustomobject]@{ Name="avp_upside_only"; Enabled="true"; Timeframe="16385"; Atr="20"; Baseline="126"; Minimum="1.00"; Maximum="1.25" }
)
$windows = @(
...
$stopRule = "Discovery only: both eras positive; continuous PF >= 1.35, trades >= 180, DD <= 5%; at least 5% control-relative net or return/DD improvement under the frozen quality tolerances; one adjacent adaptive pass."
foreach($variant in $variants) {
   $candidateRank++
   $inputs = Import-SetInputs -Path $baseProfileFull
   foreach($pair in @(
      @("InpPortfolioMagic","26071781"), @("InpRVMagicNumber","26071722"), @("InpMOMagicNumber","26071762"),
      @("InpUseAdaptiveVolatilityRisk",$variant.Enabled), @("InpVolatilityRiskTimeframe",$variant.Timeframe),
      @("InpVolatilityRiskATRPeriod",$variant.Atr), @("InpVolatilityRiskBaselineBars",$variant.Baseline),
      @("InpVolatilityRiskMinimumScale",$variant.Minimum), @("InpVolatilityRiskMaximumScale",$variant.Maximum),
      @("InpEvidenceSourceHash",$sourceHash), @("InpEvidenceRunLabel","adaptive_volatility_portfolio_discovery_model1"),
   ...
   foreach($window in $windows) {
      ...
      $queueRows.Add([pscustomobject]@{
         QueueRank=$rank; Candidate=$variant.Name; CandidateRank=$candidateRank; SourceType="adaptive_volatility_portfolio"
         ...
         AdaptiveEnabled=$variant.Enabled; VolatilityTimeframe=$variant.Timeframe; VolatilityATRPeriod=$variant.Atr
         BaselineBars=$variant.Baseline; MinimumScale=$variant.Minimum; MaximumScale=$variant.Maximum; StopRule=$stopRule
      }) | Out-Null
      $runRows.Add([pscustomobject]@{
         QueueRank=$rank; Candidate=$variant.Name; Phase="discovery_model1"
         PhaseLabel="Adaptive volatility portfolio discovery Model1"; Window=$window.Name; Model=1; Deposit=10000
...
@(
   "# Adaptive Volatility Portfolio Discovery Package", "",
   "Exact released entries/exits with a separately testable bounded inverse-volatility risk controller. No configuration includes data after 2020.", "",
