...
A new standalone multiscale time-series-momentum strategy produced a real 2015-2020 discovery plateau, then **failed the frozen future-like holdout**. Four adjacent profiles passed discovery with `+$314.47` to `+$402.59`, PF `1.20-1.50`, `198-444` trades, and only `0.60%-1.96%` drawdown. Their exact settings and hashes were frozen before opening post-2020 data. All four then lost in 2021-2023 (`-$74.79` to `-$206.87`, PF `0.76-0.85`), even though all four made money in 2024 through July 16, 2026 (`+$103.27` to `+$135.47`, PF `1.45-2.20`). The combined 2021-2026 PF was only `0.91-1.09`, so `0 / 4` profiles passed and Model4 was skipped. This is direct evidence that fitting or validating only on 2024-now cannot be expected to work automatically in the future. See `outputs/INDEPENDENT_MULTISCALE_MOMENTUM_HOLDOUT_DECISION.md`.
...
A standalone M15 compression failed-breakout trap was then compiled and screened, including a bounded liveness follow-up, and also **did not produce a new best**. All `84 / 84` Model 1 reports parsed using only 2015-2020. A coherent 16-bar fixed-R neighborhood was profitable in both disjoint eras at PF `1.26-1.45` and `0.58%-0.65%` drawdown, but every row had only `54` trades and annualized just `0.09%-0.16%`, below the frozen 60-trade activity floor. The only 60+ numeric gate row made `+$112.52` (`0.19%/yr`) continuously but only `+$0.76`, PF `1.00`, in 2019-2020; its neighboring exits lost in that era. The branch was rejected without weakening the rules, opening 2021-2026, or spending time on Model 4. See `outputs/INDEPENDENT_M15_FAILED_BREAKOUT_DECISION.md`.

A standalone XAUUSD/XAGUSD M15 relative-value lane was then built after a no-trading broker diagnostic proved synchronized silver history was usable throughout 2015-2020: yearly XAU/XAG alignment was `99.91%-100%`, with `100%` readiness for the frozen 32-bar feature lookback. The trading hypothesis still **did not produce a new best**. The corrected source compiled with `0 errors, 0 warnings`, all `32 / 32` Model 1 reports parsed across equal 2015-2017 and 2018-2020 eras, and every one of 16 nearby divergence, correlation, lookback, and payoff shapes lost in both eras. Profit factors were only `0.45-0.76`; `0 / 16` profiles were profitable in both eras and many reached the `5%` research drawdown stop. The fade-divergence family was rejected without continuous, post-2020, or Model 4 testing. See `outputs/INDEPENDENT_M15_XAG_RELATIVE_VALUE_DECISION.md`.

The distinct XAUUSD/XAGUSD M15 synchronized-continuation hypothesis was tested next and also **did not produce a new best**. It required positive rolling correlation, same-direction ATR-normalized moves in both metals, and a fresh closed-bar XAU channel breakout. The source compiled with `0 errors, 0 warnings`, and all `32 / 32` Model 1 reports parsed across equal 2015-2017 and 2018-2020 eras. Every one of 16 nearby move, breakout, correlation, buffer, and payoff shapes lost in both eras; the least-bad aggregate validation score was `-$257.45`, while the base lost `-$414.26`. Profit factors stayed below `1.00`, so `0 / 16` profiles passed the preregistered gate. Continuous, post-2020, and Model 4 testing were correctly skipped. See `outputs/INDEPENDENT_M15_XAG_SYNCHRONIZED_CONTINUATION_DECISION.md`.

A delayed XAG-lead/XAU-pullback family was then tested and initially looked more promising, but it also **did not produce a new best**. The first `32 / 32` Model 1 reports found one numeric pass: the four-bar lead made `+$42.08` (`0.14%/yr`), PF `1.13`, on `75` trades in 2015-2017 and `+$135.46` (`0.45%/yr`), PF `1.50`, on `76` trades in 2018-2020. A frozen 2-7 bar neighborhood added `12 / 12` reports and reproduced four bars exactly, but both immediate neighbors failed both eras: three bars scored `-$45.42` across the restart windows with minimum PF `0.90`, and five bars scored `-$59.30` with minimum PF `0.92`. The edge was isolated to one exact lookback, so the adjacency gate failed and continuous, post-2020, and Model 4 testing remained closed. See `outputs/INDEPENDENT_M15_XAG_LEAD_LAG_PULLBACK_DECISION.md`.

The strongest sparse M15 failed-breakout clue was then moved to native M5 bar geometry to test whether a lower timeframe could add usable activity. It **did not produce a new best**. The source compiled with `0 errors, 0 warnings`, and all `24 / 24` Model 1 reports parsed across equal 2015-2017 and 2018-2020 eras. Every frozen 14/16/18-bar structural and fixed-R shape lost in both eras; the least-bad aggregate validation score was `-$85.11`. No candidate reached PF `1.10` in both eras, and none reached the frozen 100-trade floor in both eras. The lower timeframe added noise rather than robust activity, so continuous, post-2020, and Model 4 testing were skipped. See `outputs/INDEPENDENT_M5_FAILED_BREAKOUT_DECISION.md`.
