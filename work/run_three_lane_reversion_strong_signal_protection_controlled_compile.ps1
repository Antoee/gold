...
$source = Join-Path $repo 'outputs\three_lane_reversion_strong_signal_protection_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath = Join-Path $repo 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_PROTECTION_MODEL1_COMPILE_AUDIT.csv'
$roots = @(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Strong-signal protection source identity changed.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Reversion strong-signal protection Model1 compile $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Reversion strong-signal protection Model1 focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
...
$audit = [pscustomobject][ordered]@{
   ...
   PortableBinarySha256 = $binaryHashes[0]
   Workers = $binaryRows.Count
   CompileErrors = 0
   CompileWarnings = 0
