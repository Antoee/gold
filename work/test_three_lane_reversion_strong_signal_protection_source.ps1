...
$forkPath = Join-Path $repo 'work\Professional_XAUUSD_Three_Lane_Reversion_Strong_Signal_Protection_Research.mq5'
$expectedBaseHash = '36300BA97B4384C1860ED7754495C5EFC74D2C75603BF0CDCD24BC31D9EAB1DF'
$expectedForkHash = '9810F048C62E93C217FB955DA51A4364055BF7806EFC739B37621E3B251F9206'
...
if($forkHash -ne $expectedForkHash) { throw "Strong-signal protection source identity changed: $forkHash" }
...
foreach($token in $required) {
   if($fork.IndexOf($token, [StringComparison]::Ordinal) -lt 0) {
      throw "Strong-signal protection source is missing required token: $token"
   }
}

$protection = [regex]::Match(
   $fork,
   'void ImproveStrongProtectiveStop\(const ulong ticket\)[\s\S]*?void ManageStrongPositionOnBar\(\)',
   [Text.RegularExpressions.RegexOptions]::CultureInvariant
)
if(!$protection.Success) { throw 'Strong-signal protection block could not be isolated.' }
...
[pscustomobject][ordered]@{
   ...
   SignalData = 'COMPLETED_H1_BODY_AND_OPEN_POSITION_PRICE_PROGRESS'
