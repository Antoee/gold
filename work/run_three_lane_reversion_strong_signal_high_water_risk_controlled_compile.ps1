...
$source = Join-Path $repo 'outputs\three_lane_reversion_strong_signal_high_water_risk_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath = Join-Path $repo 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_HIGH_WATER_RISK_COMPILE_AUDIT.csv'
$roots = @(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'High-water risk source identity changed.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "High-water risk compile $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "High-water risk focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
