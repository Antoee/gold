...
param(
   ...
   [string]$PackageDir = 'outputs\three_lane_momentum_same_side_exit_cooldown_recent_model1_package',
   [string]$ManifestPath = 'outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_RECENT_MODEL1_MANIFEST.csv'
)
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_momentum_same_side_exit_cooldown_recent_model1' -StringValue
...
$windows = @(
   [pscustomobject]@{Name='early_2021_2023';From='2021.01.01';To='2023.12.31'},
   [pscustomobject]@{Name='recent_2024_2026';From='2024.01.01';To='2026.07.12'},
   [pscustomobject]@{Name='continuous_2021_2026';From='2021.01.01';To='2026.07.12'}
)
$stopRule = 'Frozen 2021-2026 historical confirmation after the preregistered discovery pass. Every report must parse exactly. Center net must be no worse than control in both disjoint eras and strictly improve continuous net; continuous PF/recovery/return-DD must be no worse than control; DD <=control +0.03 point; trades >=98% of control. No post-result rescue.'
...
foreach($variant in $variants) {
   ...
   foreach($window in $windows) {
      ...
      $rows.Add([pscustomobject][ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='momentum_same_side_exit_cooldown_recent_model1'
...
@(
   '# Momentum Same-Side Exit Cooldown Recent Confirmation Contract','',
   '**Status: FROZEN 2021-2026 HISTORICAL CONFIRMATION. THE PUBLISHED LEADER AND FORWARD CANDIDATE ARE UNCHANGED.**','',
