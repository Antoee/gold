...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_PROFIT_RATCHET_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'Momentum profit-ratchet source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Momentum profit-ratchet compile $startedAtUtc",[Text.Encoding]::ASCII)
