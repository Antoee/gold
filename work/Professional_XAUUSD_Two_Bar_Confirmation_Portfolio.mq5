...
input ulong  InpPortfolioMagic = 26071784;
...
input group "Closed-Bar Two-Step Entry Confirmation"
input bool   InpRVUseTwoBarReclaimConfirmation = true;
input bool   InpRVRequirePriorCloseOutsideBand = false;
input bool   InpRVRequireConfirmationProgress = true;
input bool   InpMOUseTwoCloseBreakoutConfirmation = true;
input bool   InpMORequireConfirmationProgress = false;

input group "H1 Band VWAP Reversion Lane"
input bool   InpRVEnabled = true;
input ulong  InpRVMagicNumber = 26071725;
...
input ulong  InpMOMagicNumber = 26071765;
...
bool SharedSafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "TBC-LIVE-ACK-v1"))
   {
...
private:
   ...
   void TryEntry()
   {
      ...
      if(!BufferValue(m_atrHandle, 0, 1, atr) || atr <= 0.0 ||
         ...
         return;
      if(InpRVUseTwoBarReclaimConfirmation &&
         (!BufferValue(m_bandsHandle, 1, 2, upper2) ||
          !BufferValue(m_bandsHandle, 2, 2, lower2)))
         return;
      ...
      double high2 = iHigh(_Symbol, InpRVSignalTimeframe, 2);
      double low2 = iLow(_Symbol, InpRVSignalTimeframe, 2);
      double close2 = iClose(_Symbol, InpRVSignalTimeframe, 2);
      ...
      if(InpRVUseTwoBarReclaimConfirmation)
      {
         bool priorBuyExtension = low2 <= lower2 - penetration &&
                                  (!InpRVRequirePriorCloseOutsideBand || close2 <= lower2);
         bool priorSellExtension = high2 >= upper2 + penetration &&
                                   (!InpRVRequirePriorCloseOutsideBand || close2 >= upper2);
         buy = priorBuyExtension && close1 > lower && close1 > open1 &&
               lowerWickPercent >= InpRVMinimumWickPercent &&
               closeLocation >= minimumCloseLocation && rsi <= InpRVBuyMaximumRSI &&
               (!InpRVRequireConfirmationProgress || close1 > close2);
         sell = priorSellExtension && close1 < upper && close1 < open1 &&
                upperWickPercent >= InpRVMinimumWickPercent &&
                closeLocation <= 1.0 - minimumCloseLocation && rsi >= InpRVSellMinimumRSI &&
                (!InpRVRequireConfirmationProgress || close1 < close2);
      }
...
private:
   ...
   void TryEntry(const double atr)
   {
      ...
      if(InpMOUseTwoCloseBreakoutConfirmation)
      {
         if(!ChannelBounds(3, InpMOEntryLookbackBars, channelHigh, channelLow))
            return;
         buyBreak = InpMOAllowBuy && close2 > channelHigh + buffer && close1 > channelHigh + buffer &&
                    (!InpMORequireConfirmationProgress || close1 > close2);
         sellBreak = InpMOAllowSell && close2 < channelLow - buffer && close1 < channelLow - buffer &&
                     (!InpMORequireConfirmationProgress || close1 < close2);
      }
      else
      {
         if(!ChannelBounds(2, InpMOEntryLookbackBars, channelHigh, channelLow))
            return;
         buyBreak = InpMOAllowBuy && close1 > channelHigh + buffer;
         sellBreak = InpMOAllowSell && close1 < channelLow - buffer;
...
void UpdateDashboard()
{
   ...
   Comment("XAUUSD Two-Bar Confirmation Portfolio\n",
