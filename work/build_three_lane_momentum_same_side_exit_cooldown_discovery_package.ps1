...
param(
   ...
   [string]$PackageDir = 'outputs\three_lane_momentum_same_side_exit_cooldown_discovery_model1_package',
   [string]$ManifestPath = 'outputs\THREE_LANE_MOMENTUM_SAME_SIDE_EXIT_COOLDOWN_DISCOVERY_MODEL1_MANIFEST.csv'
)
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_momentum_same_side_exit_cooldown_discovery_model1' -StringValue
...
$variants = @(
   ...
   [pscustomobject]@{Name='msec_030';Role='lower_support';Enabled='true';CooldownMinutes='30'},
   [pscustomobject]@{Name='msec_center_060';Role='center';Enabled='true';CooldownMinutes='60'},
   [pscustomobject]@{Name='msec_090';Role='upper_support';Enabled='true';CooldownMinutes='90'},
   [pscustomobject]@{Name='msec_120';Role='upper_boundary';Enabled='true';CooldownMinutes='120'}
)
$windows = @(
...
$stopRule = 'Frozen pre-2021 same-side momentum exit-cooldown discovery. Every row must parse exactly and remain profitable. Center net must be no worse than control in both disjoint eras; continuous net >=control +1.5%; CAGR >=control +0.03 point; PF/recovery/return-DD no worse than control; DD no worse than control; trades >=97% of control. At least 2 of 3 enabled neighbors must be no worse in both eras, improve continuous net >=1%, CAGR >=control +0.01 point, retain 99% of PF/recovery/return-DD, have no worse DD, retain >=97% of control trades, and change behavior. No post-result rescue.'
...
foreach($variant in $variants) {
   ...
   foreach($window in $windows) {
      ...
      $rows.Add([pscustomobject][ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='momentum_same_side_exit_cooldown_discovery_model1'
...
@(
   '# Momentum Same-Side Exit Cooldown Discovery Contract','',
   ...
   '- Frozen support ladder: disabled control plus 30, 60, 90, and 120 elapsed minutes. Requested risk, targets, stops, sizing, and the 0.75% account-wide exposure cap are unchanged.',
