param(
   [string]$QueuePath = "outputs\MARKET_PHASE_PORTFOLIO_DISCOVERY_MODEL1_QUEUE.csv",
   [string]$ReportDir = "outputs\market_phase_portfolio_discovery_model1_package\reports_here",
   ...
   [string]$DecisionCsvPath = "outputs\MARKET_PHASE_PORTFOLIO_DISCOVERY_DECISION.csv",
   [string]$DecisionMarkdownPath = "outputs\MARKET_PHASE_PORTFOLIO_DISCOVERY_DECISION.md",
   [string]$MetricsPath = "outputs\MARKET_PHASE_PORTFOLIO_DISCOVERY_MODEL1_METRICS.md"
)
...
$rawResults = Join-Path $repo "work\MPP_RAW_RESULTS.csv"
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "collect_validation_results.ps1") `
   ...
   -OutResults "work\MPP_RAW_RESULTS.csv" -OutSummary "work\MPP_RAW_SUMMARY.csv" `
   -OutMarkdown "work\MPP_RAW_METRICS.md" | Out-Null
...
foreach($item in ($queue | Sort-Object { [int]$_.QueueRank })) {
   ...
   $identityPass = $reportText.IndexOf($sourceHash,[StringComparison]::OrdinalIgnoreCase) -ge 0 -and
                   $reportText.IndexOf("Closed-Bar Market Phase Allocation=",[StringComparison]::Ordinal) -ge 0
   ...
   $results.Add([pscustomobject]@{
      ...
      BaseProfileSha256=$item.BaseProfileSha256; PhaseEnabled=$item.PhaseEnabled
      PhaseTimeframe=$item.PhaseTimeframe; LookbackBars=$item.LookbackBars
      RangeEfficiency=$item.RangeEfficiency; TrendEfficiency=$item.TrendEfficiency
      HostileRiskScale=$item.HostileRiskScale; StopRule=$item.StopRule
...
$control = $candidateRows['mpp_fixed_control'].Continuous
...
$adjacency = @{
   mpp_fixed_control=@()
   mpp_center=@('mpp_scale75','mpp_scale25','mpp_lookback12','mpp_lookback48','mpp_narrow','mpp_wide')
   mpp_scale75=@('mpp_center','mpp_scale25'); mpp_scale25=@('mpp_center','mpp_scale75')
   mpp_lookback12=@('mpp_center'); mpp_lookback48=@('mpp_center')
   mpp_narrow=@('mpp_center','mpp_wide'); mpp_wide=@('mpp_center','mpp_narrow')
}
$basicPass = @{}; $qualityPass = @{}
foreach($candidate in $candidateRows.Keys) {
   $set = $candidateRows[$candidate]
   $basicPass[$candidate] = $candidate -ne 'mpp_fixed_control' -and
...
foreach($candidate in ($candidateRows.Keys | Sort-Object)) {
   ...
   $summary.Add([pscustomobject]@{
      Candidate=$candidate; PhaseEnabled=$set.Continuous.PhaseEnabled
      ...
      Decision=$(if($eligible){"DISCOVERY_ELIGIBLE"}elseif($candidate -eq 'mpp_fixed_control'){"CONTROL_ONLY"}else{"REJECT_BEFORE_HOLDOUT"})
   }) | Out-Null
}
...
$md.Add("# Market Phase Portfolio Discovery Decision")
$md.Add("")
$md.Add($(if($eligibleRows.Count -gt 0) { "**Decision: freeze eligible market-phase profiles for untouched holdout; Model 4 and promotion remain closed.**" } else { "**Decision: rejected in pre-2021 discovery; holdout, Model 4, and promotion remain closed.**" }))
$md.Add("")
$md.Add("The exact released Band/VWAP-reversion and E20 momentum entries/exits were retained. A closed-H1-bar efficiency-ratio controller could only reduce hostile-phase lane risk inside the existing 0.75% open-risk cap.")
$md.Add("")
...
$md.Add("- Eligible market-phase profiles: ``$($eligibleRows.Count) / 7``")
$md.Add("")
...
@(
   "# Market Phase Portfolio Discovery Metrics", "",
