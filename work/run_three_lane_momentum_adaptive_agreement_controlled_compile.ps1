...
$source=Join-Path $repo 'outputs\three_lane_momentum_adaptive_agreement_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_ADAPTIVE_AGREEMENT_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'Agreement research source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Momentum adaptive agreement compile $startedAtUtc",[Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck,"Momentum adaptive agreement focus acknowledgement $startedAtUtc",[Text.Encoding]::ASCII)
