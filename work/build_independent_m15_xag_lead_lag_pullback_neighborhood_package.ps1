param(
   ...
   [string]$PackageDir = "outputs\independent_m15_xag_lead_lag_pullback_neighborhood_model1_package",
   [string]$QueueManifestPath = "outputs\INDEPENDENT_M15_XAG_LEAD_LAG_PULLBACK_NEIGHBORHOOD_MODEL1_QUEUE.csv",
   [string]$PackageManifestPath = "outputs\INDEPENDENT_M15_XAG_LEAD_LAG_PULLBACK_NEIGHBORHOOD_MODEL1_PACKAGE_MANIFEST.csv",
   [string]$MarkdownPath = "outputs\INDEPENDENT_M15_XAG_LEAD_LAG_PULLBACK_NEIGHBORHOOD_MODEL1_PACKAGE.md"
)
...
$variants = @(
   (New-Variant "xmll_lead2" @{ Lead="2" }),
   (New-Variant "xmll_lead3" @{ Lead="3" }),
   (New-Variant "xmll_lead4" @{ Lead="4" }),
   (New-Variant "xmll_lead5" @{ Lead="5" }),
   (New-Variant "xmll_lead6" @{ Lead="6" }),
   (New-Variant "xmll_lead7" @{ Lead="7" })
)
$windows = @(
...
$rank = 0
$candidateRank = 0
$stopRule = "Frozen support gate: both eras must be profitable with PF >= 1.10, DD <= 5%, and >= 30 trades. Lead-4 must reproduce and at least one adjacent lead lookback (3 or 5) must pass before continuous testing; recent years remain closed."
foreach($variant in $variants) {
   ...
   Set-InputLine -Inputs $inputs -Name "InpEvidenceRunLabel" -Value "independent_m15_xag_lead_lag_pullback_neighborhood_model1"
   ...
   foreach($window in $windows) {
      ...
      $queueRows.Add([pscustomobject]@{
         ...
         SourceType="independent_m15_xag_lead_lag_pullback"; SourceRank=1; Phase="neighborhood_model1"
      ...
      $runRows.Add([pscustomobject]@{
         QueueRank=$rank; Candidate=$variant.Name; Phase="neighborhood_model1"
         PhaseLabel="Independent M15 XAG lead-lag lookback neighborhood Model1"; Window=$window.Name
