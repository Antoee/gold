...
param(
   ...
   [string]$PackageDir = 'outputs\three_lane_reversion_strong_reward_quality_model1_package',
   [string]$QueuePath = 'outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL1_QUEUE.csv',
   [string]$ManifestPath = 'outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL1_MANIFEST.csv',
   [string]$PackageMarkdownPath = 'outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL1_PACKAGE.md',
   [string]$ContractPath = 'outputs\THREE_LANE_REVERSION_STRONG_REWARD_QUALITY_MODEL1_CONTRACT.md'
)
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_reversion_strong_reward_quality_model1' -StringValue
...
$variants = @(
   ...
   [pscustomobject]@{Name='rvsrq_unfiltered';Role='diagnostic';Enabled='true';Quality='false';MinimumRR='1.20'},
...
foreach($variant in $variants) {
   ...
   foreach($window in $windows) {
      ...
      $common = [ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='reversion_strong_reward_quality_model1'
...
@(
   '# Three-Lane Reversion Strong-Reward Quality Model 1 Package','',
...
@(
   '# Reversion Strong-Reward Quality Model 1 Contract','',
   ...
   '- Freeze completed-H1 directional body ratio at 0.25 and requested strong risk at 0.70%. Test only disabled control, unfiltered diagnostic, and adjusted reward/risk thresholds 1.35, 1.50, and 1.65.',
