...
#property description "Post-hoc diversified XAUUSD research portfolio; tester-only until independently validated."
...
input double          InpPrimaryLaneRiskMultiplier = 1.00;
...
input bool            InpBandVWAPReversionUseD1MomentumCap = false;
input int             InpBandVWAPReversionD1MomentumLookbackBars = 126;
input double          InpBandVWAPReversionMaxAbsoluteD1MomentumPercent = 12.0;

input group "Independent Multiscale Momentum Research Lane"
input bool            InpMOEnabled = false;
input long            InpMOMagicNumber = 26071761;
...
private:
   ...
   double PeriodProfit(const ENUM_TIMEFRAMES period)
   {
      ...
      for(int i = 0; i < total; i++)
      {
         ...
         if(HistoryDealGetString(ticket, DEAL_SYMBOL) != _Symbol)
            continue;
         if(!IsResearchPortfolioMagic(HistoryDealGetInteger(ticket, DEAL_MAGIC)))
            continue;
   ...
   double OpenBasketProfit(int &positions)
   {
      ...
      for(int i = PositionsTotal() - 1; i >= 0; i--)
      {
         ...
         if(PositionGetString(POSITION_SYMBOL) != _Symbol ||
            !IsResearchPortfolioMagic(PositionGetInteger(POSITION_MAGIC)))
            continue;
   ...
   double PositionRiskMoney(const ulong ticket, bool &unprotected)
   {
      ...
      if(PositionGetString(POSITION_SYMBOL) != _Symbol)
         return 0.0;
      if(!IsResearchPortfolioMagic(PositionGetInteger(POSITION_MAGIC)))
         return 0.0;
...
public:
   ...
   SSignal BuildBandVWAPReversion()
   {
      ...
      if(InpBandVWAPReversionUseD1MomentumCap)
      {
         int momentumLookback = MathMax(20, InpBandVWAPReversionD1MomentumLookbackBars);
         double recentD1Close = iClose(_Symbol, PERIOD_D1, 1);
         double pastD1Close = iClose(_Symbol, PERIOD_D1, 1 + momentumLookback);
         if(recentD1Close <= 0.0 || pastD1Close <= 0.0)
            return signal;
         double absoluteMomentumPercent = 100.0 * MathAbs(recentD1Close - pastD1Close) / pastD1Close;
         if(absoluteMomentumPercent > MathMax(0.0, InpBandVWAPReversionMaxAbsoluteD1MomentumPercent))
            return signal;
      }
...
private:
   ...
   bool SessionAllows()
   {
      if(!sessionFilter.TradingDayAllowed())
         return false;
      MqlDateTime now;
   ...
   double ClosedProfitSince(const datetime fromTime)
   {
      ...
      for(int i = 0; i < HistoryDealsTotal(); ++i)
      {
         ...
         if(ticket == 0 || HistoryDealGetString(ticket, DEAL_SYMBOL) != _Symbol ||
            HistoryDealGetInteger(ticket, DEAL_MAGIC) != InpMOMagicNumber)
            continue;
   ...
   {
      if(!TradeEnvironmentAllows(reason))
         return false;
      if(!riskManager.CanOpen(reason))
         return false;
      if(WeekendCloseWindowActive())
      {
         reason = "momentum weekend close";
         return false;
      }
      if(newsFilter.IsBlocked())
      {
         reason = "momentum manual news filter";
         return false;
      }
      datetime dayStart = CurrentDayStart();
      double dailyProfit = ClosedProfitSince(dayStart);
      ...
      if(InpMOMaximumTradesPerDay > 0 && EntriesSince(dayStart) >= InpMOMaximumTradesPerDay)
      {
      ...
      if(CLogger::SpreadPoints() > InpMOMaximumSpreadPoints)
      {
   ...
   int MomentumDirection()
   {
      ...
      double pastClose = iClose(_Symbol, InpMOMomentumTimeframe, 1 + InpMOMomentumLookbackBars);
   ...
   {
      MqlTick tick;
      if(!SymbolInfoTick(_Symbol, tick) || atr <= 0.0 || InpRiskPercent <= 0.0)
         return false;
      ...
      if(stopDistance <= 0.0 || stopDistance > InpMOMaximumStopATR * atr ||
         (InpMOMaximumStopPriceDistance > 0.0 && stopDistance > InpMOMaximumStopPriceDistance))
         return false;
      ...
      ENUM_TRADE_BIAS bias = buy ? BIAS_BUY : BIAS_SELL;
      double riskMultiplier = InpMORiskPercent / InpRiskPercent;
      double lots = riskManager.LotsForRisk(bias, entryPrice, stopDistance, riskMultiplier);
...
public:
   ...
   bool Init()
   {
      ...
      if(InpMOMagicNumber == InpMagicNumber || InpMORiskPercent <= 0.0 || InpRiskPercent <= 0.0 ||
         ...
         InpMOTakeProfitR <= 0.0 || InpMOMaximumHoldBars < 0 || InpMOMaximumPositionLots <= 0.0 ||
   ...
   void OnTick()
   {
      ...
      double atr = 0.0;
      if(!IndicatorBufferValue(m_atrHandle, 0, 1, atr) || atr <= 0.0)
         return;
   ...
   void OnTradeTransaction(const MqlTradeTransaction &transaction)
   {
      ...
      if(HistoryDealGetString(transaction.deal, DEAL_SYMBOL) != _Symbol ||
         HistoryDealGetInteger(transaction.deal, DEAL_MAGIC) != InpMOMagicNumber)
         return;
      ...
      long positionId = HistoryDealGetInteger(transaction.deal, DEAL_POSITION_ID);
      ulong analyticsTicket = positionId > 0 ? (ulong)positionId : transaction.deal;
      logger.Write("momentum_exit", analyticsTicket, "close",
...
bool OpenSignal(const SSignal &signal)
{
   ...
   if(!signal.isBandVWAPReversion && !signal.isDailyDonchianBreakout)
...
bool ResearchCapitalContractAllows()
{
   ...
   {
      Print("Research safety lock failed: this diversified candidate is tester-only until independently validated.");
      return false;
   }

   if(InpUseAccountCurrencyLock && AccountInfoString(ACCOUNT_CURRENCY) != InpRequiredAccountCurrency)
   {
      Print("Account currency contract failed: expected ", InpRequiredAccountCurrency,
            " but found ", AccountInfoString(ACCOUNT_CURRENCY), ".");
      return false;
   }

   if(InpUseInitialBalanceContract)
   {
      double expected = InpExpectedInitialBalance;
      double tolerance = expected * MathMax(0.0, InpInitialBalanceTolerancePercent) / 100.0;
      double balance = AccountInfoDouble(ACCOUNT_BALANCE);
      if(expected <= 0.0 || MathAbs(balance - expected) > tolerance)
      {
         Print("Initial-balance contract failed: expected ", DoubleToString(expected, 2),
               " +/- ", DoubleToString(tolerance, 2), " but found ", DoubleToString(balance, 2), ".");
         return false;
      }
   }
   return true;
...
int OnInit()
{
   ...
   if(!g_momentum.Init())
   {
      Print("Failed to initialize the multiscale momentum lane.");
      indicators.Release();
      logger.Close();
      return INIT_PARAMETERS_INCORRECT;
   }
