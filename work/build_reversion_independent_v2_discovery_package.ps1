param(
   ...
   [string]$PackageManifestPath = "outputs\REVERSION_INDEPENDENT_V2_DISCOVERY_MODEL1_PACKAGE_MANIFEST.csv",
   [string]$MarkdownPath = "outputs\REVERSION_INDEPENDENT_V2_DISCOVERY_MODEL1_PACKAGE.md"
)
...
& (Join-Path $PSScriptRoot "test_reversion_independent_v2_source.ps1") -SourcePath $SourcePath | Out-Null
...
$variants = @(
   [pscustomobject]@{ Name="ri2_control"; Independent="false"; DIEdge="-12"; BandRisk="0.10"; MaxPositions="1" },
   [pscustomobject]@{ Name="ri2_m12_r30"; Independent="true"; DIEdge="-12"; BandRisk="0.30"; MaxPositions="2" },
   [pscustomobject]@{ Name="ri2_m12_r40"; Independent="true"; DIEdge="-12"; BandRisk="0.40"; MaxPositions="2" },
   [pscustomobject]@{ Name="ri2_m10_r30"; Independent="true"; DIEdge="-10"; BandRisk="0.30"; MaxPositions="2" },
   [pscustomobject]@{ Name="ri2_m10_r40"; Independent="true"; DIEdge="-10"; BandRisk="0.40"; MaxPositions="2" }
)
$windows = @(
...
$rank = 0
$candidateRank = 0
$stopRule = "Corrected discovery only through 2020: require exact control compatibility, each candidate broad-era net at least control, no active annual result worse than control, continuous net above control, PF at least 3.00, DD at or below 1.00%, and neighboring DI/risk support before any retrospective 2021-2026 implementation check."
foreach($variant in $variants) {
   ...
   Set-InputLine -Inputs $inputs -Name "InpEvidenceRunLabel" -Value "reversion_independent_v2_discovery_model1"
   ...
   foreach($window in $windows) {
      ...
      $queueRows.Add([pscustomobject]@{
         ...
         SourceType="reversion_independent_v2"; SourceRank=1; Phase="corrected_discovery_model1"
      ...
      $runRows.Add([pscustomobject]@{
         QueueRank=$rank; Candidate=$variant.Name; Phase="corrected_discovery_model1"
         PhaseLabel="Corrected reversion independent scheduling discovery Model1"; Window=$window.Name; Model=1; Deposit=10000
...
$md.Add("# Corrected Reversion Independent Scheduling Discovery Package")
$md.Add("")
$md.Add("Tests whether the already-validated H1 reversion lane should receive an independent entry attempt while the primary and reversion lanes retain separate one-position slots. No configuration includes data after 2020.")
$md.Add("")
...
$md.Add("")
$md.Add('Candidate profiles retain the `$0.75%` open-risk cap, permit one primary plus one H1 reversion position, size through broker-native risk calculation, keep the scheduling switch default off in source, and keep real-account trading disabled.')
