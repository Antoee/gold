...
private:
   ...
   string FeatureTelemetry(const bool buy,
                           ...
                           const double atr)
   {
      double open1 = iOpen(_Symbol, InpMOSignalTimeframe, 1);
      double high1 = iHigh(_Symbol, InpMOSignalTimeframe, 1);
      double low1 = iLow(_Symbol, InpMOSignalTimeframe, 1);
      double range1 = high1 - low1;
      ...
      double closeLocation = range1 > 0.0
                             ? (buy ? (close1 - low1) / range1
                                    : (high1 - close1) / range1)
                             : -1.0;
      double breakoutDistance = buy ? close1 - channelHigh : channelLow - close1;
      double recentDailyClose = iClose(_Symbol, InpMOMomentumTimeframe, 1);
      double pastDailyClose = iClose(_Symbol, InpMOMomentumTimeframe,
                                     1 + InpMOMomentumLookbackBars);
      double dailyMomentumPercent = pastDailyClose > 0.0
                                    ? 100.0 * MathAbs(recentDailyClose - pastDailyClose) /
                                      pastDailyClose
                                    : -1.0;
      double volumeSum = 0.0;
      int volumeCount = 0;
      for(int shift = 2; shift < 2 + InpMOEntryLookbackBars; ++shift)
      {
         long volume = iVolume(_Symbol, InpMOSignalTimeframe, shift);
         if(volume > 0)
         {
      ...
      double currentVolume = (double)iVolume(_Symbol, InpMOSignalTimeframe, 1);
      double volumeRatio = volumeCount > 0 && volumeSum > 0.0
                           ? currentVolume / (volumeSum / volumeCount)
                           : -1.0;
      return StringFormat("channel_width_atr=%.6f;breakout_atr=%.6f;h1_efficiency=%.6f;d1_efficiency=%.6f;d1_momentum_pct=%.6f;atr_pct=%.6f;body_ratio=%.6f;close_location=%.6f;range_atr=%.6f;volume_ratio=%.6f",
                          atr > 0.0 ? (channelHigh - channelLow) / atr : -1.0,
                          atr > 0.0 ? breakoutDistance / atr : -1.0,
                          EfficiencyRatio(InpMOSignalTimeframe, 1,
                                          InpMOEntryLookbackBars),
                          EfficiencyRatio(InpMOMomentumTimeframe, 1,
                                          InpMOMomentumLookbackBars),
                          dailyMomentumPercent,
                     ...
                     const string featureTelemetry)
   {
      ...
      string entryReason = "daily momentum plus fresh H1 breakout;" +
                           featureTelemetry +
                           StringFormat(";stop_atr=%.6f", stopDistance / atr);
   ...
   void TryEntry(const double atr)
   {
      ...
      if(buyBreak && RegimeAllows(true, close1, atr))
         OpenPosition(true, atr,
                      FeatureTelemetry(true, close1, channelHigh, channelLow, atr));
      else if(sellBreak && RegimeAllows(false, close1, atr))
         OpenPosition(false, atr,
                      FeatureTelemetry(false, close1, channelHigh, channelLow, atr));
   }
