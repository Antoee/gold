...
$momentum=@($window|Where-Object Lane -eq 'momentum')
$features=foreach($trade in $momentum){
...
$portfolioNet=Sum-Profit $window;$momentumNet=Sum-Profit $momentum
$rows=foreach($threshold in 1440,2160,2880,4320){
   $blocked=@($features|Where-Object AgeMinutes -le $threshold)
   $wins=Sum-Profit @($blocked|Where-Object Profit -gt 0)
   $losses=Sum-Profit @($blocked|Where-Object Profit -lt 0)
   $blockedNet=$wins+$losses
   [pscustomobject][ordered]@{
      ...
      MomentumControlTrades=$momentum.Count;MomentumControlNet=[Math]::Round($momentumNet,2)
