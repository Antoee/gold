...
$forkPath = Join-Path $repo 'work\Professional_XAUUSD_Three_Lane_Momentum_D1_Strength_Gate_Research.mq5'
$expectedBaseHash = 'B6810B305549968E2273DAAF736A63759FE5C16F3B416F5C69E39840FBE5173E'
$expectedForkHash = 'F405E62145EB3353D26A6EEC9095505AD3E674CB3FB5E4C1F58F0049AE4117D8'
...
if($forkHash -ne $expectedForkHash) { throw "D1-strength source identity changed: $forkHash" }
$base = Get-Content -LiteralPath $basePath -Raw
$fork = Get-Content -LiteralPath $forkPath -Raw

foreach($token in @(
   '#property version   "1.66"', 'InpMOUseMinimumD1MomentumStrength = false',
   'InpMOMinimumAbsoluteD1MomentumPercent = 2.0',
   ...
   if($fork.IndexOf($token, [StringComparison]::Ordinal) -lt 0) {
      throw "D1-strength source is missing required token: $token"
   }
}
...
if(!$strengthBlock.Success) { throw 'D1-strength regime block could not be isolated.' }
foreach($forbidden in @('DEAL_PROFIT','DEAL_SWAP','DEAL_COMMISSION','consecutive','drawdown','ACCOUNT_PROFIT')) {
   if($strengthBlock.Value.IndexOf($forbidden, [StringComparison]::OrdinalIgnoreCase) -ge 0) {
      throw "Outcome or account-performance token found in D1-strength block: $forbidden"
   }
}
...
[pscustomobject][ordered]@{
   ...
   StrengthState='ABSOLUTE_126_BAR_D1_CLOSE_RETURN';OutcomeIndependent=$true
   NewTradePaths=0;NewClosePaths=0;NewModifyPaths=0;FrozenCenterPercent=4.0
