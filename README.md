...
The promoted default is now the robust no-date BOS + liquidity-sweep profile at `1.50%` risk. It sacrifices raw historical profit to improve start-date robustness and remove hard-coded calendar blocks.

Latest promoted real-tick validation, XAUUSD M15, deposit `$1,000`, period `2024.01.01` to `2026.07.02`:

- Full period net profit: `+$452.75`
- Final balance: `$1,452.75`
- Yearly windows: `2024 +$176.99`, `2025 +$245.12`, `2026 YTD $0.00`
- Half-year windows: one losing window, `2024 H1 -$149.76`
- Quarterly windows: `+$487.46`, 2 profitable, 7 flat, 1 losing
- Monthly windows: `+$487.46`, 2 profitable, 27 flat, 1 losing
- Worst month/quarter: `-$149.76`

A `2.00%` risk variant had zero losing monthly/quarterly/split windows, but lower full-period profit at `+$321.72`. The `1.50%` profile is promoted as the better profit/robustness tradeoff.
...
- `BOS_SWEEP_WINDOWS_risk1p5.csv` - promoted robust profile quarterly/monthly windows.
- `BOS_SWEEP_WINDOW_SUMMARY_risk1p5.csv` - promoted robust profile quarterly/monthly summary.
- `BOS_SWEEP_SPLITS_risk1p5.csv` - promoted robust profile yearly/half/full windows.
- `BOS_SWEEP_SPLIT_SUMMARY_risk1p5.csv` - promoted robust profile split summary.
- `BOS_SWEEP_WINDOWS_risk1.csv` - previous robust profile quarterly/monthly windows.
- `BOS_SWEEP_WINDOW_SUMMARY_risk1.csv` - previous robust profile quarterly/monthly summary.
- `BOS_SWEEP_SPLITS_risk1.csv` - previous robust profile yearly/half/full windows.
- `BOS_SWEEP_SPLIT_SUMMARY_risk1.csv` - previous robust profile split summary.
...
- Robust BOS+sweep at `1.50%` risk: full period `+$452.75`, monthly `+$487.46`, 1 losing month, promoted default.
- Robust BOS+sweep at `2.00%` risk: full period `+$321.72`, 0 losing monthly/quarterly/split windows, safer but lower profit.
- Robust BOS+sweep at `1.00%` risk: full period `+$319.26`, monthly `+$314.76`, 1 losing month, previous default.
