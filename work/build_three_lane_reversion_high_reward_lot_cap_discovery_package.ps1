[CmdletBinding()]
param(
   [string]$SourcePath = 'work\Professional_XAUUSD_Three_Lane_Reversion_High_Reward_Lot_Cap_Research.mq5',
   ...
   [string]$PackageDir = 'outputs\three_lane_reversion_high_reward_lot_cap_discovery_model1_package',
   [string]$ManifestPath = 'outputs\THREE_LANE_REVERSION_HIGH_REWARD_LOT_CAP_DISCOVERY_MODEL1_MANIFEST.csv'
)
...
$expectedSourceHash = '3CB0574945CD4A7A486408EF5BCE58648392383C43BFEE6EB1F58B424698302F'
...
& (Join-Path $PSScriptRoot 'test_three_lane_reversion_high_reward_lot_cap_source.ps1') | Out-Null
...
if($sourceHash -ne $expectedSourceHash) { throw "High-reward source identity changed: $sourceHash" }
...
foreach($line in Get-Content -LiteralPath $leader) {
   ...
   $name = $Matches[1]
   if(!$base.Contains($name)) { throw "Leader input missing from high-reward source: $name" }
   $base[$name] = $line
   $leaderCount++
}
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_reversion_high_reward_lot_cap_discovery_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='hrlc_control';Role='control';StrongCap='0.15';HighReward='false';Threshold='2.50';HighRewardCap='0.20'},
   [pscustomobject]@{Name='hrlc_strong020_reference';Role='broad_reference';StrongCap='0.20';HighReward='false';Threshold='2.50';HighRewardCap='0.20'},
   [pscustomobject]@{Name='hrlc_rr200';Role='threshold_lower_neighbor';StrongCap='0.15';HighReward='true';Threshold='2.00';HighRewardCap='0.20'},
   [pscustomobject]@{Name='hrlc_center_rr250_cap020';Role='center';StrongCap='0.15';HighReward='true';Threshold='2.50';HighRewardCap='0.20'},
   [pscustomobject]@{Name='hrlc_rr300';Role='threshold_upper_neighbor';StrongCap='0.15';HighReward='true';Threshold='3.00';HighRewardCap='0.20'},
   [pscustomobject]@{Name='hrlc_cap018';Role='cap_lower_neighbor';StrongCap='0.15';HighReward='true';Threshold='2.50';HighRewardCap='0.18'},
   [pscustomobject]@{Name='hrlc_cap022';Role='cap_upper_neighbor';StrongCap='0.15';HighReward='true';Threshold='2.50';HighRewardCap='0.22'}
)
$windows = @(
...
$stopRule = 'Sealed pre-2021 discovery. Every report positive. Center no worse than control in both eras; continuous net >= control +4%; CAGR >= control +0.06 point; PF/recovery/return-DD >= control; DD <=1.18% and <=control +0.08 point; trades >=control -2. Center retains >=50% of broad-reference increment and improves DD/recovery/return-DD versus the reference. At least 3 of 4 orthogonal neighbors pass both eras, >=2.5% net improvement, >=97% efficiency, DD <=1.22%, and changed behavior. No post-result rescue.'
...
foreach($variant in $variants) {
   ...
   Set-FixedInput $inputs 'InpRVUseHighRewardLotCap' $variant.HighReward
   Set-FixedInput $inputs 'InpRVHighRewardMinimumRiskReward' $variant.Threshold
   Set-FixedInput $inputs 'InpRVHighRewardMaximumPositionLots' $variant.HighRewardCap
   ...
   foreach($window in $windows) {
      ...
      $rows.Add([pscustomobject][ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Role=$variant.Role;Phase='high_reward_lot_cap_discovery_model1'
         Window=$window.Name;From=$window.From;To=$window.To;Model=1;Deposit=10000;StrongSignalMinimumBodyRatio='0.25'
         StrongSignalMaximumPositionLots=$variant.StrongCap;HighRewardLotCapEnabled=$variant.HighReward
         HighRewardMinimumRiskReward=$variant.Threshold;HighRewardMaximumPositionLots=$variant.HighRewardCap
...
@(
   '# High-Reward Strong-Signal Reversion Lot-Cap Discovery Contract','',
   ...
   '- Frozen center: completed-H1 body ratio >= 0.25 plus spread-adjusted entry RR >= 2.50 raises only the reversion lot ceiling from 0.15 to 0.20; requested risk remains 0.45%.',
   '- Frozen neighborhood: adjusted-RR thresholds 2.00/3.00 and high-reward caps 0.18/0.22, plus exact control and broad strong-signal 0.20 reference.',
