...
#property description "Research fork: default-off protected trend runners"
...
input bool   InpMOUseProtectedRunner = false;
input double InpMORunnerTakeProfitR = 4.00;
input double InpMORunnerLockTriggerR = 1.50;
input double InpMORunnerLockR = 0.75;
...
input bool   InpATBUseProtectedRunner = false;
input double InpATBRunnerTakeProfitR = 4.00;
input double InpATBRunnerLockTriggerR = 1.50;
input double InpATBRunnerLockR = 0.75;
...
private:
   ...
   void ImproveProtectiveStop(const ulong ticket, const bool buy)
   {
      ...
      {
         double runnerLock = buy ? openPrice + InpMORunnerLockR * initialRisk
                                 : openPrice - InpMORunnerLockR * initialRisk;
...
private:
   ...
   void ImproveProtectiveStop(const ulong ticket, const bool buy)
   {
      ...
      {
         double runnerLock = buy ? openPrice + InpATBRunnerLockR * initialRisk
                                 : openPrice - InpATBRunnerLockR * initialRisk;
         if((buy && runnerLock > newSl) || (!buy && runnerLock < newSl))
            newSl = runnerLock;
      }
      newSl = NormalizeDouble(newSl, _Digits);
...
bool InputsValid()
{
   ...
   if(InpMOMomentumLookbackBars < 20 || InpMOEntryLookbackBars < 3 ||
      ...
      InpMOATRPeriod < 2 || InpMOStopBufferATR < 0.0 ||
       ...
       (InpMOUseProtectedRunner &&
        (InpMORunnerTakeProfitR <= InpMORunnerLockTriggerR ||
         InpMORunnerLockTriggerR <= 0.0 || InpMORunnerLockR < 0.0 ||
         InpMORunnerLockR >= InpMORunnerLockTriggerR)) ||
   ...
   if(InpATBMomentumLookbackBars < 20 || InpATBEntryLookbackBars < 3 ||
      ...
      InpATBTickVolumeLookbackBars < 2 || InpATBMinimumTickVolumeRatio < 0.0 ||
       ...
       InpATBTakeProfitR <= 0.0 ||
       (InpATBUseProtectedRunner &&
        (InpATBRunnerTakeProfitR <= InpATBRunnerLockTriggerR ||
         InpATBRunnerLockTriggerR <= 0.0 || InpATBRunnerLockR < 0.0 ||
         InpATBRunnerLockR >= InpATBRunnerLockTriggerR)) ||
       InpATBMaximumHoldBars < 0 ||
