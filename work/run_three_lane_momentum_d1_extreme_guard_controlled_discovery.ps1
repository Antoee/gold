...
if(!$UserAuthorizedFocusRisk) { throw 'Controlled D1 momentum extreme-guard testing requires explicit focus/window-risk authorization.' }
...
$manifest = Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_D1_EXTREME_GUARD_DISCOVERY_MODEL1_MANIFEST.csv'
$source = Join-Path $repo 'outputs\three_lane_momentum_d1_extreme_guard_discovery_model1_package\source\Professional_XAUUSD_EA.mq5'
$expectedManifestHash = '0FB09416715B50FE58FEEEFB5CB1B8A6E5B00F6B4EFB3EDE03968EF327E21886'
$expectedSourceHash = '6493A292B8126FD03596A0062BBC065144AEE949D63E55E7B4F10D8469989A11'
$expectedBinaryHash = 'FA394FDB8767BD7E4549FA5D05920EF0A548EE7A39E65A1119873E4711977C63'
$roots = @(
...
foreach($required in @($repoLock,$outerLock,$runner,$manifest,$source) + $roots) {
   if(!(Test-Path -LiteralPath $required)) { throw "Controlled D1 momentum extreme-guard prerequisite missing: $required" }
}
if((Get-FileHash -LiteralPath $manifest -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedManifestHash) {
   throw 'D1 momentum extreme-guard manifest identity changed.'
}
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'D1 momentum extreme-guard package source identity changed.'
}
...
if((Test-Path -LiteralPath $unlockFile) -or (Test-Path -LiteralPath $focusAck) -or
   ...
   throw 'Controlled D1 momentum extreme-guard testing refuses a pre-existing partial unlock state.'
}
if(@(Get-Process -Name terminal,terminal64,metatester,metatester64,MetaEditor,metaeditor64 -ErrorAction SilentlyContinue).Count -ne 0) {
   throw 'Controlled D1 momentum extreme-guard testing requires zero pre-existing MT5-family processes.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "D1 momentum extreme-guard discovery $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "D1 momentum extreme-guard focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
   ...
   & $runner -ManifestPath $manifest -PortableRoots $roots -UserAuthorizedFocusRisk `
      -OutputPrefix $(if($SingleWorkerRecovery) { 'THREE_LANE_MOMENTUM_D1_EXTREME_GUARD_RECOVERY_WORKER' } else { 'THREE_LANE_MOMENTUM_D1_EXTREME_GUARD_WORKER' }) `
...
if(!$completed) { throw 'Controlled D1 momentum extreme-guard discovery did not complete.' }
if(!(Test-Path -LiteralPath $repoLock) -or !(Test-Path -LiteralPath $outerLock) -or
   ...
   throw 'Controlled D1 momentum extreme-guard discovery did not restore hard-lock state.'
}
