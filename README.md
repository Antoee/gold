...
**2026-07-18 restart-safety update: still no new best. Static review found that the uncompiled combined `v1` source compared current balance with the original `$10,000` deposit on every initialization and reset its lifetime peak-equity baseline. That could stop a valid account after its first closed trade or weaken the `5%` drawdown guard after a restart. The separately identified tester-only `v2` package now persists starting capital, funding count, and lifetime peak equity; it fails closed on funding drift, foreign activity, missing state, or an invalid peak while allowing ordinary profit/loss restarts. Its deterministic package suite passes `74/74` and the nominated-source MQL preflight passes `37/37` with all `588` inputs frozen. Compilation and every MT5 run remain locked and unclaimed. See the [restart-safe package](outputs/RDMC_DIVERSIFIED_REPAIR_RESTART_SAFE_MODEL1_PACKAGE.md) and [replacement queue](outputs/RDMC_DIVERSIFIED_REPAIR_RESTART_SAFE_MODEL1_QUEUE.csv).**
