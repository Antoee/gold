...
#property description "Research fork with completed-bar strong-signal and reward-quality reversion allocation"
...
input bool   InpRVUseStrongRewardQuality = false;
input double InpRVStrongSignalMinimumAdjustedRiskReward = 1.50;
...
private:
   ...
   void TryEntry()
   {
      ...
      OpenPosition(buy, entryPrice, stopPrice, targetPrice, diEdge,
                   requestedRiskPercent, signalBodyRatio);
   }
...
bool InputsValid()
{
   ...
   if(InpRVATRPeriod < 2 || InpRVADXPeriod < 2 || InpRVRSIPeriod < 2 ||
      ...
      (InpRVUseStrongRewardQuality &&
       (!InpRVUseStrongSignalRisk ||
        InpRVStrongSignalMinimumAdjustedRiskReward < InpRVMinimumRiskReward ||
        InpRVStrongSignalMinimumAdjustedRiskReward > 10.0)) ||
