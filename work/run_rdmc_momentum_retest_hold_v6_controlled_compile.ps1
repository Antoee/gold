...
$source = Join-Path $repo 'outputs\rdmc_momentum_retest_hold_v6_package\source\Professional_XAUUSD_EA.mq5'
$expectedSourceHash = 'B99BEFF28BB0D28596F4D7786C65BDA11001B0BC4924F269F534817EADC6CDCC'
...
$auditPath = Join-Path $repo 'outputs\RDMC_MOMENTUM_RETEST_HOLD_V6_COMPILE_AUDIT.csv'
$roots = @(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash) {
   throw 'Entry/regime rewrite source identity changed.'
}
...
try {
   ...
   [IO.File]::WriteAllText($unlockFile, "Entry/regime rewrite compile $startedAtUtc", [Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck, "Entry/regime rewrite focus acknowledgement $startedAtUtc", [Text.Encoding]::ASCII)
   ...
   $compileOutput = @(& $preparer -SourcePath $source -ExpectedSourceSha256 $expectedSourceHash `
      -PortableRoots $roots -MaxCpuPercent $MaxCpuPercent -CompileTimeoutMinutes 5 `
      -UserAuthorizedFocusRisk)
...
if($binaryRows.Count -ne 2 -or $binaryHashes.Count -ne 1) { throw 'Controlled compile binary identity is not exact across both workers.' }
