...
input ulong  InpMagicNumber = 26071772;
...
input group "Volatility Squeeze Engine"
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_M15;
input int    InpSqueezeBars = 3;
...
input double InpMaximumBreakoutChannelATR = 3.50;
input double InpBreakBufferATR = 0.03;
input double InpMinimumBreakRangeATR = 0.40;
input double InpMaximumBreakRangeATR = 1.60;
input double InpMinimumExpansionRatio = 1.10;
...
input double InpMaximumStopATR = 1.25;
...
input string InpLogFileName = "Independent_XAUUSD_M15_Volatility_Squeeze_Trades.csv";
...
{
   return "IM15SQ_RISK_" + IntegerToString((long)ticket);
}
...
bool SafetyAllows(string &reason)
{
   ...
   if(!MQLInfoInteger(MQL_TESTER) &&
      ...
      (InpUseRealAccountSafetyLock || !InpAllowRealAccountTrading ||
       InpRealAccountApprovalCode != "M15SQ-LIVE-ACK"))
   {
...
bool SqueezeBarAllows(const int shift)
{
   ...
   if(!BufferValue(g_atrHandle, 0, shift, atr) || atr <= 0.0 ||
      ...
      upperBand <= lowerBand)
      return false;
   double keltnerOffset = InpKeltnerATRMultiplier * atr;
...
bool TryVolatilitySqueezeEntry(const double atr)
{
   ...
   if(!SqueezeWindowAllows(2, InpSqueezeBars))
   {
      g_squeezeRejects++;
      return false;
   }
   double channelHigh = 0.0;
   double channelLow = 0.0;
   if(!ChannelBounds(2, InpBreakoutLookbackBars, channelHigh, channelLow))
      return false;
   double channelRange = channelHigh - channelLow;
   double averageSqueezeBarRange = AverageBarRange(2, InpSqueezeBars);
   if(channelRange <= 0.0 ||
      channelRange > InpMaximumBreakoutChannelATR * atr ||
   ...
   bool breakShapeAllows = directionCandle &&
                           ...
                           range1 >= InpMinimumExpansionRatio * averageSqueezeBarRange &&
...
int OnInit()
{
   if(InpSqueezeBars < 2 || InpSqueezeBars > 12 ||
      InpBollingerPeriod < 5 || InpBollingerDeviation <= 0.0 ||
      InpKeltnerEMAPeriod < 5 || InpKeltnerATRMultiplier <= 0.0 ||
      InpBreakoutLookbackBars < InpSqueezeBars || InpMaximumBreakoutChannelATR <= 0.0 ||
   ...
   g_bollingerHandle = iBands(_Symbol, InpSignalTimeframe, InpBollingerPeriod, 0,
                              InpBollingerDeviation, PRICE_CLOSE);
   g_keltnerEmaHandle = iMA(_Symbol, InpSignalTimeframe, InpKeltnerEMAPeriod, 0,
...
{
   PrintFormat("M15SQ_DIAGNOSTIC squeezes=%d signals=%d squeeze_rejects=%d break_rejects=%d regime_rejects=%d safety_rejects=%d opened=%d stop_shape_rejects=%d minimum_lot_rejects=%d exposure_rejects=%d order_failures=%d",
               g_squeezeCandidates, g_breakoutSignals, g_squeezeRejects,
