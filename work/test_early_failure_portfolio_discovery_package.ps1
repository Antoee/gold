...
foreach($candidate in ($queue | Group-Object Candidate)) {
   ...
   foreach($required in @(
      ...
      "InpRVUseNoFollowThroughExit=", "InpRVNoFollowThroughBars=", "InpRVNoFollowThroughMaximumR=",
      "InpMOUseNoFollowThroughExit=", "InpMONoFollowThroughBars=", "InpMONoFollowThroughMaximumR=",
...
$control = @($queue | Where-Object Candidate -eq "efp_fixed_control")
Assert-True ($control.Count -eq 3 -and @($control.RVFailureExitEnabled | Sort-Object -Unique)[0] -eq "false" -and @($control.MOFailureExitEnabled | Sort-Object -Unique)[0] -eq "false") "Unchanged control missing."
