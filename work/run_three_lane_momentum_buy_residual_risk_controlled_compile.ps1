...
$auditPath=Join-Path $repo 'outputs\THREE_LANE_MOMENTUM_BUY_RESIDUAL_RISK_COMPILE_AUDIT.csv'
$roots=@(
...
if((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash.ToUpperInvariant() -ne $expectedSourceHash){throw 'Buy residual-risk research source identity changed.'}
...
try{
   ...
   [IO.File]::WriteAllText($unlockFile,"Momentum buy residual-risk compile $startedAtUtc",[Text.Encoding]::ASCII)
   [IO.File]::WriteAllText($focusAck,"Momentum buy residual-risk focus acknowledgement $startedAtUtc",[Text.Encoding]::ASCII)
