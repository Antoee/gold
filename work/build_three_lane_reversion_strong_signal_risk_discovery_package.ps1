...
param(
   ...
   [string]$PackageDir = 'outputs\three_lane_reversion_strong_signal_risk_discovery_model1_package',
   [string]$QueuePath = 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_RISK_DISCOVERY_MODEL1_QUEUE.csv',
   [string]$ManifestPath = 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_RISK_DISCOVERY_MODEL1_MANIFEST.csv',
   [string]$PackageMarkdownPath = 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_RISK_DISCOVERY_MODEL1_PACKAGE.md',
   [string]$ContractPath = 'outputs\THREE_LANE_REVERSION_STRONG_SIGNAL_RISK_DISCOVERY_CONTRACT.md'
)
...
Set-FixedInput $base 'InpEvidenceRunLabel' 'three_lane_reversion_strong_signal_risk_discovery_model1' -StringValue
Set-FixedInput $base 'InpLogTrades' 'false'
Set-FixedInput $base 'InpShowDashboard' 'false'

$variants = @(
   [pscustomobject]@{Name='rvsr_control';Enabled='false';BodyRatio='0.15';StrongRisk='0.60'},
   [pscustomobject]@{Name='rvsr_b100_r060';Enabled='true';BodyRatio='0.10';StrongRisk='0.60'},
   [pscustomobject]@{Name='rvsr_b150_r055';Enabled='true';BodyRatio='0.15';StrongRisk='0.55'},
   [pscustomobject]@{Name='rvsr_b150_r060';Enabled='true';BodyRatio='0.15';StrongRisk='0.60'},
   [pscustomobject]@{Name='rvsr_b150_r065';Enabled='true';BodyRatio='0.15';StrongRisk='0.65'},
   [pscustomobject]@{Name='rvsr_b200_r060';Enabled='true';BodyRatio='0.20';StrongRisk='0.60'},
   [pscustomobject]@{Name='rvsr_b250_r060';Enabled='true';BodyRatio='0.25';StrongRisk='0.60'}
)
$windows = @(
...
$stopRule = 'Strong-signal reversion risk discovery. Require every era positive; continuous CAGR >= control + 0.15 points; PF >= 1.85; DD <= 1.35% and <= control + 0.20 points; recovery and return/DD >= 95% of control; at least 400 trades; no era worse than control by more than $25; and two adjacent rows meeting the frozen support floor before Model4.'
...
foreach($variant in $variants) {
   ...
   foreach($window in $windows) {
      ...
      $common = [ordered]@{
         QueueRank=$ordinal;Candidate=$variant.Name;CandidateRank=$candidateRank;Phase='reversion_strong_signal_risk_discovery_model1'
...
@(
   '# Three-Lane Reversion Strong-Signal Risk Discovery Package','',
   '**Status: RISK-ALLOCATION DISCOVERY. THE ATB150 CHAMPION AND FORWARD CANDIDATE ARE UNCHANGED.**','',
   "- Research source SHA-256: ``$sourceHash``",
   "- Frozen champion profile SHA-256: ``$championHash``",
   "- Variants: ``$($variants.Count)``",
   "- Configurations: ``$ordinal``",
   '- The optional allocator may raise requested reversion risk only when the already-valid completed H1 signal candle meets a frozen directional-body ratio.',
...
@(
   '# Reversion Strong-Signal Risk Discovery Contract','',
   ...
   '- Require every disjoint era profitable, continuous CAGR at least 0.15 points above control, PF at least 1.85, drawdown at most 1.35% and no more than 0.20 points above control, recovery and return/drawdown at least 95% of control, and at least 400 continuous trades.',
   '- Reject if any era trails control by more than $25. Require two adjacent body/risk rows to reach CAGR at least control + 0.10 points, PF at least 1.80, drawdown at most 1.40%, and both efficiency measures at least 90% of control.',
